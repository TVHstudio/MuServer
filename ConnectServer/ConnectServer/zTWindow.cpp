...
void CWindow::Load()
{
	...
	this->m_BrushWhiteGray = CreateSolidBrush(RGB(255, 255, 255));
...
int CWindow::CreateMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	wcex.hbrBackground	= (HBRUSH)this->m_BrushWhiteGray;
...
{
	g_WindowZt.UpdateStatusBar(0, "Port: %d", gSettings.ServerPort);
	g_WindowZt.UpdateStatusBar(1, "Game Server: %d / %d", g_LiveGSCount, g_ServerListCount);

	if(g_bJoinServerLive)
	{
		g_WindowZt.UpdateStatusBar(2, "Join Server: On");
	}
	else
	{
		g_WindowZt.UpdateStatusBar(2, "Join Server: Off");
	}

	if(gSettings.WhiteList)
	{
		g_WindowZt.UpdateStatusBar(3, "White List: On");
	}
	else
	{
		g_WindowZt.UpdateStatusBar(3, "White List: Off");
	}
}
// ----------------------------------------------------------------------------------------------
...
void CWindow::Paint()
{
	...
	FillRect(hdc, &rect, this->m_BrushWhiteGray);
	rect.bottom -= 230;
	...
	switch(uMsg)
	{
		case WM_COMMAND:
			...
			case WM_MENU_RELOAD:
				...
				// ----
			case WM_MENU_ABOUT:
				DialogBoxParam(g_WindowZt.m_Instance, MAKEINTRESOURCE(IDD_ABOUTBOX), NULL, ProcessAbout, NULL);
				break;
				// ----
			case WM_TRAY_ABOUT:
				// ----
...
BOOL CALLBACK ProcessAbout(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        g_WindowZt.SetTitle(hWnd, "%s [%s] :: About", APP_NAME, APP_VERSION_T);
        // ----
        CString Text;
        // ----
    ...
    case WM_COMMAND:
    {
        ...
        case IDOK:
        {
            ShellExecute(NULL, "open", "http://forum.ex-team.net/", NULL, NULL, SW_SHOWNORMAL);
