...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinAccWarning))
	{
		return;
	}
...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinAccMenu))
	{
		return;
	}
...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinAccPC))
	{
		return;
	}
	...
	ContentY += 20;
	gInterface.DrawCheckLineZt(this->m_bActivePCID, eACC_PC_POINT1, eACC_PC_CHECK1, eACC_PC_LINE1, ContentX, ContentY, eWhite, "Active Protect");

	ContentY += 20;
	gInterface.DrawCheckLineZt(this->m_bChangePCID, eACC_PC_POINT2, eACC_PC_CHECK2, eACC_PC_LINE2, ContentX, ContentY, eWhite, "Change PC ID");
...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinAccOption))
	{
		return;
	}
	...
	ContentY += 25;
	gInterface.DrawCheckLineZt(this->m_bOnlyMyPC, eACC_OPTION_POINT1, eACC_OPTION_CHECK1, eACC_OPTION_LINE1, ContentX, ContentY, eWhite, "Access only from my PC");

	ContentY += 20;
	gInterface.DrawCheckLineZt(this->m_bTrade, eACC_OPTION_POINT2, eACC_OPTION_CHECK2, eACC_OPTION_LINE2, ContentX, ContentY, eWhite, "Active Trade");

	ContentY += 20;
	gInterface.DrawCheckLineZt(this->m_bInventory, eACC_OPTION_POINT3, eACC_OPTION_CHECK3, eACC_OPTION_LINE3, ContentX, ContentY, eWhite, "Active Inventory");

	ContentY += 20;
	gInterface.DrawCheckLineZt(this->m_bWerehouse, eACC_OPTION_POINT4, eACC_OPTION_CHECK4, eACC_OPTION_LINE4, ContentX, ContentY, eWhite, "Active Warehouse");

	ContentY += 20;
	gInterface.DrawCheckLineZt(this->m_bDisconnectFriend, eACC_OPTION_POINT5, eACC_OPTION_CHECK5, eACC_OPTION_LINE5, ContentX, ContentY, eWhite, "Active Disconnect Friend");

	ContentY += 20;
	gInterface.DrawCheckLineZt(this->m_bSellChar, eACC_OPTION_POINT5, eACC_OPTION_CHECK6, eACC_OPTION_LINE5, ContentX, ContentY, eWhite, "Active Selling Character");
...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinAccMessage))
	{
		return;
	}
...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinAccNumber))
	{
		return;
	}
...
void CAccountSecurity::ButtonWarning(DWORD Event)
{
	...
	if(gInterface.ButtonZt(Event, eACC_WARNGNG_OK, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccWarning);

		if(this->m_bNewSecretNumber == true)
		{
			gInterface.OpenWindowZt(ObjWindowsZt::ztWinAccNumber);
		}
		else
		{
			gInterface.OpenWindowZt(ObjWindowsZt::ztWinAccMenu);
		}		
	}
	else if(gInterface.ButtonZt(Event, eACC_WARNGNG_CLOSE, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccWarning);
	}
}
...
void CAccountSecurity::ButtonMenu(DWORD Event)
{
	...
	if(gInterface.ButtonZt(Event, eACC_MENU_CLOSE, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccMenu);
	}
	else if(gInterface.ButtonZt(Event, eACC_MENU_PCSETTING, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccMenu);
		gInterface.OpenWindowZt(ObjWindowsZt::ztWinAccPC);
	}
	else if(gInterface.ButtonZt(Event, eACC_MENU_OPTION, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccMenu);
		gInterface.OpenWindowZt(ObjWindowsZt::ztWinAccOption);
	}
}
...
void CAccountSecurity::ButtonPC(DWORD Event)
{
	...
	if(gInterface.ButtonZt(Event, eACC_PC_CLOSE, false))
	{
		this->RestoreTempData();
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccPC);
		gInterface.OpenWindowZt(ObjWindowsZt::ztWinAccMenu);
	}
	else if(gInterface.ButtonZt(Event, eACC_PC_OK, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccPC);
		this->CGSendSettingPC();
	}
	else if(gInterface.ButtonZt(Event, eACC_PC_CHECK1, false))
	{
		this->m_bActivePCID = !this->m_bActivePCID;
	}
	else if(gInterface.ButtonZt(Event, eACC_PC_CHECK2, false))
	{
...
void CAccountSecurity::ButtonOption(DWORD Event)
{
	...
	if(gInterface.ButtonZt(Event, eACC_OPTION_CLOSE, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccOption);
		gInterface.OpenWindowZt(ObjWindowsZt::ztWinAccMenu);
		this->RestoreTempData();
	}
	else if(gInterface.ButtonZt(Event, eACC_OPTION_OK, true))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccOption);
		gInterface.OpenWindowZt(ObjWindowsZt::ztWinAccMenu);
	...
	{
		if(gInterface.ButtonZt(Event, eACC_OPTION_CHECK1, false))
		{
			this->m_bOnlyMyPC = !this->m_bOnlyMyPC;
		}
		else if(gInterface.ButtonZt(Event, eACC_OPTION_CHECK2, false))
		{
			this->m_bTrade = !this->m_bTrade;
		}
		else if(gInterface.ButtonZt(Event, eACC_OPTION_CHECK3, false))
		{
			this->m_bInventory = !this->m_bInventory;
		}
		else if(gInterface.ButtonZt(Event, eACC_OPTION_CHECK4, false))
		{
			this->m_bWerehouse = !this->m_bWerehouse;
		}		
		else if(gInterface.ButtonZt(Event, eACC_OPTION_CHECK5, false))
		{
			this->m_bDisconnectFriend = !this->m_bDisconnectFriend;
		}		
		else if(gInterface.ButtonZt(Event, eACC_OPTION_CHECK6, false))
		{
...
void CAccountSecurity::ButtonMessage(DWORD Event)
{
	...
	else if(gInterface.ButtonZt(Event, eACC_MESSAGE_CLOSE, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccMessage);
...
void CAccountSecurity::ButtonNumber(DWORD Event)
{
	...
	if(gInterface.ButtonZt(Event, eACC_NUMBER_OK, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccNumber);
		gInterface.OpenWindowZt(ObjWindowsZt::ztWinAccMenu);
		this->CGSendSaveNumber();
		this->RestoreTempData();
	}
	else if(gInterface.ButtonZt(Event, eACC_NUMBER_CLOSE, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinAccNumber);
		this->RestoreTempData();
	}
...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinAccPC) && !gInterface.CheckWindowZt(ObjWindowsZt::ztWinAccNumber))
	{
		return;
	}
...
void CAccountSecurity::GCRecvSettingPC(PMSG_ANS_ACC_SETTING_PC* lpMsg)
{
	...
	gInterface.OpenWindowZt(ObjWindowsZt::ztWinAccMessage);
}
