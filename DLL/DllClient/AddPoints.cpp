...
{
	pLoadImage("ZtData\\Interface\\newui_cha_textbox03.tga", zt_ADDPOINT_BOX, 0x2601, 0x2900, 1, 0);
	pLoadImage("ZtData\\Interface\\Title.tga", zt_ADDPOINT_FRAME, 0x2601, 0x2900, 1, 0);
}
// ----------------------------------------------------------------------------------------------
...
void CAddPoints::BindImage()
{
	...
	gInterface.BindObject(eADDPOINT_INFOBG, zt_ADDPOINT_BOX, 170, 21, -1, -1);
	...
	gInterface.BindObject(eADDPOINT_TABLE, zt_ADDPOINT_FRAME, 50, 16, -1, -1);
	gInterface.BindObject(eADDPOINT_TABLE2, zt_ADDPOINT_FRAME, 50, 16, -1, -1);
	gInterface.BindObject(eADDPOINT_TABLE3, zt_ADDPOINT_FRAME, 50, 16, -1, -1);
...
void CAddPoints::Draw()
{
	...
	if(!gInterface.CheckWindowZt(ztWinAddPoints))
	{
		return;
	}
	...
	else
	{
		if(this->PointX > -230)
		{
			...
			{
				this->PointX = -230;
				gInterface.CloseWindowZt(ztWinAddPoints);
				return;
			}
		}
	}
	...
	gInterface.DrawFormat(eGold, StartX + 10, ContentY - 20, 210, 3, g_ZtText.GetText(122));
	// -> Content
	gInterface.DrawGUI(eADDPOINT_INFOBG, ContentX, ContentY);
	gInterface.DrawFormat(eAncient, ContentX + 15, ContentY + 5, 50, 3, g_ZtText.GetText(123));
	...
	gInterface.DrawFormat(eYellow, ContentX + 15, ContentY + 5, 50, 3, g_ZtText.GetText(124));
	...
	gInterface.DrawFormat(eYellow, ContentX + 15, ContentY + 5, 50, 3, g_ZtText.GetText(125));
	...
	gInterface.DrawFormat(eYellow, ContentX + 15, ContentY + 5, 50, 3, g_ZtText.GetText(126));
	...
	gInterface.DrawFormat(eYellow, ContentX + 15, ContentY + 5, 50, 3, g_ZtText.GetText(127));
	...
	if(gObjUser.GetClass() == CLASS_DARKLORD)
	{
		...
		gInterface.DrawFormat(eYellow, ContentX + 15, ContentY + 5, 50, 3, g_ZtText.GetText(128));
	...
	gInterface.DrawFormat(eWhite, StartX + 35, StartY+10, 50, 3, g_ZtText.GetText(120));
	...
	gInterface.DrawFormat(eWhite, StartX + 75 + 62, StartY+10, 50, 3, g_ZtText.GetText(129));
...
void CAddPoints::Button(DWORD Event)
{
	...
	{
#if(ADD_ADD_POINT==1)
		if(gInterface.ButtonZt(Event, eADDPOINT_POINTUP, false))
		{
			if(gInterface.CheckWindowZt(ztWinAddPoints))
			{
				this->Close();
		...
		if( g_ZtLicense.CheckUser(eZtUB::Local) ||
			...
			THINNAKORN_MAC == 1 ||
			g_ZtLicense.CheckUser(eZtUB::Escalate) || g_ZtLicense.CheckUser(eZtUB::eternalmu) ||
			...
			g_ZtLicense.CheckUser(eZtUB::GloryMU) || g_ZtLicense.CheckUser(eZtUB::ArcMu) ||
		...
		{
			if(gInterface.ButtonZt(Event, eADDPOINT_RESET, false))
			{
				if(gInterface.CheckWindowZt(ztWinAddResetPoint))
				{
					gInterface.CloseWindowZt(ztWinAddResetPoint);
				}
				else
				{
					gInterface.OpenWindowZt(ztWinAddResetPoint);
				}
			}
		}
	}
	...
	if(gInterface.CheckWindowZt(ztWinAddPoints))
	{
		if(gInterface.ButtonZt(Event, eADDPOINT_OK, true))
		{
		...
		if(gInterface.ButtonZt(Event, eADDPOINT_CLOSE, 0))
		{
			this->Close();
			return;
		}
		if(gInterface.ButtonZt(Event, eADDPOINT_INFOBG1, 0))
		{
			this->ActivePage = 0;
		}
		else if(gInterface.ButtonZt(Event, eADDPOINT_INFOBG2, 0))
		{
			this->ActivePage = 1;
		}
		else if(gInterface.ButtonZt(Event, eADDPOINT_INFOBG3, 0))
		{
			this->ActivePage = 2;
		}
		else if(gInterface.ButtonZt(Event, eADDPOINT_INFOBG4, 0))
		{
			this->ActivePage = 3;
		}
		else if(gInterface.ButtonZt(Event, eADDPOINT_INFOBG5, 0) && gObjUser.GetClass() == CLASS_DARKLORD)
		{
			this->ActivePage = 4;
		}
	}
	    ...
	    )
	{
		if(gInterface.CheckWindowZt(ztWinAddResetPoint))
		{
			if(gInterface.Button(Event, ztWinAddResetPoint, eADDPOINTRESET_CHECK1, 0))
			{
			...
			if(gInterface.Button(Event, ztWinAddResetPoint, eADDPOINTRESET_CHECK2, 0))
			{
			...
			if(gInterface.ButtonZt(Event, eADDPOINTRESET_CLOSE, 0))
			{
				gInterface.CloseWindowZt(ztWinAddResetPoint);
			}
			if(gInterface.ButtonZt(Event, eADDPOINTRESET_OK, true))
			{
...
void CAddPoints::Open(int type)
{
	...
	gInterface.OpenWindowZt(ztWinAddPoints);
}
...
void CAddPoints::CharInfoDraw()
{
	...
	if( !g_ZtLicense.CheckUser(eZtUB::Local) &&
		...
		!g_ZtLicense.CheckUser(eZtUB::masonX2) &&
	...
	float plusY = 0;
	// ----
	gInterface.DrawFormat(eYellow, PosX, PosY, 100, 1, "%s: %d", g_ZtText.GetText(51),  lpPlayer->Level);
	gInterface.DrawFormat(eRed, PosX + 50, PosY, 100, 1, "%s: %d", g_ZtText.GetText(52), gObjUser.Reset);
	gInterface.DrawFormat(eExcellent, PosX + 90, PosY, 100, 1, "%s: %d", g_ZtText.GetText(130), gObjUser.GReset);
	// ----
	plusY = 17; 
#if(ADD_ADD_POINT==1)
	if(gInterface.Data[eADDPOINT_POINTUP].OnClick)
	...
	gInterface.DrawGUIY(eADDPOINT_POINTUP, PosX, PosY + plusY, drawUP);
	gInterface.DrawFormat(eYellow, PosX + 20, PosY + plusY + 2, 100, 1, g_ZtText.GetText(131));
#endif
	gInterface.DrawGUI(eADDPOINT_TABLE, PosX + 101, PosY + plusY);
	gInterface.DrawFormat(eAncient, PosX + 76, PosY + plusY + 3, 100, 3, "%d", gVisualFix.UpPoint);
	...
	gInterface.DrawGUIY(eADDPOINT_RESET, PosX, PosY + plusY, drawUP2);
	gInterface.DrawFormat(eYellow, PosX + 20, PosY + plusY + 2, 100, 1, g_ZtText.GetText(132));
	gInterface.DrawGUI(eADDPOINT_TABLE2, PosX + 101, PosY + plusY);
	gInterface.DrawFormat(eAncient, PosX + 76, PosY + plusY + 3, 100, 3, "%d", g_AddPoints.ReturnTotalPoint());
...
void CAddPoints::DrawResetStats()
{
	...
	gInterface.DrawFormat(eGold, StartX + 10, 110, 210, 3, g_ZtText.GetText(132));

	if( g_ZtLicense.CheckUser(eZtUB::SILVER1) || 
		g_ZtLicense.CheckUser(eZtUB::SILVER2) )
	{
		ContentY += 35;
		sprintf(szText, g_ZtText.GetText(133));
	...
	{
		ContentY += 35;
		sprintf(szText, g_ZtText.GetText(134), this->ResetStatsPriceBonus);
		gInterface.DrawCheckLine(this->ResetStatsForBonus, eADDPOINTRESET_POINT1, eADDPOINTRESET_CHECK1, eADDPOINTRESET_LINE1, ContentX, ContentY, eWhite, szText);
	}
	ContentY += 20;
	sprintf(szText, g_ZtText.GetText(135), this->ResetStatsPriceCredit);
	...
	ContentY += 19;
	int pX = 10;
	gInterface.DrawFormat(eYellow, ContentX + 50 + pX, ContentY + 3, 100, 1, g_ZtText.GetText(136));
	...
	gInterface.DrawFormat(eWhite, StartX + 35, StartY+5, 50, 3, g_ZtText.GetText(137));
	...
	gInterface.DrawFormat(eWhite, StartX + 75 + 62, StartY+5, 50, 3, g_ZtText.GetText(129));
...
bool CAddPoints::ResetsTatsCheckAccept()
{
	...
	{
		if(gObjUser.ZtCred >= this->ResetStatsPriceCredit)
		{
			Result = true;
		}
	}
