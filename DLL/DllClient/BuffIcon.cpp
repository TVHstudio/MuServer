...
void BuffIcon::Load()
{
	...
	this->Read(".\\Data\\ZtData\\BuffEffect.txt");
...
void BuffIcon::Draw()
{
	...
	for(int i = 0; i < MAX_STATE_COUNT; i++)	
	{
		...
		pDrawZttraColor(0x7880, LineX, LineY, LineWind * pBuffHp / 100, 0.5, 0, 0, 0, 0, Color4f(0, 255, 0, 255));	//(Wind * pHP) / 100
...
void BuffIcon::GC_BuffInfo(PMSG_SEND_BUFF_ICON_ZT* aRecv)
{
	...
	else if( BuffIndex == AT_PARTY_EXP_BONUS_ZT )
	{
		// --
	}
...
void BuffIcon::GC_ExBuffData(GC_PMSG_ExBuff* aRecv)
{
	...
	for(int i=0; i<MAX_STATE_COUNT;i++)
	{
		this->UserExBuff[i].Index = aRecv->zt_buff[i].BuffIndex;
		this->UserExBuff[i].Time = aRecv->zt_buff[i].BuffTime;
		this->UserExBuff[i].MaxTime = aRecv->zt_buff[i].BuffMaxTime;
	}
}
// ------------------------------------------------------------------------------
