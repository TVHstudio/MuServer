...
{
	gInterface.OpenWindowZt(ztWinNPCBuffer);
}
// ----------------------------------------------------------------------------------------------
...
{
	if(!gInterface.CheckWindowZt(ztWinNPCBuffer))
	{
		return;
	}
...
void BufferSystem::ButtonEvent(DWORD Event)
{
	...
	if( gInterface.ButtonZt(Event, eNPC_BUFF_CLOSE, false ) )
	{
		gInterface.CloseWindowZt(ztWinNPCBuffer);
		return;
	}

	if( gInterface.ButtonZt(Event, eNPC_BUFF_BT1, true ) )
	{
		this->CG_SendResult(1);
		return;
	}

	if( gInterface.ButtonZt(Event, eNPC_BUFF_BT2, true ) )
	{
		this->CG_SendResult(2);
		return;
	}

	if( gInterface.ButtonZt(Event, eNPC_BUFF_BT3, true ) )
	{
		this->CG_SendResult(3);
		return;
	}

	if( gInterface.ButtonZt(Event, eNPC_BUFF_BT4, true ) )
	{
		this->CG_SendResult(4);
		return;
	}

	if( gInterface.ButtonZt(Event, eNPC_BUFF_BT5, true ) )
	{
		this->CG_SendResult(5);
		return;
	}

	if( gInterface.ButtonZt(Event, eNPC_BUFF_BT6, true ) )
	{
