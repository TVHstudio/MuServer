...
void DonateShop::ShopButtonDraw()
{
	...
	if(!gInterface.CheckWindow(ExpandInventory) && !gInterface.CheckWindow(ChatWindow))
	{
		...
		else
		{
			...
			gInterface.DrawFormat(eWhite, StartX + 115, StartY + 5, 70, 1, "%d",(int)gObjUser.ZtCred); // if full reset
		}
	}
...
void DonateShop::ButtonClick(DWORD Event)
{
	...
	{
		if(gInterface.ButtonZt(Event, eDONATE_CHECKWIN_OK, false))
		{
			...
			gInterface.CloseWindowZt(ztWinDonateCheck);
		}
		if(gInterface.ButtonZt(Event, eDONATE_CHECKWIN_CLOSE, false))
		{		
			PMSG_DONATE_ITEMBUY pMsg;
			pMsg.h.set((LPBYTE)&pMsg, 0xFB, 0x96, sizeof(pMsg));	
			pMsg.Result = 0;
	...
	if( gInterface.ButtonZt(Event, eDONATE_LEFT, false ) )
	{
	...
	else if( gInterface.ButtonZt(Event, eDONATE_RIGHT, false ) )
	{
...
{
	gInterface.OpenWindowZt(ztWinDonateCheck);
}
