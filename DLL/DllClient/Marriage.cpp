...
{
	pLoadImage("ZtTeam\\Interface\\Marriage.tga", MaiObjID::zt_MARRIAGE, 0x2601, 0x2900, 1, 0);
}
...
void CMarriage::BindImages()
{
	...
	gInterface.BindObject(eMARRIAGE_IMG, MaiObjID::zt_MARRIAGE, 200, 50, -1, -1);
}

void CMarriage::Draw()
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinMarriage))
	{
		return;
	}
...
void CMarriage::Button(DWORD Event)
{
	...
	if(gInterface.ButtonZt(Event, eMARRIAGE_CLOSE, false))
	{
		gInterface.CloseWindowZt(ObjWindowsZt::ztWinMarriage);	//need packet for gs
	...
	if(this->bDrawOK)
	{
		if(gInterface.ButtonZt(Event, eMARRIAGE_OK, true))
		{
			//need packet for gs
			PMSG_CG_MARRIAGE_CLOSE pRequest = { 0 };
...
void CMarriage::GCInfoNames(PMSG_GC_MARRIAGE_NAMES* aRecv)
{
	...
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinMarriage))
	{
		gInterface.OpenWindowZt(ObjWindowsZt::ztWinMarriage);
	}
}
...
{
	gInterface.CloseWindowZt(ObjWindowsZt::ztWinMarriage);
}
#endif