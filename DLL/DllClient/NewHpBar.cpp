...
int HpResMode()
{
	if(ztWinWidth == 800 & ztWinHeight == 600)
	{
		return 5;
	}
	else if (ztWinWidth == 1024 & ztWinHeight == 768)
	{
		return 10;
	}
	else if (ztWinWidth == 1280 & ztWinHeight == 720)
	{
		return 10;
	}
	else if (ztWinWidth == 1280 & ztWinHeight == 800)
	{
		return 10;
	}
	else if (ztWinWidth == 1366 & ztWinHeight == 768)
	{
		return 10;
	}
	else if (ztWinWidth == 1440 & ztWinHeight == 900)
	{
		return 13;
	}
	else if (ztWinWidth == 1280 & ztWinHeight == 1024)
	{
		return 14;
	}
	else if (ztWinWidth == 1600 & ztWinHeight == 1200)
	{
		return 16;
	}
	else if (ztWinWidth == 1680 & ztWinHeight == 1050)
	{
		return 13;
	}
	else if (ztWinWidth == 1920 & ztWinHeight == 1080)
	{
		return 13;
	}
	else if (ztWinWidth == 1920 & ztWinHeight == 1200)
	{
		return 16;
	}
	else
...
void ViewePortInfo::TargetMonsterHpBar()
{
	...
	if(lpViewTarget->m_Model.ObjectType == emMonster)
	{
		...
		sprintf(LifeDisplay, g_ZtText.GetText(17), Level, Life);
...
void ViewePortInfo::TargetUserHpBar()
{
	...
	if(lpViewTarget->m_Model.ObjectType == emPlayer)
	{
		...
		{
			sprintf(LifeDisplay, g_ZtText.GetText(18), Life);
			pSetTextColor(pTextThis(), 0xFF, 0xE6, 0xD2, 0xFF));
			pDrawText(pTextThis(), PosX-DevX, PosY - DevY, LifeDisplay, Wind, 16, (LPINT)3, 0);	
		}
		else
		{
			sprintf(LifeDisplay, g_ZtText.GetText(19), Level, Reset,GrandReset);
			pSetTextColor(pTextThis(), 0xFF, 0xE6, 0xD2, 0xFF));
			pDrawText(pTextThis(), PosX-DevX, PosY - DevY, LifeDisplay, Wind, 16, (LPINT)3, 0);	
#ifdef PREM_TYPE
			if(g_ZtLicense.CheckUser(eZtUB::ulasevich)	|| 
	...
	else
	{
...
#endif
		{
			...
			//	gConsole.Output(cGREEN,"PartySlot: %d ,PosX: %d, PosY: %d",PartySlot,PosX,PosY);
				...
				{
#if _NEW_PARTY_SYSTEM_ == TRUE
						sprintf(LifeDisplay, g_ZtText.GetText(20), g_Party.PartyData[PartySlot].LifePercent);
#else
						sprintf(LifeDisplay, g_ZtText.GetText(20), PartyMember.LifePercent);
#endif
					pSetTextColor(pTextThis(), 0xFF, 0xE6, 0xD2, 0xFF));
...
void ViewePortInfo::PlayerTitle()
{
	...
	for(int i = 0; i < 400; i++)
	{
		...
		if(lpViewTarget->m_Model.ObjectType == emPlayer)
		{
			...
			pDrawGUI(zt_PLAYERTITLE_A, (float)PosX - 57, (float)PosY - 0, 48, 53);
		}
	}


	//pLoadImage("ZtData\\Title\\a.tga", zt_PLAYERTITLE_A, 0x2601, 0x2900, 1, 0);
}
