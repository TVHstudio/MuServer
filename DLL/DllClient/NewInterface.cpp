...
void CNewInterface::DrawSettingsWindow()
{
	...
	//this->TextDraw(StartX+MainWidth-7,StartY+3,0,0,eGold,0,0,gFont[ARIAL_16]->m_Font,"X");
	...
	for	(int i=0;i<7;i++)
	{
		this->TextDraw(this->m_Data[eSettingsMain].X+10,this->m_Data[eSettingsMain].Y+(20*(i+1)),0,0,eGold,0,0,gFont[ARIAL_16]->m_Font,szNames[i]);

		if (i != 3)
		{
			//this->m_Data[eSettingsChat+i].State = gServerInfo.GetSettingsInfo(i);
...
bool CNewInterface::EventSettingsWindow(DWORD Event)
{
	...
	//if (IsWorkZone(eBankEx))
	{
		if (this->IsWorkZone(eSettingsClose))
		{
			...
			{
				this->m_Data[eSettingsClose].OnClick = true;
				return false;
			}*/
			// ----
	...
	for	(int i=0;i<7;i++)
	{
		...
		{
			//g_Console.AddMessage(5,"WorkZone = %d",i);
...
void CNewInterface::DrawBankEx()
{
	if (!this->m_Data[eBankEx].OnShow)
	{
		return;
	}
	//pSetCursorFocus = true;
	...
	/*float MainWidth			= 120.0;
	float MainHeight		= 135.0;
	float StartY			= 50.0;
	float StartX = 5.0;*/
	if (this->m_Data[eBankEx].leftButtonState == true)
	{
		this->m_Data[eBankEx].X = pCursorX-diffX;
		this->m_Data[eBankEx].Y = pCursorY-diffY;
		this->m_Data[eBankEx].MaxX = this->m_Data[eBankEx].X + this->m_Data[eBankEx].Width;
		this->m_Data[eBankEx].MaxY = this->m_Data[eBankEx].Y + this->m_Data[eBankEx].Height;
	}

	this->DrawBackground(eBankEx,m_Data[eBankEx].X,m_Data[eBankEx].Y,m_Data[eBankEx].Width,m_Data[eBankEx].Height,"[ BankEx ]");

	this->DrawClose(eBankExClose,this->m_Data[eBankEx].MaxX-7,this->m_Data[eBankEx].Y+3,eGold);
	...
	for	(int i = 0;i<8;i++)
	{
		this->TextDraw(m_Data[eBankEx].X+10,m_Data[eBankEx].Y+(12*i+15),0,0,eGold,0,0,gFont[ARIAL_16]->m_Font,szNames[i]);
		this->TextDraw(m_Data[eBankEx].X+85,m_Data[eBankEx].Y+(12*i+15),0,0,eWhite,0,0,gFont[ARIAL_16]->m_Font,"%s",gHPBar.ParseReducedValue(*((long*)&gObjUser.BankEx+(i))));

		if (i!=0)
		{
			this->DrawBarForm(m_Data[eBankEx].X+5,m_Data[eBankEx].Y+(12*i+12),m_Data[eBankEx].Width-10,2,0.0,0.0,0.0,0.5);//Lines
		}
	}
}

bool CNewInterface::EventBankZtWindow(DWORD Event)
{
	if (IsWorkZone(this->m_Data[eBankEx].X,this->m_Data[eBankEx].Y,this->m_Data[eBankEx].MaxX,this->m_Data[eBankEx].Y + 15))
	//if (IsWorkZone(eBankEx))
	{
		DWORD CurrentTick	= GetTickCount();

		if (this->IsWorkZone(eBankExClose))
		{
			//g_Console.AddMessage(5,"KEK?0");
			DWORD Delay	= (CurrentTick - this->m_Data[eBankExClose].EventTick);
			...
			// ----
			this->m_Data[eBankExClose].OnClick = false;
			// ----
			if( Delay < 500 )
			{
				return false;
			...
			this->SwitchBankZtWindowState();
			//g_Console.AddMessage(5,"KEK?");
			return true;
		}
		...
		if (Event == WM_LBUTTONDOWN)
		{
			this->m_Data[eBankEx].leftButtonState = true;
			diffX = pCursorX - this->m_Data[eBankEx].X;
			diffY = pCursorY - this->m_Data[eBankEx].Y;
		}
		else if (Event == WM_LBUTTONUP)
		{
			this->m_Data[eBankEx].leftButtonState = false;
			diffX = 0;
			diffY = 0;
		}
	...
	/*float diffX = pCursorX - this->m_Data[eBankEx].X;
	float diffY = pCursorY - this->m_Data[eBankEx].Y;*/
...
{
	if (gUserPanel.EventUserPanel(Event,Mouse) == true)
	{
		return 1;
	}

	if (this->EventBankZtWindow(Event) == true)
	{
		return 1;
	}
	...
	gPackSystem.EventPackWindow(Event);
