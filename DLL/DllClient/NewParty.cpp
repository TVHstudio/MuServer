...
{
	pLoadImage("ZtData\\Interface\\pm_flag.tga", zt_PM_FLAGNEW_LEADER, 0x2601, 0x2900, 1, 0);
}
#endif
...
void NewParty::Load()
{
	...
	WriteMemory((LPVOID)0x0084A578, bPartyD, sizeof(bPartyD));

	DWORD AdressNewParty = (DWORD)&this->PartyData;

	//TestOPEx((LPVOID)0x0051C3D5, &AdressNewParty, ADD_EAX);// <<==
	//TestOPEx((LPVOID)0x0051C4D6, &AdressNewParty, ADD_EAX);// <<==
	TestOPEx((LPVOID)0x0051C629, &AdressNewParty, ADD_EDX_1, ADD_EDX_2);
	//TestOPEx((LPVOID)0x00578DC1, &AdressNewParty, ADD_EAX);
	//TestOPEx((LPVOID)0x005A7B4C, &AdressNewParty, PUSH_OFFSET);
	//TestOPEx((LPVOID)0x005B7C02, &AdressNewParty, ADD_EDX_1, ADD_EDX_2);// <<==	загрузка
	//TestOPEx((LPVOID)0x005B7F63, &AdressNewParty, ADD_EDX_1, ADD_EDX_2);// <<==	освобождение
	//TestOPEx((LPVOID)0x005BA7B7, &AdressNewParty, ADD_EDX_1, ADD_EDX_2);
	//TestOPEx((LPVOID)0x0064E171, &AdressNewParty, ADD_ECX_1, ADD_ECX_2);
	//TestOPEx((LPVOID)0x0064E240, &AdressNewParty, ADD_ECX_1, ADD_ECX_2);//
	//TestOPEx((LPVOID)0x0064E397, &AdressNewParty, ADD_EAX);
	//TestOPEx((LPVOID)0x0064E3AF, &AdressNewParty, ADD_ECX_1, ADD_ECX_2);
	TestOPEx((LPVOID)0x0078F13A, &AdressNewParty, PUSH_OFFSET);	//прие??парт?
	//TestOPEx((LPVOID)0x0084A1EC, &AdressNewParty, PUSH_OFFSET);
	//TestOPEx((LPVOID)0x0084A20D, &AdressNewParty, ADD_EAX);//hz
	TestOPEx((LPVOID)0x0084A5AA, &AdressNewParty, ADD_EAX);//original
	TestOPEx((LPVOID)0x0084A5C0, &AdressNewParty, PUSH_OFFSET);//original
	TestOPEx((LPVOID)0x0084A5E1, &AdressNewParty, ADD_EAX);	//original
	//TestOPEx((LPVOID)0x0084BA31, &AdressNewParty, PUSH_OFFSET);
	//TestOPEx((LPVOID)0x0084BA51, &AdressNewParty, ADD_ECX_1, ADD_ECX_2);
	//TestOPEx((LPVOID)0x0084BB32, &AdressNewParty, ADD_EDX_1, ADD_EDX_2);
	//TestOPEx((LPVOID)0x0084C0F9, &AdressNewParty, ADD_EDX_1, ADD_EDX_2);
	//TestOPEx((LPVOID)0x0084C1AF, &AdressNewParty, ADD_ECX_1, ADD_ECX_2);
	//TestOPEx((LPVOID)0x0084C288, &AdressNewParty, PUSH_OFFSET);
	//TestOPEx((LPVOID)0x0084C2A9, &AdressNewParty, ADD_EDX_1, ADD_EDX_2);
	//TestOPEx((LPVOID)0x0084C33D, &AdressNewParty, ADD_EDX_1, ADD_EDX_2);
	//TestOPEx((LPVOID)0x0091CB9C, &AdressNewParty, ADD_EAX);

	DWORD AddressLifePercent = (DWORD)&this->LifePercent;

	//NewLifePercent
	//TestOPEx((LPVOID)0x0084C208, &AddressLifePercent, 0x0F, 0xB6, 0x82);
	//TestOPEx((LPVOID)0x0084C1ED, &AddressLifePercent, 0x0F, 0xB6, 0x88);
	TestOPEx((LPVOID)0x0095FD59, &AddressLifePercent, 0x0F, 0xB6, 0x89);//+

	DWORD AddressViewportID = (DWORD)&this->ViewportID;

	//NewViewportID
	//TestOPEx((LPVOID)0x0084B954, &AddressViewportID, 0x8B, 0x82);
	//TestOPEx((LPVOID)0x0084B99D, &AddressViewportID, 0x83, 0xB9);
	//TestOPEx((LPVOID)0x0084B9BD, &AddressViewportID, 0x83, 0xB9);
	//TestOPEx((LPVOID)0x0084BAE9, &AddressViewportID, 0x8B, 0x88);
	//TestOPEx((LPVOID)0x0084BC4D, &AddressViewportID, 0xC7, 0x81);	//-
	//TestOPEx((LPVOID)0x0084BDA7, &AddressViewportID, 0x83, 0xBA);
	//TestOPEx((LPVOID)0x0084BE4A, &AddressViewportID, 0x8B, 0x91);
	//TestOPEx((LPVOID)0x0084C03A, &AddressViewportID, 0x83, 0xB9);
	//TestOPEx((LPVOID)0x0084C13C, &AddressViewportID, 0x83, 0xB9);
	//TestOPEx((LPVOID)0x0095FD2A, &AddressViewportID, 0x8B, 0x91);
	TestOPEx((LPVOID)0x0095FE11, &AddressViewportID, 0x8B, 0x91);//+

	DWORD AddressPartyMemberCount = (DWORD)&this->PartyMemberCount;

	TestOPEx((LPVOID)0x0095E537, &AddressPartyMemberCount, 0x83, 0x3D);//+
	TestOPEx((LPVOID)0x0095FD18, &AddressPartyMemberCount, 0x3B, 0x05);//+
	TestOPEx((LPVOID)0x0095E58D, &AddressPartyMemberCount, 0x83, 0x05);//+
	TestOPEx((LPVOID)0x0095FDFF, &AddressPartyMemberCount, 0x3B, 0x05);//+
	TestOPEx((LPVOID)0x0051C61B, &AddressPartyMemberCount, 0x3B, 0x0D);//+
	TestOPEx((LPVOID)0x0064E2CC, &AddressPartyMemberCount, 0xC7, 0x05);//+ пр?разрыв?партии
	TestOPEx((LPVOID)0x0084A59C, &AddressPartyMemberCount, 0x3B, 0x15);//+
	TestOPEx((LPVOID)0x0084A578, &AddressPartyMemberCount, 0x83, 0x3D);//+ My Hook
	TestOPEx((LPVOID)0x005C60CB, &AddressPartyMemberCount, 0x83, 0x3D);	//DL Summon
	TestOPEx((LPVOID)0x00813EE3, &AddressPartyMemberCount, 0x83, 0x3D);	//DL Summon
	TestOPEx((LPVOID)0x005AD1B2, &AddressPartyMemberCount, 0x83, 0x3D);	//DL Summon
	TestOPEx((LPVOID)0x0078F124, &AddressPartyMemberCount, 0x83, 0x3D);	//Принимае??пт любо?фикс
	TestOPEx((LPVOID)0x0084B901, &AddressPartyMemberCount, 0x0F, 0xAF, 0x0D);

	BYTE NoLimitPartyMember[] = { 0xEB, 0x07 };
	WriteMemory((LPVOID)0x0084BC75, NoLimitPartyMember, sizeof(NoLimitPartyMember));

	BYTE LimitPtMemberFix2[] = { 0xEB, 0x04 };
	WriteMemory((LPVOID)0x0084BB75, LimitPtMemberFix2, sizeof(LimitPtMemberFix2));
	
	BYTE NoAddedPartyMember[] = { 0x83, 0xC0, 0x00 };
	WriteMemory((LPVOID)0x0084BCF5, NoAddedPartyMember, sizeof(NoAddedPartyMember));

	BYTE OnlyOnePartyCord[] = { 0x90, 0x90, 0x90, 0x90, 0x90, 0x90 };
	WriteMemory((LPVOID)0x0084BD04, OnlyOnePartyCord, sizeof(OnlyOnePartyCord));

	BYTE OriginalPartyBlock[] = { 0xE9, 0x3C, 0x05, 0x00, 0x00, 0x90 };
	WriteMemory((LPVOID)0x0084BD9E, OriginalPartyBlock, sizeof(OriginalPartyBlock));
	
	SetOp((LPVOID)0x005B83EE, this->MainPartyVieweID, ASM::CALL);
	SetOp((LPVOID)0x005B84D3, this->MainPartyVieweID, ASM::CALL);
	SetOp((LPVOID)0x005B856C, this->MainPartyVieweID, ASM::CALL);
	SetOp((LPVOID)0x005B857A, this->MainPartyVieweID, ASM::CALL);

	SetOp((LPVOID)0x005B8395, this->MainVieweID, ASM::CALL);
	SetOp((LPVOID)0x005B83AD, this->MainVieweID, ASM::CALL);
	SetOp((LPVOID)0x005B83C5, this->MainVieweID, ASM::CALL);
	SetOp((LPVOID)0x005B84EB, this->MainVieweID, ASM::CALL);
	SetOp((LPVOID)0x005B8506, this->MainVieweID, ASM::CALL);
	SetOp((LPVOID)0x005B851E, this->MainVieweID, ASM::CALL);

	SetOp((LPVOID)0x0084BD8F, this->DrawBarForm, ASM::CALL);
	SetOp((LPVOID)0x0084BB7E, this->TargetPartyMember, ASM::CALL);
	SetOp((LPVOID)0x0084A3F6, this->GetCoordPanelD, ASM::CALL);
	SetOp((LPVOID)0x0084A585, this->DrawPanelD, ASM::CALL);

	SetOp((LPVOID)0x00664418, this->GCPartyListAll, ASM::CALL);
	SetOp((LPVOID)0x0066443A, this->PartyMemberLifeRecv, ASM::CALL);

	BYTE bHookPtD[] = { 0xE9, 0x2B, 0x02, 0x00, 0x00 };
	WriteMemory((LPVOID)0x0084A58A, bHookPtD, sizeof(bHookPtD));
...
int NewParty::PartyVieweID()
{
			...
			&& !*(BYTE *)(v12 + 36) )
		{
			for (int j = 0; j < this->PartyMemberCount; j++ )
			{
				...
				if ( this->PartyData[j].ViewportID == -2 && this->PartyData[j].ViewportID <= -1 )
				{
					...
					PartyUser* lpParty = &this->PartyData[j];
	...
	for (int k = 0; k < this->PartyMemberCount; k++ )
	{
		...
		if ( this->PartyData[k].ViewportID < 0 )
		{
			...
			lpViewObj lpUser	= &*(ObjectPreview*)oUserPreviewStruct;
			PartyUser* lpParty = &this->PartyData[k];
			result = this->FilterPartyNumberEx(lpParty, lpUser->Name, v3);
			if ( result )
...
//#endif
		...
		if( p )
		{
				...
				g_ZtLicense.CheckUser(eZtUB::Artem2))
			{
				if(!strcmp(this->PartyData[0].Name,lpPlayer->Name))
				{
					...
					pDrawGUI(zt_PM_FLAGNEW_LEADER, X, Y, 9.0, 10.0);

					if(gInterface.IsWorkZone(X, Y, pmMaxX, pmMaxY))
					{
						if(this->pmID > 0)
						{
							pDrawColorButton(zt_PM_FLAGNEW_LEADER, X, Y, 9, 10, NULL, NULL, eRed);
						}
						else
						{
							pDrawColorButton(zt_PM_FLAGNEW_LEADER, X, Y, 9, 10, NULL, NULL, eBlue);
						}
					}
				}
			}
		...
		if(!strcmp(this->PartyData[0].Name,lpPlayer->Name) || !strcmp(this->PartyData[p].Name,lpPlayer->Name))
		{
			...
			pDrawGUFULL(0x7B42, X, Y, Whith, High, 0.0, CritY, standartWhith, standartHight, 1, 1, 0.0);
		}
		//перенест?отрисовк?баффов
#if(BUFF_PARTY==1)
		if(gPARTY_SYSTEM || g_ZtLicense.CheckUser(eZtUB::MedoniAndrei))
		{
...
void NewParty::MouseController(int Click)
{
	...
	pRequest.h.set((LPBYTE)&pRequest, 0x43, sizeof(pRequest));
			...
			if ( pViewAttackTargetID == -1 )
			{
				...
				if ( ObjectID )
				{
					if ( ObjectID != LODWORD(*(DWORD*)0x7BC4F04) )
						sub_5996A0(ObjectID + 56, (const CHAR *)&*(DWORD*)0xD47C64, ObjectID, 0, -1);
				}
			}
...
{
	if(this->PartyMemberCount <= 0 || !gInterface.CheckWindow(Party) ) 
	{
		return;
	}
...
{
	if(/*this->PartyMemberCount <= 5 ||*/ !gInterface.CheckWindow(Party) ) 
	{
		return;
	}
...
int NewParty::FilterPartyNumberEx(PartyUser* lpUser, char* Name, unsigned int a3)
{
