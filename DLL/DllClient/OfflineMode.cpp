...
void OfflineMode::Draw()
{
	...
	{
		if(g_PremiumSystemZt.m_iOfflineModeAttackFreePremiumRang > 0)
		{
			gInterface.DrawFormat(eYellow, StartX + 10, 110 + 50 - 25, 210, 3, "Attack free only %s premium players", 
				g_PremiumSystemZt.GetName(g_PremiumSystemZt.m_iOfflineModeAttackFreePremiumRang));
		}
		else
		{
	...
	*/
	...
	if(g_ZtLicense.CheckUser(eZtUB::SILVER1) || g_ZtLicense.CheckUser(eZtUB::SILVER2))
	{
		if(g_PremiumSystemZt.m_iOfflineModePickUpPremiumRang > 0)
		{
			gInterface.DrawFormat(eYellow, ContentX, ContentY, 210, 3, "Pick up options only %s premium players", 
				g_PremiumSystemZt.GetName(g_PremiumSystemZt.m_iOfflineModePickUpPremiumRang));
		}
		else
		{
	...
	if(gObjUser.InSafeZone())
	{
		...
		gObjUser.ZtCred >= this->iHourCredit
...
void OfflineMode::Click(DWORD Event)
{
	...
	if( gInterface.Button(Event, ztWinCheckOffAfk, eCHECKWIN_OK, true ))
	{
	...
	if( gInterface.Button(Event, ztWinCheckOffAfk, eCHECKWIN_CLOSE, false ) )
	{
		gInterface.CloseWindowZt(ztWinCheckOffAfk);
		return;
	}
	...
	if(gInterface.Button(Event, ztWinCheckOffAfk, eCHECKWIN_CHECK, 0))
	{
	...
	if(g_ZtLicense.CheckUser(eZtUB::SILVER1) || g_ZtLicense.CheckUser(eZtUB::SILVER2))
	{
		if(gObjUser.PremiumType == g_PremiumSystemZt.m_iOfflineModePickUpPremiumRang && gObjUser.Premium > 0)
		{
			PremiumResult = true;
		}		
	}
	...
	if(PremiumResult)
	{
		if(gInterface.Button(Event, ztWinCheckOffAfk, eOFFMODE_CHECK1, 0))
		{
			this->PickZen = !this->PickZen;
		}
		if(gInterface.Button(Event, ztWinCheckOffAfk, eOFFMODE_CHECK2, 0))
		{
			this->PickJewel = !this->PickJewel;
		}
		if(gInterface.Button(Event, ztWinCheckOffAfk, eOFFMODE_CHECK3, 0))
		{
			this->PickExel = !this->PickExel;
		}
		if(gInterface.Button(Event, ztWinCheckOffAfk, eOFFMODE_CHECK4, 0))
		{
	...
	if(g_ZtLicense.CheckUser(eZtUB::eternalmu))
	{
		if(gInterface.ButtonZt(Event, eOFFMODE_PAGEUP, 0))
		{
		...
		else if(gInterface.ButtonZt(Event, eOFFMODE_PAGEDN, 0))
		{
