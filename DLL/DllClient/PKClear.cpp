...
void PKClear::GC_Price(GC_PKCLEAR_PRICE* aRecv)
{
	...
	gInterface.OpenWindowZt(ztWinPKClear);
}
// ----------------------------------------------------------------------------------------------
...
void PKClear::Draw()
{
	if(!gInterface.CheckWindowZt(ztWinPKClear))
	{
		return;
	}
...
void PKClear::EventButton(DWORD Event)
{
	...
	if( gInterface.ButtonZt(Event, ePKCLEAR_OK, true ) )
	{
		gInterface.CloseWindowZt(ztWinPKClear);
		this->CG_Result();
		return;
	}

	if( gInterface.ButtonZt(Event, ePKCLEAR_CLOSE, false ) )
	{
		gInterface.CloseWindowZt(ztWinPKClear);
		return;
	}
