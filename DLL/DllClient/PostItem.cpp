...
bool PostItem::RecvPostItem(PMSG_POSTITEM * msg) 
{
    ...
    sprintf_s ( Messase_API, "%s %s_[%08X]", g_ZtText.GetText(121), Messase, ( DWORD ) item_post );
...
void PostItem::PostItemToWorldChatChannelImp ( void * item_data ) 
{
    ...
    if ( GetKeyState ( VK_RBUTTON ) & 0x8000 && GetKeyState ( VK_SCROLL ) & 0x8000 ) 
	{
        gPostItem.last_tickcount_post_ = GetTickCount();

		ObjectItem * lItem = (ObjectItem*)item_data;

		//g_ItemMarket.CGReqItemSell(g_ItemMarket.GetItemPos(lItem));

		g_ItemMarket.m_SelectItemPos = g_ItemMarket.GetItemPos(lItem);

		gInterface.OpenWindowZt(ObjWindowsZt::ztWinItemMarketPrice);
    }
