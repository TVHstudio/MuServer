...
PremiumSystemZt g_PremiumSystemZt;
// ----------------------------------------------------------------------------------------------

void PremiumSystemZt::Init()
{
...
void PremiumSystemZt::Load()
{

	this->Init();

	//gInterface.OpenWindowZt(ztWinPremium);
	...
	op2_ch.OZT -> галочк?
	newui_option_check.OZT - галочк?2 - 0x7B69

	newui_commamd_Line.OZJ - линия
	newui_option_line.OZJ - линия по длиньш?
...
void PremiumSystemZt::BindImages()
{
...
void PremiumSystemZt::DrawWindow()
{
	//return;
	...
	if( gInterface.IsWorkZone(ePREMIUM_CLOSE) )
	{
		...
		gInterface.DrawToolTip(gInterface.Data[ePREMIUM_CLOSE].X + 5, gInterface.Data[ePREMIUM_CLOSE].Y + 25, g_ZtText.GetText(24));
	}
	// ----
	gInterface.DrawFormat(eGold, StartX + 10, 110, 210, 3, g_ZtText.GetText(151));
	// ----
	...
	if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
	{
		...
		{
			gInterface.DrawFormat(eGold, StartX + 35, 145, 70, 3, g_ZtText.GetText(46));
			gInterface.DrawFormat(eWhite, StartX + 115, 145, 70, 3, "%d", (int)gObjUser.ZtCred); // if full reset
		}
		else
		{
			gInterface.DrawFormat(eGold, StartX + 35, 145, 70, 3, g_ZtText.GetText(141));
			gInterface.DrawFormat(eWhite, StartX + 115, 145, 70, 3, "%d", (int)gObjUser.WCoinC); // if full reset
		}
	}
	else
	{
		if(g_ZtLicense.CheckUser(eZtUB::Artem) || g_ZtLicense.CheckUser(eZtUB::Artem2) || g_ZtLicense.CheckUser(eZtUB::Chebuk))
		{
			gInterface.DrawFormat(eGold, StartX + 35, 145, 70, 3, g_ZtText.GetText(140));
			gInterface.DrawFormat(eWhite, StartX + 115, 145, 70, 3, "%d", (int)gObjUser.WCoinC); // if full reset
		}
		else
		{
	...
	gInterface.DrawFormat(eGold, StartX + 35, 165, 70, 3, g_ZtText.GetText(152));
	...
	gInterface.DrawFormat(eGold, StartX + 35, 185, 70, 3, g_ZtText.GetText(153));
	...
	gInterface.DrawFormat(eWhite, StartX + 60, 235, 70, 1, g_ZtText.GetText(154));
	...
	if(!g_ZtLicense.CheckUser(eZtUB::SILVER1) && !g_ZtLicense.CheckUser(eZtUB::SILVER2))
	{
		...
		gInterface.DrawFormat(eWhite, StartX + 60, 255, 70, 1, g_ZtText.GetText(155));
		...
		gInterface.DrawFormat(eWhite, StartX + 60, 275, 70, 1, g_ZtText.GetText(156));
	...
	gInterface.DrawFormat(eWhite, StartX + 60, 295, 70, 1, g_ZtText.GetText(157));
	...
	if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
	{
		...
		gInterface.DrawFormat(eGold, StartX + 40, 332-10, 210, 1, g_ZtText.GetText(46));


		gInterface.DrawGUI(ePREMIUM_MONEYBG, StartX + 30, 327+10);

		gInterface.DrawFormat(eGold, StartX + 40, 332+10, 210, 1, g_ZtText.GetText(140));
		...
		//if(this->PriceSwitch == 0)
		{
			...
			if( this->NeedCredit > gObjUser.ZtCred )
			{
	...
	else
	{
		if(g_ZtLicense.CheckUser(eZtUB::Artem) || g_ZtLicense.CheckUser(eZtUB::Artem2) || g_ZtLicense.CheckUser(eZtUB::Chebuk))
		{
			gInterface.DrawFormat(eGold, StartX + 60, 315, 210, 1, g_ZtText.GetText(158));
		}
		else
		{
			gInterface.DrawFormat(eGold, StartX + 60, 315, 210, 1, g_ZtText.GetText(159));
		}
	...
	gInterface.DrawFormat(eWhite, StartX + 8, gInterface.Data[ePREMIUM_FOOTER].Y + 20, 210, 3, g_ZtText.GetText(160));
	...
	if(this->PriceSwitch == 0)
	{
		...
		{
			if(gObjUser.ZtCred <= 0 || this->NeedCredit > gObjUser.ZtCred)
			{
				gInterface.Data[ePREMIUM_FINISH].Attribute = false;
				gInterface.DrawColoredGUI(ePREMIUM_FINISH, gInterface.Data[ePREMIUM_FINISH].X, gInterface.Data[ePREMIUM_FINISH].Y, eGray150);
				return;
...
void PremiumSystemZt::Button(DWORD Event)
{
	if( gInterface.Button(Event, ztWinPremium, ePREMIUM_CLOSE, false ) )
	{
		gInterface.CloseWindowZt(ztWinPremium);
		return;
	}

	if( gInterface.Button(Event, ztWinPremium, ePREMIUM_PAGEUP, false ) )
	{
		this->PremiumDay++;
	...
	if( gInterface.Button(Event, ztWinPremium, ePREMIUM_PAGEDN, false ) )
	{
		this->PremiumDay--;
	...
	if( gInterface.Button(Event, ztWinPremium, ePREMIUM_CHECK1, false ) )
	{
		this->PremiumSwitch = eVip1;
	}
	...
	if(!g_ZtLicense.CheckUser(eZtUB::SILVER1) && !g_ZtLicense.CheckUser(eZtUB::SILVER2))
	{
		if( gInterface.Button(Event, ztWinPremium, ePREMIUM_CHECK2, false ) )
		{
			this->PremiumSwitch = eVip2;
		}

		if( gInterface.Button(Event, ztWinPremium, ePREMIUM_CHECK3, false ) )
		{
	...
	if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
	{
		if( gInterface.Button(Event, ztWinPremium, ePREMIUM_CHECK4, false ) )
		{
			this->PriceSwitch = 0;
		}
		else if( gInterface.Button(Event, ztWinPremium, ePREMIUM_CHECK5, false ) )
		{
			this->PriceSwitch = 1;
		}
	}

	if( gInterface.Button(Event,ztWinPremium, ePREMIUM_FINISH, true ) )
	{
...
void PremiumSystemZt::GC_RecvInfo(GC_PREMIUM_SEND* Recv)
{
	this->PremiumSwitch = eVip1;
	this->PremiumDay = 1;
	...
	gObjUser.ZtCred = Recv->UserCredit;
...
void PremiumSystemZt::CG_SendOpenWindows()
{
	if(gInterface.CheckWindowZt(ztWinPremium))
	{
		return;
...
char* PremiumSystemZt::GetName(int Number)
{
	if(Number == eVip1)
	{
		return g_ZtText.GetText(162);
	}
	if(Number == eVip2)
	{
		return g_ZtText.GetText(163);
	}
	if(Number == eVip3)
	{
		return g_ZtText.GetText(164);
	}

	return g_ZtText.GetText(165);
}
// ----------------------------------------------------------------------------------------------

void PremiumSystemZt::GC_Config(GC_PREMIUM_CONFIG* lpMsg)
{
