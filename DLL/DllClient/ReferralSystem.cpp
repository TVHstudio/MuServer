...
{
	gInterface.BindObject(eREFERRAL_ICON, zt_ICON_REFFERAL, 16, 11.4, -1, -1);
	gInterface.BindObject(eREFERRAL_BORDER, zt_BORDER_REFFERAL, 19, 15, -1, -1);
...
{
	pLoadImage("ZtData\\Interface\\referral.jpg", zt_ICON_REFFERAL, GL_LINEAR, GL_CLAMP, 1, 0);
	pLoadImage("ZtData\\Interface\\rama.tga", zt_BORDER_REFFERAL, GL_LINEAR, GL_CLAMP, 1, 0);
}

void CReferralSystem::DrawMain()
{
	if(!g_ZtLicense.user.ReferralSystem)
	{
		return;
	}
...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinReferralMenu))
	{
		return;
	}
...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinReferralSystem))
	{
		return;
	}
...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinReferralSystem2))
	{
		return;
	}
...
{
	if(!g_ZtLicense.user.ReferralSystem || pPlayerState != GameProcess)
	{
		return;
	}
...
void CReferralSystem::Button(DWORD Event)
{
	...
	if(gInterface.ButtonZt(Event, eREFERRAL_ICON, false))
	{
		if(gInterface.CheckWindowZt(ObjWindowsZt::ztWinReferralMenu))
		{
			gInterface.CloseWindowZt(ObjWindowsZt::ztWinReferralMenu);
		}
		else
		{
			gInterface.OpenWindowZt(ObjWindowsZt::ztWinReferralMenu);
		}
	...
	if(gInterface.CheckWindowZt(ObjWindowsZt::ztWinReferralMenu))
	{
		if(gInterface.ButtonZt(Event, eREFERRAL_MENU_CLOSE, false))
		{
			gInterface.CloseWindowZt(ObjWindowsZt::ztWinReferralMenu);
		...
		if(gInterface.ButtonZt(Event, eREFERRAL_MENU_REWARD, false))
		{
			gInterface.CloseWindowZt(ObjWindowsZt::ztWinReferralMenu);
		...
		if(gInterface.ButtonZt(Event, eREFERRAL_MENU_REFERRAL, false))
		{
			gInterface.CloseWindowZt(ObjWindowsZt::ztWinReferralMenu);
			this->m_iPage = 0;
	...
	else if(gInterface.CheckWindowZt(ObjWindowsZt::ztWinReferralSystem))
	{
		if(gInterface.ButtonZt(Event, eREFERRAL_CLOSE, false))
		{
			gInterface.CloseWindowZt(ObjWindowsZt::ztWinReferralSystem);
		}

		if(this->m_iPage > 0)
		{
			if(gInterface.ButtonZt(Event, eREFERRAL_L, false))
			{
				this->m_iOpen = 0;
				this->m_iPage--;
		...
		{
			if(gInterface.ButtonZt(Event, eREFERRAL_R, false))
			{
				this->m_iOpen = 0;
				this->m_iPage++;
				this->CGReqMasterInfo();
		...
		{
			if(gInterface.ButtonZt(Event, eREFERRAL_PAGEUP1 + i, false))
			{
				this->m_iOpen = i;
				break;
			}

			if(gInterface.ButtonZt(Event, eREFERRAL_BT1 + i, true))
			{
	...
	else if(gInterface.CheckWindowZt(ObjWindowsZt::ztWinReferralSystem2))
	{
		if(gInterface.ButtonZt(Event, eREFERRAL2_CLOSE, false))
		{
			gInterface.CloseWindowZt(ObjWindowsZt::ztWinReferralSystem2);
		}

		if(gInterface.ButtonZt(Event, eREFERRAL2_BT1, true))
		{
...
void CReferralSystem::GCAnsMasterInfo(PMSG_ANS_REFERRAL_MASTER_INFO* lpMsg)
{
	...
	gInterface.OpenWindowZt(ObjWindowsZt::ztWinReferralSystem);
}
...
void CReferralSystem::GCAnsUserInfo(PMSG_ANS_REFERRAL_USER_INFO* lpMsg)
{
	...
	gInterface.OpenWindowZt(ObjWindowsZt::ztWinReferralSystem2);
}
