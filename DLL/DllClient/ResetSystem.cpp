...
void CResetSystem::BindImages()
{
	...
	gInterface.BindObject(eRESETSYSTEM_FINISH, 0x7A5E, 128, 29, -1, -1);
	gInterface.BindObject(eRESETSYSTEM_COUNT, 0x7BAC, 168, 18, -1, -1);
	gInterface.BindObject(eRESETSYSTEM_TEXT, zt_SMITHY_TEXT, 170, 21, -1, -1);
	...
	gInterface.BindObject(eGRANDSYSTEM_TEXT, zt_SMITHY_TEXT, 170, 21, -1, -1);
...
void CResetSystem::DrawReset()
{
	...
	gInterface.DrawFormat(eAncient, StartX + 10, 130, 210, 3, "Reset:  %d / %d", res->Reset , res->MaxReset);
...
void CResetSystem::Button(DWORD Event)
{
	...
	if(gInterface.ButtonZt(Event, eRESETSYSTEM_FINISH, false))
	{
	...
	if(gInterface.ButtonZt(Event, eRESETSYSTEM_CLOSE, false))
	{
		gInterface.Data[eRESETSYSTEM_MAIN].OnShow = false;
	}
}
...
void CResetSystem::DrawGrand()
{
	...
	{
		strcpy(szGetItem, this->GetItemString(pGrand->AddItem, pGrand->AddItemLevel, pGrand->AddItemSkill, pGrand->AddItemLuck, pGrand->AddItemOpt, pGrand->AddItemExl, pGrand->AddItemAnc));
		gInterface.DrawGUI(eSMITHY_TEXT, StartX + 30, CountMissionY);
		gInterface.DrawFormat(ItemNameColor, StartX + 30 + 5, CountMissionY + 5, 300, 1, "%s / %d", szGetItem, pGrand->AddItemCount);
		CountMissionY += 20;
	}
	#endif
...
void CResetSystem::GrandButton(DWORD Event)
{
	...
	if(gInterface.ButtonZt(Event, eGRANDSYSTEM_FINISH, false))
	{
		GC_RESET_RESULT pMSg;
	...
	if(gInterface.ButtonZt(Event, eGRANDSYSTEM_CLOSE, false))
	{
		gInterface.Data[eGRANDSYSTEM_MAIN].OnShow = false;
	}
}
