...
{
	if( gInterface.Button(Event, ztWinSettings, eSETTINGS_CLOSE, false ) )
	{
		gInterface.CloseWindowZt(ztWinSettings);
		return;
	}

	if( gInterface.Button(Event,ztWinSettings, eSETTINGS_FINISH, false ) )
	{
		gInterface.CloseWindowZt(ztWinSettings);
		//this->Save();
		//this->CG_ExSettings();
	...
	if( gInterface.Button(Event, ztWinSettings, eSETTINGS_CHECK1, false ) )
	{
		gCamera.Switch();
		return;
	}

	if( gInterface.Button(Event, ztWinSettings, eSETTINGS_CHECK2, false ) )
	{
		g_bGlowGraphic = !g_bGlowGraphic;
		return;
	}

	if( gInterface.Button(Event, ztWinSettings, eSETTINGS_CHECK3, false ) )
	{
		iPartyHpBar = !iPartyHpBar;
		return;
	}

	if( gInterface.Button(Event, ztWinSettings, eSETTINGS_CHECK4, false ) )
	{
	...
	if( gInterface.Button(Event, ztWinSettings, eSETTINGS_CHECK5, false ) )
	{
		iMonsterHPBar = !iMonsterHPBar;
	}

#if(NEW_GLOW_EFFECT==TRUE)
	if( gPARTY_SYSTEM && g_Party.PartyMemberCount > 0 && gInterface.Button(Event, ztWinSettings, eSETTINGS_CHECK6, false ) )
	{
