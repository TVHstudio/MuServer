...
void CTeamVsTeam::BindImage()
{
	...
	gInterface.BindObject(eLORA_BATTLE_SCORE, zt_LORA_BATTLE_SCORE, 131, 70, -10, 359 + 22);
	//gInterface.BindObject(eTEAMVSTEAM_STATISTIC_INFO, zt_TEAM_VS_TEAM_STATISTIC, 170, 20, -1, -1);
...
{
	if(!gInterface.CheckWindowZt(ztWinTeamVsTeam))
	{
		return;
	}
...
{
	if(!gInterface.CheckWindowZt(ObjWindowsZt::ztWinTeamVsTeamStatistic))
	{
		return;
	}
...
{
	if(gInterface.CheckWindowZt(ObjWindowsZt::ztWinTeamVsTeam))
	{
		if(gInterface.ButtonZt(Event, eTEAMVSTEAM_OK, true))
		{
			gInterface.CloseWindowZt(ObjWindowsZt::ztWinTeamVsTeam);
	...
	if(gInterface.CheckWindowZt(ObjWindowsZt::ztWinTeamVsTeamStatistic))
	{
		if(gInterface.ButtonZt(Event, eTEAMVSTEAM_STATISTIC_FINISH, true))
		{
			gInterface.CloseWindowZt(ObjWindowsZt::ztWinTeamVsTeamStatistic);
		}
	}
}
...
void CTeamVsTeam::GCOpenWindow(PMSG_TVTGC_WINOPEN* lpMsg)
{
	...
	gInterface.OpenWindowZt(ObjWindowsZt::ztWinTeamVsTeam);
}
...
void CTeamVsTeam::GCEventStatistic(PMSG_TVT_STATISTIC* lpMsg)
{
	...
	gInterface.OpenWindowZt(ObjWindowsZt::ztWinTeamVsTeamStatistic);
}
