...
CMuTextZt* g_TextZt = new CMuTextZt;


void CMuTextZt::Out(char* text, int x, int y)
{
	__asm {
		PUSH 0;
		PUSH 0;
...
void CMuTextZt::SetFont(HFONT hFont)
{
	__asm {
		PUSH hFont;
		MOV EDX, TEXT_GET_CONTEXT;
...
void CMuTextZt::SetColor(COLORREF color)
{
	__asm {
		PUSH color;
...
void CMuTextZt::SetBack(COLORREF color)
{
...
CGlFont::CGlFont(TCHAR* Name, int Size, int Weight, BOOL bItalic, BOOL bUnderline)
{
	m_Font = CreateFont(Size, 0, 0, 0, Weight, bItalic, bUnderline, FALSE,
		/*g_ServerInfo->GetCharset()*/0x4E4, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS,
...
//	gluOrtho2D(0.0f, 800.0f, 600.0f, 0.0f);
	...
	int WindowSize[2];
	gInterface.muGetWindowSize(WindowSize);
...
void CGlFont::PrintZt( float xs, float ys, TCHAR* text )
{
//	glMatrixMode(GL_PROJECTION);
//	glPushMatrix();
//	glLoadIdentity();
...
void CGlFont::DrawColorZt(float x, float y, COLORREF color, TCHAR* text)
{
	float r = (float)GetRValue(color);
	float g = (float)GetGValue(color);
	float b = (float)GetBValue(color);
...
void CGlFont::DrawColorFormatZt( float x, float y, COLORREF color, TCHAR* format, ... )
{
	TCHAR TextBuff[1024] = {0};
	va_list va;
	va_start(va, format);
	tvsprintf(TextBuff, format, va);
	va_end(va);
	DrawColorZt(x, y, color, TextBuff);
}
