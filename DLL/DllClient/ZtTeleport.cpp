...
ZtTeleport g_ZtTeleport;
// ----------------------------------------------------------------------------------------------

ZtTeleport::ZtTeleport()
{
	this->Init();
}
// ----------------------------------------------------------------------------------------------
...
void ZtTeleport::Draw()
{
	if(!gInterface.CheckWindowZt(ztWinTeleport))
	{
		return;
	}
	...
	if(this->ClickPage >= 0 && this->ClickPage < MAXZTTELEPORT)
	{
		...
		if(this->data[i].level > 0)
		{
			...
			gInterface.DrawFormat(ListColor, ListX + 135, ListY + 5 + PlusY, 70, 1, "%d", this->data[i].level);
		}
		// -> reset
		if(this->data[i].reset > 0)
		{
			PlusY += iLenPlus;
			ListColor = eWhite;
			...
			gInterface.DrawFormat(ListColor, StartX + 135, ListY + 5 + PlusY, 70, 1, "%d", this->data[i].reset);
		}
		// -> grand
		if(this->data[i].grand > 0)
		{
			PlusY += iLenPlus;
			ListColor = eWhite;
			...
			gInterface.DrawFormat(ListColor, StartX + 135, ListY + 5 + PlusY, 70, 1, "%d", this->data[i].grand);
		}
		// -> premium
...
void ZtTeleport::GCWindows(PMSG_GC_TELEPORTWIN* aRecv)
{
	this->Init();
	...
	for(int i=0; i < this->iLoadedCount; i++)
	{
		...
		this->data[i].level = aRecv->tdata[i].level;
		this->data[i].reset = aRecv->tdata[i].reset;
		this->data[i].grand = aRecv->tdata[i].grand;
		...
		this->LenCount = 0;
		this->SpeedCalc(this->data[i].level, this->LenCount);
		this->SpeedCalc(this->data[i].reset, this->LenCount);
		this->SpeedCalc(this->data[i].grand, this->LenCount);
	...
	{
		gInterface.OpenWindowZt(ztWinTeleport);
	}
}
// ----------------------------------------------------------------------------------------------

void ZtTeleport::CGResult(BYTE num)
{
	PMSG_CG_TELEPORTWINRES pRequest;
	pRequest.h.set((LPBYTE)&pRequest, 0xFB, 0x17, sizeof(pRequest));
	pRequest.result = (BYTE)num;
	gProtocol.DataSend((LPBYTE)&pRequest, pRequest.h.size);
	gInterface.CloseWindowZt(ztWinTeleport);
	this->ClickPage = -1;
}
// ----------------------------------------------------------------------------------------------

void ZtTeleport::Button(DWORD Event)
{
	if(!gInterface.CheckWindowZt(ztWinTeleport))
	{
		return;
...
void ZtTeleport::SpeedCalc(int number, int &countplus)
{
	if(number > 0)
	{
		countplus++;
...
char* ZtTeleport::ActiveText(int status)
{
	if(status)
	{
