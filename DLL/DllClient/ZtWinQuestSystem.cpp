...
void ZtWinQuestSystem::ImageLoad()
{
	pLoadImage("ZtData\\Interface\\info_block.tga", zt_INFO_2, 0x2601, 0x2900, 1, 0);
	pLoadImage("ZtData\\Interface\\newui_cha_textbox03.tga", zt_TEXT_BOX3, 0x2601, 0x2900, 1, 0);
}
// ----------------------------------------------------------------------------------------------

void ZtWinQuestSystem::BindImages()
{
	...
	gInterface.BindObject(eWIN_QUEST_INFO, zt_INFO_2, 22, 19, -1, -1);
	gInterface.BindObject(eWIN_QUEST_INFOBG3, zt_TEXT_BOX3, 170, 21, -1, -1);
}
// ----------------------------------------------------------------------------------------------

void ZtWinQuestSystem::Draw()
{
	if(!gInterface.CheckWindowZt(ztWinQuest))
	{
		return;
	}
	...
	if(this->OpenTab == 0)
	{
		gInterface.DrawGUIZt(eWIN_QUEST_TAB1, TabX, TabY, true );
		gInterface.DrawGUIZt(eWIN_QUEST_TAB2, TabX + gInterface.Data[eWIN_QUEST_TAB2].Width, TabY, false );
		gInterface.DrawGUIZt(eWIN_QUEST_TAB3, TabX + gInterface.Data[eWIN_QUEST_TAB3].Width + gInterface.Data[eWIN_QUEST_TAB3].Width, TabY, false );
	}
	else if(this->OpenTab == 1)
	{
		gInterface.DrawGUIZt(eWIN_QUEST_TAB1, TabX, TabY, false );
		gInterface.DrawGUIZt(eWIN_QUEST_TAB2, TabX + gInterface.Data[eWIN_QUEST_TAB2].Width, TabY, true );
		gInterface.DrawGUIZt(eWIN_QUEST_TAB3, TabX + gInterface.Data[eWIN_QUEST_TAB3].Width + gInterface.Data[eWIN_QUEST_TAB3].Width, TabY, false );
	}
	else
	{
		gInterface.DrawGUIZt(eWIN_QUEST_TAB1, TabX, TabY, false );
		gInterface.DrawGUIZt(eWIN_QUEST_TAB2, TabX + gInterface.Data[eWIN_QUEST_TAB2].Width, TabY, false );
		gInterface.DrawGUIZt(eWIN_QUEST_TAB3, TabX + gInterface.Data[eWIN_QUEST_TAB3].Width + gInterface.Data[eWIN_QUEST_TAB3].Width, TabY, true );		
	}
...
void ZtWinQuestSystem::DrawMiniInfo()
{
...
void ZtWinQuestSystem::Button(DWORD Event)
{
	if(!gInterface.CheckWindowZt(ztWinQuest))
	{
		return;
	...
	if( gInterface.ButtonZt(Event, eWIN_QUEST_FINISH, true ) )
	{
		this->CG_AcñeptQuest();
	}
	else if( gInterface.ButtonZt(Event, eWIN_QUEST_CLOSE, false ) )
	{
		gInterface.CloseWindowZt(ztWinQuest);
	}
	else if( gInterface.ButtonZt(Event, eWIN_QUEST_TAB1, false ) )
	{
		this->OpenTab = 0;
	}
	else if( gInterface.ButtonZt(Event, eWIN_QUEST_TAB2, false ) )
	{
		this->OpenTab = 1;
	}
	else if( gInterface.ButtonZt(Event, eWIN_QUEST_TAB3, false ) )
	{
		this->OpenTab = 2;
	}
}
...
void ZtWinQuestSystem::GC_RecvMain(GC_MainPacket* aRecv)
{
	for(int a=0; a < EW_MAX_ACT; a++)
	{
		strncpy(this->ewdata[a].NameQuest, aRecv->ztQuest[a].NameQuest, 25);
		this->ewdata[a].b_accept = aRecv->ztQuest[a].b_accept;
		this->ewdata[a].b_done = aRecv->ztQuest[a].b_done;
		this->ewdata[a].b_End = aRecv->ztQuest[a].b_End;
		// -> user send
		this->ewdata[a].Start = aRecv->ztQuest[a].Start;
		this->ewdata[a].Number = aRecv->ztQuest[a].Number;
		for(int m = 0; m < EW_MAX_MISSION; m++)
		{
			this->ewdata[a].Count[m] = aRecv->ztQuest[a].Count[m];
		}	
		// -> need send
		this->ewdata[a].b_Level = aRecv->ztQuest[a].b_Level;
		this->ewdata[a].b_Reset = aRecv->ztQuest[a].b_Reset;
		this->ewdata[a].b_Grand = aRecv->ztQuest[a].b_Grand;

		this->ewdata[a].NeedLevel = aRecv->ztQuest[a].NeedLevel;
		this->ewdata[a].NeedReset = aRecv->ztQuest[a].NeedReset;
		this->ewdata[a].NeedGrand = aRecv->ztQuest[a].NeedGrand;
		// -> mission send
		for(int m = 0; m < EW_MAX_MISSION; m++)
		{
			this->ewdata[a].b_Mission[m] = aRecv->ztQuest[a].b_Mission[m];
			this->ewdata[a].MissionType[m] = aRecv->ztQuest[a].MissionType[m];
			this->ewdata[a].MissionID[m] = aRecv->ztQuest[a].MissionID[m];
			this->ewdata[a].MissionIndex[m] = aRecv->ztQuest[a].MissionIndex[m];
			this->ewdata[a].MissionLevel[m] = aRecv->ztQuest[a].MissionLevel[m];
			this->ewdata[a].MissionLuck[m] = aRecv->ztQuest[a].MissionLuck[m];
			this->ewdata[a].MissionOpt[m] = aRecv->ztQuest[a].MissionOpt[m];
			this->ewdata[a].MissionExl[m] = aRecv->ztQuest[a].MissionExl[m];
			this->ewdata[a].MissionCount[m] = aRecv->ztQuest[a].MissionCount[m];
			this->ewdata[a].MissionProcent[m] = aRecv->ztQuest[a].MissionProcent[m];
		}
		for(int r = 0; r < EW_MAX_REWARD; r++)
		{
			
			this->ewdata[a].RewardType[r] = aRecv->ztQuest[a].RewardType[r];
			this->ewdata[a].RewardCount[r] = aRecv->ztQuest[a].RewardCount[r];
			this->ewdata[a].RewardItemType[r] = aRecv->ztQuest[a].RewardItemType[r];
			this->ewdata[a].RewardItemIndex[r] = aRecv->ztQuest[a].RewardItemIndex[r];
			this->ewdata[a].RewardItemLevel[r] = aRecv->ztQuest[a].RewardItemLevel[r];
		}
	}

	if(!gInterface.CheckWindowZt(ztWinQuest))
	{
		gInterface.OpenWindowZt(ztWinQuest);
	}
}
// ----------------------------------------------------------------------------------------------

void ZtWinQuestSystem::CG_AcñeptQuest()
{
...
void ZtWinQuestSystem::GC_MiniInfoRecv(GC_ZtWinMiniInfo* aRecv)
{
	for(int a = 0; a < EW_MAX_ACT; a++)
	{
		strncpy(this->ewdata[a].NameQuest, aRecv->ztQuest[a].NameQuest, 25);
		...
		for(int m = 0; m < EW_MAX_MISSION; m++)
		{
			this->ewdata[a].Count[m] = aRecv->ztQuest[a].ZtWQuestCount[m];
			this->ewdata[a].MissionType[m] = aRecv->ztQuest[a].MissionType[m];
			this->ewdata[a].MissionID[m] = aRecv->ztQuest[a].MissionID[m];
			this->ewdata[a].MissionIndex[m] = aRecv->ztQuest[a].MissionIndex[m];
			this->ewdata[a].MissionLevel[m] = aRecv->ztQuest[a].MissionLevel[m];
...
void ZtWinQuestSystem::GC_MiniInfoRecvZt(GC_ZtWinMiniInfoZt* aRecv)
{
	int a = aRecv->a;
	int m = aRecv->m;
...
void ZtWinQuestSystem::GC_Start(GC_ZtWinStart* aRecv)
{
	int a = aRecv->Act;
	int s = aRecv->Start;
