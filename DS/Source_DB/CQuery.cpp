...
BOOL CQuery::Connect(int Type, char *ConStr, char *UID, char *PWD)
{
	...
	Temp QueryMuOnline[] = 
	{
		...
		{"alter table [Character] add [ZtFreePoints] int not null default(0)","ZtFreePoints","Character"},
		...
		{"alter table [Character] add [ZtQuestNum] int not null default(0)","ZtQuestNum","Character"},
		{"alter table [Character] add [ZtQuestKill] int not null default(0)","ZtQuestKill","Character"},
...
#endif
		{"alter table [Character] add [ZtCred] float not null default(0)","ZtCred","Character"},
#ifdef _CR_MEMB_INFO_
		{"alter table [MEMB_INFO] add [ZtCredit] float not null default(0)","ZtCredit","MEMB_INFO"},
#endif
#ifdef CONFLICT_PREM
		{"alter table [MEMB_INFO] add [PremiumTime] int not null default(0)","ZtCredit","MEMB_INFO"},
#endif
		...
		{"alter table [MEMB_INFO] add [ATDate] int not null default(0)","ATDate","MEMB_INFO"},
	...
	//char szQuery2[] = "EXEC OFFLINEMODE_SAVE 'Helper', 1, 1, 'est' 4e?'";
	//this->Exec(szQuery2);
...
bool CQuery::ExecQuery(char* query, ...)
{
	char szSQL[4096];

	va_list arg;
	va_start(arg,query);
	vsprintf_s(szSQL,query,arg);
	va_end(arg);
