...
//////////////////////////////////////////////////////////////////////
	...
	strcat(chBuffer, ",ZtFreePoints");
	strcat(chBuffer, ",Resets");
	strcat(chBuffer, ",gr_res");
	strcat(chBuffer, ",ZtQuestNum");
	strcat(chBuffer, ",ZtQuestKill");
	strcat(chBuffer, ",PremiumTime");
	strcat(chBuffer, ",PremiumTimeType");
	strcat(chBuffer, ",ZtCred");
...
int CCharDBSet::CreateCharacter_Multi(CString accountId, CString Name, BYTE Class, int Page)
{
	CString qSql;

	int result = 3;

	qSql.Format("WZ_CreateCharacter%d '%s', '%s', '%d'", Page, accountId, Name, Class);
...
BYTE CCharDBSet::SaveCharacter(char* Name, LPCharacterInfo_Struct lpObj)
{
	...
	qSql.Format("EXEC ZT_CHARACTER_SAVE '%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", 
	...
	//----
	lpObj->PCPoint, lpObj->ZtFreePoints, lpObj->Reset, lpObj->GReset, lpObj->ZtQuestNum, lpObj->ZtQuestKill, lpObj->PremiumTime, lpObj->PremiumTimeType, (int)lpObj->ZtCred, lpObj->BanChat,
	...
	//qSql.Format("EXEC ZT_MEMB_INFO_SAVE '%s',%d,%d,%d,%d", Name, lpObj->PremiumTime, (int)lpObj->ZtCred, lpObj->MTDate, lpObj->QTDate);
	...
	sprintf(szQouery, "ZT_MEMB_INFO_SAVE '%s',%d,%d,%d,%d,%d", 
		lpObj->AccountID, lpObj->PremiumTime, (int)lpObj->ZtCred, lpObj->MTDate, lpObj->QTDate, lpObj->DonateCredit); 
	...
	sprintf(szQouery, "UPDATE MEMB_INFO SET ATDate = %d WHERE memb___id = '%s'", lpObj->ATDate, lpObj->AccountID);	
...
{
	CString qSql;
	qSql.Format("EXEC ZT_CHARACTER_LOAD '%s'",Name);
	...
	lpObj->ZtFreePoints = m_DBQuery.GetInt("ZtFreePoints");
	lpObj->Reset = m_DBQuery.GetInt("Resets");
	lpObj->GReset = m_DBQuery.GetInt("gr_res");
	lpObj->ZtQuestNum = m_DBQuery.GetInt("ZtQuestNum");
	lpObj->ZtQuestKill = m_DBQuery.GetInt("ZtQuestKill");
	lpObj->PremiumTime = m_DBQuery.GetInt("PremiumTime");
	lpObj->PremiumTimeType = m_DBQuery.GetInt("PremiumTimeType");
	lpObj->ZtCred = m_DBQuery.GetFloat("ZtCred");
	...
	// ----
	qSql.Format("EXEC ZT_MEMB_INFO_LOAD '%s'", szAccountID);
	...
	lpObj->ZtCred = m_MEMBQuery.GetInt("ZtCredit");
	...
	sprintf(szQouery, "SELECT ATDate FROM MEMB_INFO WHERE memb___id = '%s'", szAccountID);	
	...
	lpObj->ATDate = m_MEMBQuery.GetInt("ATDate");
