...
bool GensDBSet::SaveMemberInfo(char * Character, BYTE Influence, int Contribute, int Class)
{
	...
	LogAdd("%s %d %d %d",Character,Influence,Contribute,Class);
	// ----
...
bool GensDBSet::UpdateRanking()
{
	...
	bool res = false;
	CString qSql;
	qSql.Format("EXEC GensRankingUpdate 1");
	...
	CString qSql2;
	qSql2.Format("EXEC GensRankingUpdate 2");
	if (m_DBQuery.Exec(qSql2) == TRUE)
	{
		res = true;
	}
	else 
