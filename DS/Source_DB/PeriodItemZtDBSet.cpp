...
CPeriodItemZtDBSet::CPeriodItemZtDBSet()
{
...
BOOL CPeriodItemZtDBSet::Connect()
{
	...
	{
		//MsgBox("PeriodtemZtDBSet ODBC Connect Fail");
		return FALSE;
	}
...
int CPeriodItemZtDBSet::Insert(DWORD dwUserGUID, char* pchCharacterName, BYTE btItemType, WORD wItemCode, BYTE btEffectCategory, BYTE btEffectType1, BYTE btEffectType2, DWORD dwSerial, DWORD dwDuration, long lBuyDate, long lExpireDate)
{
	int iReturnCode = 0;
...
int CPeriodItemZtDBSet::Delete(DWORD dwUserGUID, char* pchCharacterName, WORD wItemCode, BYTE btItemType, DWORD dwSerial)
{
	CString QuerySql;

	if(SQLSyntztCheck(pchCharacterName) == FALSE)
	{
		return 2;
	}
...
int CPeriodItemZtDBSet::Select(LPPERIOD_ITEMLIST lpPeriodItemList)
{
	int iReturnCode = 0;
	int iItemCount = 0;
	...
	while(i != SQL_NO_DATA && i != SQL_NULL_DATA)
	{
		...
		if(iItemCount >= 100)
		{
			LogAdd("[PeriodItemZt][Select] Period Item List Count Over %d, User Id : %s(%d)", 100, lpPeriodItemList->chCharacterName, lpPeriodItemList->dwUserGuid);
			break;
		}
...
int CPeriodItemZtDBSet::SelectExpiredItemList(DWORD dwUserGuid, char* pchCharacterName, LPPERIOD_EXPIRED_ITEMLIST lpExpiredItemList)
{
	int iReturnCode = 0;
	int iItemCount = 0;
