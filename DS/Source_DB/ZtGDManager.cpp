...
void CZtGDManager::Protocol(BYTE protoNum, BYTE *aRecv, int uIndex)
{
	switch(protoNum)
	{
	case 0x00:
	...
	case 0x01:
		...
		break;
	case 0x02:
		this->DG_ZtBuffLoad((PMSG_REQ_ZTBUFF_LOAD *)aRecv, uIndex);
		break;
	case 0x03:
		this->GD_ZtBuffSave((PMSG_REQ_ZTBUFF_SAVE *)aRecv);
		break;
	case 0x04:
	...
	case 0x11:
		...
		break;
	case 0x14:
		this->DG_ZtQuestSystemLoad((PMSG_REQ_ZTQUESTSYSTEM_LOAD *)aRecv, uIndex);
		break;
	case 0x15:
		this->GD_ZtQuestSystemSave((PMSG_REQ_ZTQUESTSYSTEM_SAVE *)aRecv);
		break;
	}
}
// -------------------------------------------------------------------------------

void CZtGDManager::DG_PartySearchLoad(PMSG_REQ_PARTYSEARCH_LOAD *aRecv, int uIndex)
{
	CString qSql;
	char szName[11];
	...
	pMsg.bResult = true;
	qSql.Format("EXEC PARTYSEARCH_LOAD '%s'", szName);
...
void CZtGDManager::GD_PartySearchSave(PMSG_REQ_PARTYSEARCH_SAVE* aRecv)
{
	CString qSql;
	char szName[11];
	memcpy(szName, aRecv->Name, sizeof(szName)-1);
	szName[10] = 0;
	qSql.Format("EXEC PARTYSEARCH_SAVE '%s', ?", szName);
	m_DBQuery.WriteBlob(qSql, aRecv->btPartySearch, PARTY_SEARCH_SIZE);
}
// -------------------------------------------------------------------------------

void CZtGDManager::DG_ZtBuffLoad(PMSG_REQ_ZTBUFF_LOAD *aRecv, int uIndex)
{
	PMSG_ANS_ZTBUFF_LOAD pMsg;
	...
	qSql.Format("EXEC BUFF_LOAD '%s'", szName);
...
void CZtGDManager::GD_ZtBuffSave(PMSG_REQ_ZTBUFF_SAVE* aRecv)
{
	CString qSql;
	char szName[11];
	memcpy(szName, aRecv->Name, sizeof(szName)-1);
	szName[10] = 0;
	qSql.Format("EXEC BUFF_SAVE '%s', %d, %d, %d, %d, %d, %d", 
...
void CZtGDManager::DG_BMQuestLoad(PMSG_REQ_BMQUEST_LOAD *aRecv, int uIndex)
{
	...
	qSql.Format("EXEC BMQUEST_LOAD '%s'", szName);
...
void CZtGDManager::GD_BMQuestSave(PMSG_REQ_BMQUEST_SAVE* aRecv)
{
	CString qSql;
	char szName[11];
	memcpy(szName, aRecv->Name, sizeof(szName)-1);
	szName[10] = 0;
	qSql.Format("EXEC BMQUEST_SAVE '%s', %d, %d, %d, %d, %d, %d, %d, %d, %d", 
...
void CZtGDManager::DG_MonsterQuestLoad(PMSG_REQ_MONSTERQUEST_LOAD *aRecv, int uIndex)
{
	...
	qSql.Format("EXEC MONSTERQUEST_LOAD '%s'", szName);
...
void CZtGDManager::GD_MonsterQuestSave(PMSG_REQ_MONSTERQUEST_SAVE *aRecv)
{
	CString qSql;
	char szName[11];
	memcpy(szName, aRecv->Name, sizeof(szName)-1);
	szName[10] = 0;
	qSql.Format("EXEC MONSTERQUEST_SAVE '%s', %d, %d, %d, %d, %d, %d", 
...
void CZtGDManager::DG_RageSystemLoad(PMSG_REQ_RAGESYSTEM_LOAD *aRecv, int uIndex)	//незабыть обнулени?рейджа
{
	CString qSql;
	...
	qSql.Format("EXEC RAGELOAD_NEW '%s', '%s'", szAccountID, szName);

	if(m_DBQuery.Exec(qSql) == FALSE || m_DBQuery.Fetch() == SQL_NO_DATA)
	{
		m_DBQuery.Clear();

		qSql.Format("EXEC RAGEINSERT_NEW '%s', '%s'", szAccountID, szName);
...
void CZtGDManager::GD_RageSystemSave(PMSG_REQ_RAGESYSTEM_SAVE *aRecv)
{
	CString qSql;
	...
	qSql.Format("EXEC RAGESAVE_NEW '%s','%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", 
		szAccountID,
		szName,
		aRecv->rLevel,
		aRecv->rPoints,
...
void CZtGDManager::DG_CreateonBankLoad(PMSG_REQ_CREATEON_LOAD *aRecv, int uIndex)
{
	...
	qSql.Format("EXEC CREATEON_LOAD '%s'", szName);
...
void CZtGDManager::GD_CreateonBankSave(PMSG_REQ_CREATEON_SAVE* aRecv)
{
	CString qSql;
	char szName[11];
	memcpy(szName, aRecv->szName, sizeof(szName)-1);
	szName[10] = 0;
	qSql.Format("EXEC CREATEON_SAVE '%s', %d, %d, %d, %d, %d, %d", 
...
void CZtGDManager::GD_TvTInfo(PMSG_TVTGD_INFO* aRecv)
{
	char szSql[128] = { 0 };
	char szName[11] = { 0 };
...
void CZtGDManager::DG_AchievementsLoad(PMSG_GDREQ_ACH_LOAD* aRecv, int uIndex)
{
...
void CZtGDManager::GD_AchievementsSave(PMSG_GDREQ_ACH_SAVE* aRecv)
{
	CString qSql;
	char szName[11];
	szName[10] = 0;
...
void CZtGDManager::GD_OfflineAfk(PMSG_REQ_OFFLINE_AFK* lpMsg)
{
	char szQuery[256];
	char szName[11];
	szName[10] = 0;
...
void CZtGDManager::DG_VoteBuffLoad(PMSG_REQ_VOTEBUFF_LOAD *aRecv, int uIndex)
{
	...
	qSql.Format("EXEC VOTEBUFF_LOAD '%s'", szName);
...
void CZtGDManager::GD_VoteBuffSave(PMSG_REQ_VOTEBUFF_SAVE* aRecv)
{
	CString qSql;
	char szName[11];
	memcpy(szName, aRecv->szName, sizeof(szName)-1);
	szName[10] = 0;
	qSql.Format("EXEC VOTEBUFF_SAVE '%s', %d, %d", szName, aRecv->MTBuffTime, aRecv->QTBuffTime);
	m_DBQuery.Exec(qSql);
	m_DBQuery.Clear();
}
// -------------------------------------------------------------------------------

void CZtGDManager::DG_ZtQuestSystemLoad(PMSG_REQ_ZTQUESTSYSTEM_LOAD *aRecv, int uIndex)
{
	PMSG_ANS_ZTQUESTSYSTEM_LOAD pMsg;
	ZeroMemory(&pMsg, sizeof(pMsg));
	...
	qSql.Format("EXEC EX_ZTQUESTSYSTEM_LOAD '%s'", szName);
	...
	{
		pMsg.ZtWQuestStart[0]    = m_DBQuery.GetInt("ZtQuestStart_1");
		pMsg.ZtWQuestNum[0]      = m_DBQuery.GetInt("ZtQuestNum_1");
		pMsg.ZtWQuestCount[0][0] = m_DBQuery.GetInt("ZtQuestCount1_1");
		pMsg.ZtWQuestCount[0][1] = m_DBQuery.GetInt("ZtQuestCount2_1");
		pMsg.ZtWQuestCount[0][2] = m_DBQuery.GetInt("ZtQuestCount3_1");
		pMsg.ZtWQuestCount[0][3] = m_DBQuery.GetInt("ZtQuestCount4_1");
		pMsg.ZtWQuestCount[0][4] = m_DBQuery.GetInt("ZtQuestCount5_1");

		pMsg.ZtWQuestStart[1]    = m_DBQuery.GetInt("ZtQuestStart_2");
		pMsg.ZtWQuestNum[1]      = m_DBQuery.GetInt("ZtQuestNum_2");
		pMsg.ZtWQuestCount[1][0] = m_DBQuery.GetInt("ZtQuestCount1_2");
		pMsg.ZtWQuestCount[1][1] = m_DBQuery.GetInt("ZtQuestCount2_2");
		pMsg.ZtWQuestCount[1][2] = m_DBQuery.GetInt("ZtQuestCount3_2");
		pMsg.ZtWQuestCount[1][3] = m_DBQuery.GetInt("ZtQuestCount4_2");
		pMsg.ZtWQuestCount[1][4] = m_DBQuery.GetInt("ZtQuestCount5_2");

		pMsg.ZtWQuestStart[2]    = m_DBQuery.GetInt("ZtQuestStart_3");
		pMsg.ZtWQuestNum[2]      = m_DBQuery.GetInt("ZtQuestNum_3");
		pMsg.ZtWQuestCount[2][0] = m_DBQuery.GetInt("ZtQuestCount1_3");
		pMsg.ZtWQuestCount[2][1] = m_DBQuery.GetInt("ZtQuestCount2_3");
		pMsg.ZtWQuestCount[2][2] = m_DBQuery.GetInt("ZtQuestCount3_3");
		pMsg.ZtWQuestCount[2][3] = m_DBQuery.GetInt("ZtQuestCount4_3");
		pMsg.ZtWQuestCount[2][4] = m_DBQuery.GetInt("ZtQuestCount5_3");
	}
...
void CZtGDManager::GD_ZtQuestSystemSave(PMSG_REQ_ZTQUESTSYSTEM_SAVE *aRecv)
{
	CString qSql;
	char szName[11];
	memcpy(szName, aRecv->Name, sizeof(szName)-1);
	szName[10] = 0;
	qSql.Format("EXEC EX_ZTQUESTSYSTEM_SAVE '%s',%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", szName,
		aRecv->ZtWQuestStart[0], aRecv->ZtWQuestNum[0], aRecv->ZtWQuestCount[0][0], aRecv->ZtWQuestCount[0][1], aRecv->ZtWQuestCount[0][2], aRecv->ZtWQuestCount[0][3], aRecv->ZtWQuestCount[0][4],
		aRecv->ZtWQuestStart[1], aRecv->ZtWQuestNum[1], aRecv->ZtWQuestCount[1][0], aRecv->ZtWQuestCount[1][1], aRecv->ZtWQuestCount[1][2], aRecv->ZtWQuestCount[1][3], aRecv->ZtWQuestCount[1][4],
		aRecv->ZtWQuestStart[2], aRecv->ZtWQuestNum[2], aRecv->ZtWQuestCount[2][0], aRecv->ZtWQuestCount[2][1], aRecv->ZtWQuestCount[2][2], aRecv->ZtWQuestCount[2][3], aRecv->ZtWQuestCount[2][4]);
	m_DBQuery.Exec(qSql);
	m_DBQuery.Clear();
}
// -------------------------------------------------------------------------------