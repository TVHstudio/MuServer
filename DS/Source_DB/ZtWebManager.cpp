...
void CZtWebManager::Protocol(BYTE protoNum, BYTE *aRecv, int uIndex)
{
...
void CZtWebManager::GameShopUpdate(GAMESHOP_WEB_UPDATE* lpMsg, int uIndex)
{
	bool Result = false;
	char szQuery[256];
...
void CZtWebManager::CreditsUpdate(CREDIT_WEB_UPDATE* lpMsg, int uIndex)
{
	bool Result = false;
	char szQuery[256];

	sprintf(szQuery, "UPDATE MEMB_INFO SET ZtCredit = %d WHERE memb___id = '%s'", 
...
void CZtWebManager::ReqCredit(CREDIT_WEB_REQ* lpMsg)
{
	CREDIT_WEB_REQ pMsg;
