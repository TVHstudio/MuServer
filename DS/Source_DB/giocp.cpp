...
#if(THREAD_FIFTH)
DWORD	WINAPI ServerRecvWorkerThreadFifth(LPVOID p);
DWORD	WINAPI ServerRecvWorkerThreadSix(LPVOID p);
#endif
...
CQueue g_ServerQueue;
HANDLE g_ServerQueueSemaphore;
HANDLE g_ServerQueueThread;

DWORD WINAPI ServerQueueThread() // OK
{
	while(true)
	{
		if(WaitForSingleObject(g_ServerQueueSemaphore, INFINITE) == WAIT_FAILED)
		{
			LogAddC(LOGC_RED,"[SocketManager] WaitForSingleObject() failed with error: %d", GetLastError());
			break;
		}

		static QUEUE_INFO QueueInfo;

		if(g_ServerQueue.GetFromQueue(&QueueInfo) != 0)
		{
			//if(SERVER_RANGE(QueueInfo.index) != 0 && gServerManager[QueueInfo.index].CheckState() != 0)
			if(gSObj[QueueInfo.index].Connected != 0)
			{
				ProtocolCore(QueueInfo.head, QueueInfo.buff, QueueInfo.size, QueueInfo.index);
...
DWORD GetQueueSize() // OK
{
	return g_ServerQueue.GetQueueSize();
}

bool CreateServerQueue() // OK
{
	if((g_ServerQueueSemaphore = CreateSemaphore(0, 0, MAX_QUEUE_SIZE, 0)) == 0)
	{
		LogAddC(LOGC_RED, "[SocketManager] CreateSemaphore() failed with error: %d", GetLastError());
		return 0;
	}

	if((g_ServerQueueThread = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)ServerQueueThread, NULL, 0, 0)) == 0)
	{
		LogAddC(LOGC_RED,"[SocketManager] CreateThread() failed with error: %d",GetLastError());
		return 0;
	}

	if(SetThreadPriority(g_ServerQueueThread,THREAD_PRIORITY_HIGHEST) == 0)
	{
		LogAddC(LOGC_RED,"[SocketManager] SetThreadPriority() failed with error: %d",GetLastError());
		return 0;
	}
...
int CreateGIocp(int server_port)
{
	...
	if ((ThreadHandle = CreateThread(NULL, 0, IocpServerWorker, (LPVOID)NULL, 0, &ThreadID)) == NULL)
	{
		LogAddC(LOGC_RED, "CreateThread() failed with error %d", GetLastError());
		return FALSE;
	}
...
{	
	...
	{
		LogAddC(LOGC_RED, "bind() failed with error %d", WSAGetLastError());
		return FALSE;
	}
	...
	if (nRet == SOCKET_ERROR)
	{
		LogAddC(LOGC_RED, "listen() failed with error %d", WSAGetLastError());
		return FALSE;
	} 
	return TRUE;
}
...
DWORD WINAPI IocpServerWorker(LPVOID p)
{
	...
	if(g_dwThreadCount > MAX_WORKER_THREAD) //webzen season 6.2 fix
	{
		...
		LogAddC(LOGC_RED, " **** Worker Thread Max Count Fix 16ea ****");
	}
    ...
    { 
		...
		{
			LogAddC(LOGC_RED, "CreateIoCompletionPort failed with error: %d", GetLastError());
			__leave;
		}
		...
		{
			LogAddC(LOGC_RED, "SetThreadPriority() failed with error %d", GetLastError());
		}

		ThreadHandle = CreateThread(NULL, 0, ServerRecvWorkerThreadSecond, (LPVOID)NULL, 0, &ThreadID); //webzen season6.2 add-on
		if (ThreadHandle == NULL)
		{
			LogAddC(LOGC_RED, "RecvWorkerthread Second CreateThread() failed with error %d", GetLastError());
			__leave;
		}
		...
		if (ThreadHandle == NULL)
		{
			LogAddC(LOGC_RED, "RecvWorkerthread Third CreateThread() failed with error %d", GetLastError());
			__leave;
		}
		...
		if (ThreadHandle == NULL)
		{
			LogAddC(LOGC_RED, "RecvWorkerthread Fourth CreateThread() failed with error %d", GetLastError());
			__leave;
		}

		#if(THREAD_FIFTH)
		ThreadHandle = CreateThread(NULL, 0, ServerRecvWorkerThreadFifth, (LPVOID)NULL, 0, &ThreadID); //ztteam multi emulator add-on
		if (ThreadHandle == NULL)
		{
			LogAddC(LOGC_RED, "RecvWorkerthread Fifth CreateThread() failed with error %d", GetLastError());
			__leave;
		}

		ThreadHandle = CreateThread(NULL, 0, ServerRecvWorkerThreadSix, (LPVOID)NULL, 0, &ThreadID); //ztteam multi emulator add-on
		if (ThreadHandle == NULL)
		{
			LogAddC(LOGC_RED, "RecvWorkerthread Six CreateThread() failed with error %d", GetLastError());
			__leave;
		}
		...
		while(TRUE)		
		{
			...
			if (Accept==SOCKET_ERROR)
			{
				...
				LogAddC(LOGC_RED, "WSAAccept() failed with error %d", WSAGetLastError());
			...
			{
				EnterCriticalSection(&criti);
				LogAddC(LOGC_RED, "setsockopt() failed with error %d", WSAGetLastError());
				LeaveCriticalSection(&criti);
				__leave;
			}
			...
			if(g_WZIPCheck.CheckIP(inet_ntoa(cInAddr)) == FALSE) //webzen season6.2 add-on
			{
				closesocket(Accept);
				LeaveCriticalSection(&criti);
				LogAddC(LOGC_RED, "[ANTI-HACKING] Attempted to connect illegal IP : %d.%d.%d.%d ", cInAddr.S_un.S_un_b.s_b1, cInAddr.S_un.S_un_b.s_b2, cInAddr.S_un.S_un_b.s_b3, cInAddr.S_un.S_un_b.s_b4);
				continue;
			}
			...
			if( ClientIndex == -1 ) 
			{
				LogAddC(LOGC_RED, "Error : Max User Index");
				closesocket(Accept);
				LeaveCriticalSection(&criti);
				continue;
			}
			...
			{
				LogAddC(LOGC_RED, "WSARecv() failed with error %d", WSAGetLastError());
...
DWORD WINAPI ServerRecvWorkerThread(LPVOID p)
{
	...
	while(TRUE)
	{
		...
		{
			loopN = MAX_NODE-1;
			LogAddC(LOGC_RED, "error : Loop MAX %s %d", __FILE__, __LINE__);
		}
		LeaveCriticalSection(&criti);
		if( loopN > 0 )
		{
...
DWORD WINAPI ServerWorkerThread(LPVOID CompletionPortID)
{
	...
	while(TRUE)
	{
		...
		{
			EnterCriticalSection(&criti);
			LogAddC(LOGC_RED, "GetQueuedCompletionStatus failed with error %d", GetLastError() );
			CloseClient(lpPerSocketContext, FALSE);
			LeaveCriticalSection(&criti);
			continue;
		}
		...
		{
			LogAddC(LOGC_RED, "GetQueuedCompletionStatus dwIoSize is zero (%d)", GetLastError() );
			CloseClient(lpPerSocketContext, FALSE);
			LeaveCriticalSection(&criti);
			continue;
		}
...
BOOL RecvDataParse(LPPER_IO_CONTEXT	lpIOContext, int uIndex)
{
	...
	while(1)
	{
		...
		{
			LogAddC(LOGC_RED, "error-L1 : Header error (%s %d)lOfs:%d, size:%d", __FILE__, __LINE__, lOfs, lpIOContext->nSentBytes);
			lpIOContext->nSentBytes = 0;
			return FALSE;
		}
		if( size <= 0 )
		{
			LogAddC(LOGC_RED, "error : size가 %d이다.", size);
			return FALSE;
		}
		if( size <= lpIOContext->nSentBytes )	// 데이터가 크기만큼 존재하면..
		{
			...
			QueueInfo.index = uIndex;

			QueueInfo.head = headcode;

			memcpy(QueueInfo.buff, &recvbuf[lOfs], size);

			QueueInfo.size = size;

			if(g_ServerQueue.AddToQueue(&QueueInfo) != 0)
			{
				ReleaseSemaphore(g_ServerQueueSemaphore,1,0);
			}
			...
			{
			#if(THREAD_FIFTH)
			case 0x06:	//-> GJPGetCharacterInfo
			case 0x07:  //-> CharacterSave
				WzRecvQ6.AddToQueue((LPBYTE)(recvbuf+lOfs), size, headcode, uIndex, g_DelayHandler.GetQuerySessionId());
				break;
			case 0xE2:	//-> ZtTeam Custom
				WzRecvQ5.AddToQueue((LPBYTE)(recvbuf+lOfs), size, headcode, uIndex, g_DelayHandler.GetQuerySessionId());
				break;
			#endif
			case 0xF8:	//-> Gens
			case 0xD1:	//-> LuckyItem & PeriodItem
			case 0x08:	//-> GDPGetWarehouse
			case 0x09:  //-> GDPSetWarehouseSave
			case 0x12:	//-> GDPSetWarehouseMoney	
				WzRecvQ4.AddToQueue((LPBYTE)(recvbuf+lOfs), size, headcode, uIndex, g_DelayHandler.GetQuerySessionId());
				break;
			case 0x52:	//-> ItemCreate
			case 0x54:	//-> GDPropertyItemCreate
			case 0x55:	//-> PetItemCreate
				WzRecvQ3.AddToQueue((LPBYTE)(recvbuf+lOfs), size, headcode, uIndex, g_DelayHandler.GetQuerySessionId());
				break;
			case 0xD0:	//-> CashShop
			case 0xD2:	//-> PeriodItem
			case 0xD3:  //-> GameShopStorage
			...
			if(headcode == 0x52 || 
			   headcode == 0x54 || 
			   headcode == 0x55)	//Create Item
			{
				WzRecvQ3.AddToQueue((LPBYTE)(recvbuf+lOfs), size, headcode, uIndex, g_DelayHandler.GetQuerySessionId());
			}
			else if(headcode == 0xF6 || //empty
					headcode == 0xF7)	//emptry
			{
				WzRecvQ2.AddToQueue((LPBYTE)(recvbuf+lOfs), size, headcode, uIndex, g_DelayHandler.GetQuerySessionId());	WzRecvQ2.qsize += size;
			}
			else if(headcode == 0x13 || //class quest
					headcode == 0x14)	//switch character & class quest
			{
				WzRecvQ4.AddToQueue((LPBYTE)(recvbuf+lOfs), size, headcode, uIndex, g_DelayHandler.GetQuerySessionId());
			}
			else
		...
		{	// recv 데이터가 아직 완성되지 않았다면..
			if( lOfs > 0 )  // 하나이상 데이터를 보낸 후라면..
			{
				...
				{
					LogAddC(LOGC_RED, "error : recvbuflen 이 1보다 작다..");
					break;
				}
				else 
				{
					if( lpIOContext->nSentBytes < MAX_BUFF_SIZE )
					{
						...
						LogAddC(LOGC_RED, "error : copy %d",lpIOContext->nSentBytes);
					}
					break;
				}
			}
...
{	
	...
	{
		LogAddC(LOGC_RED, "Error : Max msg %s %d", __FILE__, __LINE__);
		LeaveCriticalSection(&criti);
		//CloseClient( aIndex );
		return FALSE;
	}
	...
	if(lpPerSocketContext == NULL)
	{
		LogAddC(LOGC_RED, "error : user socket context buffer free.");
		LeaveCriticalSection(&criti);
		return FALSE;
	}

	//LogAddC(LOGC_RED, "Head: 0x%02X", lpMsg[0]);
		...
		&(lpIoCtxt->Overlapped), NULL) == SOCKET_ERROR)
	{
		...
		{
			LogAddC(LOGC_RED, "WSASend() failed with error %d", WSAGetLastError());			
			//lpIoCtxt->nWaitIO = 3;
			//CloseClient( aIndex );
			LeaveCriticalSection(&criti);
			return FALSE;
...
{ //completed
	...
	{
		LogAddC(LOGC_RED, "HeapAlloc PER_SOCKET_CONTEXT Failed: %d", GetLastError());
	}
...
DWORD WINAPI ServerRecvWorkerThreadSecond(LPVOID p)
{
	...
	while(TRUE)
	{
		...
		{
			loopN = MAX_NODE-1;
			LogAddC(LOGC_RED, "error : Q2 Loop MAX %s %d", __FILE__, __LINE__);
		}
		LeaveCriticalSection(&criti);
		if( loopN > 0 )
		{
...
DWORD WINAPI ServerRecvWorkerThreadThird(LPVOID p)
{
	...
	while(TRUE)
	{
		...
		{
			loopN = MAX_NODE-1;
			LogAddC(LOGC_RED, "error : Q3 Loop MAX %s %d", __FILE__, __LINE__);
		}
		LeaveCriticalSection(&criti);
		if( loopN > 0 )
		{
...
DWORD WINAPI ServerRecvWorkerThreadFourth(LPVOID p)
{
	...
	while(TRUE)
	{
		...
		{
			loopN = MAX_NODE-1;
			LogAddC(LOGC_RED, "error : Q4 Loop MAX %s %d", __FILE__, __LINE__);
		}
		LeaveCriticalSection(&criti);
		if( loopN > 0 )
		{
...
DWORD WINAPI ServerRecvWorkerThreadFifth(LPVOID p)
{
	...
	{
		EnterCriticalSection(&criti);
		loopN = WzRecvQ5.GetCount();	

		if( loopN > MAX_NODE-1 )
		{
			loopN = MAX_NODE-1;
			LogAddC(LOGC_RED, "error : Q5 Loop MAX %s %d", __FILE__, __LINE__);
		}
		...
		if(loopN > 0)
		{
			if( WzRecvQ5.GetFromQueue((LPBYTE)RecvData, &nSize, &headcode, &uindex, &iSessionId) == TRUE )
			{
				bProcessPacket = true;
				if(iSessionId != g_DelayHandler.GetQuerySessionId())
				{
...
DWORD WINAPI ServerRecvWorkerThreadSix(LPVOID p)
{
	...
	{
		EnterCriticalSection(&criti);
		loopN = WzRecvQ6.GetCount();	

		if( loopN > MAX_NODE-1 )
		{
			loopN = MAX_NODE-1;
			LogAddC(LOGC_RED, "error : Q6 Loop MAX %s %d", __FILE__, __LINE__);
		}
		...
		if(loopN > 0)
		{
			if( WzRecvQ6.GetFromQueue((LPBYTE)RecvData, &nSize, &headcode, &uindex, &iSessionId) == TRUE )
			{
				bProcessPacket = true;
				if(iSessionId != g_DelayHandler.GetQuerySessionId())
				{
