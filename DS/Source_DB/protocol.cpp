...
void GDBanCharacter(BAN_CHAR_GS * lpMsg, int aIndex);
...
void GD_SetPartyCount(GD_PMSG_PARTYCOUNT* aRecv);
#endif
...
} SDHP_GETCHARLIST_MULTI, *LPSDHP_GETCHARLIST_MULTI;
...
typedef struct
{
	...
	BYTE		ClassSkin;
	int			Page;
} SDHP_CREATECHAR_MULTI, *LPSDHP_CREATECHAR_MULTI;
...
typedef struct
{
	...
	int			Page;
} SDHP_CHARDELETE_MULTI, *LPSDHP_CHARDELETE_MULTI;

void GJPCharacterListRequestCS_Multi(LPSDHP_GETCHARLIST_MULTI lpCLMsg, int aIndex);
void GJPCharacterCreate_Multi(LPSDHP_CREATECHAR_MULTI lpMsg, short aIndex);
void GJPCharacterDelete_Multi(LPSDHP_CHARDELETE_MULTI lpMsg, short aIndex);
...
//#ifdef _RAGE_SYSTEM_
	case 0xE0:
		...
		break;
	case 0xE1:
		gRageSystem.SaveRageSkill((DBRageUserSave *)aRecv);
		break;
//#endif
	case 0xF8: //-> Gens
		{
	...
	case 0xE2:
		{
			...
			{
			case 0x00:
				GDBanCharacter((BAN_CHAR_GS *)aRecv, aIndex);
				break;
			case 0x01:
				gWinQuest.GetMemberInfo((GDSelectQuest *)aRecv,aIndex);
				break;
			case 0x02:
				gWinQuest.UpdateMemberInfo((GDQuestUser *)aRecv);
				break;
			case 0x03:
				gRanking.CharacterRanking(aIndex);
				break;
			case 0x04:
				gRanking.RecvBrother((GD_CheckBrother*) aRecv, aIndex);
				break;
#if(CUSTOM_RETURNSPOT==TRUE)
			case 0x05:
				g_SpotReturn.GDStatus((PMSG_GD_SPOTSTATUS*)aRecv, aIndex);
				break;
			case 0x06:
				g_SpotReturn.GDStatusUpdate((PMSG_GD_SPOTSTATUS_UPDATE*)aRecv);
				break;
#endif
			case 0x07:
#if(CUSTOM_SAVE_PT==TRUE)
				GD_SetPartyCount((GD_PMSG_PARTYCOUNT*) aRecv);
#endif
				break;
			}
		}
...
#if(ZT_LICENSE==TRUE)
			case 0x00:
				g_ZtLicense.GDLicenseUser((PMSG_LICENSE_DATA*)aRecv);
				break;
#endif
	...
	case 0xEA:
		{
			PMSG_DEFAULT2* lpDef = (PMSG_DEFAULT2*)aRecv;
			g_ZtGDManager.Protocol(lpDef->subcode, aRecv, aIndex);
		}
		break;
	case 0xEB:
		{
			...
			{
			case 0x00:
				g_Marriage.GDLoad((PMSG_REQ_MARRIAGE_LOAD*) aRecv, aIndex);
				break;
			case 0x01:
				g_Marriage.GDInset((PMSG_REQ_MARRIAGE_INSERT*) aRecv, aIndex);
				break;
			case 0x02:
				g_Marriage.GDDelete((PMSG_REQ_MARRIAGE_DELETE*) aRecv, aIndex);
				break;
#if(EVENT_DUNGEON_SIEGE)
			case 0x03:
				g_DungeonSiege.GDRegSiegeList(aIndex);
				break;
			case 0x04:
				g_DungeonSiege.GDRegSiege((PMSG_GD_DUNGEON_SIEGE_GUILD_REG*) aRecv, aIndex);
				break;		
			case 0x05:
				g_DungeonSiege.GDSiegeClearList();
				break;	
			case 0x06:
				g_DungeonSiege.GDSiegeOwner(aIndex);
				break;	
			case 0x07:
				g_DungeonSiege.GDSiegeOwnerUpdate((PMSG_GD_DUNGEON_SIEGE_UPDATE_OENER*)aRecv, aIndex);
				break;
#endif
			case 0x11:
				#if(OFFLINE_MODE_RESTORE)
				g_OfflineMode.GDReqAllPlayer(aIndex);
				#endif
				break;
			case 0x12:
				#if(OFFLINE_MODE_RESTORE)
				g_OfflineMode.GDReqOffCharacter((PMSG_GDREQ_OFFMODE_CHAR*)aRecv, aIndex);
				#endif
				break;
			case 0x13:
				#if(OFFLINE_MODE_RESTORE)
				g_OfflineMode.UpdateStatus((PMSG_GDREQ_OFFMODE_STATUS*)aRecv, aIndex);
				#endif
				break;
			case 0x14:
				g_ReferralSystem.GDReqMasterInfo((PMSG_GDREQ_REFERRAL_INFO *)lpMsg, aIndex);
				break;
			case 0x15:
				g_ReferralSystem.GDReqMasterUpdate((PMSG_GD_REFERRAL_MASTER_UPDATE *)lpMsg, aIndex);
				break;
			case 0x16:
				g_ReferralSystem.GDReqUserInfo((PMSG_GDREQ_REFERRAL_USER_INFO *)lpMsg, aIndex);
				break;
			case 0x17:
				g_ReferralSystem.GDReqUserUpdate((PMSG_GD_REFERRAL_USER_UPDATE *)lpMsg, aIndex);
				break;
	#if(DEV_BANSYSTEM)
			case 0x20:
				g_BanSystem.SelectTable((PMSG_REQ_BANSYSTEM *)lpMsg, aIndex);
				break;
			case 0x21:
				g_BanSystem.Insert((PMSG_INSERT_BANSYSTEM *)lpMsg);
				break;
			case 0x22:
	...
	case 0xEC:
		{
			...
			PMSG_DEFAULT2* lpDef = (PMSG_DEFAULT2*)aRecv;
			//g_ZtGDManager.Protocol(lpDef->subcode, aRecv, aIndex);
			g_ItemMarket.Protocol(lpDef->subcode, aRecv, aIndex);
			#endif
		}
		break;
	case 0xED:
		#if(OFFLINE_MODE_RESTORE)
		g_OfflineMode.UpdateData((PMSG_GDSAVE_OFFMODE_DATA*) aRecv, aIndex);
		#endif
		break;
	case 0xEE:
		{
			PMSG_DEFAULT2* lpDef = (PMSG_DEFAULT2*)aRecv;
			g_ZtWebManager.Protocol(lpDef->subcode, aRecv, aIndex);
		}
		break;
	case 0xEF:
		{
			PMSG_DEFAULT2 * lpDef = (PMSG_DEFAULT2 *)aRecv;
			g_AccountSecurity.Protocol(lpDef->subcode, aRecv, aIndex);
		}
		break;
	case 0xE9:
		{
			PMSG_DEFAULT2 * lpDef = (PMSG_DEFAULT2 *)aRecv;
			//g_Referral.Protocol(lpDef->subcode, aRecv, aIndex);
		}
		break;
	//--
	case Protocol_GSSend_DelGuildWarehouseList:
		g_GuildBank.GDDelGuildWarehouseList((SDHP_GETGUILDWAREHOUSEDB *) aRecv);
		break;
	case Protocol_GSSend_SetGuildWarehouseList:
		g_GuildBank.GDSetGuildWarehouseList((SDHP_GETGUILDWAREHOUSEDB_SAVE *) aRecv);
		break;
	case Protocol_GSSend_GetGuildWarehouseList:
		g_GuildBank.DGGetGuildWarehouseList(aIndex, (SDHP_GETGUILDWAREHOUSEDB *) aRecv);
		break;
	//--
	#if(ENABLE_MULTI_CHAR)
	case 0xF0:
		GJPCharacterListRequestCS_Multi((LPSDHP_GETCHARLIST_MULTI)aRecv, aIndex);
		break;
	case 0xF1:
		GJPCharacterCreate_Multi((LPSDHP_CREATECHAR_MULTI)aRecv, aIndex);
		break;
	case 0xF2:
		GJPCharacterDelete_Multi((LPSDHP_CHARDELETE_MULTI)aRecv, aIndex);
		break;
		#endif
	}
}
...
void GetJoinInfo(LPSDHP_SERVERINFO lpMsg, short aIndex)
{
...
#if(CUSTOM_SAVE_PT==TRUE)
	pResult.PartyCount = gGSDbSet.GetPartyCount();
#endif

#if(ENABLE_SOCKET_MANAGER)

	gServerManager[aIndex].SetServerInfo(lpMsg->ServerName,lpMsg->Port,lpMsg->ServerCode);
	...
	{
		pResult.Result = 0;
		LogAddC(LOGC_RED, "Client Join Fail %s %d", __FILE__, __LINE__);
	}
...
void GJPCharacterListRequestCS(LPSDHP_GETCHARLIST lpCLMsg, int aIndex)
{
	...
	Result.bSummoner = 1; //gACDbSet.GetSummonerCardInfo(Result.AccountId);
...
void GJPCharacterListRequestCS_Multi(LPSDHP_GETCHARLIST_MULTI lpCLMsg, int aIndex)
{
	...
	if(lpCLMsg->Page == 1)
	{
		GJPCharacterListRequestCS((LPSDHP_GETCHARLIST)lpCLMsg, aIndex);
		return;
	}
	...
	if(result == 1)
	{
		if(gACDbSet.GetAccountId_Multi(szId, lpCLMsg->Page) == 1)
		{
			result = 1;
		}
		else
		{
			...
			if(gACDbSet.CreateAccountCharacter_Multi(szId, lpCLMsg->Page) == 1)
			{
				result = 1;
			}
		}
	...
	if(result == 1)
	{
		...
		if(gACDbSet.GetAccountInfo_Multi(szId, &ACInfo, lpCLMsg->Page) == FALSE)
		{
			result = 0;
		}
		else
...
void GDCharacterClose(LPSDHP_USERCLOSE lpMsg, short aIndex)
{
	...
	for(int n=0;n < MAX_SERVER;n++)
	{
		...
		{
			gSocketManager.DataSend(n,(BYTE*)&lpMsg,lpMsg->h.size);
		}
	}
...
void GJPCharacterCreate(LPSDHP_CREATECHAR lpMsg, short aIndex)
{
	...
	if(CurCharCount < 0)
	{
		LogAddC(LOGC_RED, "error-L3 : Character Max Over :%d User(%s)", 5, accountId);
		Result = 2;
	}
	...
	if ( Result == 1 )
	{
		...
		if ( defclass > 7 )
		{
			LogAddC(LOGC_RED, "error-L3 : Class over value %s %d",__FILE__, __LINE__);
			defclass = 1;
		}
		...
		//lpItem->m_Number = s_num;
		//ItemByteConvert16(tCharInfo.dbInventory, lpItem, MAX_EQUIPMENT);
		//DCInfo.DefClass[defclass].Equipment[MAX_EQUIPMENT].
		ItemByteConvert16(tCharInfo.dbInventory, DCInfo.DefClass[defclass].Equipment, MAX_EQUIPMENT);
		#else
	...
	{
		LogAddC(LOGC_RED, "[ERROR!!] CreateCharacter Fail Result : %d", pMsgResult.Result);
	}
...
void GJPCharacterCreate_Multi(LPSDHP_CREATECHAR_MULTI lpMsg, short aIndex)
{
	if(lpMsg->Page == 1)
	{
		GJPCharacterCreate((LPSDHP_CREATECHAR)lpMsg, aIndex);
		return;
	}
	...
	CurCharCount = gACDbSet.GetAccountCharBlank_Multi(accountId, lpMsg->Page);
	...
	if ( Result == 1 )
	{
		...
		pMsgResult.Result = gCharDbSet.CreateCharacter_Multi(accountId, szId, lpMsg->ClassSkin, lpMsg->Page);
	...
	if ( pMsgResult.Result == 1 )
	{
		gACDbSet.SaveAccountCharacter_Multi(accountId, CurCharCount, szId, lpMsg->Page);
	}
	else
	{
		LogAddC(LOGC_RED, "[ERROR!!] CreateCharacter Fail Result : %d", pMsgResult.Result);
...
void GJPCharacterDelete(LPSDHP_CHARDELETE lpMsg, short aIndex)
{
	...
	if ( SQLSyntztCheck(szAccountId) == FALSE || SQLSyntztCheck(szCharName) == FALSE)
	{
		pResult.Result = 0;
	}
...
void GJPCharacterDelete_Multi(LPSDHP_CHARDELETE_MULTI lpMsg, short aIndex)
{
	if(lpMsg->Page == 1)
	{
		GJPCharacterDelete((LPSDHP_CHARDELETE)lpMsg, aIndex);
		return;
	}
	...
	if ( pResult.Result == 1 )
	{
		...
		{
			if( gACDbSet.DeleteAccountCharacter_Multi(szAccountId, szCharName, lpMsg->Page) >= 0 )
			{
				pResult.Result = 1;
			}
...
void GDPGetWarehouse(LPSDHP_GETWAREHOUSEDB lpMsg, int aIndex)
{
	...
	CWhDBSet.Load(szId, pMsg.dbItems, pMsg.Money, pMsg.DbVersion, pMsg.pw, lpMsg->Number);
...
void GDPSetWarehouse(LPSDHP_GETWAREHOUSEDB_SAVE lpMsg, int aIndex)
{
	...
	CWhDBSet.Save(szId, lpMsg->dbItems, lpMsg->Money, lpMsg->pw, lpMsg->Number);
}
...
#endif
			pChar.DonateCredit = CharObj.DonateCredit;
			pChar.ATDate = CharObj.ATDate;
...
void GJPSetCharacterInfo(LPSDHP_DBCHAR_INFOSAVE lpMsg, short aIndex)
{
	...
	{
		LogAddC(LOGC_RED, "error-L2 : Name is zero %s %d", __FILE__, __LINE__);
		return;
	}
	...
	CharObject.Brother[MAX_IDSTRING] = '\0';
	memcpy(CharObject.Brother, lpMsg->Brother, MAX_IDSTRING);
	...
	CharObject.PCPoint = lpMsg->PCPoint;
	CharObject.ZtFreePoints = lpMsg->ZtFreePoints;
	CharObject.Reset = lpMsg->Reset;
	CharObject.GReset = lpMsg->GReset;
	CharObject.ZtQuestNum = lpMsg->ZtQuestNum;
	CharObject.ZtQuestKill = lpMsg->ZtQuestKill; 
	CharObject.PremiumTime = lpMsg->PremiumTime;
#ifdef CONFLICT_PREM_TYPE
	CharObject.PremiumTimeType = lpMsg->PremiumTimeType;
#endif
	CharObject.ZtCred = lpMsg->ZtCred;
	CharObject.BanChat = lpMsg->BanChat;
	CharObject.BanPost = lpMsg->BanPost;
	CharObject.BanChar = lpMsg->BanChar;
	...
	CharObject.AutoParty = lpMsg->AutoParty;
	CharObject.Zen = lpMsg->Zen;
	CharObject.QTDate = lpMsg->QTDate;
#if(CUSTOM_SAVE_PT==1)
	CharObject.PartyIndex = lpMsg->PartyIndex;
#endif
#ifdef S6E3
	...
	CharObject.ATDate = lpMsg->ATDate;
...
void GDLoadMedalInfo(LPSDHP_LOAD_MEDALINFO lpMsg, int aIndex)
{
	...
	{
		LogAddC(LOGC_RED, "error-L2 : GetPropertyItemInfo() serial:%u index: %d", lpMsg->dwItemSerial, lpMsg->wIndex);
	}
}
...
void GDItemCreate(LPSDHP_ITEMCREATE lpMsg, int aIndex)
{
		...
		lpMsg->Type == ITEM_POTION+31 )
	{
		...
		ITEM_SERIAL_LOG.Output("[Item Serial] [%s][%d] Type(%d) Serial(%u)",
			gServerManager[aIndex].m_IpAddr, gServerManager[aIndex].m_ServerPort, lpMsg->Type,s_num);
#else
...
void GDPropertyItemCreate(LPSDHP_PROPERTY_ITEMCREATE lpMsg, int aIndex)
{
	...
	{
		LogAddC(LOGC_RED, "error-L1 : CreatePropertyItemInfo %u ", s_num);
	}
...
void GDPetItemCreate(LPSDHP_PET_ITEMCREATE lpMsg, int aIndex)
{
	...
	{
		LogAddC(LOGC_RED, "error-L1 : CreatePetItemInfo %u ", s_num);
	}
...
void GDReqCrywolfSync(BYTE *lpRecv, int aIndex)
{
	...
	for(int n=0;n < MAX_SERVER;n++)
	{
		if(gServerManager[n].CheckState() != 0)
		{
			gSocketManager.DataSend(n,(BYTE*)&pMsgSend, sizeof(CWP_ANS_CRYWOLFSYNC));
		}
	}
...
void DGChangeName(LPSDHP_CHANGE_NAME lpMsg, int aIndex)
{
	...
	if ( SQLSyntztCheck(NewName) == FALSE)
	{
		Result = 1;
	}
...
void DGBroadCastGlobalMessage(LPBYTE lpData)
{
	...
	for(int n=0;n < MAX_SERVER;n++)
	{
		if(gServerManager[n].CheckState() != 0)
		{
			gSocketManager.DataSend(n,(BYTE*)&res,sizeof(MSG_SYS_BROADCAST));
		}
	}
...
void GDReqWcoinInfo(LPREQ_WCOIN_INFO lpMsg, int aIndex)	//old ztteam season 4 
{
	...
	pMsgSend.h.set((LPBYTE)&pMsgSend, 0xE6, sizeof(ANS_WCOIN_INFO));
	pMsgSend.iUserIndex = lpMsg->iUserIndex;
	pMsgSend.btResult = 1;
...
void GensGetMemberInfo(PMSG_GENS_USERINFO_REQ * aRecv, int aIndex)
{
	...
	{
		LogAddC(LOGC_RED, "[%s] Failed to GetMemberInfo", __FUNCTION__);
		return;
	}
	// ----
...
void GensSaveMemberInfo(PMSG_GENS_SAVEINFO * aRecv, int aIndex)
{
	...
	{
		LogAdd("[%s] Failed to SaveMemberInfo", __FUNCTION__);
		return;
	}
	// ----
...
void GensRegMemberInfo(PMSG_GENS_REGUSER_REQ * aRecv, int aIndex)
{
	...
	{
		LogAddC(LOGC_RED, "[%s] Failed to RegMemberInfo", __FUNCTION__);
		Result.Result = true;
	}
	// ----
	Result.Influence	= aRecv->Influence;
...
void GensDelMemberInfo(PMSG_GENS_DELUSER_REQ * aRecv, int aIndex)
{
	...
	{
		LogAddC(LOGC_RED, "[%s] Failed to DelMemberInfo", __FUNCTION__);
		Result.Result = true;
	}
	// ----
	Result.Result = false;
...
void GensGetRewardResult(PMSG_GENS_REWARD_REQ * aRecv, int aIndex)
{
	...
	{
		LogAdd("[%s] Reward false", __FUNCTION__);
		Result.Result = false;
	}
	// ----
...
void GensSetRewardDate(PMSG_GENS_REWARD_SET * aRecv, int aIndex)
{
	...
	{
		LogAddC(LOGC_RED, "[%s] Error update reward date ", __FUNCTION__);
	}
}
...
void GDGameShopReqPoint(GAMESHOP_GD_REQ_POINT * aRecv, int aIndex)
{
	...
	{
		LogAddC(LOGC_RED, "[%s] Error", __FUNCTION__);
		return;
	}
#endif
	// ----
...
void GDReqPeriodItemExInsert(BYTE *lpRecv, int aIndex)
{
	...
	pMsg.btResultCode = PeriodItemZtDBSet.Insert(lpMsg->dwUserGuid, lpMsg->chCharacterName, lpMsg->btItemType, lpMsg->wItemCode, lpMsg->btOptionType, lpMsg->btEffectType1, lpMsg->btEffectType2, lpMsg->dwSerial, lpMsg->dwDuration, lpMsg->lBuyDate, lpMsg->lExpireDate);
	pMsg.dwUserGuid = lpMsg->dwUserGuid;
...
void GDReqPeriodItemExDelete(BYTE *lpRecv, int aIndex)
{
	...
	pMsg.btResultCode = PeriodItemZtDBSet.Delete(lpMsg->dwUserGuid, lpMsg->chCharacterName, lpMsg->wItemCode, lpMsg->btItemType, lpMsg->dwSerial);
	pMsg.dwUserGuid = lpMsg->dwUserGuid;
	pMsg.wUserIndex = lpMsg->wUserIndex;
...
void GDReqPeriodItemExSelect(BYTE *lpRecv, int aIndex)
{
	...
	/*int iReturnCode = 0;
	iReturnCode = PeriodItemZtDBSet.SelectExpiredItemList(lpMsg->dwUserGuid, lpMsg->chCharacterName, &ExpiredItemList);
	...
	pListCount.btResult = PeriodItemZtDBSet.Select(&ItemData);
	...
	for(int iItemCount = 0; iItemCount < pListCount.btListCount; iItemCount++)
	{
		...
		LogAdd("[%d] [%d]", pList.wItemCode, pList.dwSerial);
...
{
	BAN_CHAR_DS pMsg;
	pMsg.h.set((LPBYTE)&pMsg, 0xE2, 0x00, sizeof(BAN_CHAR_DS));
	pMsg.Result = gCharPreDBSet.BanCharacter(lpMsg->Name, lpMsg->Type);
	pMsg.Type = lpMsg->Type;
	pMsg.aIndex = lpMsg->aIndex;
	memcpy(pMsg.Name, lpMsg->Name, MAX_IDSTRING+1);
	wsjServer.DataSend(aIndex, (char*)&pMsg, sizeof(BAN_CHAR_DS));
}
...
void GDSetExtendedWarehouseCount(LPSDHP_REQ_SET_EXTENDEDWAREHOUSE_COUNT lpMsg, int aIndex)
{
	...
	if( (lpMsg->ItemPos > 203 || lpMsg->ItemPos < 0) && lpMsg->BuyAtInGameShop == 0 && lpMsg->IsReplace == 0)
	{
		...
		LogAdd("Extend Warehouse fail %d %d", lpMsg->ItemPos, pMsg.ExtendedWarehouseCount);
		return;
	}
...
void GD_SetPartyCount(GD_PMSG_PARTYCOUNT* aRecv)
{
	unsigned int ptcount = aRecv->PartyCount;
	gGSDbSet.SetPartyCount(ptcount);
}
#endif
