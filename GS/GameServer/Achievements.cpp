...
void CAchievements::Load()
{
	...
	this->Init();
	this->Read(gDirPath.GetNewPath("ZtData\\Achievements.ini"));
...
#endif
#if(GAMESERVER_ACHIVEMENTS==ZTTEAM_SERVER)
		if(ireward > 0)
		{
			if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || 
				g_ZtLicense.CheckUser(eZtUB::AlterMU) || g_ZtLicense.CheckUser(eZtUB::Artem) || g_ZtLicense.CheckUser(eZtUB::Artem2) ||
				g_ZtLicense.CheckUser(eZtUB::RevoMU) || g_ZtLicense.CheckUser(eZtUB::GloryMU) || g_ZtLicense.CheckUser(eZtUB::mu4you) || g_ZtLicense.CheckUser(eZtUB::Escalate) ||
...
void CAchievements::PlayerKill(int aIndex, int aTargetIndex)
{
	...
	{
#if(GAMESERVER_ACHIVEMENTS==ZTTEAM_SERVER || GAMESERVER_ACHIVEMENTS==ZTEAM_SERVER)
