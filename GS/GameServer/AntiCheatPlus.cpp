...
#include "..\\include\\ReadScript_zt.h"
#include "GameMain.h"
#include "LogToFile.h"
#include "logproc.h"
#include "user.h"
...
void AntiCheatPlus::Load()
{
	...
	if(!g_ZtLicense.user.AntiCheatPlus)
	{
		return;
	}
...
void AntiCheatPlus::ReadConfig(char* File)
{
	...
	this->DumpActive = GetPrivateProfileInt("ZtTeam", "DumpActive", 0, File);
	this->TitleActive = GetPrivateProfileInt("ZtTeam", "TitleActive", 0, File);
	this->ClassActive = GetPrivateProfileInt("ZtTeam", "ClassActive", 0, File);
	this->InjectActive = GetPrivateProfileInt("ZtTeam", "InjectActive", 0, File);
	this->ProcessActive = GetPrivateProfileInt("ZtTeam", "ProcessActive", 0, File);
	this->EliteActive  = GetPrivateProfileInt("ZtTeam", "EliteActive", 0, File);
	this->MultiWindows = GetPrivateProfileInt("ZtTeam", "MultiWindows", 1, File);

	this->m_EnableMaxWindow = GetPrivateProfileInt("ZtTeam", "EnableMaxWindow", 0, File);
	this->m_MaxWindow = GetPrivateProfileInt("ZtTeam", "MaxWindow", 100, File);
	#if(ANTI_CHEAT_PLUS_CRC)
	this->CRCActive = GetPrivateProfileInt("ZtTeam", "CheckFileActive", 0, File);
	#endif
	this->DumpTime = GetPrivateProfileInt("ZtTeam", "DumpTime", 10000, File);
	this->TitleTime = GetPrivateProfileInt("ZtTeam", "TitleTime", 10000, File);
	this->ClassTime = GetPrivateProfileInt("ZtTeam", "ClassTime", 10000, File);
	this->InjectTime = GetPrivateProfileInt("ZtTeam", "InjectTime", 10000, File);
	this->ProcessTime = GetPrivateProfileInt("ZtTeam", "ProcessTime", 10000, File);
	#if(SCANER_ITEM_DUPE)
	this->ScanItemDupeActive = GetPrivateProfileInt("ZtTeam", "ScanItemDupeActive", 0, File);
	this->ScanItemDupeDisconnect = GetPrivateProfileInt("ZtTeam", "ScanItemDupeDisconnect", 0, File);
	this->ScanItemDupeClean = GetPrivateProfileInt("ZtTeam", "ScanItemDupeClean", 0, File);
	#endif
}
// ----------------------------------------------------------------------------------------------
...
#ifdef _RECONNECT_				
		g_ConnectZt.SendClose(UserIndex,NORM_DC);
#else
	...
	for(int aIndex = OBJ_STARTUSERINDZT; aIndex < OBJMAX; aIndex++)
	{
		...
		for(int i = 0; i < INVENTORY_SIZE; i++)
		{
			...
			if(!this->CheckItemDupe(aIndex, iItemSerial, i))
			{
				...
				if(this->ScanItemDupeClean)
				{
					gObjInventoryItemSet(lpUser->m_Index, i, -1);
					lpUser->pInventory[i].Clear();
					GCInventoryItemDeleteSend(lpUser->m_Index, i, 1);
				}

				if(this->ScanItemDupeDisconnect)
				{
					#ifdef _RECONNECT_				
					g_ConnectZt.SendClose(aIndex,NORM_DC);
					#else
		...
		for(int i = 0; i < WAREHOUSE_SIZE; i++)
		{
			...
			if(!this->CheckWareDupe(aIndex, iNextSerial, i))
			{
				...
				{
					//gObjInventoryItemSet(lpUser->m_Index, i, -1);
					//lpUser->pWarehouse[i].Clear();
					//GCInventoryItemDeleteSend(lpUser->m_Index, i, 1);
					gObjWarehouseDeleteItem(aIndex,i);
				}
