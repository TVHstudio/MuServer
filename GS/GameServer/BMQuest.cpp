...
void cBMQuest::StartUP()
{
	...
	if(file == NULL)
	{
		MessageBoxA(0,"Data\\ZtData\\BMQuest.ini Load Error","CRITICAL ERROR",0);
		return;
	}
	char Buff[256];
	int Flag = 0;
...
void cBMQuest::NPCdialogue(int aIndex,int aNPC)
{
	...
	//Проверяем не закончен ли квес?
	if(lpUser->BMQuest_num < Count[QClass])
	{
		//Проверяем нача?ли уж?квес?
		...
		else if(lpUser->BMQuest_start == 1)
		{
			...
			if(lpUser->BMQuest_kill >= Quest[QClass][lpUser->BMQuest_num].Coun)
			{
				Presents(aIndex,
					...
					Quest[QClass][lpUser->BMQuest_num].gift); //Функция вознагражден?
