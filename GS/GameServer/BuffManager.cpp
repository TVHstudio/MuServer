...
BUFF_EFFECT_DATA* CBuffManager::GetEffectDataFromItemCode(WORD wItemCode)
{
	BUFF_EFFECT_DATA* lpBuffData = 0;

	lpBuffData = g_BuffEffect.GetBuffDataFromItemCode(wItemCode);
	return lpBuffData;
}
...
#ifdef DEBUG_CODE
	LogAddC(1,"Type: %d, Index: %d iEffectOption: %d",pEffect->btItemType,pEffect->btItemIndex,iEffectOption);
#endif
	...
	case AT_GENERAL_NOPERIOD_ITEM: //CashShop + PcBang General
		{
			for(i = 0; i < MAX_STATE_COUNT; i++)
			{
				...
				case AT_BT_BROTHERHOOD_BUFF:

				case AT_VIP_SILVER:
				case AT_VIP_GOLD:
				case AT_VIP_PLATINUM:
				case AT_GUILD_BUFF:
				case AT_NPC_BUFF_EXP:
				case AT_NPC_BUFF_DAMAGE:
				case AT_NPC_BUFF_DEFENCE:
				case AT_NPC_BUFF_EXL_CRIT:
				case AT_NPC_BUFF_DROP:
				case AT_NPC_BUFF_DOUBLE:
				case AT_NOOB_BUFF:
				case AT_QUEST_BUFF:
				case AT_DUNGEON_BUFF:
				case AT_MARRI_BUFF:
				case AT_MARRI_BUFF2:
				//
				case AT_MMOTOP_BUFF:
...
void GCUseBuffEffect(LPOBJ lpObj, BYTE btEffectIndex, BYTE btEffectUseOption, WORD wOptionType, WORD wEffectType, int iLeftTime)
{
	...
	//GCUseBuffEffect
	//GCDisplayBuffeffectPartyMember(v10);
	GCDisplayBuffeffectPartyMember(lpObj->m_Index);
#endif
}
...
bool gObjApplyBuffEffectDurationForInGameShop(LPOBJ lpObj,WORD wItemCode,int iDuration)
{
	...
	lpBuffData = (LPBUFF_EFFECT_DATE)g_BuffManager.GetBuffInfo(wItemCode);	//nowork
	...
	if(g_ItemAddOption.SearchForClearItemEffect(iItemCode) == FALSE)
		return false;

	lpItemEffectInfo = g_ItemAddOption.SearchItemEffectType1(iItemCode);

	int iRemoveBuffIndex = 0;

	iRemoveBuffIndex = g_BuffManager.InsertEffect(lpObj, lpBuffData->btIndex, lpItemEffectInfo->wEffectType1, lpItemEffectInfo->wEffectType2, lpItemEffectInfo->iEffectValue1, lpItemEffectInfo->iEffectValue2, iDuration);
