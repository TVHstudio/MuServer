...
BOOL CCashItemPeriodSystem::ClearPeriodItemEffect(LPOBJ lpObj, int ItemCode)//Identical
{
...
#ifdef DEBUG_CODE2
	LogAddC(2,"%d --- %d",this->ItemPeriodInfo[iItemInfoIndex].btCountItems, ItemCode);
#endif
	...
	{
		int iOptionIndex = 0;//Seasl Of Hellin nado perenesti na if

		for( iOptionIndex = 0; iOptionIndex < MAX_ITEM_PERIOD_EFFECT; iOptionIndex++ )
		{
//#ifdef DEBUG_CODE2
//			LogAddC(2,"%d -> %d",this->ItemPeriodInfo[iItemInfoIndex].vItemEffect[iOptionIndex].iItemCode ,ItemCode);
//#endif
...
int CCashItemPeriodSystem::CheckBuffUser(LPOBJ lpObj, int ItemBuffType)
{
	bool Result = FALSE;
	...
	for( int iOptionIndex = 0; iOptionIndex < MAX_ITEM_PERIOD_EFFECT; iOptionIndex++ )
	{
		if( this->ItemPeriodInfo[iItemInfoIndex].vItemEffect[iOptionIndex].bInUse == true )
		{
			//if(ItemBuffType == this->ItemPeriodInfo[iItemInfoIndex].vItemEffect[iOptionIndex].iItemCode)
			//{
			//	return TRUE;
			//}
#ifdef DEBUG_CODE2
			LogAddC(2,"%d %d", ItemBuffType, this->ItemPeriodInfo[iItemInfoIndex].vItemEffect[iOptionIndex].iItemCode);
#endif
			//6700
			//6699
			//6750
