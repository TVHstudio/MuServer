...
void CCashShop::CashShopOptioNReload()
{
	...
	this->LoadShopOption(gDirPath.GetNewPath("CashShop\\CashShopOption.dat"));
	this->Load(gDirPath.GetNewPath("CashShop\\CashShopList.txt"));
			...
			if ( iType == 1 )
			{
				...
				if(g_SocketOption.CheckItemType(iItemCode) == 1) 
				{
					...
					ItemByteConvert(ItemStatus.btItemInfo, iItemCode, ItemStatus.btSkillOption,
						ItemStatus.btLuckOption, ItemStatus.btAddOption,ItemStatus.btItemLevel, 1, ItemStatus.btExOption, ItemStatus.btAncOption, 0, 0, CA_Item_socket, 0xFF, 0);
				}
				else
				{
					ItemByteConvert(ItemStatus.btItemInfo, iItemCode, ItemStatus.btSkillOption,
						ItemStatus.btLuckOption, ItemStatus.btAddOption,ItemStatus.btItemLevel, 1, ItemStatus.btExOption, ItemStatus.btAncOption, 0, 0, NULL, 0xFF, 0);
				}
#else
				ItemByteConvert(ItemStatus.btItemInfo, iItemCode, ItemStatus.btSkillOption,
					ItemStatus.btLuckOption, ItemStatus.btAddOption,ItemStatus.btItemLevel, 1, ItemStatus.btExOption, ItemStatus.btAncOption, 0, 0, NULL, 0xFF, 0);	//add
#endif
...
void CCashShop::MakeItemList(CASHSHOP_ITEM_STATUS * lpItemStatus, sellItem * lpItemSellInfo)
{
	...
	for ( iIndex=0;iIndex<iItemCount;iIndex++)
	{
		...
		this->CashItemList[iCategory][iItemIndex+iIndex].ItemInfo.Convert(ITEMGET(lpItemStatus->btItemType, lpItemStatus->wItemIndex),
			lpItemStatus->btSkillOption, lpItemStatus->btLuckOption, lpItemStatus->btAddOption, lpItemStatus->btExOption, 0, 0, NULL, 0xFF, 0, CURRENT_DB_VERSION);
...
BOOL CCashShop::GiveBuyItemToInventory(LPOBJ lpObj, int iItemGuid)
{
	...
	pItem.Convert(iItemCode, lpItemInfo->btItemSkillOpion, lpItemInfo->btItemLuckOption, lpItemInfo->btItemAddOption,
		lpItemInfo->btItemExOption, lpItemInfo->btItemAncOption, 0,NULL, 0xFF, 0, CURRENT_DB_VERSION);
