	...
	if ( gObj[iUserIndex].Type == OBJ_USER )
	{
		while ( iCAL_EXP > 0 )
		{
			if ( iCAL_EXP > 0 )
			{
				...
				gObjUserSetExp(&gObj[iUserIndex],iCAL_EXP);	//Panda Use
...
void CChaosCastle::GiveUserDamage(int iUserIndex, int iDamage)
{
	...
	//Fix HP/SD Bug
#if(FIX_HPSDBAR)
	pResult.AttackDamage = iDamage;
	pResult.iShieldDamage = 0;
	pResult.Life	= 0;
	pResult.MaxLife = 0;
	pResult.SD	= 0;
	pResult.MaxSD = 0;
...
bool CChaosCastle::CheckCanStartPlay(int iChaosCastleIndex)
{
	...
	if ( iEnteredUser < ZtConfig.Event.Zt_CCMinPlayer )
	{
...
void CChaosCastle::GiveWinnerItem(int iChaosCastleIndex, int iWinnerIndex)
{
...
#if(SYSTEM_ACHIEVEMENTS)
	g_Achievements.WinCC(iWinnerIndex);
#endif
	...
	// -> Chaos castle win
	gChaosCastleRewardItem.Main(iWinnerIndex, iChaosCastleIndex);

	if(ZtConfig.Event.Zt_CCRewardLevelUp)
	{
		#if(ENABLE_NSGAME_UPDATE)
		for(int i = 0; i < ZtConfig.Event.Zt_BCRewardLevelUp; i++)
		{
			__int64 exp = gLevelExperience[gObj[iWinnerIndex].Level];
			gObj[iWinnerIndex].Experience = exp;
			gObjLevelUp(&gObj[iWinnerIndex], exp, gObj[iWinnerIndex].Class, 0);
		}
		#endif
	}
