...
void DataServerProtocolCore(BYTE protoNum, BYTE *aRecv, int aLen)
{
	switch ( protoNum )
	{
		...
		case 0xEA:
			{
				...
				g_ExGDManager.Protocol(lpDef->subcode, aRecv);
			}
			break;
		case 0xEB:
			{
		...
		case 0xEE:
			{
				PMSG_DEFAULT2* lpDef = (PMSG_DEFAULT2*)aRecv;
				g_ExWebManager.Protocol(lpDef->subcode, aRecv);
			}
			break;
		case 0xEF:
			{
...
void JGPGetCharList(BYTE *lpRecv)
{
...
#ifdef EXPINV
#ifdef DEBUG_CODE
	LogAddC(2, "lpCount->ExpandedWarehouse: %d", lpCount->ExpandedWarehouse);
	...
	{
		gObj[aIndex].ExpandedWarehouse = 1;
	}
	...
	if( pCLCount.Count > 0 )
	{
		...
		{
			pCList.CharSet[5] |= 12;
			pCList.CharSet[17] |= (TempInventory[7] - 129) << 5;
		}
#endif
#ifdef NEWWINGS
			...
			default:
				#if(ENABLE_PETEX)
				if(g_PetEx.IsPet(ITEMGET(13, TempInventory[8])))
				{
...
#ifdef CHARCREATE_TEMP
		if( HightLevel >= ExConfig.ExCustom.CreateSUMLevel )  //gCreateSUMLevel
		{
			GenerableClass += 1;
		}
		// ----
		if( HightLevel >= ExConfig.ExCustom.CreateMONKLevel ) //gCreateMONKLevel
		{
			GenerableClass += 8;
		}
		// ----
		if( HightLevel >= ExConfig.ExCustom.CreateMGLevel )	  //gCreateMGLevel
		{
			GenerableClass += 4;
			gObj[aIndex].Magumsa = 1;
		}
		// ----
		if( HightLevel >= ExConfig.ExCustom.CreateDLLevel )	  //gCreateDLLevel
		{
			GenerableClass += 2;
...
{
	g_ExLicense.GCLicenseUser(aIndex);
...
void GJSetCharacterInfo(LPOBJ lpObj, int aIndex, BOOL bMapServerMove)
{
	...
	pCSave.ExFreePoints = lpObj->ExFreePoints;
...
void GDUpdateExtendedWarehouseCount(LPOBJ lpObj, BYTE btExtendedWarehouseCount, DWORD dwEventIndex, int iItemPos, bool bBuyAtInGameShop, bool bIsReplace)
{
	...
	LogAddC(1,"ExpandedInventory: %d, ExpandedWarehouse: %d",lpObj->ExpandedInventory,lpObj->ExpandedWarehouse);
