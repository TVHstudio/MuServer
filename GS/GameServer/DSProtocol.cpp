...
void DataServerProtocolCore(BYTE protoNum, BYTE *aRecv, int aLen)
{
	switch ( protoNum )
	{
		...
		case 0xD1:	//-> PeriodItemZt Main
			{
				...
				{
				case 1:
					g_PeriodItemZt.DGPeriodItemZtInsert((PMSG_ANS_PERIODITEMZT_INSERT*)aRecv);
					break;
				case 2:
					g_PeriodItemZt.DGPeriodItemZtDelete((PMSG_ANS_PERIODITEMZT_DELETE*)aRecv);
					break;
				case 3:
					g_PeriodItemZt.DGPeriodItemZtExpiredItemList((PMSG_ANS_PERIODITEMZT_EXPIRED_ITEMLIST*)aRecv);
					break;
				case 4:
					g_PeriodItemZt.DGPeriodItemZtListCount((PMSG_ANS_PERIODITEMZT_LIST_COUNT*)aRecv);
					break;
				case 5:
					g_PeriodItemZt.DGPeriodItemZtList((PMSG_ANS_PERIODITEMZT_LIST*)aRecv);
					break;
				}
			}
			break;
		...
		case 0xE2:
			{
				...
				case 0x01:
					...
					{
						gWinQuestSystem.RecvQuestInfo((DGQuestUserInfo*)aRecv);
					}
					break;
				case 0x02:
					{
						gBrotherhood.DG_RecvBrotherCount((DG_CheckBrother*)aRecv);
					}
					break;
		...
		case 0xE3:
			{
				gRanking.RecvDS((DGCharTop*)aRecv);
			}
			break;
		case 0xE4:
			{
		...
		case 0xE5:
			{
				...
				case 0x00:
					g_SpotReturn.DGStatus((PMSG_DG_SPOTSTATUS*)aRecv);
					break;
#endif
				}
			};
		...
		case 0xEA:
			{
				...
				g_ZtGDManager.Protocol(lpDef->subcode, aRecv);
			}
			break;
		case 0xEB:
			{
...
#if(CUSTOM_MARRIAGE)
				case 0x00:
					g_Marriage.DGLoad((PMSG_ANS_MARRIAGE_LOAD*) aRecv);
					break;
				case 0x01:
					g_Marriage.DGInsert((PMSG_ANS_MARRIAGE_INSERT*) aRecv);
					break;
				case 0x02:
					g_Marriage.DGDelete((PMSG_ANS_MARRIAGE_DELETE*) aRecv);
					break;
#endif
#if(EVENT_DUNGEON_SIEGE)
				case 0x03:
					g_DungeonSiege.DGRegSiegeList((PMSG_DG_DUNGEON_SIEGE_REG_LIST*)aRecv); 
					break;
				case 0x04:
					g_DungeonSiege.DGRegSiegeResult((PMSG_GD_DUNGEON_SIEGE_GUILD_REG_RESULT*)aRecv); 
					break;
				case 0x06:
					g_DungeonSiege.DGSiegeOwner((PMSG_DG_DUNGEON_SIEGE_SELECT_OENER*)aRecv); 
					break;
#endif
				case 0x11:
					#if(OFFLINE_MODE_RESTORE)
					g_OfflineMode.DGAnsAccauntPassword((PMSG_DGANS_OFFMODE_START*)aRecv);
					#endif
#if(CUSTOM_REFERAL_SYSTEM)
				case 0x14:
					//g_ReferralSystem.DGAnsMasterInfo((PMSG_DGANS_REFERRAL_MASTER_INFO*)aRecv);
					break;
				case 0x15:
					g_ReferralSystem.DGAnsMasterUpdate((PMSG_GDANS_REFERRAL_MASTER_UPDATE*)aRecv);
					break;
				case 0x16:
					g_ReferralSystem.DGAnsUserInfo((PMSG_DGANS_REFERRAL_USER_INFO*)aRecv);
					break;
				case 0x17:
					g_ReferralSystem.DGAnsUserUpdate((PMSG_GDANS_REFERRAL_USER_UPDATE*)aRecv);
					break;
#endif
#if(DEV_BANSYSTEM)
				case 0x20:
		...
		case 0xEC:
			#if(OFFLINE_MODE_RESTORE)
			g_OfflineMode.DGAnsOffCharacter((SDHP_DBCHAR_INFORESULT*)aRecv);
			#endif
			break;
		case 0xED:
			#if(OFFLINE_MODE_RESTORE)
			g_OfflineMode.DGPSelectData((PMSG_GDLOAD_OFFMODE_DATA*) aRecv);
			#endif
			break;
		case 0xEE:
			{
				PMSG_DEFAULT2* lpDef = (PMSG_DEFAULT2*)aRecv;
				g_ZtWebManager.Protocol(lpDef->subcode, aRecv);
			}
			break;
		case 0xEF:
			{
				...
				{
				case 0x00:
					g_AccountSecurity.DGAnsLoad((PMSG_ANS_ACC_DATABASE_LOAD*)aRecv);
					break;
				case 0x01:
					g_AccountSecurity.DGAnsSave((PMSG_ANS_ACC_DATABASE_SAVE*)aRecv);
					break;
				}
			}
			break;
		case 0xFD:
		{
			...
			switch (lpMsg->subcode)
			{
				...
				case 0x01:
					GDGetWarehouseNumber((LOADWAREHOUSENUMBER_SAVE*)aRecv);
					break;
#endif
			}
		}
...
#if (DEV_GUILDBANK)
		//case Protocol_GSRecv_GetWarehouseList:
		//	DGGetWarehouseList((SDHP_GETWAREHOUSEDB_SAVE *)aRecv);
		//	break;
		case Protocol_GSRecv_GetWarehouseNoItem:
			GDGetWarehouseNoItem((SDHP_GETWAREHOUSEDB_RESULT *)aRecv);
			break;
		//case Protocol_GSRecv_GetWarehouseList:
		//	DGGetWarehouseList((SDHP_GETWAREHOUSEDB_SAVE *)aRecv);
		//	break;
		case Protocol_GSRecv_GetGuildWarehouseList:
			DGGetWarehouseList((SDHP_GETWAREHOUSEDB_SAVE *)aRecv);
			break;
#endif
		#if(DEV_PLAYERSHOP)
		case 0xFE:
			{
				if(aRecv[0] == 0xC2 && aRecv[4] == 0x00)
				{
					g_ItemMarket.DGAnsItemList((SDHP_ANS_MARKET_ITEM*)aRecv);
				}

				if(aRecv[0] == 0xC1 && aRecv[3] == 0x01)
				{
					g_ItemMarket.DGAnsItemSell((SDHP_ANS_MARKET_SELL*)aRecv);
				}

				if(aRecv[0] == 0xC1 && aRecv[3] == 0x02)
				{
					g_ItemMarket.DGAnsItemBuy((SDHP_ANS_MARKET_BUY*)aRecv);
				}

				if(aRecv[0] == 0xC1 && aRecv[3] == 0x03)
				{
					g_ItemMarket.DGAnsItemStatus((SDHP_ANS_MARKET_STATUS*)aRecv);
				}
				#if(MARKET_ITEM_RECV)
				if(aRecv[0] == 0xC1 && aRecv[3] == 0x04)
				{
					g_ItemMarket.GDReqItemDate((SDHP_REQ_MARKET_ITEMRECV*)aRecv);
				}
				#endif
			}
			break;
...
void DataServerLoginResult(SDHP_RESULT * lpMsg)
{
	...
	{
#if(SWENG_FIX==TRUE)
		LogAddC(2, "%s", lMsg.Get(MSGGET(1, 210)));	//FIX_CRASH
#else
	...
	gParty.DBIndex = lpMsg->PartyCount;
#endif

	g_ZtGDManager.DB_Start();
...
#if(EVENT_DUNGEON_SIEGE)
	g_DungeonSiege.GDSiegeOwner();
	g_DungeonSiege.GDReqRegSiegeList();
#endif

	#if(DEV_BANSYSTEM)
	g_BanSystem.DSReqSelect();
	#endif
}
...
struct PMSG_CHARLISTCOUNT
{
	...
	bool ZtpandedWarehouse;	//-> Season 6.3
#endif
};
...
void JGPGetCharList(BYTE *lpRecv)
{
...
#ifdef ZTPINV
#ifdef DEBUG_CODE
	LogAddC(2, "lpCount->ZtpandedWarehouse: %d", lpCount->ZtpandedWarehouse);
	
#endif

	#if(!ZTTEAM_GAMECLIENT_DEV)
	pCLCount.ZtpandedWarehouse		= lpCount->ZtpandedWarehouse;
	#endif
	gObj[aIndex].ZtpandedWarehouse	= lpCount->ZtpandedWarehouse;
#endif

	if(g_ZtLicense.CheckUser(NSGames))
	{
		gObj[aIndex].ZtpandedWarehouse = 1;
	}
	...
	if( pCLCount.Count > 0 )
	{
		for ( int n=0;n<pCLCount.Count ;n++)
		{
...
#endif
#ifdef NEWWINGS_2_5
			if( IS_NEWWINGS_2_5(TempInventory[7]+ITEMGET(12, 0)) )
			{
				pCList.CharSet[5]	|= CS_SET_WING1(3);
				pCList.CharSet[17]	|= (BYTE)(TempInventory[7] - 180 + 1) << 2;
			}
#endif
#ifdef NEWWINGS_4
			if( IS_NEWWINGS_4(TempInventory[7]+ITEMGET(12, 0)) )
			{
				pCList.CharSet[5]	|= CS_SET_WING1(3);
				pCList.CharSet[17]	|= (BYTE)(TempInventory[7] - 180 + 1) << 2;
			}
#endif
#ifdef NEWWINGS_5
			if( IS_NEWWINGS_5(TempInventory[7]+ITEMGET(12, 0)) )
			{
				pCList.CharSet[5]	|= CS_SET_WING1(3);
				pCList.CharSet[17]	|= (BYTE)(TempInventory[7] - 180 + 1) << 2;
			}
#endif
#ifdef NEWWINGS_6
			if( IS_NEWWINGS_6(TempInventory[7]+ITEMGET(12, 0)) )
			{
			...
			default:
				#if(ENABLE_PETZT)
				if(g_PetZt.IsPet(ITEMGET(13, TempInventory[8])))
				{
					index |= CS_SET_HELPER(3);
					pCList.CharSet[16] |= TempInventory[8];
					break;
				}
				#endif
				index |= (TempInventory[8] ) & 0x03;
...
#ifdef CHARCREATE_TEMP
		if( HightLevel >= ZtConfig.ZtCustom.CreateSUMLevel )  //gCreateSUMLevel
		{
			GenerableClass += 1;
		}
		// ----
		if( HightLevel >= ZtConfig.ZtCustom.CreateMONKLevel ) //gCreateMONKLevel
		{
			GenerableClass += 8;
		}
		// ----
		if( HightLevel >= ZtConfig.ZtCustom.CreateMGLevel )	  //gCreateMGLevel
		{
			GenerableClass += 4;
			gObj[aIndex].Magumsa = 1;
		}
		// ----
		if( HightLevel >= ZtConfig.ZtCustom.CreateDLLevel )	  //gCreateDLLevel
		{
			GenerableClass += 2;
...
{
	g_ZtLicense.GCLicenseUser(aIndex);
...
void JGGetCharacterInfo( SDHP_DBCHAR_INFORESULT * lpMsg)
{
	...
	if ( lpObj->Money < 0 )
	{
		LogAddC(2, "ЎЪЎЩ ZEN MODIFY ЎЩЎЪ [%s][%s] %d -> 0", szName, gObj[aIndex].AccountID, lpObj->Money);
		lpObj->Money = 0;
	}
...
#ifdef PERIOD
	g_PeriodItemZt.RequestPeriodItemInfo(lpObj);	//1.01.00
#endif
#ifdef LUCKYITEM
...
//#endif
	...
	g_PremiumSystemZt.UserConnect(aIndex);
#endif

	#if(DEV_PLAYERSHOP)
	g_ItemMarket.ConnectPlayer(aIndex);
	#endif

#if(_RECONNECT_)
	if(g_ConnectZt.ConnectIndex == aIndex)
	{
		//g_ConnectZt.ConnectTick = 0;
	}
#endif

	if(Premium.CheckPremiumServer(aIndex) == false)
	{
		#if(_RECONNECT_)
...
void GJSetCharacterInfo(LPOBJ lpObj, int aIndex, BOOL bMapServerMove)
{
	...
	pCSave.AccountID[MAX_ACCOUNT_LEN] = 0;
	...
	pCSave.Brother[MAX_ACCOUNT_LEN] = 0;
	memcpy(pCSave.Brother, lpObj->Brotherhood, sizeof(pCSave.Brother)-1);
	...
	pCSave.PCPoint = lpObj->PCPoint;
	pCSave.ZtFreePoints = lpObj->ZtFreePoints;
	pCSave.Reset = lpObj->Reset;
	pCSave.GReset = lpObj->GReset;
	pCSave.ZtQuestNum = lpObj->ZtQuestNum;
	pCSave.ZtQuestKill = lpObj->ZtQuestKill;

	//if(g_ZtLicense.CheckUser(SILVER1) || g_ZtLicense.CheckUser(SILVER2))
	//{
	//	pCSave.PremiumTime = time(NULL) + lpObj->PremiumTime;
	//}
	//else
	//{
		pCSave.PremiumTime = lpObj->PremiumTime;
	//}

#ifdef PREM_TYPE
	pCSave.PremiumTimeType = lpObj->PremiumTimeType;
#endif
	pCSave.ZtCred = lpObj->ZtCred;
	pCSave.BanChat = lpObj->BanChat;
	pCSave.BanPost = lpObj->BanPost;
	pCSave.BanChar = lpObj->BanChar;
	...
	pCSave.AutoParty = lpObj->AutoPt;
	pCSave.Zen = lpObj->Zen;
	pCSave.QTDate = lpObj->QTDate;
...
#if(CUSTOM_SAVE_PT==TRUE)
	pCSave.PartyIndex = lpObj->PartyIndex;
#endif	

	pCSave.DonateCredit = lpObj->m_iDonateCredit;
#if(ENABLE_VOTER_ANONS)
	pCSave.ATDate = lpObj->ATDate;
	#endif
	...
	if(g_ZtLicense.user.WinQuest) 
	{
		gWinQuestSystem.DGSendInformation(aIndex);
	}

	g_ZtGDManager.DB_Save(aIndex);
}
...
void GDGetWarehouseList(int aIndex, char* AccountID, int Number)
{
	...
	#if(DEV_GUILDBANK)
	g_GuildBank.ClearWarehouse(aIndex);
	#endif
...
void DGGetWarehouseList(SDHP_GETWAREHOUSEDB_SAVE * lpMsg)
{
	...
	for ( int n=0;n<WAREHOUSE_SIZE;n++)
	{
		...
		if ( itype != -1 )
		{
			...
			{
				item.Convert(itype, item.m_Option1, item.m_Option2, item.m_Option3, DBI_GET_NOPTION(lpMsg->dbItems[n*dbsize+7]), lpMsg->dbItems[n*dbsize+8], item.m_ItemOptionEx, 0, 0xFF, item.m_PeriodItemOption, 1);
			}
			else
			{
				item.Convert(itype, item.m_Option1, item.m_Option2, item.m_Option3, DBI_GET_NOPTION(lpMsg->dbItems[n*dbsize+7]), lpMsg->dbItems[n*dbsize+8], item.m_ItemOptionEx, SocketOption, SocketOptionIndex, item.m_PeriodItemOption, 3);
			}
#else
			else if ( lpMsg->DbVersion <= 2 )
			{
				item.Convert(itype, item.m_Option1, item.m_Option2, item.m_Option3, DBI_GET_NOPTION(lpMsg->dbItems[n*dbsize+7]), lpMsg->dbItems[n*dbsize+8], item.m_ItemOptionEx, 0, 0xFF, 0, 1);
			}
			else
			{
				item.Convert(itype, item.m_Option1, item.m_Option2, item.m_Option3, DBI_GET_NOPTION(lpMsg->dbItems[n*dbsize+7]), lpMsg->dbItems[n*dbsize+8], item.m_ItemOptionEx, SocketOption, SocketOptionIndex, 0, 3);
			}
#endif
	...
	pResult.result = 2;
#ifdef DEBUG_CODE
	//if(lpObj->TESTW == 0)	//DeBug
#endif

	DataSend(lpObj->m_Index, (LPBYTE)&pResult, pResult.h.size);	//Пакет на открытие окна сундука

	GCUserWarehouseSend(&gObj[aIndex]);//Отсылает пакет вещей в Клиент
...
void GDSetWarehouseList(int aIndex)
{
	...
	{
		g_GuildBank.GDSetGuildWarehouseList(aIndex);
		/* Guild Bank Fix */
		/*LPOBJ lpObj = &gObj[aIndex];
...
void ItemSerialCreateRecv(SDHP_ITEMCREATERECV * lpMsg)
{
	...
	if ( lpMsg->MapNumber == (BYTE)-1 || lpMsg->MapNumber == (BYTE)-2 )
	{
		...
		{
			NewItem.m_Durability = lpMsg->Dur;// egfg
		}
		...
		if(g_ZtLicense.user.GrandHero)
		{
			if(GrandHero.SockeetItem(lpMsg->Type) == TRUE)
			{
				...
				NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, Sockeet, 0xFF, 0, CURRENT_DB_VERSION);
			}
			else
			{
				NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, 0, 0xFF, 0, CURRENT_DB_VERSION);
			}
		}
#endif
		if(g_ZtLicense.CheckUser(eZtUB::drynea) || g_ZtLicense.CheckUser(eZtUB::MedoniAndrei))
		{
			if(g_SocketOption.CheckItemType(lpMsg->Type))
			{
				if(lpObj->MixManagerSocket == 1)
				{
					BYTE Sockeet[5] = {0xFE,0xFF,0xFF,0xFF,0xFF};
					NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, Sockeet, 0xFF, 0, CURRENT_DB_VERSION);
				}
				else if(lpObj->MixManagerSocket == 2)
				{
					BYTE Sockeet[5] = {0xFE,0xFE,0xFF,0xFF,0xFF};
					NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, Sockeet, 0xFF, 0, CURRENT_DB_VERSION);
				}
				else if(lpObj->MixManagerSocket == 3)
				{
					BYTE Sockeet[5] = {0xFE,0xFE,0xFE,0xFF,0xFF};
					NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, Sockeet, 0xFF, 0, CURRENT_DB_VERSION);
				}
				else if(lpObj->MixManagerSocket == 4)
				{
					BYTE Sockeet[5] = {0xFE,0xFE,0xFE,0xFE,0xFF};
					NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, Sockeet, 0xFF, 0, CURRENT_DB_VERSION);
				}
				else if(lpObj->MixManagerSocket == 5)
				{
					BYTE Sockeet[5] = {0xFE,0xFE,0xFE,0xFE,0xFE};
					NewItem.Convert(lpMsg->Type,lpMsg->Op1, lpMsg->Op2, lpMsg->Op3,lpMsg->NewOption, lpMsg->SetOption,0, Sockeet, 0xFF, 0, CURRENT_DB_VERSION);
				}
	...
	if ( lpMsg->MapNumber == 235 )
	{
		...
		if ( gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED )
		{
...
#ifdef PERIOD
			//if(lpMsg->x == 1 && lpMsg->y == 1)	//Credit   Himik bla
			//{
			//	pCreateItem.m_Number = 1;//lpMsg->m_Number;
			//}
			//else if(lpMsg->x == 2 && lpMsg->y == 2)
			//{
			//	pCreateItem.m_Number = 2;//lpMsg->m_Number;
			//}
			//else
			{
			...
			pCreateItem.m_Durability = lpMsg->Dur;//Season 4.5 changed positionhfm

			if ( g_SocketOption.CheckItemType(ITEMGET(iItemType, iItemIndex)) == 1 )
			{
				BYTE CS_Item[5] = {0xFE,0xFE,0xFE,0xFE,0xFE};
				pCreateItem.Convert(ITEMGET(iItemType, iItemIndex), lpMsg->Op1, lpMsg->Op2, lpMsg->Op3, lpMsg->NewOption, lpMsg->SetOption, 0, CS_Item, 0xFF, 0, 3);
			}
			else
			{
...
//				{
//					g_PeriodItemZt.SetPeriodItemInfo(&gObj[lpMsg->aIndex], lpMsg->Type, lpMsg->m_Number, lpMsg->lDuration);
//				}
//#endif
							//LogAdd("[CREDIT DONATE][%s] Type: %d, Index: %d, Level: %d, Skill: %d, Luck: %d, Opt: %d, Exellent: %d, Ancent: %d, Serial: %X",
							//	lpObj->AccountID,iItemType,iItemIndex,lpMsg->Level,lpMsg->Op1,lpMsg->Op2,lpMsg->Op3,lpMsg->NewOption,lpMsg->SetOption,lpMsg->m_Number);
				//ExLog(0,"[%s] Type: %d, Index: %d, Level: %d, Skill: %d, Luck: %d, Opt: %d, Exellent: %d, Ancent: %d, Serial: %d",
				//	lpObj->AccountID,iItemType,iItemIndex,lpMsg->Level,lpMsg->Op1,lpMsg->Op2,lpMsg->Op3,lpMsg->NewOption,lpMsg->SetOption,lpMsg->m_Number);
				//ExLog(0,"%s",lpObj->AccountID);
				GCInventoryItemOneSend(lpMsg->aIndex, btItemPos);
...
void DGMoveOtherServer(SDHP_CHARACTER_TRANSFER_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result != 0 )
	{
		...
		GCServerMsgStringSend("№®Б¦ №Я»эЅГ change@webzen.co.kr·О №®АЗЗШ БЦЅГ±в№Щ¶шґПґЩ",lpObj->m_Index, 1);
		return;
	}

	LogAddTD("[CharTrasfer] Success [%s][%s] (%d)",	lpObj->AccountID, lpObj->Name, lpMsg->Result);

	GCServerMsgStringSend("БўјУАМ Бѕ·бµЛґПґЩ.", lpObj->m_Index, 1);
	GCServerMsgStringSend("єРЗТ ј­№ц·О БўјУЗШБЦЅГ±в №Щ¶шґПґЩ.", lpObj->m_Index, 1);
...
{
	LPOBJ lpObj = &gObj[aIndex];
	char pbuffer[WAREHOUSE_BUFF];//Season 4.5 change size in 2x times
...
{
	LPOBJ lpObj = &gObj[aIndex];
	char pbuffer[WAREHOUSE_BUFF];//Season 4.5 changed size
...
void GDRequestWcoinInfoLoad (LPOBJ lpObj,int Count,int Type)
{
	...
	LogAddTD("[%s][%s] Wcoin Send Data Server",lpObj->AccountID,lpObj->Name);
}
...
#endif
void DSBanSystem(BAN_CHAR_DS * lpMsg)
{
	if(lpMsg == NULL)
	{
		return;
	...
	if(lpMsg->Type == 3)
	{
		if(lpMsg->Result)
		{
			MsgNormal(lpMsg->aIndex,"[UnBanChar] %s",lpMsg->Name);
		}
		else
		{
			MsgNormal(lpMsg->aIndex,"[UnBanChar] %s Error",lpMsg->Name);
		}
	}
}

void GDUpdateExtendedInvenCount(LPOBJ lpObj, BYTE ExpandedInventory, DWORD dwEventIndex, int iItemPos, bool bBuyAtInGameShop, bool bIsReplace)
{
	...
	pMsg.ExtendedInvenCount = ExpandedInventory;
...
void GDUpdateExtendedWarehouseCount(LPOBJ lpObj, BYTE btExtendedWarehouseCount, DWORD dwEventIndex, int iItemPos, bool bBuyAtInGameShop, bool bIsReplace)
{
	...
	LogAddC(1,"ExpandedInventory: %d, ZtpandedWarehouse: %d",lpObj->ExpandedInventory,lpObj->ZtpandedWarehouse);
...
void DGUpdateExtendedWarehouseResult(SDHP_ANS_SET_EXTENDEDWAREHOUSE_COUNT * lpMsg)
{
	...
	// ----
	lpObj->ZtpandedWarehouse = lpMsg->ExtendedWarehouseCount;
	// ----
	//if( lpMsg->BuyAtInGameShop == 1 )
	//{
	//	CCashShopInGame::SetActiveItemResult(v8, lpMsg->EventIndex);
...
void GDGetWarehouseNumber(LOADWAREHOUSENUMBER_SAVE * aRecv)
{
	if (!gObjIsConnected(aRecv->aIndex))
	{
		LogAddC(2, "[Warehouse][%s] Error", __FUNCTION__);
		return;
	}

	// ----
	LPOBJ lpUser = &gObj[aRecv->aIndex];
	// ----
	lpUser->WarehouseNumber = aRecv->Number;
...
void GDCharacterListMultiSend(PMSG_REQ_MULTI_CHAR_RECV* lpMsg, int aIndex) // OK
{
	...
	gObj[aIndex].m_CharPage = lpMsg->Page;

	SDHP_GETCHARLIST_MULTICHAR pMsg;

	//pMsg.header.set(0xE3,sizeof(pMsg));

	pMsg.h.set((LPBYTE)&pMsg, 0xF0, sizeof(pMsg));
