...
void CDarkSpirit::ModeAttackRandom()
{
	...
	if( 30 + lpObj->m_MPSkillOpt.MpsDarkSpiritStr2 > rand() % 100 )
	{
		criticaldamage	= 1;
		//iActionType		= 1;	
	}
	// ----
	if( 30 + lpObj->m_MPSkillOpt.MpsDarkSpiritStr3 > rand() % 100 )
	{
		criticaldamage	= 2;
		//iActionType		= 1;
	}
...
void CDarkSpirit::SetMode(ePetItem_Mode mode, int iTargetindex)
{
	...
	{
		for(int i = 0; i < g_Manager.m_DarkRevenAttackBlock.size(); i++)
		{
			if(lpObj->MapNumber == g_Manager.m_DarkRevenAttackBlock[i].Map)
			{
				return;
			}
		}
...
void CDarkSpirit::Set(int aIndex, CItem * pPetItem)
{
	...
	this->m_AttackDamageMin = petitemlevel * ZtConfig.Pet.RavenAttackDamageMinLvl + leadership/ZtConfig.Pet.RavenAttackDamageMinCmd + 180;
	this->m_AttackDamageMax = petitemlevel * ZtConfig.Pet.RavenAttackDamageMaxLvl + leadership/ZtConfig.Pet.RavenAttackDamageMaxCmd + 200;
	this->m_AttackSpeed = petitemlevel * 4 / 5 + leadership / 50 + 20;
	this->m_SuccessAttackRate= petitemlevel + petitemlevel / 15 + 1000;
...
//#endif
	...
	if ( lpObj->pInventory[1].m_DurabilitySmall >= ZtConfig.Pet.RavenDurabilitySmall /*800*/ + lpObj->m_MPSkillOpt.MpsPetDurDownSpeed) //season4 changed
	{
	...
	if ( gObjWingSprite(lpTargetObj) == TRUE )
	{
		...
		if ( AttackDamage > 1 )
		{
			...
			else if ( IS_NEWWINGS_6(Wing->m_Type) )
			{
				...
				float damage = (float)(AttackDamage * (45/*50*/ - (Wing->m_Level*2))) / 100.0f;   // Fix Damage wings 5lvl in Pvp
				AttackDamage = (int)(damage);	//  #formula
				//}
			}
#endif
			...
			{
				float damage = (float)(AttackDamage * (50/*50*/ - (Wing->m_Level*2))) / 100.0f;   // Fix Damage wings 5lvl in Pvp
				AttackDamage = (int)(damage);	//  #formula
			}
#endif
#ifdef NEWWINGS_4
			else if ( IS_NEWWINGS_4(Wing->m_Type) )	//Custom Wings4
			{
...
#endif
#ifdef NEWWINGS_2_5
			else if ( IS_NEWWINGS_2_5(Wing->m_Type) )	//Custom Wings2.5
			{
		...
		if ( g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
		{
			if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
			{
				if ( lpObj->MapNumber == MAP_INDEX_CASTLESIEGE && lpTargetObj->MapNumber == MAP_INDEX_CASTLESIEGE )
				{
					if( lpObj->m_btCsJoinSide == lpTargetObj->m_btCsJoinSide)
					{
						//int PetAttackByNPC = GetPrivateProfileInt("CastleSiegePet","PetAttackByNPC",20,"..\\Data\\ZtData\\Skills.ini");
						AttackDamage = ( AttackDamage * ZtConfig.Character.PetCastleAttackByNPC ) / 100;		//AttackDamage = AttackDamage * 20 / 100;
					}
					else if(g_ShieldSystemOn == FALSE)
					{
						//int PetAttackByPlayer = GetPrivateProfileInt("CastleSiegePet","PetAttackByPlayer",40,"..\\Data\\ZtData\\Skills.ini");
						AttackDamage = AttackDamage * ZtConfig.Character.PetCastleAttackByPlayer / 100;	//AttackDamage = AttackDamage * 40 / 100;
					}
				}
			}
		}
...
int CDarkSpirit::GetAttackDamage(LPOBJ lpObj, int targetDefense, BOOL bIsOnDuel, int crititcaldamage)
{
	...
	{
		if(lpObj->pInventory[0].m_Magic > 0 )
		{
