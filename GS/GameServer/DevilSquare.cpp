...
int CDevilSquare::GetUserLevelToEnter(int iUserIndex, int& iMoveGate) //0046A880
{
	...
	if(gObj[iUserIndex].ChangeUP3rd == 1)
	{
...
int CDevilSquare::gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg)
{
	...
	if(g_MasterLevelSystem.CheckIsMasterLevelCharacter(lpObj) == FALSE)
	{
		...
		exp *= NewExp;
		//exp = int(exp * gResetSystem.Main(lpObj));
	...
	if ( exp > 0 )
	{
		if ( lpObj->Type == OBJ_USER )
		{
			...
			gObjUserSetExp(lpObj,exp);	//Panda Use
...
void CDevilSquare::gObjExpParty(LPOBJ lpObj, LPOBJ lpTargetObj, int AttackDamage, BOOL MSBFlag)
{
	...
	if ( viewplayer > 1 )
	{
		if ( bApplaySetParty != FALSE )
		{
			if ( viewplayer == 3 )
			{
				viewpercent = ZtConfig.Event.DSParty3ExpPercent;//230;
			}
			else if ( viewplayer == 4 )
			{
				viewpercent = ZtConfig.Event.DSParty4ExpPercent;//270;
			}
			else if ( viewplayer >= 5 )
			{
				viewpercent = ZtConfig.Event.DSParty5ExpPercent;//300;
			}
			else
			{
				viewpercent = ZtConfig.Event.DSPartyExpPercentOther;//120;
			}
		}
		else
		{
			if ( viewplayer == 2 )
			{
				viewpercent = ZtConfig.Event.DSParty2ExpSetPercent;//160;
			}
			else if ( viewplayer == 3 )
			{
				viewpercent = ZtConfig.Event.DSParty3ExpSetPercent;//180;
			}
			else if ( viewplayer == 4 )
			{
				viewpercent = ZtConfig.Event.DSParty4ExpSetPercent;//200;
			}
			else if ( viewplayer >= 5 )
			{
				viewpercent = ZtConfig.Event.DSParty5ExpSetPercent;//220;
			}
			else
			{
				viewpercent = ZtConfig.Event.DSParty1ExpSetPercent;//120;
			}
		}
			...
			if ( lpTargetObj->MapNumber == lpPartyObj->MapNumber )
			{
				if ( dis[n] < 10 )
				{
					...
					if(g_MasterLevelSystem.CheckIsMasterLevelCharacter(lpPartyObj) == FALSE)
					{
						...
						exp *= NewExp;
						//exp = int(exp * gResetSystem.Main(lpPartyObj));
						//exp = int(exp * gAddExperience);
					}
#if(PC_BANG_POINT_SYSTEM==1)
				...
				g_DonateManager.CheckExp(lpObj->m_Index, exp);
					...
					if ( exp > 0 )
					{
						if ( lpPartyObj->Type == OBJ_USER )
						{
							...
							gObjUserSetExp(lpPartyObj,exp);	//Panda Use
