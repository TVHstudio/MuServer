...
void CDevilSquareGround::SendScore()
{
	...
	for ( ; Itor != this->m_DevilSquareRankList.end(); Itor++ )
	{
		...
		gObjUserSetExp((*(Itor)),exp);	//Panda Use
		...
		if ( count < 4 )
		{
			if(count == 1)
			{
				if(ZtConfig.Event.Zt_DSRewardLevelUp1)
				{
					for(int i = 0; i < ZtConfig.Event.Zt_DSRewardLevelUp1; i++)
					{
						__int64 exp = gLevelExperience[(*(Itor))->Level];
						(*(Itor))->Experience = exp;
						gObjLevelUp((*(Itor)), exp, (*(Itor))->Class, 0);
					}
				}
			}
			else if(count == 2)
			{
				if(ZtConfig.Event.Zt_DSRewardLevelUp2)
				{
					for(int i = 0; i < ZtConfig.Event.Zt_DSRewardLevelUp2; i++)
					{
						__int64 exp = gLevelExperience[(*(Itor))->Level];
						(*(Itor))->Experience = exp;
						gObjLevelUp((*(Itor)), exp, (*(Itor))->Class, 0);
			...
			{
				if(ZtConfig.Event.Zt_DSRewardLevelUp3)
				{
					for(int i = 0; i < ZtConfig.Event.Zt_DSRewardLevelUp3; i++)
					{
						__int64 exp = gLevelExperience[(*(Itor))->Level];
						(*(Itor))->Experience = exp;
						gObjLevelUp((*(Itor)), exp, (*(Itor))->Class, 0);
...
void CDevilSquareGround::SendRankingInfo(LPOBJ lpObj)
{
	if ( lpObj->m_nEventScore <= 0 )
	{
		return;
				...
				//////////////////////////////////////////////////////////////////////Confict
				//DevilSquareRewardItem
#ifdef DS_REWARD_ITEM
				gDevilSquareRewardItem.Main(lpObj->m_Index);
#endif // DS_REWARD_ITEM
