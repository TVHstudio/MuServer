...
enum ZTDB_DATA_TRANSFER_TYPE
{
  ZT_SEND_P2P = 0x1,
  ZT_SEND_BROADCASTING = 0x0,
};

enum ZTDB_RESULT
{
  ZT_RESULT_SUCCESS = 0x1,
  ZT_RESULT_FAIL = 0x0,
  ZT_RESULT_FAIL_FOR_CASTLE = 0x10,
};
...
void ZtDataServerProtocolCore(BYTE protoNum, LPBYTE aRecv, int aLen)
{
	switch ( protoNum )
	{
		case 0x00:
			ZtDataServerLoginResult((SDHP_RESULT *)aRecv);
			break;
		case 0x02:
...
void ZtDataServerLogin()
{
	...
	wsZtDbCli.DataSend((char*)&pInfo, pInfo.h.size);
}
...
void GDCharClose(int type, char* GuildName, char* Name)
{
	...
	wsZtDbCli.DataSend((char*)&pMsg, pMsg.h.size);
}

void ZtDataServerLoginResult(SDHP_RESULT * lpMsg)
{
	if ( lpMsg->Result == false )
	{
		LogAddTD("ZtDB Server Login Fail: %d", lpMsg->Result);
		return;
	}
	...
	if ( lpMsg->Flag == 1 )
	{
		...
		if ( gObjIsConnected(aIndex) == TRUE )
		{
			if ( szMaster[0] == gObj[aIndex].Name[0] )
			{
				...
				{
#if(DEV_GUILDBANK)					
					g_GuildBank.GDDelGuildWarehouseList(aIndex);
#endif					
...
//#endif
					...
					if ( gObj[n].lpGuild != NULL )
					{
						...
						g_DungeonSiege.GCGuildNumber(n);
#endif
...
void DGRelationShipAnsJoin(EXSDHP_RELATIONSHIP_JOIN_RESULT * lpMsg)
{
	...
	if ( lpMsg->btFlag == 1 )
	{
		...
		if ( lpMsg->btResult == ZT_RESULT_FAIL ||lpMsg->btResult == ZT_RESULT_FAIL_FOR_CASTLE )
			return;
...
void FriendMemoSend(PMSG_FRIEND_MEMO * lpMsg, int aIndex)
{
	...
	wsZtDbCli.DataSend((char*)&pMsg, bsize);
