...
bool NsEngine = GetPrivateProfileInt("ZtTeam","NsEngine",0,ZtCustom_DIR);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	ZeroMemory(Zt_User,sizeof(Zt_User));
#endif

	WORD serverport;

	memset(DataServerIp2, 0, sizeof(DataServerIp2));
	memset(ZtDbIp, 0, sizeof(ZtDbIp));
	DataServerPort2=0;
	ZtDbPort=0;

	//if (strlen(lpCmdLine)>1) 
	//{
	//	sscanf(lpCmdLine, "%s %d %s %d %d %s %d %s %d" ,JoinServerIp, &JoinServerPort, DataServerIp, &DataServerPort, &serverport, DataServerIp2, &DataServerPort2, ZtDbIp, &ZtDbPort);
	//}
	//else 
	//{ 
	//	serverport=55900;
	...
	//Ex Data Base
	GetPrivateProfileStringA("Connect","ZtDbIp","127.0.0.1",ZtDbIp,sizeof(ZtDbIp),".\\Connect.ini");
	ZtDbPort = GetPrivateProfileInt("Connect","ZtDbPort",55906,".\\Connect.ini");
	//Ranking
	RankingPort = GetPrivateProfileInt("Connect","RankingPort",44455,".\\Connect.ini");
	//Event
	...
	if ( ZtDbIp[0] == 0 )
		memcpy(ZtDbIp, DataServerIp, sizeof(ZtDbIp));
...
BOOL GameServerStart(void) 
{
...
#else
	//86400000 
	if( g_ZtLicense.CheckUser(feqan) )
	{
...
#endif
	SetTimer(ghWnd, WM_ZT_TIMER_SEC, 1000, NULL);	//1000
	...
	SetTimer(ghWnd, WM_ZT_TIMER_500MC, 500, NULL);
	SetTimer(ghWnd, WM_ZT_TIMER_100MC, 100, NULL);
	SetTimer(ghWnd, WM_ZT_LICENSE_10SEC, 1000, NULL);	//need 60000 
	SetTimer(ghWnd, WM_ZT_TIMER_10SEC, 10000, NULL);
	return TRUE;
}
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hbrBackground	= (HBRUSH)CreateSolidBrush(WHITE_BRUSH);//CreateSolidBrush ( RGB ( 225, 123, 234));
...
//#endif
		...
		case WM_GM_DATA2_CLIENT_MSG_PROC:
			...
			break;
		case WM_GM_ZTDATA_CLIENT_MSG_PROC:
			ZtDataClientMsgProc(wParam, lParam);
			break;
				...
				case WM_SECOND_MSG_PROCESS:
					{
						...
						if(g_ZtLicense.Exit == TRUE)
						{
							g_ZtLicense.ExitTime++;
							if(g_ZtLicense.ExitTime == 5)
							{
								ExitProcess(0);
							}
						}
...
//#endif
#ifdef PERIOD
					g_PeriodItemZt.CheckPeriodData();
#endif
					break;
				case WM_SET_DATE:
					gSetDate();
...
//#endif
				...
				case WM_ZT_TIMER_SEC:
					{
						...
						g_ZtUser.GCServerTime();
						#endif
#if(CUSTOM_ZT_TELEPORT==TRUE)
						g_ZtTeleport.Timer();
#endif
#if(CUSTOM_ZT_TELEPORT_EVO)
						g_ZtTeleportEvo.Timer();
#endif						
						g_ZtUser.TickCountClientDataSend();
#ifdef _VOTER_						
						gVoter.Run();
#endif
						g_ZtUser.PartyMiniMapInfo();
				...
				case WM_ZT_TIMER_500MC:
					...
					break;
				case WM_ZT_TIMER_100MC:
					{
#if(_RECONNECT_)
						g_ConnectZt.Timer();
#endif
						g_ZtUser.AutoHPClick();
					...
					break;
#if(ZTLICENSE_SERVER)
				case WM_ZT_LICENSE_10SEC:
					g_ZtLicenseProtocol.Run();
					break;
#endif
				case WM_ZT_TIMER_10SEC:
					//#if(ANTI_CHEAT_PLUS && SCANER_ITEM_DUPE)
					//g_AntiCheatPlus.RunItemDupe();
					//#endif
					break;
