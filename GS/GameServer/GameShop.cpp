...
void GameShop::RequestPoint(int aIndex)	//-> Complete
{
	...
	{
		pPoint.WCoinC[0]	= (double)lpUser->ZtCred;
		pPoint.WCoinC[1]	= (double)lpUser->ZtCred;
	}
	else
	{
	...
	case 0:	//-> WCoinC
		{
			...
			if( g_ZtLicense.CheckUser(SILVER1) || g_ZtLicense.CheckUser(SILVER2))
			{
				if( lpUser->ZtCred >= lpItem->Price )
				{
					lpUser->ZtCred	-= lpItem->Price;
					pBuy.Result = false;
#if(SYSTEM_ACHIEVEMENTS)
					g_Achievements.BuyDonateShopItem(aIndex, lpItem->Price);
#endif
				}
			}
			else
	...
	case 0:	//-> WCoinC
		{
			...
			if(g_ZtLicense.CheckUser(SILVER1) || g_ZtLicense.CheckUser(SILVER2))
			{
				if( lpUser->ZtCred >= lpPackage->Price )
				{
					lpUser->ZtCred	-= lpPackage->Price;
					pBuy.Result = false;
				}
			}
			else
...
void GameShop::RequestStorageItemUse(int aIndex, GAMESHOP_REQ_STORAGEITEMUSE * lpRequest)
{
	...
	//ZtTeam Code
	//if ( g_CashItemPeriodSystem.CheckBuffUser(lpUser,lpItem->ItemID) == TRUE )
	//{
	//	pAnswer.Result = 1;	//true
	//	DataSend(aIndex, (LPBYTE)&pAnswer, pAnswer.h.size);
	//	LogAddC(3, "[GameShop] %d", __LINE__);
	...
	// ----
	//ZTeam
	//if(g_CashShop.IsGetAmountFromShopItem(lpItem->ItemID) && !this->IsInventoryBuff(lpItem->ItemID))
	//{
	...
	//if( g_CashShop.CheckPeriodItem(lpItem->ItemID) //-> Buff
	//	&& !this->IsInventoryBuff(lpItem->ItemID) )	
	//{
	//	g_CashItemPeriodSystem.SearchAndDeleteItemPeriodEffect(lpUser, lpItem->ItemID);	//fix
	//	g_CashItemPeriodSystem.GDReqPeriodItemInsert(lpUser, lpItem->ItemID, lpItem->Duration);
	...
	//ZtTeam
	if( g_BuffEffect.GetBuffDataFromItemCode(lpItem->ItemID) //-> Buff
		&& !this->IsInventoryBuff(lpItem->ItemID) )	
	{
		g_CashItemPeriodSystem.SearchAndDeleteItemPeriodEffect(lpUser, lpItem->ItemID);	//fix
		g_CashItemPeriodSystem.GDReqPeriodItemInsert(lpUser, lpItem->ItemID, lpItem->Duration);
	...
	//if( g_CashShop.CheckPeriodItem(lpUser, lpItem->ItemID) //-> Buff
	//	&& !this->IsInventoryBuff(lpItem->ItemID) )	
	//{
	//	g_CashItemPeriodSystem.SearchAndDeleteItemPeriodEffect(lpUser, lpItem->ItemID);	//fix
	//	g_CashItemPeriodSystem.GDReqPeriodItemInsert(lpUser, lpItem->ItemID, lpItem->Duration);
...
{
	//LogAdd("IBSCategory: %d",IBSCategory);
	for( int i = 0; i < 30; i++ )
	{
...
bool GameShop::CanBeBuy(int aIndex, short ItemID)
{
	...
	{
		if( !lpUser->ChangeUP3rd  )//ChangeUP3rd
		{
...
void GameShop::DGGetPoint(GAMESHOP_DG_GET_POINT * aRecv)
{
	...
	//-> Send Client
	ZtUserDataSend(aRecv->UserIndex);

	if(!lpUser->WaitWcoinConnect)
	{
		lpUser->WaitWcoinConnect = true;
		if(g_ZtLicense.CheckUser(eZtUB::MedoniAndrei) || g_ZtLicense.CheckUser(eZtUB::absolute))
		{
			if(ZtConfig.PlayerConnect.ShowWcoinC)
			{
				MsgNormal(lpUser->m_Index,"[WCoinC]: %d",(int)lpUser->GameShop.WCoinC);
			}
			if(ZtConfig.PlayerConnect.ShowGP)
			{
				MsgNormal(lpUser->m_Index,"[Goblin Point]: %d",(int)lpUser->GameShop.GoblinPoint);
			}
			if(ZtConfig.PlayerConnect.ShowZtCredit)
			{
				MsgNormal(lpUser->m_Index,"[Credits]: %d",(int)lpUser->ZtCred);
			}
		}
	}
}
