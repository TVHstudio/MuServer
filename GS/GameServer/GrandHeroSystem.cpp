...
{
	if(!g_ZtLicense.user.GrandHero) return;
	...
	this->ZtStoneDropRate = GetPrivateProfileInt("Common","ZtStoneDropRate",0,GRANDHERO_DIR);
...
{
	if(!g_ZtLicense.user.GrandHero) return false;
...
bool cGrandHero::BossDrop(LPOBJ lpObj, LPOBJ lpTargetObj)	//lpTargetObj - то?кт?убивае?
{
	if(!this->Enable)return false;
	...
	{
		//srand(time(NULL));
		if(this->ZtStoneDropRate > rand()%10000)
		{
