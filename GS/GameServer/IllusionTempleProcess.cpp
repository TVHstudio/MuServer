...
void CIllusionTempleProcess::ProcState_Closed(int iTick) 
{
	...
	if(this->m_iLeftMinutesBeforeOpen <= 0)
	{
		if(this->m_iEventBattleTimer <= 0)
		{
			...
			//int Zt_IllusionTemplPlaerMin = GetPrivateProfileInt("IllusionTemple","ItMinPlayers",4,"..\\Data\\ZtData\\Events.ini");

			if(this->m_iEnteredUserCount < ZtConfig.Event.Zt_IllusionTemplPlaerMin) //UserCount
			{
...
void CIllusionTempleProcess::CalcSendRewardEXP() 
{
	...
	for(i = 0;i < MAX_FLOOR_USER;i++)
	{
		...
		if(this->m_FloorData[i].m_iTeamPlayerIndex != -1 && gObj[this->m_FloorData[i].m_iTeamPlayerIndex].Connected > PLAYER_LOGGED && gObj[this->m_FloorData[i].m_iTeamPlayerIndex].MapNumber == this->m_btFloorIndex+MAP_INDEX_ILLUSIONTEMPLE &&	this->m_FloorData[i].m_btUsedKillCountPoint >= g_iIllusionTempleMinUseSkillCnt)
		{
			...
			gObjUserSetExp(&gObj[this->m_FloorData[i].m_iTeamPlayerIndex],this->m_FloorData[i].m_i64ExpReward);	//Panda Use
...
void CIllusionTempleProcess::DropRewardItem(int aIndex)
{
	...
	int loc2 = aIndex;
	//Configs
	//int Zt_ItDropType	= GetPrivateProfileInt("IllusionTemple","ItDropType",12,"..\\Data\\ZtData\\Events.ini");
	//int Zt_ItDropID		= GetPrivateProfileInt("IllusionTemple","ItDropID",15,"..\\Data\\ZtData\\Events.ini");
	//int Zt_ItDropLevel	= GetPrivateProfileInt("IllusionTemple","ItDropLevel",0,"..\\Data\\ZtData\\Events.ini");
	//int Zt_ItDropSkill	= GetPrivateProfileInt("IllusionTemple","ItDropSkill",0,"..\\Data\\ZtData\\Events.ini");
	//int Zt_ItDropLuck	= GetPrivateProfileInt("IllusionTemple","ItDropLuck",0,"..\\Data\\ZtData\\Events.ini");
	//int Zt_ItDropOpt	= GetPrivateProfileInt("IllusionTemple","ItDropOpt",0,"..\\Data\\ZtData\\Events.ini");
	//int Zt_ItDropExcOpt	= GetPrivateProfileInt("IllusionTemple","ItDropExcOpt",0,"..\\Data\\ZtData\\Events.ini");
	//int Zt_ItDropAncOpt	= GetPrivateProfileInt("IllusionTemple","ItDropAncOpt",0,"..\\Data\\ZtData\\Events.ini");

	int Exe1 = 0;
	int Anc1 = 0;

	if ( ZtConfig.Event.Zt_ItDropExcOpt > 0 )
		Exe1 = GenExcOpt(ZtConfig.Event.Zt_ItDropExcOpt);

	if ( ZtConfig.Event.Zt_ItDropAncOpt == 5 || ZtConfig.Event.Zt_ItDropAncOpt == 10 )
		Anc1 = ZtConfig.Event.Zt_ItDropAncOpt;

	int loc3 = ItemGetNumberMake(ZtConfig.Event.Zt_ItDropType, ZtConfig.Event.Zt_ItDropID);	// Jewel of Chaos
	ItemSerialCreateSend(gObj[aIndex].m_Index, (BYTE)gObj[aIndex].MapNumber, (BYTE)gObj[aIndex].X, gObj[aIndex].Y,loc3,
		(BYTE)ZtConfig.Event.Zt_ItDropLevel, 0, (BYTE)ZtConfig.Event.Zt_ItDropSkill, (BYTE)ZtConfig.Event.Zt_ItDropLuck, 
		(BYTE)ZtConfig.Event.Zt_ItDropOpt, loc2, (BYTE)Exe1, (BYTE)Anc1);
}
...
int CIllusionTempleProcess::LevelUp(int iUserIndex, int iAddExp) 
{
	...
	if(g_MasterLevelSystem.MasterLevelUp(&gObj[iUserIndex], (__int64&)iAddExp,1,0) != 0) //season3 add-on
	{
		return 0;
	}
	...
	if(gObj[iUserIndex].Level >= ZtConfig.ZtCustom.MaxLevel)
	{
	...
	g_Achievements.LevelUp(iUserIndex);
#endif
