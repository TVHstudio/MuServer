...
{
	for (int iCount = OBJ_STARTUSERINDZT; iCount < OBJMAX; ++iCount )
	{
...
void CImperialGuardian::CGEnterPortal(int nUserIndex, int nDestZoneIndex)	//OK
{
		...
		&& m_ZoneInfo[nDestZoneIndex].vtPlayers.size() < 1 )
	{
		...
		*/
		...
		{
			nMaxLevel = gObj[nUserIndex].MLevel + gObj[nUserIndex].Level;
	...
	if ( nGateIndex != -1 )
	{
		...
		if ( gObjSearchActiveEffect(lpObj, 3) == 1 )
...
bool CImperialGuardian::GCNotifyAllZoneClear(int nZoneIndex)	//OK
{
	...
	for ( auto it = vtTemp.begin(); it != vtTemp.end(); ++it )
	{
		...
		if ( IsEventMap(gObj[nResult].MapNumber) )
		{
			...
			g_ShopPointZt.AddEventBonus(nResult, ShopPointZtEvent::IG);
#endif
			...
			if ( gObjSearchActiveEffect(&gObj[nResult], 42)
				|| gObjSearchActiveEffect(&gObj[nResult], 31) )
			{
				pMsg.RewardExp = 0;
			}
			...
			if( !g_MasterLevelSystem.CheckIsMasterLevelCharacter(&gObj[nResult]) )
							GCSendExp_INT64(nResult, -1, pMsg.RewardExp, 0, 0);
...
void CImperialGuardian::DropItem(int nIndex, int nMonsterIndex)	//OK
{
    ...
    if ( nMaxLevel == 0)
		nMaxLevel = gObj[nIndex].Level + gObj[nIndex].MLevel;
#ifdef IMPERIAL
...
int CImperialGuardian::ImperialGuardianLevelUp(int iIndex, int iAddExp)	//OK
{
    ...
    if ( g_MasterLevelSystem.MasterLevelUp(&gObj[iIndex], (__int64 &)iAddExp, true, 0) )
    {
		return 0;
    }
...
void CImperialGuardian::MonsterBaseAct(OBJECTSTRUCT *lpObj)	//OK
{
	...
	if(lpObj->m_ActState.Emotion == 0)
	{
		...
		{
			if ( !gObjSearchActiveEffect(lpObj, 57) )
