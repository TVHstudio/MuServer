...
void cLoraBattle::LoadConfigs()
{
	...
	if(!g_ZtLicense.user.LoraBattle)
	{
		Enable = 0;
	}
	...
	RewardZtCred = GetPrivateProfileInt("Reward","RewardZtCred",0,LORABATTLE_DIR);
...
{
	for(int j=OBJ_STARTUSERINDZT; j<OBJMAX; j++)
	{
		LPOBJ lpObj = &gObj[j];
...
void cLoraBattle::EndEvent()
{
	...
	for(int l=OBJ_STARTUSERINDZT; l<OBJMAX; l++)
	{
		if(KillSocer[l] == 0)
			continue;
...
void cLoraBattle::Reward(int aIndex)
{
	...
	if(this->RewardZtCred > 0)
	{
			lpObj->ZtCred += this->RewardZtCred;
#ifndef FIX_DS_CRASH
			GJSetCharacterInfo(lpObj, lpObj->m_Index, 0);
#endif
			ZtUserDataSend(aIndex);
			MessageChat(aIndex,"~[Credit]: Online Bonus: %d",this->RewardZtCred);
	}
