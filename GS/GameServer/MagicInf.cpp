...
int CMagicInf::Set(int aSkill, BYTE aLevel)
{
	...
	this->m_Level = aLevel;
	// ----
	if( g_MasterSkillSystem.CheckRequireStatus(aSkill) )
	{
		if( MagicDamageC.GetSkillUseType(aSkill) == 4 )
		{
			int nBrandSkill = MagicDamageC.GetSkillReqBrandOfSkill(aSkill);
			// ----
...
int CMagicInf::UpdateML(DWORD aSkill, BYTE aLevel) //00490E60
{
	...
	if( this->m_Skill != 0xFF )
	{
		...
		{
			int nBrandSkill = MagicDamageC.GetSkillReqBrandOfSkill(this->m_Skill);
			// ----
