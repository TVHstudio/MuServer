...
void CMapQuest::Load()
{
	...
	this->m_Data.clear();
	this->Read(gDirPath.GetNewPath("ZtData\\MapQuest.ini"));
}
...
void CMapQuest::AddBoss(int Map, int BossClass, int BossX, int BossY, int OrcClass, int OrcCount)
{
	...
	char szMsg[128];
	sprintf(szMsg,"[Map Quest] Boss %s in %s map", gObj[BossIndex].Name, ztMapName(gObj[BossIndex].MapNumber));
