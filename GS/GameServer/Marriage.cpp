...
void CMarriage::Load()
{
	...
	if(!g_ZtLicense.user.Marriage)
	{
		return;
	}

	this->Read(gDirPath.GetNewPath("ZtData\\Marriage.ini"));
}
...
void CMarriage::DGInsert(PMSG_ANS_MARRIAGE_INSERT* aRecv)
{
	...
	ZtFireCracker(aIndex1, lpUser1->X, lpUser1->Y);
	ZtFireCracker(aIndex1, lpUser2->X, lpUser2->Y);
	ZtFireCracker(aIndex2, lpUser1->X, lpUser1->Y);
	ZtFireCracker(aIndex2, lpUser2->X, lpUser2->Y);
