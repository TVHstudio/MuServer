...
{
	this->Enable = GetPrivateProfileInt("ZtTeam","Enable",0,MOB_BONUS_DIR);
...
void cMonsterBonus::Main(LPOBJ lpMobObj, LPOBJ lpTargetObj)
{
	...
	for(int i = 0; i < this->Count; i++)
	{
		...
		if(lpMobObj->Class == this->MobData[i].MobID)
		{
			...
			{
				lpTargetObj->ZtCred += this->MobData[i].BonusCount;
				ZtUserDataSend(lpTargetObj->m_Index);	//Може?быть реконект если быстро убиваешь монстров нужн?пересматривать
				MsgNormal(lpTargetObj->m_Index,g_ZtText.GetText(109),this->MobData[i].BonusCount);
				return;
			}
			else if(this->MobData[i].BonusType == 2)	//WcoinC
			{
				...
				MsgNormal(lpTargetObj->m_Index,g_ZtText.GetText(110),this->MobData[i].BonusCount);
				return;
			}
			else if(this->MobData[i].BonusType == 3)	//Wcoin
			{
				...
				gGameShop.GDSaveUserInfo(lpTargetObj->m_Index);
				MsgNormal(lpTargetObj->m_Index,g_ZtText.GetText(111),this->MobData[i].BonusCount);
				return;
			}
			else if(this->MobData[i].BonusType == 4)	//Wcoin
			{
				...
				gGameShop.GDSaveUserInfo(lpTargetObj->m_Index);
				MsgNormal(lpTargetObj->m_Index,g_ZtText.GetText(112),this->MobData[i].BonusCount);
				return;
			}
			...
			if(g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::ulasevich) || g_ZtLicense.CheckUser(eZtUB::ulasevich2))
			{
				 if(this->MobData[i].BonusType == 5)
				 {
					...
					ZtUserDataSend(lpTargetObj->m_Index);
				 }
