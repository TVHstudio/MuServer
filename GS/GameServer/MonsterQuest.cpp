...
{
	this->Init();
	this->Read(gDirPath.GetNewPath("ZtData\\MonsterQuest.ini"));
}
// -------------------------------------------------------------------------------
	...
	for(int n = 0; n < MAX_MISSION_QUEST; n++)
	{
		...
		if( lpMonster->Class == this->MonsterData[UserQuest][n].Mission)
		{
			...
			if(this->CheckDoneQuest(UserIndex) == true)
			{
				...
				ZtFireCracker(lpUser->m_Index, lpUser->X, lpUser->Y);
			}
...
void MonsterQuest::MissionReward(int UserIndex)
{
	...
	for(int n = 0; n < MAX_REWARD_QUEST; n++)
	{
		if(this->RewardData[UserQuest][n].Active)
		{
			...
			case eZen:
				{
					MapC[lpUser->MapNumber].MoneyItemDrop(this->RewardData[UserQuest][n].Reward, lpUser->X, lpUser->Y);	//Скорее всег?нужн?буде?переписать
				}
				break;
			case eFreePoints:
				{
					MessageChat(lpUser->m_Index,"@[MonsterQuest]: FreePoints %d!",this->RewardData[UserQuest][n].Reward);
					lpUser->ZtFreePoints += this->RewardData[UserQuest][n].Reward;
			...
			case eCredit:
				{
					...
					lpUser->ZtCred += this->RewardData[UserQuest][n].Reward;
					ZtUserDataSend(UserQuest);
				}
				break;
			}
		}
...
void MonsterQuest::GCMonsterKill(int aIndex, int n)
{
	...
	GCMonsterQuestKill rSend;	//Добавить структур?
	ZeroMemory(&rSend,sizeof(rSend));	
	rSend.h.set((LPBYTE)&rSend, 0xFB, 18, sizeof(rSend));
	rSend.MissionNum = n;
	rSend.MissionKill = lpUser->m_MonsterQuest.KillCount[n];	//нуэн?урезат?паке?можн?прос?++ ?клиент?
...
{
	//GCMonsterQuestConnect	rSend;	//Добавить структур?
	//ZeroMemory(&rSend,sizeof(rSend));
	//rSend.h.set((LPBYTE)&rSend, 0xFB, 19, sizeof(rSend));
...
void MonsterQuest::GCWindowsOpen(int UserIndex)
{
	...
	GCQuestWindowsOpen rSend;	//Добавить структур?
	ZeroMemory(&rSend,sizeof(rSend));
