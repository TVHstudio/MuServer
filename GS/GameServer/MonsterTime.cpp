...
void CMonsterTime::Load()
{
	...
	this->Read(gDirPath.GetNewPath("ZtData\\MonsterTime.ini"));
}
...
void CMonsterTime::AddMonster(int Num, int Map)
{
	...
	char szMsg[128];
	sprintf(szMsg, "Attack %s in %s", lpMonsterAttr->m_Name, ztMapName(Map));
...
void CMonsterTime::DieMonster(int aIndex, int aMonsterIndex)
{
	...
	for(int i = 0; i < MAX_MONSTERTIME; i++)
	{
		if(aMonsterIndex == this->m_Data[i].Index)
		{
			...
			char szMsg[128];
			sprintf(szMsg, "%s killed %s in %s", lpMonsterAttr->m_Name, lpUser->Name, ztMapName(lpMonster->MapNumber));
			AllSendServerMsg(szMsg);
