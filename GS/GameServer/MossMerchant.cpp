...
{
	SMDFile = fopen("..\\Data\\ZtData\\Events\\MossMerchant.ini", "r");

	if ( SMDFile == NULL )
	{
		MessageBoxA(0,"[MossMerchant] Info file Load Fail [..\\Data\\ZtData\\Events\\MossMerchant.ini]","CRITICAL ERROR",0);
		ExitProcess(1);
		return;
	}
...
void CMossMerchant::ItemBuy(int aIndex,int Item)
{
	...
	ItemSerialCreateSend(aIndex,235,0,0,NewItem,(BYTE)NewLevel,(BYTE)NewDur,(BYTE)NewSkill,(BYTE)NewLuck,(BYTE)NewOpt,aIndex,(BYTE)NewExe,(BYTE)NewAnc);
