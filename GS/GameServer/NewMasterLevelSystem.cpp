...
void CMasterLevelSystem::SetExperienceTable()	//OK
{
	...
	for( int iLevel = 1; iLevel < MAX_MASTER_LEVEL + 1; iLevel++ )
	{
		...
		nTotalLevel			= iLevel + ZtConfig.ZtCustom.MaxLevel;
...
int	CMasterLevelSystem::MasterLevelUp(LPOBJ lpObj, __int64 & iAddExp, bool bEventMapReward, int iMonsterType)	//OK
{
	...
	if( iAddExp > 0 )
	{
		...
		if( iAddExp + lpObj->MLExp >= lpObj->MLNextExp )
		{
			...
			{
				lpObj->MLPoint += ZtConfig.AddMasterLvlPoint.DwLevelPoints;
			}
			else if(lpObj->Class == CLASS_KNIGHT)
			{
				lpObj->MLPoint += ZtConfig.AddMasterLvlPoint.DkLevelPoints;
			}
			else if(lpObj->Class == CLASS_ELF)
			{
				lpObj->MLPoint += ZtConfig.AddMasterLvlPoint.ElfLevelPoints;
			}
			else if ( lpObj->Class == CLASS_MAGUMSA )
			{
				lpObj->MLPoint += ZtConfig.AddMasterLvlPoint.MgLevelPoints;
			}
			else if ( lpObj->Class == CLASS_DARKLORD )
			{
				lpObj->MLPoint += ZtConfig.AddMasterLvlPoint.DlLevelPoints;
			}
			else if(lpObj->Class == CLASS_SUMMONER)
			{
				lpObj->MLPoint += ZtConfig.AddMasterLvlPoint.SumLevelPoints;
			}
#ifdef MONK
			else if ( lpObj->Class == CLASS_MONK )
			{
				lpObj->MLPoint += ZtConfig.AddMasterLvlPoint.RfLevelPoints;
			}
#endif
#else
...
int CMasterLevelSystem::CheckIsMasterLevelCharacter(LPOBJ lpObj)	//
{
	...
	if( lpObj->Level >= ZtConfig.ZtCustom.MaxLevel && lpObj->ChangeUP3rd == 1 )
	{
		return true;
	}
