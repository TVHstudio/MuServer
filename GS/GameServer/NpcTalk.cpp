...
BOOL NpcTalk(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	ZtQuestSystem.NPCClick(lpNpc,lpObj);

	if(g_ZtLicense.user.TvTEvent)
	{
		TvT.NPCClick(lpNpc,lpObj);
	}

	if(g_ZtLicense.user.WinQuest) gWinQuestSystem.NPCClicker(lpNpc,lpObj);

	SearchEvent.NPCClick(lpObj->m_Index,lpNpc->m_Index);

	gTeleportManager.NPCClick(lpNpc,lpObj);
	...
	{
		if(gMonsterQuest.NPCDialog(lpNpc->m_Index,lpObj->m_Index) != FALSE )
		{
			return true;
		}
	}
	...
	{
		BloodHound.NPCClick(lpObj->m_Index,lpNpc->m_Index);
	}

#if(CUSTOM_PERSONAL_SHOP==TRUE)
	if( g_PersonalShopZt.NPC_Dialog(lpObj->m_Index,lpNpc->m_Index) )
	{
		return true;
	}
#endif
#if(CUSTOM_NPC_BUFFER==TRUE)
	if(g_BufferSystem.Dialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
#endif
	if(BMQuest.NPCClick(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}

#if(CUSTOM_MARRIAGE)
	if(g_Marriage.NpcDialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
#endif
#if(ADD_RESET_WINDOW)
	if(gResetSystem.NpcDialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
#endif
#if(EVENT_DUNGEON_SIEGE)
	if(g_DungeonSiege.NpcDialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
#endif
#if(EVENT_TEAM_VS_TEAM)
	if(g_TeamVsTeam.NpcDialog(lpObj->m_Index, lpNpc->m_Index))
	{
		return true;
	}
#endif
#if(CUSTOM_ZT_TELEPORT==TRUE)
	if(g_ZtTeleport.Dialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
#endif
#if(CUSTOM_ZT_TELEPORT_EVO)
	if(g_ZtTeleportEvo.Dialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
#endif

	if(gSystemOfRage.NpcDialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}

	if(gGrandReset.NpcDialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}

#if(CUSTOM_PKCLEAR_NPC==TRUE)
	if( g_PKClear.NPCDialog(lpObj->m_Index,lpNpc->m_Index) )
	{
		return true;
	}
#endif
	...
	if(g_ZtWinQuestSystem.Dialog(lpObj->m_Index,lpNpc->m_Index))
	{
		return true;
	}
#endif
#if(CUSTOM_LOTTERY)
	if(g_Lottery.NpcDialog(lpObj->m_Index, lpNpc->m_Index))
	{
		return true;
	}
#endif
#if(EVENT_CARNAGE)
	if(g_CarnageEvent.NpcDialog(lpObj->m_Index, lpNpc->m_Index))
	{
		return true;
	}
#endif
...
//#endif
		...
		case 492:
			...
			break;
		case 547:
			if (NpcJuliaTalk(lpNpc, lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		...
		case 542:
			...
			break;
		case 522:	//Season 5 ImperialFort
			if(NpcJerint(lpNpc,lpObj) == TRUE)
			{
				return TRUE;
			}
			break;
		case 543:	//Season 5 Gens
		case 544:
			{
				if(Npc_Dialog(lpNpc,lpObj) == TRUE)
				{
					return true;
				}
			}
...
BOOL NpcCastleCrown(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	{
		if(CheckInCharacterItem(lpObj->m_Index,ITEMGET(13,4)))
		{
			return TRUE;
		}
	}
	...
	//if( (abs(lpObj->Y - lpNpc->Y)) > 2 || (abs(lpObj->X - lpNpc->X)) > 2)
	//{
	//	return TRUE;
	//}
	...
	if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
	{
		if(lpObj->X != lpObj->TX || lpObj->Y != lpObj->TY)
		{
			return TRUE;
		}
	}
...
BOOL NpcJuliaTalk(LPOBJ lpNpc, LPOBJ lpObj)	//Season 5
{
	if (!gObjIsConnected(lpObj)) return TRUE;

	PMSG_TALKRESULT pResult;
	PHeadSetBE((LPBYTE)&pResult, 0x30, 0x0B);
	pResult.result = 0x25;
	pResult.level1 = 0;
	DataSend(lpObj->m_Index, (LPBYTE)&pResult, 0x0B);

	return TRUE;
}
BOOL SartigantheAngel(OBJECTSTRUCT *lpNpc, OBJECTSTRUCT *lpObj)	//Season 5 
{
	...
	pMsg.level2 = ((g_DoppelGanger.GetEventTime(lpObj)/1000)/10%60);
	lpObj->TargetShopNumber = 540;
	DataSend(lpObj->m_Index, (LPBYTE)&pMsg, sizeof(pMsg));////5
	return true;
}
BOOL NpcSilverBox(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	g_DoppelGanger.SilverChestOpen(lpNpc);
	return true;
}
BOOL NpcGoldBox(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if(DG_MAP_RANGE(lpNpc->MapNumber))
	{
	...
	{
		#ifdef PANDORA_EVENT
		if(gPandoraBoxEvent.BoxClick(lpNpc, lpObj))
		{
			return true;
		}
		#endif
