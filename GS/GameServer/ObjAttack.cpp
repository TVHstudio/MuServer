...
//#endif
	...
	if(!lpObj->CtrlKey)
	{
		if ( g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
		{
			if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
			{
				...
				{
					if(g_ZtUser.CheckAlliancePlayer(lpObj->m_Index, lpTargetObj->m_Index))
					{
						return false;
					}
				}
	...
	if ( AttackDamage == 0 )
	{
		...
		//	targetdefense -= ( targetdefense * ZtConfig.Skills.Ex_BerserkerDex ) / 100;
		//}
		...
		{
			//int Ex_InnovationPower = GetPrivateProfileInt("Summoner","InnovationPower",30,"..\\Data\\ZtData\\Skills.ini");
		...
		else
		{
			...
			g_MonsterZt.AttackMonster(lpTargetObj->m_Index, lpObj->m_Index, AttackDamage);
#endif
		}
		...
		if ( lpTargetObj->Live )
		{
				...
				case 57:
					...
					else
					{
						if(g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::Artem) || g_ZtLicense.CheckUser(eZtUB::Artem2))
						{
							...
							if(ZtConfig.Character.DKSkillDMGPercentMax > 0)
							{
								if(iSkillDMGPercent > ZtConfig.Character.DKSkillDMGPercentMax)
								{
									iSkillDMGPercent = ZtConfig.Character.DKSkillDMGPercentMax;
								}
							}
				...
				case 78:
					{
						if(g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::Artem) || g_ZtLicense.CheckUser(eZtUB::Artem2))
						{
							...
							if(ZtConfig.Character.DLSkillDMGPercentMax > 0)
							{
								if(iSkillDMGPercentDL > ZtConfig.Character.DLSkillDMGPercentMax)
								{
									iSkillDMGPercentDL = ZtConfig.Character.DLSkillDMGPercentMax;
								}
							}
			...
			if( g_MasterSkillSystem.CheckRequireStatus(skill) )
			{
				...
				if(g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::Artem) || g_ZtLicense.CheckUser(eZtUB::Artem2))
				{
					if(ZtConfig.Character.DKSkillDMGPercentMax > 0)
					{
						if(iMasterSkillDMGPercentDK > ZtConfig.Character.DKSkillDMGPercentMax)
						{
							iMasterSkillDMGPercentDK = ZtConfig.Character.DKSkillDMGPercentMax;
						}
					}					

					if(ZtConfig.Character.DLSkillDMGPercentMax > 0)
					{
						if(iMasterSkillDMGPercentDL > ZtConfig.Character.DLSkillDMGPercentMax)
						{
							iMasterSkillDMGPercentDL = ZtConfig.Character.DLSkillDMGPercentMax;
						}
					}
				}
			...
			if(ZtConfig.Character.SoulBarrierAttackManaDown)
			{
						...
						if(lpObj->Type == OBJ_USER)
						{
							for(int i=205;i<=209;i++)
							{
								...
								if(iAltarIndex != -1)
								{
									...
									{
										AttackDamage = ( AttackDamage * ZtConfig.Character.CWElfAlterDMGPercent ) / 100; 
									}
								}
							}
						}
			...
			if ( g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
			{
				if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
				{
					if ( lpObj->MapNumber == MAP_INDEX_CASTLESIEGE && lpTargetObj->MapNumber == MAP_INDEX_CASTLESIEGE )
					{
						...
						{
							//int AttackByNPC = GetPrivateProfileInt("CastleSiege","AttackByNPC",20,"..\\Data\\ZtData\\Skills.ini");
							AttackDamage =  ( AttackDamage * ZtConfig.Character.CastleAttackByPlayer ) / 100;		//AttackDamage = AttackDamage * 20 / 100;	CastleAttackByPlayer
						}
						else
						{
							//int AttackByPlayer = GetPrivateProfileInt("CastleSiege","AttackByPlayer",40,"..\\Data\\ZtData\\Skills.ini");
							AttackDamage = ( AttackDamage * ZtConfig.Character.CastleAttackByNPC ) / 100;		//AttackDamage = AttackDamage * 40 / 100;
						}
						// ----
						if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal))
						{
							if(g_ZtUser.CheckAlliancePlayer(lpObj->m_Index, lpTargetObj->m_Index))
							{
								AttackDamage = ( AttackDamage * ZtConfig.Character.CastleDamageByAlliance ) / 100;
							}
						}
					}
				}
...
//#endif
			...
			if ( gObjFenrir( lpObj ) )
			{
				...
				{
					iIncPercent += ZtConfig.PrideMu.IncPercent;
				}
#endif
				if ( iIncPercent > 0 )
				{
			...
			if ( gObjFenrir( lpTargetObj ) )
			{
				...
				if(g_ZtLicense.CheckUser(eZtUB::PrideMuLocal) || g_ZtLicense.CheckUser(eZtUB::PrideMu) || g_ZtLicense.CheckUser(eZtUB::Local3) || g_ZtLicense.CheckUser(eZtUB::Local))
				{
					iDecPercent += ZtConfig.PrideMu.DecPercent;
				}
#endif
				if ( iDecPercent > 0 )
				{
		...
		if ( g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
		{
			if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
			{
				if ( lpObj->MapNumber == MAP_INDEX_CASTLESIEGE && lpTargetObj->MapNumber == MAP_INDEX_CASTLESIEGE )
				{
					if( lpObj->m_btCsJoinSide == lpTargetObj->m_btCsJoinSide)
					{
						//int AttackByNPC2 = GetPrivateProfileInt("CastleSiege","AttackByNPC",20,"..\\Data\\ZtData\\Skills.ini");
						AttackDamage = AttackDamage * ZtConfig.Character.CastleAttackByPlayer / 100;		//AttackDamage = AttackDamage * 20 / 100;
					}
					else if(g_ShieldSystemOn == 0)
					{
						//int AttackByPlayer2 = GetPrivateProfileInt("CastleSiege","AttackByPlayer",40,"..\\Data\\ZtData\\Skills.ini");
						AttackDamage = AttackDamage * ZtConfig.Character.CastleAttackByPlayer / 100;		//AttackDamage = AttackDamage * 40 / 100;
					}
					// ----
					if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal))
					{
...
//#endif
	...
	if ( selfdefense == TRUE && bDamageReflect == FALSE )
	{
		...
		{
			if(ZtConfig.PvP.ZtPkPartySystem && lpCallObj->PartyNumber >= 0) //Season 2.5 add-on
			{
				int number = 0;
	...
	if ( AttackDamage > 0 )
	{
...
#if(FIX_REF2)
			if(!ZtConfig.Character.bReflectToReflect)
			{
				if(!byReflect)
				{
					if ( atd_reflect )
		...
		if ( bCombo )
		{
			...
			if(g_ZtLicense.CheckUser(eZtUB::PrideMuLocal) || g_ZtLicense.CheckUser(eZtUB::PrideMu) /*|| g_ZtLicense.CheckUser(eZtUB::Local3) || g_ZtLicense.CheckUser(eZtUB::Local)*/)
			{
				...
				iComboDamage *= ZtConfig.PrideMu.ComboBase / 100;
			}
			else
			{
				iComboDamage = ( Strength + Dexterity + Energy ) / 2;
			...
			iComboDamage = ( iComboDamage * ZtConfig.Character.ComboDamagePercent ) / 100;
	...
	if(AttackDamage > 0)
	{
		if(byReflect)
		{
			...
			g_ZtUser.GCDamageTable(lpObj->m_Index);
		}
		else
		{
			...
			g_ZtUser.GCDamageTable(lpObj->m_Index);
			g_ZtUser.GCDamageTable(lpTargetObj->m_Index);
		}
...
int CObjAttack::GetAttackDamage(LPOBJ lpObj, int targetDefense, BYTE& effect, BOOL bIsOnDuel, CMagicInf* lpMagic) //004B9FA0
{
	...
	if(SkillWeakness != 0)
	{
		//int Ex_WeaknessPower = GetPrivateProfileInt("Summoner","WeaknessPower",30,"..\\Data\\ZtData\\Skills.ini");
...
int CObjAttack::GetAttackDamageWizard(LPOBJ lpObj, int targetDefense, CMagicInf* lpMagic, BYTE& effect, BOOL bIsOnDuel) //0x4BAD10
{
	...
	if ( Right->IsItem() )
	{
		...
		if ((Right->m_Type >= ITEMGET(5,0) && Right->m_Type < ITEMGET(6,0) ) ||
			 Right->m_Type >= ITEMGET(0,0) && Right->m_Type < ITEMGET(1,0)) //season4 add-on
		{
	...
	case 225: //season4 add-on
		{
			if(gObjSearchActiveEffect(lpObj, AT_BERSERKER) == 1)
			{
				...
				//int Ex_BerserkerPower = GetPrivateProfileInt("Summoner","BerserkerPower",50,"..\\Data\\ZtData\\Skills.ini");
