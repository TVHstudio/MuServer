	...
	if (lpObj->Class == CLASS_ELF && lpObj->Type == OBJ_USER )
	{
		...
		if(Right->m_Type >= ITEMGET(4,0) && Right->m_Type < ITEMGET(5,0))
		{
			if ( Left->m_Type == ITEMGET(4,7) )
			{
				...
				if(ZtConfig.Character.NeedArrow) Left->m_Durability -= 1.0f;
...
/*		else if ( (Left->m_Type >= ITEMGET(4,0) && Left->m_Type <= ITEMGET(4,6) ) ||
		...
		else if(Left->m_Type >= ITEMGET(4,0) && Left->m_Type < ITEMGET(5,0))
		{
			if ( Right->m_Type == ITEMGET(4,15) )
			{
				...
				if(ZtConfig.Character.NeedArrow) Right->m_Durability -= 1.0f;
...
BOOL CObjBaseAttack::PkCheck(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	if ( gLanguage == 0 || gLanguage == 2 || gLanguage == 4)
	{
		if ( lpObj->Type == OBJ_USER )
		{
			...
			{
				if(gParty.GetPkLevel(lpObj->PartyNumber) >= ZtConfig.PvP.ZtPkLimit)
				{
					bPlayerKiller = TRUE;
				}
			}

			if(lpObj->m_PK_Level >= ZtConfig.PvP.ZtPkLimit) //6
			{
				bPlayerKiller = TRUE;
			}
	...
	if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER)
	{
		...
		if( g_ZtLicense.CheckUser(eZtUB::Encode)	 || 
		...
		if ( gLanguage == 0 || gLanguage == 2 )
		{
			...
			if(lpObj->m_PK_Level >= ZtConfig.PvP.ZtPkLimit)
			{
				bPlayerKiller = TRUE;
			}
			...
			{
				if(gParty.GetPkLevel(lpTargetObj->PartyNumber) >= ZtConfig.PvP.ZtPkLimit)
				{
					bPlayerKiller = TRUE;
				}
			}

			if(lpTargetObj->m_PK_Level >= ZtConfig.PvP.ZtPkLimit)
			{
				bPlayerKiller = TRUE;
			}
...
BOOL CObjBaseAttack::MissCheckPvP(LPOBJ lpObj , LPOBJ lpTargetObj, int skill, int skillSuccess, int magicsend, BOOL & bAllMiss, BYTE byBarrageCount)
{
	...
	{
		iAttackRate = (float)(iBaseLevel * ZtConfig.Character.DKPvPAttackRateFact1 + ( lpObj->Dexterity * ZtConfig.Character.DKPvPAttackRateFact2) / ZtConfig.Character.DKPvPAttackRateDiv);	// #formula
	}
	else if ( lpObj->Class == CLASS_DARKLORD )
	{
		iAttackRate = (float)(iBaseLevel * ZtConfig.Character.DLPvPAttackRateFact1 + ( lpObj->Dexterity * ZtConfig.Character.DLPvPAttackRateFact2 ));	// #formula
	}
	else if ( lpObj->Class == CLASS_ELF )
	{
		iAttackRate = (float)(iBaseLevel * ZtConfig.Character.EEPvPAttackRateFact1 + ( lpObj->Dexterity * ZtConfig.Character.EEPvPAttackRateFact2 ) / ZtConfig.Character.EEPvPAttackRateDiv);	// #formula
	}
	else if ( lpObj->Class == CLASS_MAGUMSA )
	{
		iAttackRate = (float)(iBaseLevel * ZtConfig.Character.MGPvPAttackRateFact1 + ( lpObj->Dexterity * ZtConfig.Character.MGPvPAttackRateFact2 ) / ZtConfig.Character.MGPvPAttackRateDiv);	// #formula
	}
	else if ( lpObj->Class == CLASS_WIZARD )
	{
		iAttackRate = (float)(iBaseLevel * ZtConfig.Character.DWPvPAttackRateFact1 + ( lpObj->Dexterity * ZtConfig.Character.DWPvPAttackRateFact2 ));	// #formula
	}
	else if ( lpObj->Class == CLASS_SUMMONER )
	{
		iAttackRate = (float)(iBaseLevel * ZtConfig.Character.SUPvPAttackRateFact1 + ( lpObj->Dexterity * ZtConfig.Character.SUPvPAttackRateFact2 ) / ZtConfig.Character.SUPvPAttackRateDiv);	// #formula
	}
	else if( lpObj->Class == CLASS_MONK )
	{
		iAttackRate = (float)(iBaseLevel * ZtConfig.Character.RFPvPAttackRateFact1 + ( lpObj->Dexterity * ZtConfig.Character.RFPvPAttackRateFact2 ) / ZtConfig.Character.RFPvPAttackRateDiv);
	}
...
int CObjBaseAttack::GetTargetDefense(LPOBJ lpObj, LPOBJ lpTargetObj, BYTE& MsgDamage) //Identical gs-cs 56
{
	...
	{
		if(this->MagicDefenceCheck(lpObj, lpTargetObj))
		{
			return this->GetTargetMagicDefense(lpObj, lpTargetObj, MsgDamage);
		}
	}
...
bool CObjBaseAttack::MagicDefenceCheck(LPOBJ lpObj, LPOBJ lpTargetObj)
{
...
int CObjBaseAttack::GetTargetMagicDefense(LPOBJ lpObj, LPOBJ lpTargetObj, BYTE& MsgDamage)
{
	int targetdefense = lpTargetObj->m_MagicDefense;
