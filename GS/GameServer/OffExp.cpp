...
void cOffExp::OffExpStart(int aIndex, int Time)
{
	...
	if (CheckTerrain(lpObj))
	{
		if(g_ZtLicense.CheckUser(eZtUB::Local3) || g_ZtLicense.CheckUser(eZtUB::drynea) || g_ZtLicense.CheckUser(eZtUB::Local2) || g_ZtLicense.CheckUser(eZtUB::Local))
		{
			...
			{
				if(this->PriceCredit > lpObj->ZtCred)
				{
					MsgNormal(aIndex,"[OffExp] Need %d Credit",this->PriceCredit);
					return;
				}
				lpObj->ZtCred -= this->PriceCredit;//Conflict
		...
		else
		{
			if(this->PriceZen)
			{
				if(this->PriceZen > lpObj->Money)
				{
					MsgNormal(aIndex, g_ZtText.GetText(239),this->PriceZen);
					return;
				}
				lpObj->Money -= this->PriceZen;
				GCMoneySend(aIndex,lpObj->Money);
			...
			else if(this->PriceCredit)
			{
				if(this->PriceCredit > lpObj->ZtCred)
				{
					MsgNormal(aIndex,g_ZtText.GetText(240),this->PriceCredit);
					return;
				}
				lpObj->ZtCred -= this->PriceCredit;//Conflict
				ZtUserDataSend(aIndex);
			...
			else if(this->PriceWCoinC)
			{
				if(this->PriceWCoinC > lpObj->GameShop.WCoinC)
				{
					MsgNormal(aIndex,g_ZtText.GetText(241),this->PriceWCoinC);
					return;
				}
				lpObj->GameShop.WCoinC -= this->PriceWCoinC;
				gGameShop.GDSaveUserInfo(aIndex);
			...
			else if(this->PriceWCoinP)
			{
				if(this->PriceWCoinP > lpObj->GameShop.WCoinP)
				{
					MsgNormal(aIndex,g_ZtText.GetText(242),this->PriceWCoinP);
					return;
				}
				lpObj->GameShop.WCoinP -= this->PriceWCoinP;
				gGameShop.GDSaveUserInfo(aIndex);
			...
			else if(this->PriceGoblin)
			{
				if(this->PriceGoblin > lpObj->GameShop.GoblinPoint)
				{
					MsgNormal(aIndex,g_ZtText.GetText(243),this->PriceGoblin);
					return;
				}
				lpObj->GameShop.GoblinPoint -= this->PriceGoblin;
				gGameShop.GDSaveUserInfo(aIndex);
		...
		//******
#ifdef _RECONNECT_
		g_ConnectZt.SendClose(aIndex,OFFAFK_DC);
#else
		CloseClient(aIndex);
		lpObj->OffExp = 1;
#endif
	...
	{
		MsgNormal(aIndex,g_ZtText.GetText(244));
	}
}
...
void cOffExp::MainFunction(int aIndex)
{
...
#ifdef DRYNEA_13_11
	if(g_ZtLicense.CheckUser(eZtUB::Vadim) || g_ZtLicense.CheckUser(eZtUB::drynea))
#else
	...
	if (this->TickPriceEnable)
	{
		if (lpObj->OffExpTickPRice > (this->TickTimeMin * 60))
		{
			...
			{
				if (this->TickPrice > lpObj->ZtCred)
				{
					GJPUserClose(lpObj->AccountID);
					gObjDel(lpObj->m_Index);
					lpObj->OffExp = 0;
