...
void cOffTrade::CreateOffTrade(int aIndex, int type)
{
	...
	{
		MsgNormal(aIndex,g_ZtText.GetText(45));
		return;
	}

	if(lpObj->MapNumber == MAP_INDEX_CASTLESIEGE)
	{
		MsgNormal(aIndex,g_ZtText.GetText(46));
		return;
	}
	...
	{
		MsgNormal(aIndex,g_ZtText.GetText(47));
		return;
	}
...
//#if(CUSTOM_PERSONAL_SHOP==TRUE)
		if(g_PersonalShopZt.Enable)
		{
			...
			{
				sprintf_s(szBuff,g_ZtText.GetText(48),lpObj->Name,lpObj->X,lpObj->Y);
			}
			else if(lpObj->MapNumber==2)
			{
				sprintf_s(szBuff,g_ZtText.GetText(49),lpObj->Name,lpObj->X,lpObj->Y);
			}
			else if(lpObj->MapNumber==3)
			{
				sprintf_s(szBuff,g_ZtText.GetText(50),lpObj->Name,lpObj->X,lpObj->Y);
			}
			else if(lpObj->MapNumber==51)
			{
				sprintf_s(szBuff,g_ZtText.GetText(51),lpObj->Name,lpObj->X,lpObj->Y);
			}
			else if(lpObj->MapNumber==79)
			{
				sprintf_s(szBuff,g_ZtText.GetText(52),lpObj->Name,lpObj->X,lpObj->Y);
			}
			else
			{
				GCServerMsgStringSend(g_ZtText.GetText(53), aIndex, 1);
				return;
			}
...
//#else
			...
			if(type == OFFZEN_DC)
			{
				strcpy(lpObj->m_szPShopText,g_ZtText.GetText(54));
			}
			else if(type == OFFCR_DC)
			{
				strcpy(lpObj->m_szPShopText,g_ZtText.GetText(55));
			}
			else if(type == OFFWC_DC)
			{
				...
				{
					strcpy(lpObj->m_szPShopText,g_ZtText.GetText(56));
				}
			}
			else if(type == OFFWP_DC)
			{
				...
				{
					strcpy(lpObj->m_szPShopText,g_ZtText.GetText(57));
				}
			}
			else if(type == OFFWG_DC)
			{
				strcpy(lpObj->m_szPShopText,g_ZtText.GetText(58));
			}
			else if(type == OFFCHAOS_DC)
			{
				strcpy(lpObj->m_szPShopText,g_ZtText.GetText(59));
			}
			else if(type == OFFBLESS_DC)
			{
				strcpy(lpObj->m_szPShopText,g_ZtText.GetText(60));
			}
			else if(type == OFFSOUL_DC)
			{
				strcpy(lpObj->m_szPShopText,g_ZtText.GetText(61));
			}
			else if(type == OFFLIFE_DC)
			{
				...
				{
					strcpy(lpObj->m_szPShopText,g_ZtText.GetText(62));
				}
	#else
			...
			if(lpObj->MapNumber==0)
				MessaageAllGlobal(g_ZtText.GetText(63),lpObj->m_szPShopText,lpObj->Name,lpObj->X,lpObj->Y);
			else if(lpObj->MapNumber==2)
				MessaageAllGlobal(g_ZtText.GetText(64),lpObj->m_szPShopText,lpObj->Name,lpObj->X,lpObj->Y);
			else if(lpObj->MapNumber==3)
				MessaageAllGlobal(g_ZtText.GetText(65),lpObj->m_szPShopText,lpObj->Name,lpObj->X,lpObj->Y);
			else if(lpObj->MapNumber==51)
				MessaageAllGlobal(g_ZtText.GetText(66),lpObj->m_szPShopText,lpObj->Name,lpObj->X,lpObj->Y);
			else if(lpObj->MapNumber==79)
				MessaageAllGlobal(g_ZtText.GetText(67),lpObj->m_szPShopText,lpObj->Name,lpObj->X,lpObj->Y);
...
//#endif 
	...
	if(type == OFFZEN_DC)
	{
		g_ConnectZt.SendClose(aIndex,OFFZEN_DC);
	}
	else if(type == OFFCR_DC)
	{
		g_ConnectZt.SendClose(aIndex,OFFCR_DC);
	}
	else if(type == OFFWC_DC)
	{
		g_ConnectZt.SendClose(aIndex,OFFWC_DC);
	}
	else if(type == OFFWP_DC)
	{
		g_ConnectZt.SendClose(aIndex,OFFWP_DC);
	}
	else if(type == OFFWG_DC)
	{
		g_ConnectZt.SendClose(aIndex,OFFWG_DC);
	}
	else if(type == OFFCHAOS_DC)
	{
		g_ConnectZt.SendClose(aIndex,OFFCHAOS_DC);
	}
	else if(type == OFFBLESS_DC)
	{
		g_ConnectZt.SendClose(aIndex,OFFBLESS_DC);
	}
	else if(type == OFFSOUL_DC)
	{
		g_ConnectZt.SendClose(aIndex,OFFSOUL_DC);
	}
	else if(type == OFFLIFE_DC)
	{
		g_ConnectZt.SendClose(aIndex,OFFLIFE_DC);
	}
		//g_ConnectZt.SendClose(aIndex,type);
#else
		CloseClient(aIndex);
		lpObj->OffTrade = type;
#endif
	...
	{
		MsgNormal(aIndex,g_ZtText.GetText(68));
	}
}
...
bool cOffTrade::CheckMoney(int aSourceIndex, int aIndex, int ItemPos)
{
	...
	{
		if ( SourceObj->ZtCred < Value )
		{
			LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of ZtCred",
				gObj[aSourceIndex].AccountID, 
				gObj[aSourceIndex].Name, 
				lpObj->AccountID, 
				lpObj->Name);
			//::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 7);
			MsgNormal(aSourceIndex,g_ZtText.GetText(69),Value);
			return false;
		}
	}
	else if(lpObj->OffTrade == OFFWC_DC)
	{
		if ( SourceObj->GameShop.WCoinC < Value )
		{
			LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of WCoinC",
				...
				lpObj->AccountID, 
				lpObj->Name);
			MsgNormal(aSourceIndex,g_ZtText.GetText(70),Value);
			return false;
		}
	}
	else if(lpObj->OffTrade == OFFWP_DC)
	{
		if ( SourceObj->GameShop.WCoinP < Value )
		{
			LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of WCoinP",
				...
				lpObj->AccountID, 
				lpObj->Name);
			MsgNormal(aSourceIndex,g_ZtText.GetText(71),Value);
			return false;
		}
	}
	else if(lpObj->OffTrade == OFFWG_DC)
	{
		if ( SourceObj->GameShop.GoblinPoint < Value )
		{
			LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of WCoinG",
				...
				lpObj->AccountID, 
				lpObj->Name);
			MsgNormal(aSourceIndex,g_ZtText.GetText(72),Value);
			return false;
		}
	}
	else if(lpObj->OffTrade == OFFCHAOS_DC)
	{
		if ( SourceObj->ChaosBank < Value )
		{
			LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Chaos Bank",
				...
				lpObj->AccountID, 
				lpObj->Name);
			MsgNormal(aSourceIndex,g_ZtText.GetText(73),Value);
			return false;
		}
	}
	else if(lpObj->OffTrade == OFFBLESS_DC)
	{
		if ( SourceObj->BlessBank < Value )
		{
			...
			MsgNormal(aSourceIndex,g_ZtText.GetText(74),Value);
			return false;
		}
	}
	else if(lpObj->OffTrade == OFFSOUL_DC)
	{
		if ( SourceObj->SoulBank < Value )
		{
			LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Soul Bank",
				gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, lpObj->AccountID, lpObj->Name);
			MsgNormal(aSourceIndex,g_ZtText.GetText(75),Value);
			return false;
		}
	}
	else if(lpObj->OffTrade == OFFLIFE_DC)
	{
		if ( SourceObj->LifeBank < Value )
		{
			LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Life Bank",
				gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, lpObj->AccountID, lpObj->Name);
			MsgNormal(aSourceIndex,g_ZtText.GetText(76),Value);
			return false;
		}
	}
	else
	{
		...
		if (Value > (MAX_ZEN - lpObj->Money))
		{
			...
			lpObj->AccountID, 
			lpObj->Name);
			MsgNormal(aSourceIndex,g_ZtText.GetText(77), lpObj->Name);
			MsgNormal(aIndex,g_ZtText.GetText(78));
			return false;
		}
#endif
	}
...
void cOffTrade::UserOnlineOffSell(LPOBJ lpObj)
{
	...
	g_ZtGDManager.GD_OfflineAfk(lpObj->m_Index, 0, 0);
