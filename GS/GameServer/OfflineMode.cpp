...
{
	this->Read(gDirPath.GetNewPath("ZtData\\OfflineMode.ini"));
}
// ----------------------------------------------------------------------------------------------

void OfflineMode::Read(char* filename)
{
	//this->StartBonusPay = GetPrivateProfileInt("ZtTeam", "StartBonusPay", 0, filename);
	//this->HourZenPay = GetPrivateProfileInt("ZtTeam", "HourZenPay", 0, filename);
	this->PriceZen = GetPrivateProfileInt("ZtTeam", "PriceZen", 0, filename);
	this->PriceWcoin = GetPrivateProfileInt("ZtTeam", "PriceWcoin", 0, filename);
	this->HourCredit = GetPrivateProfileInt("ZtTeam", "HourCredit", 0, filename);
	this->bOnlyLorenMarket = GetPrivateProfileInt("ZtTeam", "OnlyLorenMarket", 0, filename);

	#if(ADD_OFFMODE_TIMER)
	if(g_ZtLicense.CheckUser(eZtUB::eternalmu))
	{
		this->m_StandartPlayerTime = GetPrivateProfileInt("ZtTeam", "StandartPlayerTime", 12, filename);
		this->m_OtherPlayerMaxTime = GetPrivateProfileInt("ZtTeam", "OtherPlayerMaxTime", 24, filename);
		this->m_PremiumPlayerMaxTime = GetPrivateProfileInt("ZtTeam", "PremiumPlayerMaxTime", 72, filename);
	}
	#endif
...
void OfflineMode::Start(CG_OFFMODE_RESULT* aRecv, int UserIndex)
{
	...
	if(!gObjIsConnectedZt(UserIndex))
	{
		return;
	}
	...
	{
		MsgNormal(UserIndex,g_ZtText.GetText(42));
		return;
	}
	...
	if(/*g_ZtUser.InSafeZone(UserIndex) && */lpUser->m_bPShopOpen == true)
	{
		char szBuff[256] = { 0 };
		if(this->bOnlyLorenMarket && lpUser->MapNumber != 79)
		{
			MsgNormal(UserIndex,g_ZtText.GetText(43));
			return;
		}
		else
		{
			sprintf_s(szBuff,g_ZtText.GetText(44),lpUser->Name);
	...
	{
		if(!g_ZtUser.InSafeZone(UserIndex))
		{
	...
	*/
#if(_RECONNECT_==TRUE)
	g_ConnectZt.SendClose(UserIndex, TRUE);
#else
...
void OfflineMode::Attack(int UserIndex)
{
	...
	if(g_ZtUser.InSafeZone(UserIndex) == true)
	{
		return;
	}
	...
	*/
	...
	{
		if(lpUser->PremiumTimeType == g_PremiumSystemZt.m_iOfflineModeAttackFreePremiumRang && lpUser->PremiumTime > 0)
		{
			AttackFree = true;
		}		
	}
	...
	if(AttackFree == false)
	{
		...
		if(lpUser->m_OfflineAttackTime == 0)
		{
			...
			{
				if(this->HourCredit > lpUser->ZtCred)
				{
					lpUser->m_OfflineMode = false;
					return;
				}
				lpUser->ZtCred -= this->HourCredit;
			}
		}
	}
	...
	bool PremiumResult = false;

	if(g_ZtLicense.CheckUser(eZtUB::SILVER1) || g_ZtLicense.CheckUser(eZtUB::SILVER2) || g_ZtLicense.CheckUser(eZtUB::MedoniAndrei))
	{
		if(lpUser->PremiumTimeType == g_PremiumSystemZt.m_iOfflineModePickUpPremiumRang && lpUser->PremiumTime > 0)
		{
			PremiumResult = true;
		}		
	}
...
void OfflineMode::DarkWizard(int UserIndex)
{
		...
		lpUser->m_OfflineSkill == 381)
	{
		...
		for (int n=0;n<MAX_VIEWPORT_MONSTER;n++)
		{
			...
			if(g_ZtUser.InSafeZone(tObjNum) == true)
			{
				continue;
			}
...
void OfflineMode::DarkKnight(int UserIndex)
{
	...
	if( lpUser->m_OfflineSkill == 41  || 
		...
		lpUser->m_OfflineSkill == 481 )	//42 - урезат?скорость ?2-3 раза
	{
	...
	// Rageful Blow
		...
		lpUser->m_OfflineSkill == 333 )	//42 - урезат?скорость ?2-3 раза
	{
	...
	// BK top skill
			...
			this->OffTimer(UserIndex, 4) == true)	//232 - урезат?скорость ?3-4 раза
	{
		int TargetIndex = this->SearchTarget(UserIndex, MaxDis);
...
void OfflineMode::PickUP(int aIndex)
{
	...
	for(int item_num = 0; item_num < MAX_MAPITEM; item_num++)
	{
		...
		if(lpMapItem->IsItem() == TRUE && lpMapItem->Give == false && lpMapItem->live == true)
		{
			...
			int dis = sqrt((float) (tx*tx+ty*ty));	//разобрат? ка?работает
...
{
	if( g_ZtLicense.CheckUser(eZtUB::SILVER1) || g_ZtLicense.CheckUser(eZtUB::SILVER2) || g_ZtLicense.CheckUser(eZtUB::Local) )
	{
