...
COptimization::COptimization()
{
	...
	this->m_UserIndex = OBJ_STARTUSERINDZT;
...
int COptimization::AddObjectUser(SOCKET aSocket, char* ip)
{
	...
	int aIndex = this->GetFirstEmptryIndex(OBJ_STARTUSERINDZT, OBJMAX);
...
{
	int aIndex = this->GetFirstEmptryIndex(OBJ_MAXMONSTER, OBJ_STARTUSERINDZT);
...
void COptimization::ViewePortCPU(short aIndex)
{
		...
		)
	{
		...
		for(int n = OBJ_STARTUSERINDZT; n < this->m_UserIndex; n++)
		{
			if(gObj[n].Connected == PLAYER_PLAYING && aIndex != n)
			{
				if(gObj[n].m_State == 1 || gObj[n].m_State == 2)
