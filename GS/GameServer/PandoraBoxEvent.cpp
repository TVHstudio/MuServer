...
{
	this->Init();
	this->Enable = GetPrivateProfileInt("ZtTeam","Enable",0,PANDORA_EVENT_DIR);
	if (!this->Enable)
	{
		return;
	}
	
	this->EventTime = GetPrivateProfileInt("ZtTeam","EventTime",10,PANDORA_EVENT_DIR);
	this->RewardCredits = GetPrivateProfileInt("ZtTeam","RewardCredits",0,PANDORA_EVENT_DIR);
	this->RewardWcoinC = GetPrivateProfileInt("ZtTeam","RewardWcoinC",0,PANDORA_EVENT_DIR);
	this->RewardWcoinP = GetPrivateProfileInt("ZtTeam","RewardWcoinP",0,PANDORA_EVENT_DIR);
	this->RewardGoblinPoints = GetPrivateProfileInt("ZtTeam","RewardGoblinPoints",0,PANDORA_EVENT_DIR);
...
#if(NEWTIMEREX)
	else if(g_TimerZt.GetSecond() == 00)
#else
...
void cPandoraBoxEvent::RespawnBox(CordsBox RandCord)
{
	...
	if (g_ZtLicense.CheckUser(eZtUB::ulasevich) || 
		g_ZtLicense.CheckUser(eZtUB::ConflictNoCustom) || 
	...
	if(result >= 0 && result < OBJ_MAXMONSTER)
	{
		...
		gMSetBase.m_Mp[MobPos].m_MapNumber	= this->ActiveMap;
		gMSetBase.m_Mp[MobPos].m_W			= cX;
		gMSetBase.m_Mp[MobPos].m_H			= cY;
		gMSetBase.m_Mp[MobPos].m_X			= cX;
		gMSetBase.m_Mp[MobPos].m_Y			= cY;
		gMSetBase.m_Mp[MobPos].m_ArrangeType = 2;

		gObj[result].m_PosNum = MobPos;
...
bool cPandoraBoxEvent::BoxClick(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	MessaageAllGlobal("[Pandora Event] Map: %s X: %d Y: %d", ztMapName(lpObj->MapNumber), lpObj->X, lpObj->Y);
	gObjDel(this->BoxIndex);
	this->BoxIndex = -1;
...
bool cPandoraBoxEvent::ProcessKill(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	//MessaageAllGlobal("[Pandora Event] %s Capture Box in Map: %s X: %d Y: %d", lpObj->Name, ztMapName(lpObj->MapNumber), lpObj->X, lpObj->Y);	
	MessaageAllGlobal("[Pandora Event] %s Capture Box", lpObj->Name);	
	MessaageAllGlobal("[Pandora Event] Map: %s X: %d Y: %d", ztMapName(lpObj->MapNumber), lpObj->X, lpObj->Y);
...
void cPandoraBoxEvent::Prize()
{
	...
	{
			lpObj->ZtCred += this->RewardCredits;
#ifndef FIX_DS_CRASH
...
void cPandoraBoxEvent::LoadMonsterPos()
{
	...
	if(g_MapServerManager.CheckMapCanMove(0) == FALSE)
	{
	...
	gMSetBase.m_Mp[gMSetBase.m_Count].m_Type		= 542;
	gMSetBase.m_Mp[gMSetBase.m_Count].m_MapNumber	= 0;
	gMSetBase.m_Mp[gMSetBase.m_Count].m_W			= 0;
	gMSetBase.m_Mp[gMSetBase.m_Count].m_H			= 0;
	gMSetBase.m_Mp[gMSetBase.m_Count].m_X			= 0;
	gMSetBase.m_Mp[gMSetBase.m_Count].m_Y			= 0;
	gMSetBase.m_Mp[gMSetBase.m_Count].m_ArrangeType = 2;
	gMSetBase.m_Mp[gMSetBase.m_Count].m_Dir			= 2;

	this->BoxIndex = -1;
	this->BoxPos = gMSetBase.m_Count;
