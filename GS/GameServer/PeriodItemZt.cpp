...
CPeriodItemZt g_PeriodItemZt;

CPeriodItemZt::CPeriodItemZt()	//OK
{
...
//006bb120
void CPeriodItemZt::Initialize()	//OK
{
...
//006bb1e0
int CPeriodItemZt::AddPeriodInfo(LPOBJ lpObj)	//OK
{
...
//006bb360
bool CPeriodItemZt::RemovePeriodInfo(LPOBJ lpObj)	//OK
{
...
//006bb400
bool CPeriodItemZt::IsCorrectUser(LPOBJ lpObj)	//OK
{
	int iPeriodItemSlotIndex = lpObj->m_PeriodDataIndex;
...
//006bb480
int CPeriodItemZt::AddPeriodItemData(LPOBJ lpObj,WORD wItemCode,BYTE btEffectType1,BYTE btEffectType2,int iDuration,long lExpireDate)	//OK
{
	int iPeriodItemSlotIndex = lpObj->m_PeriodDataIndex;
...
//006bb680
int CPeriodItemZt::AddPeriodItemData(LPOBJ lpObj,WORD wItemCode,DWORD dwSerial,int iDuration)	//OK
{
	int iPeriodItemSlotIndex = lpObj->m_PeriodDataIndex;
	long lExpireDate = 0;
...
//006bb8d0
int CPeriodItemZt::AddPeriodItemData(LPOBJ lpObj,WORD wItemCode,DWORD dwSerial,int iDuration,long lExpireDate)	//OK
{
	int iPeriodItemSlotIndex = lpObj->m_PeriodDataIndex;
...
//006bbab0
bool CPeriodItemZt::RemovePeriodItemData(LPOBJ lpObj, BYTE btItemType, WORD wItemCode, unsigned long dwSerial)	//OK
{
	if( !lpObj )
		return false;
	...
	if( btItemType == 1 )
	{
		...
		lpBuffData = (LPBUFF_EFFECT_DATE)g_BuffManager.GetBuffInfo(wItemCode);
...
//006bbc00
bool CPeriodItemZt::RemovePeriodItemData(LPOBJ lpObj,WORD wItemCode,unsigned long dwSerial)	//OK
{
	int iInventoryPosition = 0;
...
//006bbca0
void CPeriodItemZt::CheckPeriodData()	//OK
{
	LPOBJ lpObj = 0;
		...
		if( this->m_PeriodData[i].bInUse == true && this->m_PeriodData[i].lpObj != NULL )
		{
			...
			if( lpObj->Type == OBJ_USER && lpObj->Connected >= PLAYER_LOGGED )
			{
				for(int iItemSlotIndex = 0; iItemSlotIndex < MAX_ITEMDATA; iItemSlotIndex++)
				{
					...
					{
						LogAddTD("[PeriodItemZt][Remove PeriodItem] Expire PeriodItem Data. User Id : %s(%d), Name : %s, Item : %s(%d), Type : %d, Serial : %u, ExpireDate : %d",
...
//006bbfb0
long CPeriodItemZt::ConvertStringToTime(char const* pchTime)	//OK
{
	int iYear = 0;
	int iMonth = 0;
	int iDay = 0;
...
long CPeriodItemZt::GetCurrentDate()	//~OK
{
	CTime tCurrentTime(CTime::GetTickCount());
...
//006bc220
//gObjUseImprovePotion
long CPeriodItemZt::GetExpireDate(int iDuration)	//~OK
{
	CTime tExpireTime;
	CTime tCurrentTime(CTime::GetTickCount());
...
//006bc2b0
bool CPeriodItemZt::CheckExpireDate(long dwItemExpireDate)	//OK
{
...
//006bc300
unsigned long CPeriodItemZt::GetLeftDate(long lExpireDate)	//OK
{
...
//CGUseItemRecv
//CGUseItemRecv
bool CPeriodItemZt::SetPeriodItemInfo(LPOBJ lpObj,WORD wItemCode,DWORD dwSerial,DWORD dwDuration)	//~OK
{
	if( !lpObj )
		return false;
	...
	{
		lpBuffData		= (LPBUFF_EFFECT_DATE)g_BuffManager.GetBuffInfo(wItemCode);
		lpItemEffect	= g_ItemAddOption.SearchItemEffectType1(wItemCode);

		if( !lpBuffData || !lpItemEffect )
			return false;

		if( gObjApplyBuffEffectItemPeriod(lpObj,lpBuffData->btIndex,dwDuration) == TRUE )
		{
			ItemData.Clear();
			ItemData.btType = 1;
			ItemData.wItemCode = wItemCode;
...
//006bc600
bool CPeriodItemZt::SetPeriodItemInfo(LPOBJ lpObj,BYTE btItemType,WORD wItemCode,DWORD dwSerial,DWORD dwDuration)	//OK
{
	if( !lpObj )
		return false;
...
//006bc800
bool CPeriodItemZt::SetPeriodItemInfo(LPOBJ lpObj,WORD wItemCode)	//OK
{
	if( !lpObj )
		return false;
	...
	if( gObjApplyBuffEffectItemPeriod(lpObj,lpBuffData->btIndex,lpItemEffect->iEffectValidTime) == TRUE )
	{
		ItemData.Clear();
		ItemData.btType = 1;
		ItemData.wItemCode = wItemCode;
...
//006bc990
bool CPeriodItemZt::SetDisableItemToExpire(LPOBJ lpObj,int iInventoryPosition)	//OK
{
	if( !lpObj )
		return false;
	...
	lpObj->pInventory[iInventoryPosition].SetPeriodItemZtpire();
	GCItemListSend(lpObj->m_Index);
	g_PeriodItemZt.OnRequestPeriodItemList(lpObj);

}

//006bca60
void CPeriodItemZt::SetExpireNotMatchedData(LPOBJ lpObj)	//OK
{
	...
	for( i = 0; i < MAIN_INVENTORY_SIZE; i++)
	{
		...
		{
			if( !lpObj->pInventory[i].IsPeriodItemZtpire() )
			{
				...
				{
					lpObj->pInventory[i].SetPeriodItemZtpire();
...
//006bcc90
void CPeriodItemZt::RequestPeriodItemInfo(LPOBJ lpObj)	//OK
{
	PMSG_REQ_PERIODITEMZT_SELECT pMsg;
...
//006bcd10
//gObjUseImprovePotion
void CPeriodItemZt::RequestPeriodItemInsert(LPOBJ lpObj,ITEMDATA* lpItemData,DWORD dwDuration)	//~OK
{
	PMSG_REQ_PERIODITEMZT_INSERT pMsg;
	pMsg.wUserIndex = lpObj->m_Index;
	pMsg.dwUserGuid = lpObj->DBNumber;
	memcpy(pMsg.chCharacterName,lpObj->Name,11);
	...
	LogAddTD("[PeriodItemZt][Insert PeriodItem] Request Insert PeriodItem Data. User Id : %s(%d), Name : %s, Item : %s(%d), Type : %d, Serial : %u, Duration : %d",
...
//006bce80
void CPeriodItemZt::RequestPeriodItemDelete(LPOBJ lpObj,ITEMDATA* lpItemData)	//OK
{
	PMSG_REQ_PERIODITEMZT_DELETE pMsg;
	pMsg.wUserIndex = lpObj->m_Index;
	pMsg.dwUserGuid = lpObj->DBNumber;
	memcpy(pMsg.chCharacterName,lpObj->Name,11);
	...
	LogAddTD("[PeriodItemZt][Delete PeriodItem] Request Delete PeriodItem Data. User Id : %s(%d), Name : %s, Item : %s(%d), Type : %d, Serial : %u",
...
//006bcf90
void CPeriodItemZt::RequestPeriodItemUpdate(LPOBJ lpObj,ITEMDATA* lpItemData)	//OK
{
	// ----
}

//006bcfb0
void CPeriodItemZt::DGPeriodItemZtInsert(PMSG_ANS_PERIODITEMZT_INSERT* lpMsg)	//OK
{
	...
	case 0:
		...
		{
			gObjApplyBuffEffectItemPeriod(lpObj,lpMsg->wItemCode,lpMsg->dwDuration);
		}
	...
	LogAddTD("[PeriodItemZt][Insert PeriodItem] OnRequest Insert PeriodItem Data. User Id : %s(%d), Name : %s, Item : %s(%d), Type : %d, Serial : %u, Duration : %d, Result : %d",
...
//006bd190
void CPeriodItemZt::DGPeriodItemZtDelete(PMSG_ANS_PERIODITEMZT_DELETE* lpMsg)	//OK
{
	int iIndex = lpMsg->wUserIndex;
	...
	{
		LogAddTD("[PeriodItemZt][Delete PeriodItem] OnRequest Delete PeriodItem Data. User Id : %s(%d), Name : %s, Result : %d",
			lpObj->AccountID,lpObj->DBNumber,lpObj->Name,lpMsg->btResultCode);
	}
}

//006bd260
void CPeriodItemZt::DGPeriodItemZtSelect(PMSG_ANS_PERIODITEMZT_SELECT* lpMsg)	//OK
{
	int iIndex = lpMsg->wUserIndex;
	...
	if( lpMsg->btResultCode > 0 )
	{
		...
		for(int i = 0; i < iItemCount; i++)
		{
			...
			{
				gObjApplyBuffEffectItemPeriod(lpObj,ItemList.ItemData[i].wItemCode,dwDuration);
			}
			...
			LogAddTD("[PeriodItemZt][SelectItemList] User Id : %s(%d), Name : %s, Item Code : %d(%s), PeriodType : %d, Serial : %u, Duration : %d, ExpireDate : %d",
...
//006bd550
void CPeriodItemZt::DGPeriodItemZtExpiredItemList(PMSG_ANS_PERIODITEMZT_EXPIRED_ITEMLIST* lpMsg)	//OK
{
	int iIndex = lpMsg->wUserIndex;
	...
	for(int i = 0; i < lpMsg->btExpiredItemCount; i++)
	{
		...
		if( iInventoryPosition != -1 )
		{
			LogAddTD("[PeriodItemZt][SetExpiredItemList] OnRequest Expired Item Set Disable. User Id : %s(%d), Name : %s, Item : %s(%d), Serial : %u",
...
//006bd6d0
void CPeriodItemZt::DGPeriodItemZtListCount(PMSG_ANS_PERIODITEMZT_LIST_COUNT* lpMsg)	//OK
{
	int iIndex = lpMsg->wUserIndex;
	...
	LogAddTD("[PeriodItemZt][SelectItemListCount] End PeriodItem List Receive User Id : %s(%d), Name : %s",
...
//006bd7c0
void CPeriodItemZt::DGPeriodItemZtList(PMSG_ANS_PERIODITEMZT_LIST* lpMsg)	//OK
{
	int iIndex = lpMsg->wUserIndex;
	...
	if( lpMsg->btItemType == 1 )
	{
		if( !gObjApplyBuffEffectItemPeriod(lpObj,lpMsg->wItemCode,dwDuration) )
		{
			LogAddTD("[PeriodItemZt][SelectItemList] Failed User Id : %s(%d), Name : %s, Item Code : %d(%s), PeriodType : %d, Serial : %u, Duration : %d, ExpireDate : %d",
...
//006bdaf0
int CPeriodItemZt::GetItemFromInventory(LPOBJ lpObj,WORD wItemCode,DWORD dwSerial)	//OK
{
	int iReturnIndex = -1;
	...
	for(int i = 0; i < MAIN_INVENTORY_SIZE; i++)
	{
		if( lpObj->pInventory[i].IsItem() )
		{
			...
			{
#ifdef DEBUG_CODE
				LogAddC(2,"wItemCode:%d dwSerial:%d lpObj->pInventory[i].GetNumber():%d",wItemCode,dwSerial,lpObj->pInventory[i].GetNumber());
#endif
...
//ManagementProc (x2)
//CGMMng::_ClearInven
void CPeriodItemZt::OnRequestPeriodItemList(LPOBJ lpObj)	//OK
{
...
//006bde30
void CPeriodItemZt::OnRequestPeriodItemListOnce(LPOBJ lpObj,ITEMDATA* lpItemData)	//OK
{
	if( !lpObj || !lpItemData )
		return;
...
//006bdf40
void CPeriodItemZt::OnRequestPeriodExpiredItem(LPOBJ lpObj,ITEMDATA* lpItemData)	//OK
{
	// not implemented
}

//006bdf60
bool CPeriodItemZt::SendPeriodItemInfoOnce(LPOBJ lpObj,CItem* lpItem)	//OK
{
