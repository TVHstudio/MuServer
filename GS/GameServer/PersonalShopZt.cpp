...
void PersonalShopZt::Load()
{
	...
	if(!g_ZtLicense.user.PerosnalStoreZt)
	{
		return;
	}

	this->Read(gDirPath.GetNewPath("ZtData\\PersonalShopZt.ini"));
}
// ----------------------------------------------------------------------------------------------

void PersonalShopZt::Read(char* File)
{
	...
	if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || 
	   g_ZtLicense.CheckUser(eZtUB::ProtectDebug) || g_ZtLicense.CheckUser(eZtUB::mu4you) || g_ZtLicense.CheckUser(eZtUB::NSGames))
	{
...
bool PersonalShopZt::NPC_Dialog(int aIndex, int aIndexNPC)
{
	if(!this->Enable)
	{
 		return false;
...
void PersonalShopZt::Search(int aIndex, CG_PersonalPage* aRecv)
{
	if(!this->Enable)
	{
		return;
...
void PersonalShopZt::UpdatePSInventoryUser(int aIndex)
{
	//if(!this->Enable)
	//{
	//	return;
...
void PersonalShopZt::UpdatePSInventoryTarget(int aIndex, int aTargetIndex)
{
	if(!this->Enable)
	{
 		return;
...
int PersonalShopZt::GetMoney(int aIndex, int Pos)
{
...
bool PersonalShopZt::CheckPrice(int Zen, int Bonus, int Bless, int Soul, int Chaos, int Life, int Credit)
{
	bool Result = 0;
...
bool PersonalShopZt::PShop_CheckInventoryPrice(short aIndex)
{
	if(!gObjIsConnectedZt(aIndex))
	{
		return false;
	...
	for(int i = MAIN_INVENTORY_SIZE; i < INVENTORY_SIZE; i++)
	{
		if(lpObj->Inventory1[i].IsItem() == 1)
		{
			...
			#if(DEV_PERSONAL_CREDIT)
			if(g_PersonalShopZt.CreditPrice)
			{
