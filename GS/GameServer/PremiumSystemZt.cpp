...
PremiumSystemZt::PremiumSystemZt()
{
	this->Init();
}
// -------------------------------------------------------------------------------
...
void PremiumSystemZt::Load()
{
	...
	if(g_ZtLicense.user.PremiumZt == false)
	{
		return;
	}

	this->Read(gDirPath.GetNewPath("ZtData\\PremiumSystemZt.ini"));

	this->CheckValue();
}
// -------------------------------------------------------------------------------

void PremiumSystemZt::Read(char* File)
{
...
void PremiumSystemZt::CheckValue()
{
...
void PremiumSystemZt::UserCalculatorPlus(int aIndex)
{
...
int PremiumSystemZt::UserExpPlus(int aIndex)
{
	if(!this->ActiveSystem)
	{
		return 0;
...
void PremiumSystemZt::UserZenPlus(int aIndex, float& Money)
{
	if(!this->ActiveSystem)
	{
		return;
...
void PremiumSystemZt::UserConnect(int aIndex)
{
	if(!this->ActiveSystem)
	{
		return;
	...
	/*if(lpUser->PremiumTimeType == eVip1)
	{
		g_BuffManagerEx.AddZtBuffData(aIndex, AT_VIP_SILVER, lpUser->PremiumTime);
	}
	else if(lpUser->PremiumTimeType == eVip2)
	{
		g_BuffManagerEx.AddZtBuffData(aIndex, AT_VIP_GOLD, lpUser->PremiumTime);
	}
	else if(lpUser->PremiumTimeType == eVip3)
	{
		g_BuffManagerEx.AddZtBuffData(aIndex, AT_VIP_PLATINUM, lpUser->PremiumTime);
	}*/
...
void PremiumSystemZt::PremiumTime(int aIndex)
{
	if(!this->ActiveSystem)
	{
		return;
	...
	if(lpUser->PremiumTime <= 0)
	{
		...
		MessageChat(aIndex, g_ZtText.GetText(252));
	}

}
// -------------------------------------------------------------------------------

void PremiumSystemZt::CG_RecvBuy(int aIndex, CG_PREMIUM_BUY* Recv)
{
	if(!this->ActiveSystem)
	{
		return;
	...
	if(iPremiumType != lpUser->PremiumTimeType)
	{
		/*if(lpUser->PremiumTimeType == eVip1)
		{
			g_BuffManagerEx.DeleteZtBuffData(aIndex, AT_VIP_SILVER);
		}
		else if(lpUser->PremiumTimeType == eVip2)
		{
			g_BuffManagerEx.DeleteZtBuffData(aIndex, AT_VIP_GOLD);
		}
		else if(lpUser->PremiumTimeType == eVip3)
		{
			g_BuffManagerEx.DeleteZtBuffData(aIndex, AT_VIP_PLATINUM);
		}*/
...
void PremiumSystemZt::GC_Send(int aIndex, CG_SEND_OPEND_WIN* Recv)
{
	if(!this->ActiveSystem)
	{
		return;
	...
	pSend.UserCredit = (WORD)lpUser->ZtCred;
...
void PremiumSystemZt::InfoMessage(int aIndex)
{
	if ( OBJMAX_RANGE(aIndex) == FALSE )
	{
		LogAdd("error : %s %d", __FILE__, __LINE__ );
	...
	if(lpUser->PremiumTimeType == eVip1)
	{
		MessageChat(aIndex, g_ZtText.GetText(246), Plus1ExpProcent);
		MessageChat(aIndex, g_ZtText.GetText(247), Plus1DropZenProcent);
		MessageChat(aIndex, g_ZtText.GetText(248), Plus1DamageProcent);
		MessageChat(aIndex, g_ZtText.GetText(249), Plus1WizardDamageProcent);
		MessageChat(aIndex, g_ZtText.GetText(250), Plus1DefenceProcent);
	}
	else if(lpUser->PremiumTimeType == eVip2)
	{
		MessageChat(aIndex, g_ZtText.GetText(246), Plus2ExpProcent);
		MessageChat(aIndex, g_ZtText.GetText(247), Plus2DropZenProcent);
		MessageChat(aIndex, g_ZtText.GetText(248), Plus2DamageProcent);
		MessageChat(aIndex, g_ZtText.GetText(249), Plus2WizardDamageProcent);
		MessageChat(aIndex, g_ZtText.GetText(250), Plus2DefenceProcent);
	}
	else if(lpUser->PremiumTimeType == eVip3)
	{
		MessageChat(aIndex, g_ZtText.GetText(246), Plus3ExpProcent);
		MessageChat(aIndex, g_ZtText.GetText(247), Plus3DropZenProcent);
		MessageChat(aIndex, g_ZtText.GetText(248), Plus3DamageProcent);
		MessageChat(aIndex, g_ZtText.GetText(249), Plus3WizardDamageProcent);
		MessageChat(aIndex, g_ZtText.GetText(250), Plus3DefenceProcent);
	}

	MessageChat(aIndex,g_ZtText.GetText(251),(lpUser->PremiumTime/60));

	this->UserBuff(aIndex);
}

void PremiumSystemZt::GC_Config(int aIndex)
{
	if(!this->ActiveSystem)
	{
		return;
...
void PremiumSystemZt::UserBuff(int aIndex)
{
	if(!this->ActiveSystem)
	{
		return;
