...
void cPriceManager::SellPriceItem(int aIndex,int iPos,int & money)
{
	...
	for(int i=0;i<this->sCount;i++)
	{
			 ...
			 lpObj->pInventory[iPos].m_Level == this->SellPrice[i].Level )
		{
			...
			{
				lpObj->ZtCred += this->SellPrice[i].Price;
				money = 0;
	...
	for(int i=0;i<this->bCount;i++)
	{
		if ( iType == ITEMGET(this->BuyPrice[i].Type,this->BuyPrice[i].Index) && this->BuyPrice[i].Level == ItemLevel)
		{
			if(this->BuyPrice[i].PriceType == 0)	//Zen
			{
				...
				{
					MsgNormal(aIndex,g_ZtText.GetText(79),iStoreTaxMoney);
					return false;
				}
			}
			else if(this->BuyPrice[i].PriceType == 1)	//Credits
			{
				...
				//	lpObj->ZtCred -= this->BuyPrice[i].Price;
				//	ZtUserDataSend(lpObj->m_Index);
				//	iStoreTaxMoney = 0;
				//}
				//else
				//{
					if(this->BuyPrice[i].Price > lpObj->ZtCred)
					{
						MsgNormal(aIndex,g_ZtText.GetText(80),this->BuyPrice[i].Price);
						return false;
					}
					MsgNormal(aIndex,g_ZtText.GetText(81),this->BuyPrice[i].Price);
					lpObj->ZtCred -= this->BuyPrice[i].Price;
					ZtUserDataSend(lpObj->m_Index);
					iStoreTaxMoney = 0;
				//}
			...
			else if(this->BuyPrice[i].PriceType == 2)	//WcoinC
			{
				...
				{
					MsgNormal(aIndex,g_ZtText.GetText(82),this->BuyPrice[i].Price);
					return false;
				}
				MsgNormal(aIndex,g_ZtText.GetText(83),this->BuyPrice[i].Price);
			...
			else if(this->BuyPrice[i].PriceType == 3)	//Wcoin
			{
				...
				{
					MsgNormal(aIndex,g_ZtText.GetText(84),this->BuyPrice[i].Price);
					return false;
				}
				MsgNormal(aIndex,g_ZtText.GetText(85),this->BuyPrice[i].Price);
			...
			else if(this->BuyPrice[i].PriceType == 4)	//Wcoin
			{
				...
				{
					MsgNormal(aIndex,g_ZtText.GetText(86),this->BuyPrice[i].Price);
					return false;
				}
				MsgNormal(aIndex,g_ZtText.GetText(87),this->BuyPrice[i].Price);
