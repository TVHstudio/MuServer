		...
		if ( Token == 1 )
		{
			...
			else if ( st == 2 )
			{
				...
				while ( true )
				{
					...
					{
						this->m_iEventItemDropRateZt[iProbability] = (int)TokenNumber; //good
					}
					else
					{
						this->m_iEventItemDropRateZt[iProbability] = this->m_iEventItemDropRateZt[iProbability-1] + (int)TokenNumber; //great
					}
...
BOOL CProbabilityItemBag::SortItem(int n)
{
	...
	while(loc2-- != 0)
	{
		...
		if(n >= 10000 - this->m_iEventItemDropRateZt[loc2] && n < 10000 - this->m_iEventItemDropRateZt[loc2-1])
		{
...
BOOL CProbabilityItemBag::DropNewYearLuckMonsterReward(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	...
	{
		loc14 = rand()%10000;
		if ( this->m_iRateKindCount == 0 || loc14 < 10000 - this->m_iEventItemDropRateZt[this->m_iRateKindCount-1])
		{
...
{ 
	...
	// ----
	if ( !this->m_iRateKindCount || iDropItemRate < 10000 - this->m_iEventItemDropRateZt[this->m_iRateKindCount-1] )
	{
		MapC[lpObj->MapNumber].MoneyItemDrop(this->m_iDropZen, cX, cY);
	...
	// ----
	int iDropItemNum = rand() % this->GetBagCount();	// this->GetDropItemNumber(iDropItemRate);
	// ----
	if( cX || cY )
	{
		X = cX;
...
{ 
	...
	// ----
	if ( !this->m_iRateKindCount || iDropItemRate < 1000000 - this->m_iEventItemDropRateZt[this->m_iRateKindCount-1] )
	{
		MapC[lpObj->MapNumber].MoneyItemDrop(this->m_iDropZen, cX, cY);
	...
	// ----
	int iDropItemNum = rand() % this->GetBagCount();// this->GetDropItemNumberEx(iDropItemRate);
	// ----
	if( cX || cY )
	{
		X = cX;
