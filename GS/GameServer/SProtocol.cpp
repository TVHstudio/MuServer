...
#if(_RECONNECT_)
				if( gObj[aIndex].m_ConnectZt )
				{
					g_ConnectZt.SendGame(aIndex);
					bJoinResult = false;
				}
#endif
#if(OFFLINE_MODE_RESTORE)
				if(gObj[aIndex].m_ReOfflineMode)
				{
					g_OfflineMode.GDReqOffCharacter(aIndex);
					bJoinResult = false;
				}
#endif
			}
...
#if _RECONNECT_
	g_ConnectZt.SendClose(number,0);
#endif
...
void BroadCastMessage(BroadCastMessageInfo* lpResult)
{
	...
	case 1:
		...
		break;
	case 2:
		MessageSendAllEx(ZtConfig.Command.PostColor, lpResult->Sender, "%s %s", ZtConfig.Command.PostPrist, lpResult->Text);
		break;
	}
