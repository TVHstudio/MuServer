...
void cSearchEvent::NPCClick(int aIndex, int aNPC)
{
	...
	if(NpcObj->Class == this->NPC && this->IndexNPC == NpcObj->m_Index)
	{
		...
		if(this->Stage == 1)
		{
			...
			MessaageAllGlobal(g_ZtText.GetText(92), lpObj->Name);
			...
			MessaageAllGlobal(g_ZtText.GetText(93));
		...
		else if(this->Stage == 2)
		{
			...
			MessaageAllGlobal(g_ZtText.GetText(94),lpObj->Name);
			...
			/*if(g_ZtLicense.CheckUser(Encode))
			{
			...
			{
				MessaageAllGlobal(g_ZtText.GetText(95));
			}
#else
		...
		else if(this->Stage == 3)
		{
			...
			{
				MessaageAllGlobal(g_ZtText.GetText(96), lpObj->Name);
			}
#else
...
void cSearchEvent::EventStart()
{
	...
	MessaageAllGlobal(g_ZtText.GetText(97));
	this->Start = true;
	this->Stage = 1;
}
...
void cSearchEvent::StartNPC()
{
	...
	//LogAddC(2, "[DEBUG] ms: %d sec: %d :: tick: %d", t.wMilliseconds, t.wSecond, g_TimerZt.GetSecond());
			...
			if(TickTime[i].Min > 5)
			{
				for(int j=1; j <= 5; j++)
				{
					...
					if(TickTime[i].Hour == t.wHour && (TickTime[i].Min-j) == t.wMinute)
						MessaageAllGlobal(ZtConfig.MessageConfig.SearchEvent, j);
				}
			}
			else if(TickTime[i].Min == 4)
			{
					if(TickTime[i].Hour-1 == t.wHour && 59 == t.wMinute)
						MessaageAllGlobal(ZtConfig.MessageConfig.SearchEvent, 5);
					if(TickTime[i].Hour == t.wHour && 0 == t.wMinute)
						MessaageAllGlobal(ZtConfig.MessageConfig.SearchEvent, 4);
					if(TickTime[i].Hour == t.wHour && 1 == t.wMinute)
						MessaageAllGlobal(ZtConfig.MessageConfig.SearchEvent, 3);
					if(TickTime[i].Hour == t.wHour && 2 == t.wMinute)
						MessaageAllGlobal(ZtConfig.MessageConfig.SearchEvent, 2);
					if(TickTime[i].Hour == t.wHour && 3 == t.wMinute)
						MessaageAllGlobal(ZtConfig.MessageConfig.SearchEvent, 1);
...
void cSearchEvent::Present(int aIndex)
{
	...
	{
		lpObj->ZtCred += this->BonusCredits;
	}
...
void cSearchEvent::EventEnd()
{
	...
	MessaageAllGlobal(g_ZtText.GetText(98));
}