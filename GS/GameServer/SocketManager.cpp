...
bool CSocketManager::DataSend(int index,BYTE* lpMsg,int size) // OK
{
	...
	if(WSASend(gObj[index].m_socket,&lpIoContext->wsabuf,1,&SendSize,Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
...
void CSocketManager::Disconnect(int index) // OK
{
	...
	if(closesocket(gObj[index].m_socket) == SOCKET_ERROR && WSAGetLastError() != WSAENOTSOCK)
	{
...
void CSocketManager::OnRecv(int index,DWORD IoSize,IO_RECV_CONTEXT* lpIoContext) // OK
{
	...
	if(WSARecv(lpObj->m_socket,&lpIoContext->wsabuf,1,&RecvSize,&Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSA_IO_PENDING)
		{
...
void CSocketManager::OnSend(int index,DWORD IoSize,IO_SEND_CONTEXT* lpIoContext) // OK
{
	...
	if(WSASend(lpObj->m_socket,&lpIoContext->wsabuf,1,&SendSize,Flags,&lpIoContext->overlapped,0) == SOCKET_ERROR)
	{
		if(WSAGetLastError() != WSA_IO_PENDING)
		{
			//gLog.Output(LOG_CONNECT,"[SocketManager] WSASend() failed with error: %d",WSAGetLastError());
