...
// Globals
//************************************
int g_iSeedZttractMoney;//3EE3324
...
void CSocketOption::Init()//00556C60 - identical?:D
{
	...
	for( i = 0; i < MAX_SOCKET_ITEM; i++ )
	{
		this->m_SocketItemList[i].ItemID		= -1;
		this->m_SocketItemList[i].ItemIndex	= -1;
	}
	// ----
...
void CSocketOption::_InitOption()//00557A60 - identical
{
	...
	g_iSeedZttractMoney				= GetPrivateProfileInt("GameServerInfo","SeedZttractMoney",1000000,gDirPath.GetNewPath("commonserver.cfg"));
...
BYTE CSocketOption::CheckItemType(int iItemNumber)//005581B0 -identical //calls alloc 8x
{
...
#ifdef GRAND_HERO_DEV
	if(GrandHero.SockeetItem(iItemNumber))
		return true;
#endif
	// ----
	return false;
	...
	if( iItemNumber >= ITEMGET(7,60) && iItemNumber <= ITEMGET(7,65) ||
		iItemNumber >= ITEMGET(8,60) && iItemNumber <= ITEMGET(8,65) ||
		iItemNumber >= ITEMGET(9,60) && iItemNumber <= ITEMGET(9,65) ||
		iItemNumber >= ITEMGET(10,60) && iItemNumber <= ITEMGET(10,65) ||
		iItemNumber >= ITEMGET(11,60) && iItemNumber <= ITEMGET(11,65) )
	{
		return true;
	}
#endif
