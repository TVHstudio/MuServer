...
void SpotReturn::Load()
{
	...
	if(!g_ZtLicense.user.SpotReturn)
	{
		return;
	}

	this->Read(gDirPath.GetNewPath("ZtData\\SpotReturn.ini"));
}
// ----------------------------------------------------------------------------------------------

void SpotReturn::Read(char* File)
{
	this->Enable = GetPrivateProfileInt("ZtTeam", "Enable", 1, File);
	this->CheckTimeStatus = GetPrivateProfileInt("ZtTeam", "CheckTimeStatus", STATUSMAXTIME, File);
}
// ----------------------------------------------------------------------------------------------
...
void SpotReturn::StatusTime()
{
	...
	for (int aUserIndex = OBJ_STARTUSERINDZT; aUserIndex < OBJMAX; aUserIndex++)
	{
		...
		if(ZtUserInSafeZone(aUserIndex) == false)
		{
