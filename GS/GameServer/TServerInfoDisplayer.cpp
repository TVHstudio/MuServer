...
TServerInfoDisplayer::TServerInfoDisplayer()
{
	...
	this->m_bValidZTDSConnection = FALSE;
...
void TServerInfoDisplayer::CheckZTDSConnection()
{
	if ( wsZtDbCli.GetSocket() == INVALID_SOCKET )
		this->m_bValidZTDSConnection = FALSE;
	else
		this->m_bValidZTDSConnection = TRUE;
}
...
void TServerInfoDisplayer::PaintAllInfo(HWND hWnd, int iTopLeftX, int iTopLeftY)
{
	...
	rect.bottom = 100;
	FillRect(hDC, &rect, CreateSolidBrush(RGB(107, 243, 125)));
	//rect.top = 20;
#if(NEW_WINDOW==1)

	SetTextColor(hDC, RGB(210, 105, 30));
	...
	if ( this->m_bValidZTDSConnection == FALSE )
	{
		SetTextColor(hDC, RGB(255, 0, 0));
	...
	if(g_ZtLicense.m_Enable)
	{
		char szLicense[50] = { 0 };
		sprintf(szLicense, "License: %s", g_ZtLicense.GetLicenseName());
	...
	if (g_ZtLicense.m_Enable && g_ZtLicense.NewVersion)
	{
		g_ZtLicense.DrawVersion = !g_ZtLicense.DrawVersion;

		if(g_ZtLicense.DrawVersion)
		{
			SetTextColor(hDC, RGB(255, 0, 0));
			TextOutA(hDC, GAMESERVER_WIDTH - 940, 17+10, ZTNEW_VERISON, strlen(ZTNEW_VERISON));

			char InfoServer[50] = { 0 };
			sprintf(InfoServer,"Version: %s",g_ZtLicense.VersionNum);
			TextOutA(hDC, GAMESERVER_WIDTH - 940, 31+10, InfoServer, strlen(InfoServer));

			char InfoServer2[50] = { 0 };
			sprintf(InfoServer2,"Date: %s",g_ZtLicense.VersionDate);
	...
	if ( this->m_bValidJSConnection == FALSE || this->m_bValidDSConnection == FALSE || this->m_bValidFSConnection == FALSE || this->m_bValidZTDSConnection == FALSE || this->m_bValidEVDSConnection == FALSE || this->m_bValidRKDSConnection == FALSE )
	{
		SERVER_CONN_STATE_LOG.Output("Server Interconnection Check - [FS/%d] [DS/%d] [ZT/%d] [ES/%d] [RS/%d]", this->m_bValidFSConnection, this->m_bValidDSConnection, this->m_bValidZTDSConnection, this->m_bValidEVDSConnection,	this->m_bValidRKDSConnection);
	}
