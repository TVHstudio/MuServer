...
{
	if(!g_ZtLicense.user.bTeamVsTeam2)
	{
		return;
	}

	this->Read(gDirPath.GetNewPath("ZtData\\Events\\TeamVsTeam.dat"));
}

void CTeamVsTeam::Read(char* filename)
{
	this->m_Enable				= GetPrivateProfileInt("ZtTeam", "Enable", 0, filename);
	this->m_EventStartDay		= GetPrivateProfileInt("ZtTeam", "EventStartDay", 0, filename);
	this->m_EventStartHour		= GetPrivateProfileInt("ZtTeam", "EventStartHour", 0, filename);
	this->m_EventeStartMin		= GetPrivateProfileInt("ZtTeam", "EventeStartMin", 0, filename);
	this->m_EventDurationTime	= GetPrivateProfileInt("ZtTeam", "EventDurationTime", 0, filename);
	this->m_NpcClass			= GetPrivateProfileInt("ZtTeam", "NpcClass", 0, filename);
	this->m_NpcMap				= GetPrivateProfileInt("ZtTeam", "NpcMap", 0, filename);
	this->m_NpcX				= GetPrivateProfileInt("ZtTeam", "NpcX", 0, filename);
	this->m_NpcY				= GetPrivateProfileInt("ZtTeam", "NpcY", 0, filename);
	this->m_RewardBonusEnable	= GetPrivateProfileInt("ZtTeam", "RewardBonusEnable", 0, filename);
	this->m_RewardBonusCount    = GetPrivateProfileInt("ZtTeam", "RewardBonusCount", 0, filename);
	...
	this->m_RewardLevelUp		= GetPrivateProfileInt("ZtTeam", "RewardLevelUp", 0, filename);
	#endif		
	this->m_MinReset			= GetPrivateProfileInt("ZtTeam", "MinReset", 0, filename);
	
	#if(NEW_UPDATE_1019)
	this->m_EventX				= GetPrivateProfileInt("ZtTeam", "EventX", 72, filename);
	this->m_EventY				= GetPrivateProfileInt("ZtTeam", "EventY", 46, filename);
	this->m_TeamX1				= GetPrivateProfileInt("ZtTeam", "TeamX1", 36, filename);
	this->m_TeamY1				= GetPrivateProfileInt("ZtTeam", "TeamY1", 28, filename);
	this->m_TeamX2				= GetPrivateProfileInt("ZtTeam", "TeamX2", 93, filename);
	this->m_TeamY2				= GetPrivateProfileInt("ZtTeam", "TeamY2", 80, filename);
	#endif
...
void CTeamVsTeam::StatusStart()
{
	...
	for(int aIndex = OBJ_STARTUSERINDZT; aIndex < OBJMAX; aIndex++)
	{
		...
		{
			gObjTeleport(aIndex, TEAMVSTEAM_MAP, this->m_TeamX1+(rand()%6-3), this->m_TeamY1+(rand()%6-3));
		}
		else if(lpUser->m_TvTTeam == eEVENT_TVT_TEAM_2)
		{
			gObjTeleport(aIndex, TEAMVSTEAM_MAP, this->m_TeamX2+(rand()%6-3), this->m_TeamY2+(rand()%6-3));
		}
...
bool CTeamVsTeam::PlayerRegen(int aIndex)
{
	...
	if(lpUser->m_TvTTeam == eEVENT_TVT_TEAM_1)
	{
		lpUser->X = this->m_TeamX1+(rand()%6-3);
		lpUser->Y = this->m_TeamY1+(rand()%6-3);
	}
	else if(lpUser->m_TvTTeam == eEVENT_TVT_TEAM_2)
	{
		lpUser->X = this->m_TeamX2+(rand()%6-3);
		lpUser->Y = this->m_TeamY2+(rand()%6-3);
	}
...
void CTeamVsTeam::PlayerReward(int WinTeam)
{
	for(int aIndex = OBJ_STARTUSERINDZT; aIndex < OBJMAX; aIndex++)
	{
		...
		if(lpUser->m_TvTTeam == WinTeam)
		{
			...
			#if(ENABLE_NSGAME_UPDATE)
			for(int i = 0; i < this->m_RewardLevelUp; i++)
			{
				__int64 exp = gLevelExperience[lpUser->Level];
				lpUser->Experience = exp;
				gObjLevelUp(lpUser, exp, lpUser->Class, 0);
			}
			#endif
		}
	}
...
#if(NEW_UPDATE_1019)
	gObjTeleport(aIndex, TEAMVSTEAM_MAP, this->m_EventX+(rand()%6-3), this->m_EventY+(rand()%6-3));
#else
...
void CTeamVsTeam::PlayerTrap()
{
	for(int aIndex = OBJ_STARTUSERINDZT; aIndex < OBJMAX; aIndex++)
	{
		...
		if(lpUser->Connected >= PLAYER_PLAYING)
		{
			if((lpUser->X >= 66 && lpUser->X <= 69) && (lpUser->Y >= 47 && lpUser->Y <= 51)  ||
			   (lpUser->X >= 17 && lpUser->X <= 21) && (lpUser->Y >= 47 && lpUser->Y <= 51))
			{
				//lpUser->Life = 0;
				//lpUser->Live = 0;
				//lpUser->m_State = 4;
				//lpUser->RegenTime = GetTickCount();
				//lpUser->DieRegen = 1;
				//lpUser->PathCount = 0;

				//if(lpUser->Teleport == 1)
				//{
				//	lpUser->Teleport = 0;
				//}

				//GCDiePlayerSend(lpUser,lpUser->m_Index,0,0);

				if(lpUser->Life > 0)
				{
					lpUser->Life = 0.0;

					gObjLifeCheck(lpUser, lpUser, 0, 0, 0, 0, 0, 0);

					lpUser->m_TvTDie++;

					GCUserKillCount(aIndex);
				}
			}
			else if(lpUser->X == 54 && lpUser->Y == 27 ||
					lpUser->X == 52 && lpUser->Y == 71 ||
					lpUser->X == 33 && lpUser->Y == 71 ||
					lpUser->X == 33 && lpUser->Y == 27)
			{
				if(gObjSearchActiveEffect(lpUser, AT_STUN) == FALSE)
				{
					gObjApplyBuffEffectDuration(lpUser, AT_STUN, 0, 0, 0, 0, 2);
					...
					for(int n = 0; n < INVETORY_WEAR_SIZE; n++)
					{
						if(lpUser->pInventory[n].IsItem() == TRUE)
						{
							...
							{
								lpUser->pInventory[n].m_Durability -= 50;

								if(lpUser->pInventory[n].m_Durability < 0)
								{
									lpUser->pInventory[n].m_Durability = 0;
								}
