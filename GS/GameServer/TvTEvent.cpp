...
void cTvTEvent::NPCClick(LPOBJ lpNpc, LPOBJ lpObj)
{
	...
	if(lpNpc->Class == this->NPC)
	{
		...
		if(this->NeedReset > lpObj->Reset)
		{
			MsgNormal(lpObj->m_Index,"[TvT Event] Need Reset: %d",this->NeedReset);
			return;
		}

		if(this->NeedGrandReset > lpObj->GReset)
		{
		...
		if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
		{
			for(int i = 0; i < OBJMAX; i++)
			{
				if(!this->player_hdd[i])
				{
					continue;
				}

				if(this->player_hdd[i] == lpObj->hdd_id)
				{
					MsgNormal(lpObj->m_Index,"[TvT Event] Only one window pc");
					return;
				}
			}

			this->player_hdd[lpObj->m_Index] = lpObj->hdd_id;
		}
#endif
...
void cTvTEvent::Reward(int aIndex)
{
	...
	{
		lpObj->PCPoint += this->RewardExPoint;
	}
	if(this->RewardWCoin > 0)
	{
		lpObj->GameShop.WCoinC += this->RewardWCoin;
