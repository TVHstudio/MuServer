...
void cWinQuestSystem::Init()
{
	...
	this->Enable = GetPrivateProfileInt("ZtTeam","Enable",0,WIN_QUEST_SYSTEM_CFG);
	this->NPC = GetPrivateProfileInt("ZtTeam","NPC",0,WIN_QUEST_SYSTEM_CFG);
...
void cWinQuestSystem::MonsterKiller(LPOBJ lpObj, LPOBJ lpTargetObj)	//lpTargetObj - то?кт?убивае?
{
	if(!this->Enable) return;
...
void cWinQuestSystem::SendPacket(int aIndex, BYTE OpenWind)
{
	...
	for(int i=0; i < MAX_WIN_QUEST; i++)
	{
		...
		strncpy(qSend.ztQuest[i].Name,this->Data[i][n].Name,40);
		strncpy(qSend.ztQuest[i].Info,this->Data[i][n].Info,200);

		qSend.ztQuest[i].n = n;
		qSend.ztQuest[i].e = this->Count[i];

		//MsgNormal(aIndex,"%d / %d",n,this->Count[i]);

		if(n >= this->Count[i])
		{
			qSend.ztQuest[i].Finish = true;
		}
		else
		{
			qSend.ztQuest[i].Finish = false;
		}

		qSend.ztQuest[i].start = this->WinQ[aIndex].Start[i];

		// ----

		for(int k = 0; k < 5; k++)
		{
			qSend.ztQuest[i].MissionType[k] = this->Data[i][n].MissionType[k];
			qSend.ztQuest[i].MissionMain[k] = this->Data[i][n].MissionMain[k];
			qSend.ztQuest[i].MissionLvL[k] = this->Data[i][n].MissionLvL[k];
			qSend.ztQuest[i].MissionLuck[k] = this->Data[i][n].MissionLuck[k];
			qSend.ztQuest[i].MissionExl[k] = this->Data[i][n].MissionExl[k];
			qSend.ztQuest[i].MissionAnc[k] = this->Data[i][n].MissionAnc[k];
			...
			if(this->Data[i][n].MissionType[k] == 1)	//Kill Mission
			{
				...
				strncpy(qSend.ztQuest[i].MissionName[k], lpMonsterAttr->m_Name,20);

				if(this->WinQ[aIndex].Count[i][k] < this->Data[i][n].MissionLvL[k])
				{
					Reward = 0;
					qSend.ztQuest[i].MissionCheck[k] = 0;
				}
				else
				{
					qSend.ztQuest[i].MissionCheck[k] = 1;
				}
			}
			...
			if(this->Data[i][n].MissionType[k] == 2)
			{
				...
				strncpy(qSend.ztQuest[i].MissionName[k], pItem->Name,20);
				if(this->WinQ[aIndex].Count[i][k] < this->Data[i][n].MissionLvL[k])
				{
					Reward = 0;
					qSend.ztQuest[i].MissionCheck[k] = 0;
			...
			qSend.ztQuest[i].Count[k] = this->WinQ[aIndex].Count[i][k];
		}
		...
		for(int k = 0; k < 5; k++)
		{
			qSend.ztQuest[i].RewardType[k] = this->Data[i][n].RewardType[k];
			qSend.ztQuest[i].RewardMain[k] = this->Data[i][n].RewardMain[k];
			qSend.ztQuest[i].RewardLvL[k] = this->Data[i][n].RewardLvL[k];
			qSend.ztQuest[i].RewardLuck[k] = this->Data[i][n].RewardLuck[k];
			qSend.ztQuest[i].RewardExl[k] = this->Data[i][n].RewardExl[k];
			qSend.ztQuest[i].RewardAnc[k] = this->Data[i][n].RewardAnc[k];
		}
		...
		for(int k = 0; k < 5; k++)
		{
			qSend.ztQuest[i].PriceType[k] = this->Data[i][n].PriceType[k];
			qSend.ztQuest[i].PriceMain[k] = this->Data[i][n].PriceMain[k];
			qSend.ztQuest[i].PriceLvL[k] = this->Data[i][n].PriceLvL[k];

			qSend.ztQuest[i].PriceCheck[k] = 1;

			//0 - off; 1 - zen; 2 - WcoinC; 3 - WcoinP; 4 - Goblin; 5 - Cred; 6 - Item

			if(this->Data[i][n].PriceType[k] == 1) //1 - zen
			{
				qSend.ztQuest[i].PriceDone[k] = lpObj->Money;

				if(this->Data[i][n].PriceMain[k] > lpObj->Money)
				{
					Accept = 0;
					qSend.ztQuest[i].PriceCheck[k] = 0;
				}
			}
			if(this->Data[i][n].PriceType[k] == 2) //2 - WcoinC
			{
				qSend.ztQuest[i].PriceDone[k] = lpObj->GameShop.WCoinC;
			...
			{
				qSend.ztQuest[i].PriceDone[k] = lpObj->GameShop.WCoinP;
			...
			{
				qSend.ztQuest[i].PriceDone[k] = lpObj->GameShop.GoblinPoint;
			...
			{
				qSend.ztQuest[i].PriceDone[k] = lpObj->ZtCred;
				if(this->Data[i][n].PriceMain[k] > lpObj->ZtCred)
				{
					Accept = 0;
					qSend.ztQuest[i].PriceCheck[k] = 0;
				}
			...
			{
				qSend.ztQuest[i].PriceDone[k] = gObjGetItemCountInInventory(aIndex,this->Data[i][n].PriceMain[k],this->Data[i][n].PriceLvL[k]);
		...
		qSend.ztQuest[i].NeedLevel = this->Data[i][n].NeedLevel;
		qSend.ztQuest[i].NeedReset = this->Data[i][n].NeedReset;
		qSend.ztQuest[i].NeedGrand = this->Data[i][n].NeedGrand;

		qSend.ztQuest[i].LevelDone = lpObj->Level;
		if(this->Data[i][n].NeedLevel > lpObj->Level)
		{
			Accept = 0;
		}

		qSend.ztQuest[i].ResetDone = lpObj->Reset;
		if(this->Data[i][n].NeedReset > lpObj->Reset)
		{
			Accept = 0;
		}

		qSend.ztQuest[i].GrandDone = lpObj->GReset;
		if(this->Data[i][n].NeedGrand > lpObj->GReset)
		{
			Accept = 0;
		}

		qSend.ztQuest[i].accept = Accept;
		qSend.ztQuest[i].reward = Reward;
	}
...
void cWinQuestSystem::RecvPacket(int aIndex, CGSendQuestResult * Date)
{
	...
	if(Date->Result == 1)
	{
		for(int k = 0; k < 5; k++)
		{
			...
			if(this->Data[i][n].PriceType[k] == 5) //5 - Cred
			{
				lpObj->ZtCred -= this->Data[i][n].PriceMain[k];
	...
	if(Date->Result == 2)
	{
		for(int k = 0; k < 5; k++)
		{
			//0-off;1-zen;2-ZtFreePoints;3-WcoinC;4-WcoinP;5-Goblin;6-Cred;7-Damage;8-Defence;9-Life;10-SD;11-CritDmg;12-ExlDmg;13-Double;14-Ignore;15-Ref;16-Exp;17-Drop;18-Item
			...
			if(this->Data[i][n].RewardType[k] == 2)	//2-ZtFreePoints
			{
				lpObj->ZtFreePoints += this->Data[i][n].RewardMain[k];
			}
