...
CZtLicenseClient g_ZtLicenseClient;

CZtLicenseClient::CZtLicenseClient()
{
...
void CZtLicenseClient::Load()
{
	InitializeCriticalSection(&this->m_critical);

	if(g_ZtLicenseClient.Connect(LICENSE_IP,LICENSE_PORT) == 0)
	{
	...
	//g_ZtLicenseProtocol.GLReqUser();
}

bool CZtLicenseClient::Connect(char* IpAddress, unsigned short port)
{
	WSADATA wsa;
...
void CZtLicenseClient::Disconnect()
{
	if(this->m_socket != INVALID_SOCKET)
	{
...
bool CZtLicenseClient::CheckState()
{
	if(this->m_socket == INVALID_SOCKET)
	{
		return false;
...
bool CZtLicenseClient::DataRecv()
{
	int count = 0;
	int size = 0;
	...
	while(true)
	{
		...
		if(size <= this->m_RecvSize)
		{
			...
			g_ZtLicenseProtocol.ProtocolCore(head,&this->m_RecvBuff[count],size);
...
bool CZtLicenseClient::DataSend(BYTE* lpMsg,int size)
{
...
bool CZtLicenseClient::DataSendZt()
{
	EnterCriticalSection(&this->m_critical);
...
DWORD WINAPI CZtLicenseClient::ClientWorkerThread()
{
	while(true)
	{
		DWORD result = WSAWaitForMultipleEvents(1,&g_ZtLicenseClient.m_event,0,WSA_INFINITE,0);

		if(result != WSA_WAIT_FAILED && result != WSA_WAIT_TIMEOUT)
		{
			WSANETWORKEVENTS NetworkEvents;

			if(WSAEnumNetworkEvents(g_ZtLicenseClient.m_socket,g_ZtLicenseClient.m_event,&NetworkEvents) == SOCKET_ERROR)
			{
...
DWORD WINAPI CZtLicenseClient::ClientStatusThread()
{
	while(true)
	{
		if(g_ZtLicenseClient.CheckState() == 0)
		{
			if(g_ZtLicenseClient.Connect(LICENSE_IP, LICENSE_PORT) == 0)	//config
			{
				continue;
			}
		}
