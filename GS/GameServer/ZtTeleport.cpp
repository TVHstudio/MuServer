...
void ZtTeleport::Load()
{
	...
	{
		this->Read(gDirPath.GetNewPath("ZtData\\ZtTeleport.ini"));
	}
}
// ----------------------------------------------------------------------------------------------

void ZtTeleport::Read(char* File)
{
...
bool ZtTeleport::Dialog(int aUserIndex, int aNpcIndex)
{
	if(!this->Enable)
	{
 		return false;
...
void ZtTeleport::Timer()
{
#if(ENABLR_DEN_DEV)
	if(g_ZtLicense.CheckUser(eZtUB::eternalmu))
	{
		for (int aUserIndex = OBJ_STARTUSERINDZT; aUserIndex < OBJMAX; aUserIndex++)
		{
...
bool ZtTeleport::Check(int aUserIndex)
{
	if(OBJMAX_RANGE(aUserIndex) == 0)
	{
		return false;
...
void ZtTeleport::GCWindows(int aUserIndex)
{
	if(OBJMAX_RANGE(aUserIndex) == 0)
	{
		return;
...
void ZtTeleport::CGResult(PMSG_CG_TELEPORTWINRES* aRecv, int aUserIndex)
{
	if(OBJMAX_RANGE(aUserIndex) == 0)
	{
		return;
