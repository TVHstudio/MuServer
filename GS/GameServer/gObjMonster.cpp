...
BOOL gObjMonsterRegen(LPOBJ lpObj)
{
	...
	g_MonsterMsg.RegenMessage(lpObj->m_Index);
#endif
		...
		if ( tObjNum >= 0 )
		{
			if ( (gObj[tObjNum].Type == t1 || gObj[tObjNum].Type == t2) && (gObj[tObjNum].Live != FALSE) )
			{
				...
				else if ( (gObj[tObjNum].Authority &2) != 2 && (gObj[tObjNum].Authority &32) != 32 && gObj[tObjNum].Teleport == 0 )
				{
					...
					if ( dis < mindis )
					{
						if(g_DoppelGanger.IsDGMonster(lpObj))	//Season 5 DoppelGanger
						{
...
int gObjGuardSearchEnemy(LPOBJ lpObj)
{
	...
	for (n=0;n<MAX_VIEWPORT_MONSTER;n++)
	{
		...
		if ( tObjNum >= 0 )
		{
			...
			if(ZtConfig.PvP.ZtPkPartySystem && gObj[tObjNum].PartyNumber >= 0) //Season 2.5 add-on
			{
			...
			if (lpObj->Class == 249 || lpObj->Class == 247 )
			{
				if ( gEvent1 )
				{
					if ( !(rand()%30) )
					{
						...
						{
							ChatSend(lpObj,ZtConfig.CommonServer.GuardMessage);
							lpObj->m_PK_Count = 0;
						}
					}
				}
		...
		if ( lpObj->TargetNumber >= 0 && lpObj->PathStartEnd == 0)
		{
			...
			if ( dis <= attackrange )
			{
				...
				if ( MapC[map].CheckWall(lpObj->X, lpObj->Y, gObj[tuser].X, gObj[tuser].Y) == TRUE )
				{
					...
					if ( (attr&1) != 1 )
					{
						lpObj->m_ActState.Attack = 1;		//Fix Monster in main?
					}
					else
					{
						lpObj->TargetNumber = -1;
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	//g_Achievements.MonsterKill(lpTargetObj->m_Index, lpObj->m_Index);
#endif

#if(CUSTOM_MAPQUEST)
	g_MapQuest.MonsterKill(lpTargetObj->m_Index, lpObj->m_Index);
#endif
#if(CUSTOM_MONSTER_MESSAGE)
	g_MonsterMsg.DieMessage(lpTargetObj->m_Index, lpObj->m_Index);
#endif
	...
	{
		if( lpTargetObj->DropBuffTime > 0 )
		{
			int DropAdd = 3;
			if ( (rand()%10000) < DropAdd )
			{
				int iTopHitUser = gObjMonsterTopHitDamageUser(lpTargetObj);
				...
				ItemSerialCreateSend(lpTargetObj->m_Index,lpTargetObj->MapNumber,lpTargetObj->X,lpTargetObj->Y,Type,0,0,0,0,0,iTopHitUser,0,0);
			}
		}	
	}
#endif
...
#ifdef SEASON5_QUEST
	S5Quest.MainKillMonster(lpObj,lpTargetObj);	//Season 5 Quest
#endif

	ZtQuestSystem.MonsterKill(lpObj,lpTargetObj);

#ifdef PRIVATE_MODULE
	if(g_ZtLicense.user.MonsterQuest)
	{
		gMonsterQuest.MissionKill(lpObj->m_Index,lpTargetObj->m_Index);
	}
#endif
	...
	{
		gWinQuestSystem.MonsterKiller(lpObj,lpTargetObj);
	}

	gMonsterBonus.Main(lpObj,lpTargetObj);

#if(CUSTOM_ZT_QUEST_SYSTEM==TRUE)
	if(g_ZtWinQuestSystem.MonsterKiller(lpObj->m_Index, lpTargetObj->m_Index))
	{
		return;
	}
#endif
	...
	if ( lpObj->Class == 459 && lpObj->Connected == 3 )	// Season 4.5 addon
	{
		if ( g_bRaklionSelupanItemDrop != 0)
		{
			...
			if ( ItemDropRate <= g_iRaklionSelupanItemDropRate )
			{
				...
				for(int i = 0; i < ZtConfig.CommonServer.CountItemDropSelupan; i++)	//10
				{
					if ( !gObjGetRandomItemDropLocation(lpObj->MapNumber, cDropX, cDropY, 2, 2, 10))
					{
						cDropX = lpObj->X;
...
//#if GS_CASTLE == 1 
	...
	{
		int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		int iMaxCount = ZtConfig.CommonServer.CountItemDropErohim;
		//if(g_ZtLicense.CheckUser(eZtUB::drynea))
		//{
		//	iMaxCount = ZtConfig.DryneaConfig.CountErohimItemsDrop;
		//}
...
//		if ( (rand()%10000) < ZtConfig.DryneaConfig.ErohimItemsDrop )
//		{
//#endif
...
//#endif
	...
	if ( lpObj->Class == 275 ) 
	{
		...
		int iMaxNumOfRewardItem = ZtConfig.CommonServer.CountItemDropKundun;

		if(g_ZtLicense.CheckUser(eZtUB::drynea))
		{
			iMaxNumOfRewardItem = ZtConfig.DryneaConfig.CountKundumItemsDrop;
		}
		...
		for ( int i=0;i<iMaxNumOfRewardItem;i++)
		{
			...
			if ( lpObj->Class == 275 ) 
			{
				if ( (rand()%10000) < ZtConfig.CommonServer.KundunAncDrop )
				{
...
/*
	...
	*/
	if (lpObj->Class == 44)	//Dragon
	{
		...
		//level = ZtConfig.GoldenDrop.level44;
		//type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType44, ZtConfig.GoldenDrop.ItemIndex44);
		//int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
	...
	if (lpObj->Class == 43)	//GoldenBudgeDragon
	{
		...
		level = ZtConfig.GoldenDrop.level43;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType43, ZtConfig.GoldenDrop.ItemIndex43);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
	...
	if (lpObj->Class == 78)	//GoldenGoblin
	{
		...
		level = ZtConfig.GoldenDrop.level78;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType78, ZtConfig.GoldenDrop.ItemIndex78);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
	...
	if (lpObj->Class == 493)	//GoldenDarkKnight
	{
		...
		level = ZtConfig.GoldenDrop.level493;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType493, ZtConfig.GoldenDrop.ItemIndex493);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
...
*/
	if (lpObj->Class == 53)	//GoldenTitan
	{
		...
		level = ZtConfig.GoldenDrop.level53;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType53, ZtConfig.GoldenDrop.ItemIndex53);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
	...
	if (lpObj->Class == 54)	//GoldenSoldier
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level54;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType54, ZtConfig.GoldenDrop.ItemIndex54);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
...
*/
	if (lpObj->Class == 80)	//GoldenLizardKing
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level80;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType80, ZtConfig.GoldenDrop.ItemIndex80);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
	...
	if (lpObj->Class == 81)	//GoldenVepar
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level81;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType81, ZtConfig.GoldenDrop.ItemIndex81);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
	...
	if (lpObj->Class == 494)	//GoldenDevil
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level494;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType494, ZtConfig.GoldenDrop.ItemIndex494);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
...
*/
	if (lpObj->Class == 82)	//GoldenTantalos
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level82;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType82, ZtConfig.GoldenDrop.ItemIndex82);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
	...
	if (lpObj->Class == 83)	//GoldenWheel
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level83;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType83, ZtConfig.GoldenDrop.ItemIndex83);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
	...
	if (lpObj->Class == 495)	//GoldenGolem
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level495;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType495, ZtConfig.GoldenDrop.ItemIndex495);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
	...
	if (lpObj->Class == 496)	//GoldenCrust
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level496;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType496, ZtConfig.GoldenDrop.ItemIndex496);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
...
*/
	if (lpObj->Class == 497)	//GoldenSatyrus
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level497;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType497, ZtConfig.GoldenDrop.ItemIndex497);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
	...
	if (lpObj->Class == 498)	//GoldenTwinTale
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level498;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType498, ZtConfig.GoldenDrop.ItemIndex498);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
	...
	*/
	if (lpObj->Class == 499)	//GoldenIronKnight
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level499;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType499, ZtConfig.GoldenDrop.ItemIndex499);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
	...
	if (lpObj->Class == 500)	//GoldenNapin
	{
		...
		//type = ItemGetNumberMake(ItemType, ItemIndex);
		level = ZtConfig.GoldenDrop.level500;
		type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType500, ZtConfig.GoldenDrop.ItemIndex500);
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)x, (BYTE)y, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
		return;
...
//===========================//
	...
	if ( lpObj->Class == 79 )//Season 4.5 REMAKE GoldenDragon
	{
		...
		int randlevel1 = ZtConfig.GoldenDrop.randlevel1_79;
		int randlevel2 = ZtConfig.GoldenDrop.randlevel2_79;
		int randlevel3 = ZtConfig.GoldenDrop.randlevel3_79;
		int countitem = ZtConfig.GoldenDrop.countitem79;
		...
		for(int i = 0; i < countitem; i++)
		{
			...
			//type = ItemGetNumberMake(ItemType, ItemIndex);
			type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType79, ZtConfig.GoldenDrop.ItemIndex79);
	...
	if ( lpObj->Class == 501 )//Season 4.5 REMAKE	GoldenGreatDragon
	{
		...
		int randlevel1 = ZtConfig.GoldenDrop.randlevel1_501;
		int randlevel2 = ZtConfig.GoldenDrop.randlevel2_501;
		int countitem = ZtConfig.GoldenDrop.countitem501;
		...
		for(int i = 0; i < countitem; i++)
		{
			...
			//type = ItemGetNumberMake(ItemType, ItemIndex);
			type = ItemGetNumberMake(ZtConfig.GoldenDrop.ItemType501, ZtConfig.GoldenDrop.ItemIndex501);
			ItemSerialCreateSend(lpObj->m_Index, (BYTE)lpObj->MapNumber, (BYTE)cDropX, (BYTE)cDropY, type, (BYTE)level, (BYTE)dur,
			(BYTE)Option1, (BYTE)Option2, (BYTE)Option3, MaxHitUser, 0, 0);
	...
	if ( ZtConfig.CommonServer.ExlDropRate > rand()%10000 )//if ( ExtDropPer == TRUE )
	{
		ExtDropPer = TRUE;
	...
	if ( item_drop)
	{
		...
		if ( ExtDropPer == TRUE )
		{
			...
			else
			{
				//NOption = NewOptionRand(lpObj->Level);
				int iRandExl = rand() % 2;
				NOption = NewOptionRand(iRandExl);
			}
		...
		//if(g_ZtLicense.CheckUser(Local) || g_ZtLicense.CheckUser(ulasevich) || g_ZtLicense.CheckUser(ulasevich2) || g_ZtLicense.CheckUser(MU2Play))
		//{
	...
	else if ( (rand()%moneyrate) < 10 )
	{
		...
		else
		{
			...
			{
				money = money + ( money * ZtConfig.Pet.Zt_PandaRingZen / 100 );
			}
			//Master Drop Zen
		...
		if(lpTargetObj->Level == 400 && ( lpTargetObj->DbClass == 3 || lpTargetObj->DbClass == 19 || 
					lpTargetObj->DbClass == 35 || lpTargetObj->DbClass == 50 || lpTargetObj->DbClass == 66 || lpTargetObj->DbClass == 83 || lpTargetObj->DbClass ==98) )
				{
					...
					{
						if (ZtConfig.PrideMu.MasterMoney < 1)
						{
							money = money * ZtConfig.CommonServer.Zt_MasterDropZen;
						}
						else if (ZtConfig.PrideMu.DiapasonMasterMoney < 1)
						{
							money = ZtConfig.PrideMu.MasterMoney;
						}
						else
						{
							
							money = (rand() % ZtConfig.PrideMu.DiapasonMasterMoney) + ZtConfig.PrideMu.MasterMoney;
						}
		...
		if(lpTargetObj->Level == 400 && ( lpTargetObj->DbClass == 3 || lpTargetObj->DbClass == 19 || 
			...
			if(g_ZtLicense.user.WinQuest) gWinQuestSystem.ZenOption(lpObj->m_Index, money);

			gExclusiveOptions.DropZen(lpObj->m_Index, money);

			#if(ENABLE_PETZT)
			g_PetZt.Zen(lpObj->m_Index, money);
			#endif

			#if(SYSTEMOF_BROTHER==TRUE)
			gBrotherhood.UserBonusZen(lpObj,money);
			#endif

			money = ( money * ZtConfig.CommonServer.ProcentDropZen ) / 100;

			if (ZtConfig.CommonServer.MaxDropZen > 0)
			{
				if (money > ZtConfig.CommonServer.MaxDropZen)
				{
					money = ZtConfig.CommonServer.MaxDropZen;
				}
			}

			#if(CUSTOM_PREMIUM_SYSTEM==TRUE)
			g_PremiumSystemZt.UserZenPlus(lpObj->m_Index, money);
			#endif
	...
	if ( (rand()%400) == 1 )
	{
		for ( n=0;n<4;n++)
		{
			...
			if( mOn > ZtConfig.CommonServer.MaxDropZen )
			{
				mOn =	ZtConfig.CommonServer.MaxDropZen;
			}
			if (mOn > 0)
			{
...
BOOL gEventMonsterItemDrop(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( gMedalEvent )
	{
		...
		if(g_ZtLicense.CheckUser(eZtUB::NSGames))
		{
			if ( lpObj->Level >= gSilverMedalDropLevelMonsterMin && lpObj->Level <= gSilverMedalDropLevelMonsterMax )
			{
			...
			else if ( lpObj->Level >= gGoldMedalDropLevelMonsterMin && lpObj->Level <= gGoldMedalDropLevelMonsterMax )	// Gold Medal
			{
	...
	if ( (rand()%10000) < g_iKundunMarkDropRate )
	{
		...
		if ( lpObj->Level < 25 )
			...
			level = 6;
		else if ( lpObj->Level > 115 && lpObj->Level < ZtConfig.ZtCustom.MaxLevel+1 )
			level = 7;
