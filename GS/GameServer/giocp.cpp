...
BOOL HackToolCheck(BYTE headcode, int size, int uIndex, BYTE xcode) //004C20F0 
{
	...
	if(OBJMAX_RANGE(uIndex) != FALSE)
	{
		...
		if(PacketCount > MAX_PACKET_SERVER)
		{
...
DWORD WINAPI ServerQueueThread() // OK
{
	while(true)
	{
		...
		{
			LogAddC(2,"[SocketManager] WaitForSingleObject() failed with error: %d", GetLastError());
			break;
		}
		...
		{
			if(OBJMAX_RANGE(QueueInfo.index) != 0 && gObj[QueueInfo.index].Connected != PLAYER_EMPTY)
			{
...
bool CreateServerQueue() // OK
{
	...
	{
		LogAddC(2, "[SocketManager] CreateSemaphore() failed with error: %d", GetLastError());
		return 0;
	}

	if((g_ServerQueueThread = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)ServerQueueThread, NULL, 0, 0)) == 0)
	{
		LogAddC(2,"[SocketManager] CreateThread() failed with error: %d",GetLastError());
		return 0;
	}

	if(SetThreadPriority(g_ServerQueueThread, THREAD_PRIORITY_HIGHEST) == 0)
	{
		LogAddC(2,"[SocketManager] SetThreadPriority() failed with error: %d",GetLastError());
		return 0;
	}
...
BOOL RecvDataParse(LPPER_IO_CONTEXT	lpIOContext, int uIndex)	
{
	...
	BYTE headcode;
	BYTE xcode = 0;
	BYTE byDec[MAX_EXSENDBUF_SIZE];	//[7024];
	...
	while ( 1 )
	{
		...
		if ( size <= lpIOContext->nSentBytes )
		{
			...
			//LogAddC(2, "0x%02X 0x%02X 0x%02X 0x%02X 0x%02X", recvbuf[0], recvbuf[1], recvbuf[2], recvbuf[3], recvbuf[4]);
			#endif

			if ( xcode == 0xC3 )
			{
#if(ENABLE_INDEV_NETWORK)
				int iSize = g_PacketEncrypt.Decrypt( byDec+2,(recvbuf+lOfs+2), size-2);
				//int iSize = g_PacketEncrypt.Decrypt(&byDec[2], &recvbuf[lOfs + 2], size - 2);
				//memcpy( &byDec[2], &recvbuf[lOfs + 2], size - 2);
				...
				{
#if(ENABLE_INDEV_NETWORK)
					PBMSG_HEAD* lphead = (PBMSG_HEAD*)(byDec);
					headcode		= lphead->headcode;
					BYTE subhead	= byDec[2];
					byDec[0]		= 0xC1;
					byDec[1]		= ((BYTE)iSize)+2;
				...
				#if(ENABLE_INDEV_NETWORK)
				LogAddC(3, "0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
					byDec[0], byDec[1], byDec[2], byDec[3], byDec[4], byDec[5]);
				#endif
...
#if(ENABLE_INDEV_NETWORK)
					if ( PacketStream.AddData(byDec, iSize+2) == 0 )
#else
					...
					memcpy(QueueInfo.buff, byDec, size);
					QueueInfo.size = iSize;
					QueueInfo.encrypt = 1;
					QueueInfo.serial = subhead;
			...
			{
#if(ENABLE_INDEV_NETWORK)
				int iSize = g_PacketEncrypt.Decrypt( byDec+3, (recvbuf+lOfs+3), size-3);
				//int iSize = g_PacketEncrypt.Decrypt(&byDec[3], &recvbuf[lOfs + 3], size - 3);
#else
				...
				else
				{
					...
					byDec[1]		= SET_NUMBERH(wsize);
					byDec[2]		= SET_NUMBERL(wsize);
...
#if(ENABLE_INDEV_NETWORK)
					if ( PacketStream.AddData(byDec, iSize+3) == 0 )
#else
...
int DataSend(int aIndex, LPBYTE lpMsg, DWORD dwSize)
{
	...
	if ( aIndex < OBJ_STARTUSERINDZT )
	{
		return true;
	}

#if(OFFLINE_MODE == TRUE)
	if( gObj[aIndex].m_OfflineMode )
	{
		return false;
	}
#endif
...
BOOL UpdateCompletionPort(SOCKET sd, int ClientIndex, BOOL bAddToList)
{
	...
	g_Optimization.AddedMemoryObject(ClientIndex);
	#endif
...
void CloseClient ( LPPER_SOCKET_CONTEXT lpPerSocketContext, BOOL bGraceful )
{
	...
	if ( index >= OBJ_STARTUSERINDZT && index < OBJMAX )
	{
