...
CLogToFile g_OtherChatLog("Other", ".\\LOG\\Chat", TRUE);
#endif
...
{
	//LogAddC(2, "[DEBUG][1] PACKET TIME: %u", ( GetTickCount() - lpObj->m_PacketCheckTime ));

	if ( ( GetTickCount() - lpObj->m_PacketCheckTime ) < 300 )	//300
	{
		return FALSE;
	}
...
{
#ifdef DEBUG_CODE
	LogAddC(2, "protoNum: 0x%X | HEAD: 0x%X 0x%X 0x%X 0x%X WarehouseSave %d",protoNum,aRecv[0],aRecv[1],aRecv[2],aRecv[3],gObj[aIndex].WarehouseSave);
#endif
	...
	//LogAddC(2, "X: %d = TX: %d | Y: %d = TY: %d",lpObj->X, lpObj->TX, lpObj->Y, lpObj->TY);
	...
	else
	{
		...
		switch ( protoNum )
		{
#if(CUSTOM_POSTITEM)
            case 0x78:
                pPostItemProc ( ( PMSG_POSTITEM* ) aRecv, aIndex );
                break;
#endif
			case 0x00:
				tempindex = aIndex;
			...
			case PROTOCOL_ATTACK:
				CGAttack((PMSG_ATTACK *)aRecv, aIndex);	//OK
				break;
			case 0x18:
			...
			case 0x19:
				if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
				{
					CGMagicAttack((PMSG_MAGICATTACK *)aRecv, aIndex);	//OK
				}
				break;
			case 0x1B:
			...
			case 0x1C:
				if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
				{
					CGTeleportRecv((PMSG_TELEPORT *)aRecv, aIndex);	//OK
				}
				break;
			case 0xB0:
				if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
			...
			case PROTOCOL_BEATTACK:
				CGBeattackRecv(aRecv, aIndex, FALSE);	//OK
				break;
			case 0x1E:
				if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
				{
					CGDurationMagicRecv((PMSG_DURATION_MAGIC_RECV *)aRecv, aIndex);	//OK
				}
				break;
			case 0x22:
				if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
...
//#if(CUSTOM_PERSONAL_SHOP ==TRUE)
							if(g_PersonalShopZt.Enable)
							{
								CGPShopReqSetItemPriceEx((PMSG_REQ_PSHOP_SETITEMPRICE_EX *)aRecv, aIndex);
							}
							else
							{
//#else
...
//#if(CUSTOM_PERSONAL_SHOP ==TRUE)
							if(g_PersonalShopZt.Enable)
							{
								CGPShopReqBuyItemEx((PMSG_REQ_BUYITEM_FROM_PSHOP *)aRecv, aIndex);
							}
							else
							{
//#else
...
//#endif						
			...
			case 0x43:
				if ( PacketCheckTime(&gObj[aIndex]) == TRUE )
				{
					...
					gParty.SetPartyIndex((PMSG_PARTYDELUSER *)aRecv, aIndex);
#endif
			...
			case 0xBF:
				{
					...
					switch ( lpDef->subcode )
					{
						...
						case 0x17:
							...
							break;
						case 0x0E: //Season 5 DoppelGanger
			...
			case 0xF6:	//Season 5 Quest
				{
					...
					switch(lpDef->subcode)
					{
						case 0x1B:
#ifdef SEASON5_QUEST
							S5Quest.Info(aIndex);//Click T
#endif
							break;
						case 0x30:
#ifdef SEASON5_QUEST
							S5Quest.NPCQuest(aIndex);//CLick NPC Quest
#endif
							break;
						case 0x31:
#ifdef SEASON5_QUEST
							gObjElfSupportBuff(aIndex);
#endif
							break;
					}
				//	if(aRecv[3] == 0x30)
				...
				//	}
				//	LogAdd("NEW PROTOCOL F6: %x %x %x %x",aRecv[3],aRecv[4],aRecv[5],aRecv[6]);
				}
				break;
			case 0xF7:	//Season 5 ImperialFort
				{
					...
					{
						case 0x01:
							//PMSG_REQ_ENTERZONE * lpMsg = (PMSG_REQ_ENTERZONE*)aRecv;
							//LogAddC(3, "[DEBUG] %d", lpMsg->btType);
			...
			case 0xFB:
				{
					...
					switch ( lpDef->subcode )
					{
						...
						case 0x09:
							{
								//NewShopRecv((CG_NEW_CASH_SHOP*)aRecv, aIndex);
								g_PremiumSystemZt.CG_RecvBuy(aIndex, (CG_PREMIUM_BUY*) aRecv);
							}
							break;
						case 0x0A:
							{
								g_PremiumSystemZt.GC_Send(aIndex, (CG_SEND_OPEND_WIN*) aRecv);
							}
							break;
#endif
						case 11:
...
#if(_RECONNECT_)
						case 12:
							g_ConnectZt.SendLogin(aIndex, (CONNECTZT_LOGIN*)aRecv);
							break;
						case 13:
							g_ConnectZt.RecvClose(aIndex);
							break;
#endif
						case 15:
							{
								if(g_ZtLicense.user.WinQuest) gWinQuestSystem.RecvPacket(aIndex, (CGSendQuestResult*)aRecv);
							}
							break;
						case 16:
							gAutoParty.SendPartyList(aIndex);
							break;
						case 20:
							{
								...
								{
									gMonsterQuest.MissionReward(aIndex);
								}
#endif
							}
							break;
						...
						case 0x1B:
							{
								g_PartySearch.CG_RecvInfo(aIndex,(CG_AutoPartyInfo*) aRecv);
							}
							break;
						case 0x1C:
							{
								g_PartySearch.CG_RecvPartyUser(aIndex,(CG_PartyRecv*) aRecv);
							}
							break;
						case 0x1D:
							{
								g_PartySearch.Search(aIndex);
							}
							break;
#endif
						case 0x0E:
...
//#if(CUSTOM_PERSONAL_SHOP==TRUE)
								if(g_PersonalShopZt.Enable)
								{
									g_PersonalShopZt.Search(aIndex, (CG_PersonalPage*) aRecv);
								}
//#endif
							}
							break;
						case 0x20:
							{
								...
								g_BufferSystem.CG_ResultRecv(aIndex, (CG_BuffNpcResult*) aRecv);
#endif
							}
							break;
#if(CUSTOM_SMITHY == TRUE)
						case 0x21:
							g_Smithy.CGCreateItem((CG_SmithyCreateItem*) aRecv, aIndex);
							break;
#endif
						case 0x3D:
							{
#if(OFFLINE_MODE==TRUE)
								g_OfflineMode.Start((CG_OFFMODE_RESULT*)aRecv, aIndex);
#endif
							}
							break;
#if(CUSTOM_MARRIAGE)
						case 0x3E:
							...
							break;
						case 0x3F:
							g_Marriage.CGClickOK(aIndex);
							break;
#endif
#if(CUSTOM_ADD_POINTS==TRUE)
						case 0x1A:
							g_ZtUser.CGAddPoints(aIndex, (PMSG_ADD_POINTS*) aRecv);
							break;
#endif
#if(CUSTOM_ADD_POINTS==TRUE)
						case 0x26:
							g_ZtUser.CGResetPoints(aIndex, (PMSG_ADD_POINTSRESET*) aRecv);
							break;
#endif
						case 0x41:
							gCreditDonate.CGChangeClass((PMSG_CG_CHANGECLASS*) aRecv, aIndex);
							break;
						...
						case 0xFD:
							g_AntiCheatPlus.CGAnsUpdateFile((PMSG_UPDATEFILE*)aRecv, aIndex);
							break;
#endif
#endif
#if(CUSTOM_PARTY_LEADER==1)
						case 0x42:
							gParty.CGNewLeader(aIndex, (GC_PARTYNEWLEADER*) aRecv);
							break;
#endif
#if(ADD_RESET_WINDOW)
						case 0x43:
							gResetSystem.CGResulInfo(aIndex);
							break;
#endif
#if(EVENT_DUNGEON_SIEGE)
						case 0x44:
							g_DungeonSiege.CGDRegSiege(aIndex);
							break;
						case 0x45:
							g_DungeonSiege.CGHWID((PMSG_CG_DUNBEON_SIEGE_HWID*)aRecv, aIndex);
							break;
#endif
						case 0x71:
						...
						case 0x81:
							g_TeamVsTeam.CGTeleportEvent(aIndex);
							break;
#endif
#if(SYSTEM_ACHIEVEMENTS)
						case ACH_PROTOCOL1:
							g_Achievements.CGWindowOpen((PMSG_CG_ACH_WINOPEN*)aRecv, aIndex);
							break;
#endif
#if(CUSTOM_ZT_TELEPORT==TRUE)
						case 0x17:
							g_ZtTeleport.CGResult((PMSG_CG_TELEPORTWINRES*) aRecv, aIndex);
							break;
#endif
						case 0x95:
							gSystemOfRage.CGRageReset(aIndex, (PMSG_ANS_RESETRAGE*)aRecv);
							break;
						case 0x96:
							CGDonateItemBuy((PMSG_DONATE_ITEMBUY*) aRecv, aIndex);
							break;
						case 0xA0:
							gResetSystem.CGResetMenu(aIndex);
							break;
						case 0xA1:
							gGrandReset.CGGrandMenu(aIndex);
							break;
						case 0xA2:
							gGrandReset.CGGrandStart(aIndex);
							break;
#if(CUSTOM_ZT_TELEPORT_EVO)
						case 0xA3:
							g_ZtTeleportEvo.CGResult((PMSG_CG_TELEPORTWINRESEVO*) aRecv, aIndex);
							break;
						case 0xA4:
							g_ZtTeleportEvo.CGDialog((PMSG_CG_DIALOGEVO*) aRecv, aIndex);
							break;
#endif
						case 0xAF:
							{
#if(CUSTOM_PKCLEAR_NPC==TRUE)
								g_PKClear.CG_ResultRecv(aIndex, (CG_PKCLEAR_RESULT*) aRecv);
#endif
							}
							break;
#if(CUSTOM_ACCOUNT_SECURITY)
						case 0xB0:
							g_AccountSecurity.CGRecvAccountConnect(aIndex, (PMSG_ANS_ACC_CONNECT*)aRecv);
							break;
						case 0xB1:
							g_AccountSecurity.CGRecvSettingPC(aIndex, (PMSG_REQ_ACC_SETTING_PC*)aRecv);
							break;
						case 0xB2:
							g_AccountSecurity.CGRecvAccountOption(aIndex, (PMSG_REQ_ACC_OPTION*)aRecv);
							break;
						case 0xB3:
							g_AccountSecurity.CGRecvSaveNumber(aIndex, (PMSG_REQ_ACC_SAVENUMBER*)aRecv);
							break;
#endif

#if(CUSTOM_REFERAL_SYSTEM)
						case 0xB5:
							g_ReferralSystem.CGReqMasterInfo((PMSG_REQ_REFERRAL_MASTER_INFO*)aRecv, aIndex);
							break;
						case 0xB6:
							g_ReferralSystem.CGMasterResult((PMSG_CGREQ_REFERRAL_MASTER_REWARD*)aRecv, aIndex);
							break;
						case 0xB7:
							g_ReferralSystem.CGReqUserInfo((PMSG_REQ_REFERRAL_USER_INFO*)aRecv, aIndex);
							break;
						case 0xB8:
							g_ReferralSystem.CGUserResult((PMSG_CGREQ_REFERRAL_USER_REWARD*)aRecv, aIndex);
							break;
#endif
						case 0xBA:
							g_ZtUser.CG_RecvAutoHPClicker((PMSG_PROTOCOL_RESULT*)aRecv, aIndex);
							break;
#if(CUSTOM_ZT_QUEST_SYSTEM==TRUE)
						case 0x12:
							{
								g_ZtWinQuestSystem.CG_AcceptQuest(aIndex, (CG_Accept_Done*) aRecv);
							}
							break;
#endif
#if(CUSTOM_PICKUP_ITEM_INFO)
						case 0xBF:
							g_PickUpItemInfo.InfoRecv((PMSG_REQ_PICKUP_INFO*)aRecv, aIndex);
							break;
#endif
						case 0xC1:
							WarehouseChange(aIndex, (CHANGEWAREHOUSE*)aRecv);
							break;
						case 0xC2:
							CGRecvOffTrade((PMSG_OFFTRADE*)aRecv, aIndex);
							break;
						#if(CUSTOM_LOTTERY)
						case 0xD1:
							g_Lottery.CGLotteryPlay((PMSG_LOTTERY_PLAY*)aRecv, aIndex);
							break;
						#endif
#if(ADD_CTRL_KEY)
						case 0xD3:
							g_ZtUser.CGCtrlKey(aIndex, (PMSG_CONTROLLER_CTRL*) aRecv);
							break;
#endif
						case 0xD7:
							g_ZtUser.RecvConfig((CG_MINIMAP_PARTY*) aRecv, aIndex);
							break;
						case 0xD8:
							g_GuildBank.CGOpenGuildWarehouse((PMSG_GUILD_BANK*)aRecv, aIndex);
							break;
						#if(INTERFACE_JEWELBANK)
						case 0xDA:
							gJewelsBank.Packet(aIndex, (PMSG_JEWELBANK*)aRecv);
							break;
						#endif
						#if(DEV_PLAYERINFO)
						case 0xDB:
							g_ZtUser.RecvInfoPlayer((PMSG_REQ_INFOPLAYER*)aRecv, aIndex);
							break;
						#endif
						#if(CUSTOM_VALUE_MENU)
						case 0xDE:
							g_ZtUser.CGRecvValueMenu((PMSG_ANS_VALUE_MENU*)aRecv, aIndex);
							break;
						#endif
						#if(CUSTOM_MAP_TELEPORT)
						case 0xE1:
							g_MapTeleport.CGReqData((PMSG_REQ_MAPTELEPORT_DATA*)aRecv, aIndex);
							break;
						case 0xE2:
							g_MapTeleport.CGReqSelect((PMSG_REQ_MAPTELEPORT_SELECT*)aRecv, aIndex);
							break;
						#endif
						#if(FIX_VIEWE_MONSTER)
						case 0xE5:
							g_ZtUser.CGReqReViewe((PMSG_REQ_REVIEWE*)aRecv, aIndex);
							break;
						#endif
						#if(CUSTOM_CAMERA_MOVE)
						case 0xEA:
							g_CameraMove.CGReqViewe((PMSG_REQ_CAMERAMOVE_VIEWE*)aRecv, aIndex);
							break;
						#endif
						#if(ENABLE_CAMERA_PARTY)
						case 0xEC:
							g_CameraParty.CGReqPartyMember((PMSG_REQ_PARTY_MEMBER*)aRecv, aIndex);
							break;
						#endif
						#if(ENABLE_MULTI_CHAR)
						case 0xF0:
							GDCharacterListMultiSend((PMSG_REQ_MULTI_CHAR_RECV*)aRecv, aIndex);
							break;
						#endif
					}
				}
		...
		#if(MOVE_ITEM)
		case 0x79:
			CGMoveItemProc((PMSG_MOVEITEM*)lpMsg, aIndex);
			break;
		#endif
#if(ENABLE_MOVE_ITEM)
			case 0x79:
			...
			case 0x80:
				...
				{
				case 0x00:
					g_ItemMarket.CGReqItemList((PMSG_REQ_MARKET_ITEM*) aRecv, aIndex);
					break;
				case 0x01:
					g_ItemMarket.CGReqItemSell((PMSG_REQ_MARKET_SELL*) aRecv, aIndex);
					break;
				case 0x02:
					g_ItemMarket.CGReqItemBuy((PMSG_REQ_MARKET_BUY*) aRecv, aIndex);
					break;
				}			
				break;
			#endif
...
void CGLiveClient(PMSG_CLIENTTIME * lpClientTime, short aIndex) 
{
	...
	if ( gObj[aIndex].Connected == PLAYER_PLAYING )
	{
		...
		if ( ( gObj[aIndex].m_AttackSpeed + SpeedPlus ) < lpClientTime->AttackSpeed || ( gObj[aIndex].m_MagicSpeed + SpeedPlus ) < lpClientTime->MagicSpeed )
		{
			...
			if ( gObj[aIndex].m_AttackSpeedHackDetectedCount > 0 )
			{

				char szLogs[512];
				sprintf(szLogs, "[%s][%s] ip[%s] hdd[%d] -> Editing AttackSpeed [%d][%d] [%d][%d]",
					gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].Ip_addr, gObj[aIndex].hdd_id, 
					gObj[aIndex].m_AttackSpeed, lpClientTime->AttackSpeed, gObj[aIndex].m_MagicSpeed, lpClientTime->MagicSpeed);
				g_AntiCheatPlus.WraitLog(szLogs);
#ifdef _RECONNECT_
...
void CGCheckMainRecv(PMSG_CHECK_MAINEXE_RESULT * lpMsg, int aIndex) 
{
	...
	gObj[aIndex].CheckSumTime = 0;
		...
		return;
#endif
	//int EnableCheckSum = GetPrivateProfileInt("Common","CheckSum",0,"..\\Data\\ZtData\\CommonServer.ini");
	if(ZtConfig.CommonServer.EnableCheckSum)
	{
	...
	if(lpObj->Level == 1)
	{
		for ( int n = OBJ_STARTUSERINDZT ; n < OBJMAX ; n++)
		{
			...
			{
				MsgNormal(n, "Welcome [user: %s Lv%d] to %s", lpObj->Name, lpObj->Level, ztMapName(lpObj->MapNumber));	
			}
		}
	}
#endif
...
//#endif
	...
	gBrotherhood.UserConnect(aIndex);
#endif
}
...
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex) 
{
	...
	g_ChatFilter.CheckSyntax(aIndex, lpChat->chatmsg);
#endif
#if(CUSTOM_CHAT_LOG==FALSE)
	...
	for(int i = 0; ZtConfig.Command.CommandPost[i] != '\0';i++)
	{
		if( ZtConfig.Command.CommandPost[i] != lpChat->chatmsg[i])
		PostConfigConflictFlag = false;
		PostConfigConflictCount++;
	};
	for(int i = 0; ZtConfig.Command.AdminPostName[i] != '\0';i++)
	{
		if( ZtConfig.Command.AdminPostName[i] != lpChat->chatmsg[i])
		AdminConfigConflictFlag = false;
		AdminConfigConflictCount = i;
	};
	//LogAddTD("%d", ZtConfig.Command.PostColor);
	if(AdminConfigConflictFlag)
	{
			if ( slen > 2 )
			{
				if ( CheckAuthorityCondition(98, lpObj) == true) //season4 changed
				{
					if(!g_GMManager.CheckCommand(aIndex, egm_cmd_gg))
					{
						return;
					}
					...
					if(g_ZtLicense.CheckUser(eZtUB::ulasevich) || g_ZtLicense.CheckUser(eZtUB::Local3) || g_ZtLicense.CheckUser(eZtUB::ulasevich2) ||
						g_ZtLicense.CheckUser(eZtUB::Almdar) || g_ZtLicense.CheckUser(eZtUB::Almdar2) ||
					...
					{
						MessaageAllGlobal("[%s]:%s" , lpObj->Name , &lpChat->chatmsg[AdminConfigConflictCount+2]);
					}
					else
					{	
						AllSendServerMsg(&lpChat->chatmsg[AdminConfigConflictCount+2]);
					}
					
#if(CUSTOM_CHAT_LOG==TRUE)
					if ( gWriteChatLog && g_ZtLicense.user.ChatLog)
					{
						g_GMChatLog.Output("[%s] [%s] %s", lpObj->AccountID, lpObj->Name, &lpChat->chatmsg[AdminConfigConflictCount+2]);
					}
#else
	...
	if(PostConfigConflictFlag)
	{
		if (slen > PostConfigConflictCount)
		{
			if ( slen > 2 )
			{
				{
...
#endif
					#if(DEV_BANSYSTEM)
					if(!g_BanSystem.Chat(aIndex))
					{
						MsgOutput(lpObj->m_Index, "[BanChatHdd]");
						return;
					}
					#endif
					...
					{
						if(ZtConfig.DryneaConfig.PostFloodProtect == 1)
						{
							if(lpObj->PostTickCount != 0)
							{
								if ( 1000 * ZtConfig.DryneaConfig.PostFloodProtectTime >= GetTickCount() - lpObj->PostTickCount )
								{
									MsgNormal(aIndex,"Please wait %d seconds before next post", ZtConfig.DryneaConfig.PostFloodProtectTime - ((GetTickCount() - lpObj->PostTickCount) / 1000));
									return;
								}
							}
						}

						lpObj->PostTickCount = GetTickCount();
					}
#endif
					if(ZtConfig.Command.PostDelay > 0)
					{
						if ( 1000 * ZtConfig.Command.PostDelay >= GetTickCount() - lpObj->PostTickCount )
						{
							MsgNormal(aIndex,"Please wait %d seconds before next post", ZtConfig.Command.PostDelay - ((GetTickCount() - lpObj->PostTickCount) / 1000));
							return;
						}
					...
					//char ChatMessage[90];
					//ZeroMemory(&ChatMessage,sizeof(ChatMessage));
					//sprintf(ChatMessage, "%s %s",ZtConfig.Command.PostPrist,&lpChat->chatmsg[1]);break;

					//memcpy(pMessage.Message, szTemp, 90);
					lpObj->Money -= ZtConfig.Command.PostMoney;
					GCMoneySend(aIndex, lpObj->Money);

#if(CUSTOM_POST_SERVER)
					if(ZtConfig.Command.PostServer)
					{
						BroadCastMessageInfo lpRequest = { 0 };
						lpRequest.h.set((LPBYTE)&lpRequest, 0xCD, sizeof(lpRequest));
						lpRequest.Type = 2;
						sprintf(lpRequest.Text, "[%d] ", gGameServerCode+1); 
						strncat(lpRequest.Text, &lpChat->chatmsg[PostConfigConflictCount], 89-10);
						memcpy(lpRequest.Sender, lpChat->chatid, 10);		
						wsJServerCli.DataSend((PCHAR)&lpRequest, sizeof(BroadCastMessageInfo));
					}
					else
					{
						MessageSendAllEx(ZtConfig.Command.PostColor,lpChat->chatid, "%s %s", ZtConfig.Command.PostPrist,&lpChat->chatmsg[PostConfigConflictCount]);
					}
#else
					MessageSendAllEx(ZtConfig.Command.PostColor,lpChat->chatid, "%s %s", ZtConfig.Command.PostPrist,&lpChat->chatmsg[PostConfigConflictCount]);
#endif
					...
					g_QuestionAnswer.Chat(aIndex, &lpChat->chatmsg[PostConfigConflictCount]);
					#endif

#if(CUSTOM_CHAT_LOG==TRUE)
					if ( gWriteChatLog && g_ZtLicense.user.ChatLog )
					{
						g_PostLog.Output("[%s] [%s] %s", lpObj->AccountID, lpObj->Name, &lpChat->chatmsg[PostConfigConflictCount]);
					}
#endif
				}
				return;
		...
		case '!':	// Global Announcement
			if ( slen > 2 )
			{
				...
				else
				{
					...
					lpObj->Money -= ZtConfig.Command.PostMoney;
					GCMoneySend(aIndex, lpObj->Money);
					MessageSendAllEx(ZtConfig.Command.PostColor,lpChat->chatid, "%s %s", ZtConfig.Command.PostPrist,&lpChat->chatmsg[1]);
				}
#endif
				return;
			}
...
#if(_BAN_SYSTEM_)
	if ( lpObj->BanChat > time(NULL) )
	{
		MsgOutput(lpObj->m_Index, "[BanChat] %d Sec", (lpObj->BanChat-time(NULL)));
		return;
	}
#endif
		...
		if ( lpObj->GuildNumber > 0 )
		{
			...
			else if ( lpChat->chatmsg[1] == '@' && lpObj->lpGuild->iGuildUnion )
			{
				...
				if ( UnionManager.GetGuildUnionMemberList(lpObj->lpGuild->iGuildUnion, iGuildCount, iGuildList) == TRUE )
				{
					...
					if ( gWriteChatLog && g_ZtLicense.user.ChatLog )
					{
						g_AllianceChatLog.Output("[%s] [%s] [%s] %s", lpObj->AccountID, lpObj->Name, lpObj->GuildName,  &lpChat->chatmsg[1]);
					}
#endif
			...
			else
			{
				...
				if ( gWriteChatLog && g_ZtLicense.user.ChatLog )
				{
					g_GuildChatLog.Output("[%s] [%s] [%s] %s", lpObj->AccountID, lpObj->Name, lpObj->GuildName, &lpChat->chatmsg[1]);
				}
#endif
	...
	else if( lpChat->chatmsg[0] == '$' )
	{
		if (gGENS)
		{
				...
				&& lpObj->Money >= gGensSystem.ChatPrice )
			{
				...
				if ( gWriteChatLog && g_ZtLicense.user.ChatLog )
				{
					g_GensChatLog.Output("[%s] [%s] [%s] %s", lpObj->AccountID, lpObj->Name, gGensSystem.GetGensInfluenceName(lpObj), &lpChat->chatmsg[1]);
				}
#endif
			...
			// ----
			//GDGensServerGroupChattingSend(lpObj->m_GensInfluence, lpChat);	//-> Work with ZtDB, i think not need...
		}
	}
	else
	{
		...
		if ( gWriteChatLog && g_ZtLicense.user.ChatLog )
		{
			g_OtherChatLog.Output("[%s] [%s] %s", lpObj->AccountID, lpObj->Name, lpChat->chatmsg);
		}
#endif
...
void CGChatWhisperRecv(PMSG_CHATDATA_WHISPER* lpMsg, int aIndex) 
{
...
#if(CUSTOM_CHATFILTER)
	g_ChatFilter.CheckSyntax(aIndex, lpMsg->chatmsg);
#endif
#if(_BAN_SYSTEM_)
	if ( lpObj->BanChat > time(NULL) )
	{
	...
	if ( len > 0 )
	{
...
#ifdef _NEW_CHAT_FIX_
		MessageSendEx(index, 2, pWhisper.id, pWhisper.chatmsg);
#else
		...
		if( gWriteChatLog && g_ZtLicense.user.ChatLog )
		{
			g_PrivateChatLog.Output("[%s] [%s] -> [%s] [%s]: %s", lpObj->AccountID, lpObj->Name, gObj[index].AccountID, gObj[index].Name, lpMsg->chatmsg);
		}
#endif
...
void CSPJoinIdPassRequest(PMSG_IDPASS* lpMsg, int aIndex) 
{
			...
			if(id[0] == sObj->AccountID[0])
			{
				if(!strncmp(id, sObj->AccountID, 10))
				{
					//if(!strncmp(pass, sObj->Pass, 10))
					{
						...
						{
							g_ZtGDManager.GD_OfflineAfk(i, 0, 0);
		...
		if(sObj->Connected == 3)
		{
			if(!strcmp(id,sObj->AccountID))
			{
				//if(!strncmp(pass, sObj->Pass, 10))
				{
					if(sObj->OffExp == 1)
					{
						g_ZtGDManager.GD_OfflineAfk(i, 0, 1);
						GJPUserClose(sObj->AccountID);
						gObjDel(i);
						sObj->OffExp = 0;
					}
	...
	if(g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(ulasevich) || g_ZtLicense.CheckUser(ulasevich2) || 
		g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2)
		 || g_ZtLicense.CheckUser(eZtUB::MU2Play) || g_ZtLicense.CheckUser(eZtUB::Sentinel) || g_ZtLicense.CheckUser(eZtUB::Escalate) || g_ZtLicense.CheckUser(eZtUB::eternalmu) ||
		 g_ZtLicense.CheckUser(eZtUB::Artem) || g_ZtLicense.CheckUser(eZtUB::Artem2) || g_ZtLicense.CheckUser(eZtUB::RevoMU) || g_ZtLicense.CheckUser(eZtUB::GloryMU) ||
		 g_ZtLicense.CheckUser(eZtUB::mu4you) || g_ZtLicense.CheckUser(eZtUB::NSGames))
	{
#define MAX_PASS_LEN 20
		ZeroMemory(&gObj[aIndex].Pass,sizeof(gObj[aIndex].Pass));
		char pass[MAX_PASS_LEN+1];
		pass[MAX_PASS_LEN]=0;
		memcpy(pass, lpMsg->Pass, sizeof(lpMsg->Pass));
		BuxConvert(pass, MAX_ACCOUNT_LEN);//fix
		gObj[aIndex].Pass[MAX_ACCOUNT_LEN] = 0;
		memcpy(gObj[aIndex].Pass, pass, MAX_ACCOUNT_LEN);
	}
...
void CGPCharacterCreate( PMSG_CHARCREATE * lpMsg, int aIndex) 
{
	...
	{
		if(gObj[aIndex].m_CharPage > 0)
		{
			CGPCharacterCreate_Multi((PMSG_CHARCREATE *) lpMsg, aIndex);
	...
	{
			if(!ztCheckSimbol(lpMsg->Name, strlen(lpMsg->Name)))
			{
				LogAddC(2, "Simbol Character create error [%s]", gObj[aIndex].AccountID);
				JGCharacterCreateFailSend(aIndex, lpMsg->Name);
				return;
			}
	}
...
void CGPCharacterCreate_Multi( PMSG_CHARCREATE * lpMsg, int aIndex) 
{
	if ( !PacketCheckTime(&gObj[aIndex]))
	{
		JGCharacterCreateFailSend(aIndex, lpMsg->Name);
...
#ifdef SNAKER_CHN_LANG
	if(g_ZtLicense.CheckUser(eZtUB::snaker) || g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::masonX)
		|| g_ZtLicense.CheckUser(eZtUB::AlterMU))
	{
	...
	SDHP_CREATECHAR_MULTI sCreate;

	memset(&sCreate, 0, sizeof(sCreate));
	PHeadSetB((LPBYTE)&sCreate, 0xF1, sizeof(sCreate));
	sCreate.ClassSkin = lpMsg->ClassSkin;
	sCreate.Number = aIndex;
	memcpy(sCreate.AccountId, gObj[aIndex].AccountID, MAX_ACCOUNT_LEN);
	memcpy(sCreate.Name, lpMsg->Name, MAX_ACCOUNT_LEN);
	...
	sCreate.Page = gObj[aIndex].m_CharPage;
...
void CGPCharDel(PMSG_CHARDELETE * lpMsg,int aIndex) 
{
	...
	if(OBJMAX_RANGE(aIndex))
	{
		if(gObj[aIndex].m_CharPage > 0)
		{
			CGPCharDel_Multi((PMSG_CHARDELETE *) lpMsg, aIndex);
	...
	if(g_AccountSecurity.GetActivePC(aIndex))
	{
		pResult.result = 0;
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		return;
...
void CGPCharDel_Multi(PMSG_CHARDELETE * lpMsg,int aIndex) 
{
	if ( !PacketCheckTime(&gObj[aIndex]))
	{
		return;
	...
	PHeadSetB((LPBYTE)&pCDel, 0xF2, sizeof(pCDel));
	pCDel.Number = aIndex;
	memcpy(pCDel.AccountID, gObj[aIndex].AccountID, sizeof(pCDel.AccountID));
	memcpy(pCDel.Name, lpMsg->Name, sizeof(pCDel.Name));
	pCDel.Guild = 0;
	...
	pCDel.Page = gObj[aIndex].m_CharPage;
...
#if(CUSTOM_ACCOUNT_SECURITY)
	if(!g_AccountSecurity.SelectCharacter(aIndex))
	{
		return;
	}
#endif
...
void GCLevelUpMsgSend(int aIndex, int iSendEffect) 
{
	...
	PMSG_LEVELUP pMsg;	//Need Visual Fix
	...
	pMsg.exUpPoint = gObj[aIndex].LevelUpPoint;
	pMsg.exMaxLife = gObj[aIndex].MaxLife+gObj[aIndex].AddLife;
	pMsg.exMaxMana = gObj[aIndex].MaxMana+gObj[aIndex].AddMana;
	pMsg.exMaxShield = gObj[aIndex].iMaxShield + gObj[aIndex].iAddShield;
	pMsg.exMaxAG = gObj[aIndex].MaxBP + gObj[aIndex].AddBP;
...
void GCMagicListMultiSend(LPOBJ lpObj, BYTE btListType)
{
	...
	for ( int n=0;n<MAX_MAGIC;n++)
	{
		if ( lpObj->Magic[n].IsMagic() == TRUE )
		{
			...
			if( g_MasterSkillSystem.CheckPrimarySkill(lpObj, lpObj->Magic[n].m_Skill) )
			{
				continue;
			}
			// ----
	...
	if ( lpItem->IsItem() == TRUE && lpItem->Give == false && lpItem->live == true)
	{
		...
		if ( lpItem->m_Type == ITEMGET(14,15) )
		{
			if ( MapC[map_num].ItemGive(aIndex, item_num, false) == TRUE )
			{
				...
				pResult.h.size -= 3;
#if(SYSTEM_ACHIEVEMENTS)
				g_Achievements.PickUpZen(aIndex, lpItem->m_BuyMoney);
#endif
			}
		...
		else
		{
			...
			if (lpItem->m_Type == ITEMGET(14,88) ||	lpItem->m_Type == ITEMGET(14,89) ||	lpItem->m_Type == ITEMGET(14,90) || lpItem->m_Type == ITEMGET(14,85) ||	lpItem->m_Type == ITEMGET(14,86) ||	lpItem->m_Type == ITEMGET(14,87)) //season 3.5 add-on
			{
				...
				case ITEMGET(14,90):	iMaxOverLapped = 50;	break;//SEE CONFLICT 
					/*#ifdef SEASON6DOT3_ENG //zt
				case ITEMGET(14,90):	iMaxOverLapped = 255;	break;
#else*/
			...
			if ( lpItem->m_Type == ITEMGET(14,110) ) // Double Goar
			{
				...
				if ( MAIN_INVENTORY_RANGE(pos) )
				{
					...
					if ( gObj[aIndex].pInventory[pos].m_Durability >= 5.0f )
					{
						...
						LogAdd("[DoubleGoer] [%s][%s] Make Dimension Magyon (Left Marker %d)",
						...
						if ( gObj[aIndex].pInventory[pos].m_Durability >= 5.0f )
							{
								...
								LogAdd("[Imperial Fort] [%s][%s] Make Dimension Magyon (Left Marker %d)",
									gObj[aIndex].AccountID, gObj[aIndex].Name, NewDur);
			...
			//#if(SYSTEM_ACHIEVEMENTS) //start Game
			//g_Achievements.PickUp(aIndex, lpItem->m_Type, lpItem->m_Level, lpItem->m_NewOption, lpItem->m_SetOption, lpItem->m_Durability);
			//#endif
			#if(ITEM_GET_NOTICE)
			...
			{
 				if(!gWinQuestSystem.MonsterDrop(aIndex,lpItem->m_Type) && MapC[map_num].ItemGive(aIndex, item_num, false) == TRUE ) 
				{
 					lpItem->Clear();
					pResult.result = -1;
					pResult.h.size -= sizeof(pResult.Data);
...
#if(CUSTOM_ZT_QUEST_SYSTEM==TRUE)
			if( g_ZtWinQuestSystem.PickUpItem(aIndex,lpItem->m_Type) == 1 && MapC[map_num].ItemGive(aIndex, item_num, false) == TRUE ) 
			{
 				lpItem->Clear();
				pResult.result = -1;
				pResult.h.size -= sizeof(pResult.Data);
			...
			else if( g_ZtWinQuestSystem.PickUpItem(aIndex,lpItem->m_Type) == 2) 
			{
				pResult.result = -1;
				pResult.h.size -= sizeof(pResult.Data);
				DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
			...
			if ( pResult.result != 0xFF )
			{
				...
				if ( MapC[map_num].ItemGive(aIndex, item_num, false) == TRUE )
				{
					...
					if ( pos != 0xFF )
					{
						GCInventoryItemOneSend(aIndex, pos);
					}
#endif
...
#ifdef PERIOD
						g_PeriodItemZt.OnRequestPeriodItemList(&gObj[aIndex]);
#endif
					}
					else
					{
...
BOOL CGItemDropRequest(PMSG_ITEMTHROW * lpMsg, int aIndex, BOOL drop_type) //004427B0
{
...
#if(Negative_Number)
	CItem* BCheckItem = &lpObj->pInventory[lpMsg->Ipos];
	//LogAddC(1, "CGItemDropRequest Item %s (%d)", BCheckItem->GetName(), BCheckItem->m_Number);
	//==Check Seri -1
	if (ZtConfig.CommonServer.MembItemLimit && (int)BCheckItem->m_Number < 0)
	{
		//LogAddTD("[WARNING][Serial -1 Item] Item NO SELL SERI (%d)", BCheckItem->m_Number);
		MsgOutput(aIndex, "[WARNING][Serial -1 Item] Item NO Drop SERI (%d)", BCheckItem->m_Number);
		return -1;
	}
	//==Check Item Drop
	if (ZtConfig.CommonServer.NorItemLimit)
	{
		//LogAddTD("[WARNING][Max Level Sell] Item NO SELL Level >= (%d)", ZtConfig.CommonServer.NorItemLevel);
		MsgOutput(aIndex, "[WARNING][All Item] Item NO Drop All Item");
		return -1;
	}
...
#ifdef ZTCLUSIVE_AMYLET
	if(g_ZtLicense.CheckUser(eZtUB::ulasevich) || g_ZtLicense.CheckUser(eZtUB::Local3) || g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::ulasevich2))
	{
		if ( lpObj->pInventory[lpMsg->Ipos].m_Type == ITEMGET(13,147) ||
			lpObj->pInventory[lpMsg->Ipos].m_Type == ITEMGET(13,148) ||
			lpObj->pInventory[lpMsg->Ipos].m_Type == ITEMGET(13,149) ||
			lpObj->pInventory[lpMsg->Ipos].m_Type == ITEMGET(13,150) ||
			lpObj->pInventory[lpMsg->Ipos].m_Type == ITEMGET(13,151) ||
			lpObj->pInventory[lpMsg->Ipos].m_Type == ITEMGET(13,152) ||
			lpObj->pInventory[lpMsg->Ipos].m_Type == ITEMGET(13,153) ||
			lpObj->pInventory[lpMsg->Ipos].m_Type == ITEMGET(13,154) ||
			lpObj->pInventory[lpMsg->Ipos].m_Type == ITEMGET(13,155) ||
			lpObj->pInventory[lpMsg->Ipos].m_Type == ITEMGET(13,156)
			)
		{
			GCServerMsgStringSend("Move Amulet Block",lpObj->m_Index,1);
			return -1;
		}
	}
#endif
...
#if(CUSTOM_ITEM_MOVE)
	if(g_ItemMove.CheckDrop(lpObj->pInventory[lpMsg->Ipos].m_Type))
	{
		GCServerMsgStringSend("Block Drop Item",lpObj->m_Index,1);
		return -1;
	}
#endif
...
#ifdef NEWWINGS_2_5
		|| IS_NEWWINGS_2_5(lpObj->pInventory[lpMsg->Ipos].m_Type)
#endif
#ifdef NEWWINGS_4
		|| IS_NEWWINGS_4(lpObj->pInventory[lpMsg->Ipos].m_Type)
#endif
#ifdef NEWWINGS_5
		|| IS_NEWWINGS_5(lpObj->pInventory[lpMsg->Ipos].m_Type)
#endif
#ifdef NEWWINGS_6
		|| IS_NEWWINGS_6(lpObj->pInventory[lpMsg->Ipos].m_Type)
#endif
		)
	{
...
#ifdef BLOCK_MOVE_DROP_FIX
	if(gBlockMoveIteml.Move(lpObj->m_Index,lpMsg->Ipos) == false)
	{
	...
	if(pResult.Result)
	{
		pResult.Result = g_Achievements.GetItemDrop(lpObj->m_Index, lpObj->pInventory[lpMsg->Ipos].m_Type);
	}
#endif
	...
	if ( pResult.Result == 1 )
	{
		...
		if ( bIsItem == TRUE )
		{
			...
			//{
			//	gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
			//	ShiningJewelryCaseItemBagOpen(lpObj,lpObj->MapNumber,lpObj->X,lpObj->Y);
			//	LogAddTD("[%s][%s][%d]%d/%d Season 5 New Box Item Serial:%u (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)", lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, serial, szItemName, type, level, Option1, Option2, Option3);
			//}
			//else if( type == ITEMGET(14,142) )
			//{
			//	gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
			//	ElegantJewelryCaseItemBagOpen(lpObj,lpObj->MapNumber,lpObj->X,lpObj->Y);
			//	LogAddTD("[%s][%s][%d]%d/%d Season 5 New Box Item Serial:%u (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)", lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, serial, szItemName, type, level, Option1, Option2, Option3);
			//}
			//else if( type == ITEMGET(14,143) )
			//{
			//	gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
			//	SteelJewelryCaseItemBagOpen(lpObj,lpObj->MapNumber,lpObj->X,lpObj->Y);
			//	LogAddTD("[%s][%s][%d]%d/%d Season 5 New Box Item Serial:%u (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)", lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, serial, szItemName, type, level, Option1, Option2, Option3);
			//}
			//else if( type == ITEMGET(14,144) )
			//{
			//	gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
			//	OldJewelryCaseItemBagOpen(lpObj,lpObj->MapNumber,lpObj->X,lpObj->Y);
			//	LogAddTD("[%s][%s][%d]%d/%d Season 5 New Box Item Serial:%u (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)", lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, serial, szItemName, type, level, Option1, Option2, Option3);
			//}
			//else if( type == ITEMGET(14,142) )
			//{
...
void GCItemMoveResultSend(int aIndex, BYTE result, BYTE pos, BYTE ItemInfo[])  //00446170
{
	...
	if(pos >= 204)
	{
		g_PersonalShopZt.UpdatePSInventoryUser(aIndex);
	}
#endif
...
void CGInventoryItemMove(PMSG_INVENTORYITEMMOVE * lpMsg, int aIndex) //00446250
{
...
#if(Negative_Number)
	CItem* BCheckItem = &lpObj->pInventory[source];
	//LogAddC(1, "CGInventoryItemMove Item %s (%d) %d -> %d", BCheckItem->GetName(), BCheckItem->m_Number, lpMsg->sFlag, lpMsg->tFlag);

	//==Check Seri -1
	if ((int)BCheckItem->m_Number < 0)
	{
		if (ZtConfig.CommonServer.MembItemLimit && lpMsg->tFlag == 1 || lpMsg->tFlag == 4) //Cam Giao Dich, Treo Shop
		{
			MsgOutput(aIndex, "[WARNING][Serial -1 Item] Item NO Trade, Ps Shop");
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)& ItemInfo);
			return;
		}
		if (ZtConfig.CommonServer.MembItemMixLimit && lpMsg->tFlag == 3) //Cam Chao Mix
		{
			MsgOutput(aIndex, "[WARNING][Serial -1 Item] Item NO Mix");
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)& ItemInfo);
			return;
		}
		if (ZtConfig.CommonServer.MembItemWareLimit && lpMsg->tFlag == 2) //Cam Ware
		{
			MsgOutput(aIndex, "[WARNING][Serial -1 Item] Item NO Ware");
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)& ItemInfo);
			return;
		}
	}
	//==Check All Item
	if (ZtConfig.CommonServer.NorItemLimit && lpMsg->tFlag == 1 || lpMsg->tFlag == 4)
	{
		//LogAddTD("[WARNING][Max Level Sell] Item NO SELL Level >= (%d)", ZtConfig.CommonServer.NorItemLevel);
		MsgOutput(aIndex, "[WARNING][All Item] Item NO Trade, Ps Shop");
		::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)& ItemInfo);
		return;
	}
	//==Check Item Level Max
	if ((int)BCheckItem->m_Level >= ZtConfig.CommonServer.NorItemLevel && lpMsg->tFlag >= 1 && lpMsg->tFlag <= 4)
	{
		//LogAddTD("[WARNING][Max Level Sell] Item NO SELL Level >= (%d)", ZtConfig.CommonServer.NorItemLevel);
		MsgOutput(aIndex, "[WARNING][Max Level Move] Item NO Move Level >= (%d)", ZtConfig.CommonServer.NorItemLevel);
		::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)& ItemInfo);
		return;
	}
	//==Check Item Count Exl
	if (GetExcOptionCount(BCheckItem->m_NewOption) >= ZtConfig.CommonServer.NorItemExt && lpMsg->tFlag >= 1 && lpMsg->tFlag <= 4)
	{
		//LogAddTD("[WARNING][Max Ext Sell] Item NO SELL Ext >= (%d)", ZtConfig.CommonServer.NorItemLevel);
		MsgOutput(aIndex, "[WARNING][Max Ext Move] Item NO Move Ext >= (%d)", ZtConfig.CommonServer.NorItemLevel);
		::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)& ItemInfo);
		return;
	}
#endif
	...
	if( (lpMsg->tFlag == lpMsg->sFlag) && (source == target)) //season 3.0 add-on (New-Antihack)
	{
	 ...
	 if(THINNAKORN_MAC || g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
	 {
...
//			if (lpObj->WarehouseNumber < 0 || lpObj->WarehouseNumber > 5  )
//#endif
//			{
//				::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
...
//		}
	...
	if ( lpMsg->sFlag == 2 && lpMsg->tFlag == 0 )
	{
		...
		#if(CUSTOM_ACCOUNT_SECURITY)
		if(g_AccountSecurity.CheckWerehouse(aIndex) == false)
		{
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
			//::GCServerMsgStringSend(lMsg.Get(MSGGET(6, 70)), lpObj->m_Index, 1);
			return;
	...
	if( it_type != 8 )
	{
		...
		#if(CUSTOM_ITEM_MOVE)
		if(g_ItemMove.CheckVault(lpObj->pInventory[source].m_Type))
		{
			GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
			GCServerMsgStringSend("Block Vault Item",lpObj->m_Index,1);
			return;
		}
		#endif	
	}
		 ...
		 (lpMsg->sFlag == 3 && lpMsg->tFlag == 3) )
	{
		...
		if( loc77 == ITEMGET(14,110)) //Season 5 DoppelGanger
		{
			if( lpObj->pInventory[target].m_Durability == 0.0f && (lpMsg->sFlag == FALSE) && (lpMsg->tFlag == FALSE))
			{
				GCInventoryItemDeleteSend(aIndex,target,0);
		...
		if( loc77 == ITEMGET(14,101)) //Season 5 ImperialFort
		{
			if( lpObj->pInventory[target].m_Durability == 0.0f && (lpMsg->sFlag == FALSE) && (lpMsg->tFlag == FALSE))
			{
				GCInventoryItemDeleteSend(aIndex,target,0);
...
#ifdef PERIOD
		g_PeriodItemZt.SendPeriodItemInfoOnce(&gObj[aIndex], &lpObj->pInventory[target]);
#endif
		return;
	}
...
//#ifdef GENS
		if( !gPkLimitFree && !gGensSystem.IsMapBattleZone(lpObj->MapNumber)
//#endif	
			)
		{
			BOOL bPlayerKiller = FALSE; //season 2.5 add-on

			if(ZtConfig.PvP.ZtPkPartySystem && lpObj->PartyNumber >= 0) //season 2.5 add-on
			{
	...
	#if(CUSTOM_VALUE_MENU)
	if(ZtConfig.ZtCustom.MultiWarehouse)
	{
		if ( ShopNum == 100 )
		{
			if(g_ZtLicense.m_MultiWarehouseType)
			{
				...
				if(g_GuildBank.m_iEnable == 1 && 
					...
					gObj[DealerNumber].X == g_GuildBank.m_iGuildBankX &&
					gObj[DealerNumber].Y == g_GuildBank.m_iGuildBankY)
				{
				...
				if(g_ZtUser.GCSendValueMenu(aIndex, 1))
				{
					return;
				}
			}
	...
	else
	{
		...
		pRequest.h.set((LPBYTE)&pRequest, 0xFB, 0x06, sizeof(GC_SHOP_CLICK));
		// ----
...
void GCUserWarehouseSend(LPOBJ lpObj) 
{
	...
	DataSend(lpObj->m_Index, SendByte, lOfs);	//Îòñûëàåò ïàêå?âåùå??êëèåíò
...
bool CheckBuyDonateItem(PMSG_BUYREQUEST* lpMsg, int aIndex)
{
	if(!OBJMAX_RANGE(aIndex))
	{
		return false;
	...
	if(lpUser->m_bCheckBuyDonateItem)
	{
		return false;
	}

	if(lpUser->TargetShopNumber < DONAT_SHOP_START && DONAT_SHOP_END > lpUser->TargetShopNumber)
	{
		return false;
	}

	lpUser->m_bCheckBuyDonateItem = true;
	lpUser->m_btCheckBuyDonatePos = lpMsg->Pos;

	PMSG_DONATE_CHECKBUY pMsg;
...
void CGDonateItemBuy(PMSG_DONATE_ITEMBUY* lpMsg, int aIndex)
{
	if(!OBJMAX_RANGE(aIndex))
	{
		return;
	...
	{
		PMSG_BUYREQUEST pMsg;
		pMsg.Pos = lpUser->m_btCheckBuyDonatePos;
		CGBuyRequestRecv(&pMsg, aIndex);
	}

	lpUser->m_bCheckBuyDonateItem = false;
	lpUser->m_btCheckBuyDonatePos = 0;
}

void CGBuyRequestRecv(PMSG_BUYREQUEST * lpMsg, int aIndex)
{
#if(THINNAKORN_MAC)
	if(CheckBuyDonateItem(lpMsg, aIndex) == true)
	{
		return;
	}
#endif
	...
	if ( ShopC[tShop].m_item[lpMsg->Pos].IsItem() == TRUE )
	{
		...
		int BuyItemLevel = ShopC[tShop].m_item[lpMsg->Pos].m_Level;

		if(!PriceManager.BuyPriceItem(aIndex,ShopC[tShop].m_item[lpMsg->Pos].m_Type,iStoreTaxMoney,BuyItemLevel))
		{
			DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
			return;
		}
...
#if(DONATE_SHOP==TRUE)
		bool StandartShop = true;

		if(tShop >= 0 && tShop < DONAT_SHOP_START)
		{
			StandartShop = true;
		}
		else
		...
		if(StandartShop == false)
		{
			if(!ShopC[tShop].CheckBuyItemEx(aIndex,lpMsg->Pos, iStoreTaxMoney))
			{
				DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
				return;
			}
		...
		else
		{
			...
			if ( bNoItem != FALSE )
			{
				...
				if ( pResult.Result != 0xFF )
				{
					...
					if(StandartShop == false)
					{
						if(!ShopC[tShop].BuyItemEx(aIndex,lpMsg->Pos))
						{
							lpObj->Money -= iStoreTaxMoney;
							::g_CastleSiegeSync.AddTributeMoney(iStoreTaxMoney2);
...
void CGSellRequestRecv(PMSG_SELLREQUEST * lpMsg, int aIndex) 
{
...
#if(Negative_Number)
	CItem* BCheckItem = &lpObj->pInventory[lpMsg->Pos];
	//LogAddC(1, "CGSellRequestRecv Item %s (%d)", BCheckItem->GetName(), BCheckItem->m_Number);
	//==Check Seri -1
	if (ZtConfig.CommonServer.MembItemLimit && (int)BCheckItem->m_Number < 0)
	{
		//LogAddTD("[WARNING][Serial -1 Item] Item NO SELL SERI (%d)", BCheckItem->m_Number);
		MsgOutput(aIndex, "[WARNING][Serial -1 Item] Item NO SELL SERI (%d)", BCheckItem->m_Number);
		DataSend(aIndex, (LPBYTE)& pResult, pResult.h.size);
		return;
	}
	//==Check Item Level Max
	if ((int)BCheckItem->m_Level >= ZtConfig.CommonServer.NorItemLevel)
	{
		//LogAddTD("[WARNING][Max Level Sell] Item NO SELL Level >= (%d)", ZtConfig.CommonServer.NorItemLevel);
		MsgOutput(aIndex, "[WARNING][Max Level Sell] Item NO SELL Level >= (%d)", ZtConfig.CommonServer.NorItemLevel);
		DataSend(aIndex, (LPBYTE)& pResult, pResult.h.size);
		return;
	}
	//==Check Item Count Exl
	if (  GetExcOptionCount(BCheckItem->m_NewOption) >= ZtConfig.CommonServer.NorItemExt)
	{
		//LogAddTD("[WARNING][Max Ext Sell] Item NO SELL Ext >= (%d)", ZtConfig.CommonServer.NorItemLevel);
		MsgOutput(aIndex, "[WARNING][Max Ext Sell] Item NO SELL Ext >= (%d)", ZtConfig.CommonServer.NorItemLevel);
		DataSend(aIndex, (LPBYTE)& pResult, pResult.h.size);
		return;
	}
#endif
	...
	if(g_ZtLicense.CheckUser(eZtUB::ulasevich) || g_ZtLicense.CheckUser(eZtUB::Local3) || g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::ulasevich2))
	{
		if ( lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(13,147) ||
			lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(13,148) ||
			lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(13,149) ||
			lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(13,150) ||
			lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(13,151) ||
			lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(13,152) ||
			lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(13,153) ||
			lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(13,154) ||
			lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(13,155) ||
			lpObj->pInventory[lpMsg->Pos].m_Type == ITEMGET(13,156)
			)
		{
			DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
			return;
	...
	//if(gBlockMoveIteml.Sell(aIndex, lpMsg->Pos) == false)
	//{
	//	GCServerMsgStringSend("Move Item Block",lpObj->m_Index,1);
	//	DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
	//	return;
...
#ifdef PERIOD
	if( lpObj->pInventory[lpMsg->Pos].IsPeriodItem() && !lpObj->pInventory[lpMsg->Pos].IsPeriodItemZtpire() )
	{
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		return;
	}
...
#if(CUSTOM_ITEM_MOVE)
	if(g_ItemMove.CheckDrop(lpObj->pInventory[lpMsg->Pos].m_Type))
	{
		GCServerMsgStringSend("Block Sell Item",lpObj->m_Index,1);
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		return;
	}
#endif
		...
		if(!gCreditDonate.Sell(aIndex,lpMsg->Pos,iAddZen))
		{
			if(gBlockMoveIteml.Sell(aIndex, lpMsg->Pos) == false)
			{
				GCServerMsgStringSend("Move Item Block",lpObj->m_Index,1);
...
void ItemDurRepaire(LPOBJ lpObj, CItem * DurItem, int pos, int RequestPos) 
{
	...
	if ( itemtype >= ITEMGET(14,0) || ( itemtype >= ITEMGET(13,0) && itemtype  < ITEMGET(13,4) ) || 
		...
		itemtype < ITEMGET(12,36)) || (itemtype > ITEMGET(12,43) &&		//43
		itemtype < ITEMGET(12,49)) || (itemtype > ITEMGET(12,50) &&	
#ifdef NEWWINGS_6
			itemtype < ITEMGET(12,180))|| (itemtype > ITEMGET(12,220) &&	//Custom Wings4	196
#else
		itemtype < ITEMGET(12,180))|| (itemtype > ITEMGET(12,194) &&	//Custom Wings4
#endif
		//itemtype != ITEMGET(12, 50) || itemtype != ITEMGET(12, 49) &&
	...
	if ( itemtype == ITEMGET(13,64) ||
		 ...
		 itemtype == ITEMGET(13,80) ||
		 itemtype == ITEMGET(13,106)||
		 itemtype == ITEMGET(13,122)||
		 itemtype == ITEMGET(13,123) )
	{
		pResult.Money = 0;
		DataSend(lpObj->m_Index, (LPBYTE)&pResult, pResult.h.size);
		return;
...
//#endif
				...
				if(g_ZtLicense.CheckUser(eZtUB::ulasevich) || g_ZtLicense.CheckUser(eZtUB::Local3) || g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::ulasevich2))
				{
					if ( lpObj->pInventory[n].m_Type == ITEMGET(13,147) ||
						lpObj->pInventory[n].m_Type == ITEMGET(13,148) ||
						lpObj->pInventory[n].m_Type == ITEMGET(13,149) ||
						lpObj->pInventory[n].m_Type == ITEMGET(13,150) ||
						lpObj->pInventory[n].m_Type == ITEMGET(13,151) ||
						lpObj->pInventory[n].m_Type == ITEMGET(13,152) ||
						lpObj->pInventory[n].m_Type == ITEMGET(13,153) ||
						lpObj->pInventory[n].m_Type == ITEMGET(13,154) ||
						lpObj->pInventory[n].m_Type == ITEMGET(13,155) ||
						lpObj->pInventory[n].m_Type == ITEMGET(13,156)
						)
					{
						continue;
					}
				}
#endif // ZTCLUSIVE_AMYLET
...
void CGTradeRequestSend(PMSG_TRADE_REQUEST * lpMsg, int aIndex) 
{
	...
	if ( DG_MAP_RANGE(gObj[number].MapNumber) != FALSE )	//Season 5 DoppelGanger
	{
...
void CGPShopReqSetItemPriceEx(PMSG_REQ_PSHOP_SETITEMPRICE_EX * lpMsg, int aIndex) 
{
...
#if(CUSTOM_PERSONAL_SHOP ==TRUE)
	int iBonusPrice = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->sBonusPrice4, lpMsg->sBonusPrice3), MAKE_NUMBERW(lpMsg->sBonusPrice2, lpMsg->sBonusPrice1));
	...
	short aLifeCnt = MAKE_NUMBERW(lpMsg->sLifeCnt2,lpMsg->sLifeCnt1);
	int iCreditPrice = MAKE_NUMBERDW(MAKE_NUMBERW(lpMsg->sCreditPrice4, lpMsg->sCreditPrice3), MAKE_NUMBERW(lpMsg->sCreditPrice2, lpMsg->sCreditPrice1));
#endif
...
#if(CUSTOM_PERSONAL_SHOP ==TRUE)
	if ( iItemPrice < 0 || iBonusPrice < 0 || aBlessCnt < 0 || aSoulCnt < 0 || aChaosCnt < 0 || aLifeCnt < 0 || iCreditPrice < 0)
	{
...
#if(CUSTOM_PERSONAL_SHOP ==TRUE)
	if(!g_PersonalShopZt.CheckPrice(iItemPrice, iBonusPrice, aBlessCnt, aSoulCnt, aChaosCnt, aLifeCnt, iCreditPrice))
	{
		LogAddTD("[PShop] [%s][%s] ERROR : Personal Shop Item Check Price", gObj[aIndex].AccountID, gObj[aIndex].Name);
		CGPShopAnsSetItemPrice(aIndex, 0, lpMsg->btItemPos);
		return;
	}
#endif
...
//#if(CUSTOM_PERSONAL_SHOP==TRUE)
	if(g_PersonalShopZt.Enable)
	{
		if(!g_PersonalShopZt.PShop_CheckInventoryPrice(aIndex))
		{
			::GCServerMsgStringSend("No Price Item", aIndex, 1);
			::CGPShopAnsOpen( aIndex, 0);
			return;
		}	
	}
//#endif
	...
	if(gObj[aIndex].m_bActiveOffTrade == true)
	{
#if(_RECONNECT_==TRUE)
		g_ConnectZt.SendClose(aIndex, TRUE);
#else
		CloseClient(aIndex);
...
void CGPShopAnsBuyList(int aSourceIndex, int aTargetIndex, BYTE btResult, bool bResend) 
{
	...
	{
#if(CUSTOM_PERSONAL_SHOP==TRUE)
		g_PersonalShopZt.UpdatePSInventoryTarget(aSourceIndex, aTargetIndex);
#endif
...
//#if(CUSTOM_PERSONAL_SHOP==TRUE)
				if(g_PersonalShopZt.Enable)
				{
					pMsgIL.PShopItemValue = g_PersonalShopZt.GetMoney(aTargetIndex,n); //lpObjTarget->Inventory1[n].m_iPShopValue;
				}
				else
//#else
				{
...
#if(CUSTOM_ACCOUNT_SECURITY)
	if(g_AccountSecurity.CheckTrade(lpSourceObj->m_Index) == false)
	{
		::CGPShopAnsBuyItem(aSourceIndex, -1, 0, 10);
		return;
	}
	...
	else
	{
		...
		__try
		{
			...
			if ( lpObj->Inventory1[lpMsg->btItemPos].IsItem() == TRUE )
			{
				...
				// --
				//g_PersonalShopZt.BonusPrice;
				if( iPShopBonusValue > 0 )
				{
					if(g_PersonalShopZt.BonusPrice == 1)
					{
						if ( gObj[aSourceIndex].ZtCred < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopBonus )
						{
							LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
								lpSourceObj->AccountID, lpSourceObj->Name,lpObj->AccountID, lpObj->Name);
							//::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 7);
					...
					else if(g_PersonalShopZt.BonusPrice == 2)
					{
				...
				{
					if(g_PersonalShopZt.CreditPrice == 1)
					{
						if ( gObj[aSourceIndex].ZtCred < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopCredit )
						{
				...
				#if(CUSTOM_ITEM_MOVE)
				if(g_ItemMove.CheckTrade(lpObj->Inventory1[lpMsg->btItemPos].m_Type))
				{
					GCServerMsgStringSend("Block Trade Item",lpObj->m_Index,1);
					::CGPShopAnsBuyItem(aSourceIndex, -1, 0, 10);
					return;
				}
				#endif
				...
				else
				{
					...
					for( n = 0; n < 4; n++)
					{
						if( JewelCount[n] < aPShopJewelValue[n] )
						{
							...
							lpSourceObj->AccountID, lpSourceObj->Name,lpObj->AccountID, lpObj->Name, n);
							::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 11+n);
							MsgOutput(aSourceIndex, g_ZtText.GetText(253));
							return;
						}
				...
				if( iPShopBonusValue > 0 )
				{
					if(g_PersonalShopZt.BonusPrice == 1)
					{
						gObj[aSourceIndex].ZtCred -= iPShopBonusValue;
						lpObj->ZtCred += iPShopBonusValue;
					}
					else if(g_PersonalShopZt.BonusPrice == 2)
					{
				...
				if( iPShopCreditValue > 0 )
				{
					if(g_PersonalShopZt.CreditPrice == 1)
					{
						gObj[aSourceIndex].ZtCred -= iPShopCreditValue;
						lpObj->ZtCred += iPShopCreditValue;
					}
					ZtUserDataSend(aSourceIndex);
				}
				#endif
				...
				if(ExBankMinus == true)
				{
					...
					ZtUserDataSend(lpSourceObj->m_Index);
				}
				else
				{
	...
	else
	{
		...
		__try
		{
			...
			if ( lpObj->Inventory1[lpMsg->btItemPos].IsItem() == TRUE )
			{
...
#ifdef _OFFTRADE_
				if(!gOffTrade.CheckMoney(aSourceIndex, lpObj->m_Index, lpMsg->btItemPos))return;
#else
				if ( gObj[aSourceIndex].Money < lpObj->Inventory1[lpMsg->btItemPos].m_iPShopValue )
				{
					LogAddTD("[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
...
#ifdef _OFFTRADE_
				gOffTrade.ValueUpdate(aSourceIndex, lpObj->m_Index, dwCost);
#else
				...
				GJSetCharacterInfo(lpObj, aSourceIndex, 0);	//OffTrade
#endif
				...
				if (PShop_CheckInventoryEmpty(lpObj->m_Index) == true )
				{
					...
					::CGPShopAnsClose(lpObj->m_Index, 1);
#ifdef _OFFTRADE_
					gOffTrade.UserOnlineOffSell(lpObj);
#endif
				}
				else
				{
		...
		for ( int n=0;n<MAX_USER_IN_PARTY;n++)
		{
			...
			if ( number >= 0 )
			{
				if ( gObjIsConnected(&gObj[number], gParty.m_PartyS[pnumber].DbNumber[n]) == TRUE )
				{
					...
					lOfs += sizeof(pList);
#ifdef _ZT_DEV_BUFF_PT_
					GCDisplayBuffeffectPartyMember(number);
#endif
				}
			}
		}
...
void CGPartyListAll(int pnumber) 
{
    ...
    {
        number = gParty.m_PartyS[pnumber].Number[nb];
        if ( number >= 0 )
		{
			GCDisplayBuffeffectPartyMember(number);
		}
...
//#if(GS_CASTLE==1)
	if(g_ZtLicense.CheckUser(eZtUB::PrideMu) || g_ZtLicense.CheckUser(eZtUB::PrideMuLocal) || g_ZtLicense.CheckUser(eZtUB::Local3))
	{
...
//#endif
	...
	if(gObj[number].lpGuild->WareOpen)
	{
		GCResultSend(aIndex, 0x51, 0x06);
		return;
	}
	...
	{
#if(FIX_AGM_REQVALLIANCE || ENABLE_AGMASTER)
		if( g_ZtLicense.CheckUser(eZtUB::Gredy) || 
			...
			g_ZtLicense.CheckUser(eZtUB::MUDev))	
		{
	...
	if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal))	
	{
		...
		if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal))
		{
			if ( gObj[number].lpGuild->TotalCount >= MAX_USER_GUILD )
			{
		...
		int MaxGuildMember = MAX_USER_GUILD;
		...
		if(g_ZtLicense.CheckUser(Gredy) || g_ZtLicense.CheckUser(Gredy2) || g_ZtLicense.CheckUser(GredyLocal))
		{
			MaxGuildMember = ZtConfig.m_iGuildMaxUser;
		}
...
//#endif
	...
	if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal))	
	{
		_GUILD_INFO_STRUCT * lpGuildUser = gObj[aIndex].lpGuild;
...
void CGGuildMasterInfoSave(int aIndex, PMSG_GUILDINFOSAVE * lpMsg) 
{
	...
	{
		if ( len <= 2 || len > MAX_GUILD_LEN
#ifdef FIX_CREATE_GUILD
			|| !ztCheckSimbol(GuildName, len)
#endif
...
#if(FIX_ZEN_MOVE==TRUE)
				unsigned long long totalzen = lpObj->WarehouseMoney + money;
				if ( totalzen > MAX_WAREHOUSE_ZEN )
				{
					GCWarehouseInventoryMoneySend(aIndex, 0, 0, 0);
					return;
				}
...
//void CGWarehouseUseEndOLD(int aIndex) 
//{
//	if ( !gObjIsConnectedGP(aIndex))
//	{
//		LogAddC(2, "error-L3 [%s][%d]", __FILE__, __LINE__);
...
//
//	gObjItemTextSave(lpObj);
//	GDSetWarehouseList_(aIndex);
//	GDUserItemSave(lpObj);
//
//	//if(lpObj->WareChange == true)
//	//{
//	//	return;
//	//}
//
...
void CGWarehouseUseEnd(int aIndex) 
{
	...
	DWORD dwDelay = GetTickCount() - lpObj->WarehouseTickCount;

	if(dwDelay < 100)
	{
		LogAddC(2, "error WarehouseTickCount");
#ifdef _RECONNECT_				
		g_ConnectZt.SendClose(aIndex, NORM_DC);
#else
		CloseClient(aIndex);
	...
	if(lpObj->m_IfState.use > 0 && lpObj->m_IfState.type == 6 && lpObj->m_IfState.state == 1 && lpObj->WarehouseSave == 1)
	{
		PMSG_DEFAULT pMsg;
		PHeadSetB((LPBYTE)&pMsg, 0x82, sizeof(pMsg));
		DataSend(aIndex, (LPBYTE)&pMsg, pMsg.h.size);
	...
	else if(lpObj->WareChange == true && lpObj->m_IfState.use == 0)
	{
		lpObj->m_IfState.use = 1;
	}
	else
	{
		LogAddC(2, "error [CGWarehouseUseEnd][%s][%d]", __FILE__, __LINE__);
...
#if(DEV_GUILDBANK)
	if(!g_GuildBank.WarehousePassword(aIndex))
	{
		return;
	}
#endif
...
//#endif
		...
		case CHAOS_TYPE_SOCKET_SPHERE_REMOVE_MIX:
			...
			break;
		case CHAOS_TYPE_SECROMICON:	//Season 5 ImperialFort
			g_MixSystem.SecromiconMix(lpObj);
			break;
		case 47:	//CHAOS_TYPE_GOLD_AND_SILBER_BOX
...
void PMoveProc(PMSG_MOVE* lpMove, int aIndex)
{
	...
	if ( lpObj->PathCount > 0 )
	{
		...
		if (g_ZtLicense.user.PandoraEvent)
		{
			if ( gObj[aIndex].Type == OBJ_USER )
			{
				if (( (attr&1) == 1 ) && (aIndex == gPandoraBoxEvent.ActivePlayer))
				{
					for ( n=0 ; n<15 ; n++)
					{
						lpObj->PathX[n] = 0;
...
void CGAttack(PMSG_ATTACK* lpMsg, int aIndex) 
{
	...
	if ( !gObj[usernumber].Live )
	{
		...
		//gObjViewportListCreate(gObj[aIndex].m_Index);
		//gObjViewportListProtocolCreate(&gObj[aIndex]);
		//MsgOutput(aIndex, "[INDEX]: %d", usernumber);
		#endif
		return;
...
void GCDamageSend(int aIndex, int TargetIndex, int AttackDamage, int MSBFlag, int MSBDamage, int iShieldDamage) 
{
	...
	pResult.iShieldDamage = iShieldDamage;

	if ( gObj[aIndex].Type == OBJ_USER && OBJMAX_RANGE(TargetIndex))
	{
		pResult.Life	= (DWORD)(gObj[TargetIndex].Life);
		pResult.MaxLife = (DWORD)(gObj[TargetIndex].MaxLife + gObj[TargetIndex].AddLife);
		pResult.SD	= (DWORD)(gObj[TargetIndex].iShield);
		pResult.MaxSD = (DWORD)(gObj[TargetIndex].iMaxShield + gObj[TargetIndex].iAddShield);
	}
	else
	{
		pResult.Life	= 0;
	...
	if ( OBJMAX_RANGE(TargetIndex) && gObj[TargetIndex].Type == OBJ_USER )
	{
...
{
#if EXP_FIX == TRUE
	GCKillPlayerExtSend(aIndex, TargetIndex, exp, AttackDamage, MSBFlag);
	return;
#endif
	//GCKillPlayerExtSend(aIndex, TargetIndex, exp, AttackDamage, MSBFlag);
	...
	pkillMsg.ExDamage = AttackDamage;
...
void GCKillPlayerExtSend(int aIndex, int TargetIndex, __int64 exp, int AttackDamage, int MSBFlag)
{
	...
	pMsg.ExDamage = AttackDamage;

	DataSend(aIndex, (LPBYTE)&pMsg, pMsg.h.size);

#if(FIX_FINAL_DMG_COLOR==TRUE)
	if(g_ZtLicense.CheckUser(Local) || g_ZtLicense.CheckUser(SILVER1) || g_ZtLicense.CheckUser(SILVER2))
	{
		GCDamageSend(aIndex, TargetIndex, AttackDamage, 0, gObj[aIndex].MSBDamage, 0);
	}
#endif
}
...
void GCMagicAttackNumberSend(LPOBJ lpObj, WORD MagicNumber, int usernumber,  BYTE skillsuccess)
{
	...
	if ( CC_MAP_RANGE(lpObj->MapNumber) )
	{
		...
		{
			int GetBaseMasterLevelSkill = g_MasterSkillSystem.GetBaseMasterLevelSkill(MagicNumber);
...
void CGMagicCancel(PMSG_MAGICCANCEL * lpMsg, int aIndex)
{
	...
	{
		if ( gObjSearchActiveEffect(lpObj,BUFF_EXP_WIZARDRY2) == TRUE )
		{
	...
	{
		if ( gObjSearchActiveEffect(lpObj,BUFF_EXP_WIZARDRY3) == TRUE )
		{
	...
	{
		if( gObjSearchActiveEffect(lpObj,BUFF_INFINITY_ARROW2) == TRUE )
		{
	...
	if ( lpMagic )
	{
		...
		if ( usemana >= 0 )
		{
			...
			if ( usebp >= 0 )
			{
				...
				gObj[iTargetIndex].MuHelperTeleportFix = true;
#endif
				return;
			}
		}
...
#if(CUSTOM_MAPQUEST)
	g_MapQuest.TeleportMap(lpObj->m_Index, MapNumber);
#endif
}
...
void CGBeattackRecv(BYTE* lpRecv, int aIndex, int magic_send) 
{
	...
	if(g_MasterSkillSystem.CheckRequireStatus(lpMagic->m_Skill) != FALSE) //Season3 add-on
	{
		...
		if(	   GetBaseMasterLevelSkill != 403
			...
			&& GetBaseMasterLevelSkill != 556
			&& GetBaseMasterLevelSkill != 558
			&& GetBaseMasterLevelSkill != 559
			&& GetBaseMasterLevelSkill != 560
			&& GetBaseMasterLevelSkill != 561
			&& GetBaseMasterLevelSkill != 562
			&& GetBaseMasterLevelSkill != 563
			&& GetBaseMasterLevelSkill != 564
			&& GetBaseMasterLevelSkill != 565
			&& GetBaseMasterLevelSkill != 566
			&& GetBaseMasterLevelSkill != 567
			&& GetBaseMasterLevelSkill != 575
			&& GetBaseMasterLevelSkill != 577 )
		{
			LogAdd("error-L3 %s %d", 
				__FILE__, 
				__LINE__);
	...
	if ( lpMagic->m_Skill == 24 || 
		...
		g_MasterSkillSystem.GetBaseMasterLevelSkill(lpMagic->m_Skill) == 525)
	{
	...
	{
		#if(EVIL_SPIRIT_SUMMONER)
		if(g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal))
		{
			if(gObj[aIndex].Class != CLASS_WIZARD && gObj[aIndex].Class != CLASS_MAGUMSA &&	gObj[aIndex].Class != CLASS_SUMMONER)
			{
				return;
			}
		}
	...
	for (int n=0;n<lpCount->Count;n++)
	{
		...
		g_MasterSkillSystem.GetBaseMasterLevelSkill(lpMagic->m_Skill) == 445 ||
		g_MasterSkillSystem.GetBaseMasterLevelSkill(lpMagic->m_Skill) == 530))
		{
			return;
		}
		...
		#if(FIX_OBJECT_VIEWE)
		g_ObjectViewe.GCIndexLive(aIndex, tNumber, gObj[tNumber].Type, gObj[tNumber].Live);
		#endif
...
void CGDurationMagicRecv(PMSG_DURATION_MAGIC_RECV* lpMsg, int aIndex)
{
	...
	{
		g_ObjectViewe.GCIndexLive(aIndex, aTargetIndex, gObj[aTargetIndex].Type, gObj[aTargetIndex].Live);
	}
	#endif
...
void CGUseItemRecv(PMSG_USEITEM* lpMsg, int aIndex) //0045C690
{
		...
		g_ZtLicense.CheckUser(eZtUB::Stone) )
	{
		if(citem->m_Type >= ITEMGET(14, 0) && citem->m_Type <= ITEMGET(14, 3) || citem->m_Type== ITEMGET(14, 70))
		{
			...
			if( Delay < ZtConfig.CommonServer.PotionDelay )
			{
				MsgOutput(aIndex, "HP Delay: %d / %d ms.", Delay, ZtConfig.CommonServer.PotionDelay);
				return;
			}
		}
		else if(g_ZtLicense.CheckUser(eZtUB::Stone))
		{
			if(citem->m_Type >= ITEMGET(14, 35) && citem->m_Type <= ITEMGET(14, 40))
			{
				...
				if( Delay < ZtConfig.CommonServer.PotionDelay )
				{
					MsgOutput(aIndex, "SD Delay: %d / %d ms.", Delay, ZtConfig.CommonServer.PotionDelay);
					return;
				}
			}
		}
...
void GCReqmoveDevilSquare(PMSG_REQ_MOVEDEVILSQUARE* lpMsg, int aIndex) 
{
	...
	if(!ZtConfig.Event.DSAllowPKPlayer)
	{
		if(lpObj->m_PK_Level >= 4)
		{
			bPlayerKiller = TRUE;
	...
	if(ZtConfig.CommonServer.DevilSqueryCount)
	{
		if(AccountDataGet(lpObj) == -1)
		{
			AccountDataAdd(lpObj);	
		}

		int iAccInx = AccountDataGet(lpObj);

		if(iAccInx != -1)
		{

			//MsgNormal(aIndex, "DS %d / %d", ExAccData[iAccInx].DevilSqueryCount, ZtConfig.CommonServer.DevilSqueryCount);

			if(ExAccData[iAccInx].DevilSqueryCount >= ZtConfig.CommonServer.DevilSqueryCount)
			{
				MsgNormal(aIndex, "You have reached the daily limit");
			...
			//ExAccData[iAccInx].DevilSqueryCount++;
		}
	}
...
void CGCloseWindow(int aIndex) 
{
	if ( gObj[aIndex].m_IfState.use && gObj[aIndex].m_IfState.type )
	{
		...
		{
#if(DONATE_SHOP==TRUE)
			if( gObj[aIndex].TargetShopNumber < DONAT_SHOP_START && DONAT_SHOP_END > gObj[aIndex].TargetShopNumber )
			{
				gObj[aIndex].TargetShopNumber = -1;
			}
#else
...
void CGRequestEnterBloodCastle(PMSG_REQ_MOVEBLOODCASTLE* lpMsg, int iIndex) 
{
	...
	if(ZtConfig.PvP.ZtPkPartySystem && gObj[iIndex].PartyNumber >= 0) //Season 2.5 add-on
	{
	...
	if(!ZtConfig.Event.BCAllowPKPlayer)
	{
	...
	if(ZtConfig.CommonServer.BloodCastleCount)
	{
		if(AccountDataGet(&gObj[iIndex]) == -1)
		{
			AccountDataAdd(&gObj[iIndex]);	
		}

		int iAccInx = AccountDataGet(&gObj[iIndex]);

		if(iAccInx != -1)
		{
			//MsgNormal(iIndex, "BC %d / %d", ExAccData[iAccInx].BloodCastleCount, ZtConfig.CommonServer.BloodCastleCount);

			if(ExAccData[iAccInx].BloodCastleCount >= ZtConfig.CommonServer.BloodCastleCount)
			{
				MsgNormal(iIndex, "You have reached the daily limit");
		...
		//ExAccData[iAccInx].BloodCastleCount++;
	}
...
//#endif
	...
	if ( !gPkLimitFree )
	{
		...
		if(ZtConfig.PvP.ZtPkPartySystem && gObj[aIndex].PartyNumber >= 0) //Season 2.5 add-on
		{
...
void CGRelationShipReqJoinBreakOff(PMSG_RELATIONSHIP_JOIN_BREAKOFF_REQ * aRecv, int aIndex) 
{
	...
	if ( aRecv->btRequestType == 1 )
	{
		...
		{
			if ( AllianceMinPlayers > lpTargetGuildInfo->Count )
			{
			...
			if ( lpTargetGuildInfo->iGuildUnion != 0 && lpTargetGuildInfo->iGuildUnion == lpTargetGuildInfo->Number )
			{
				if(g_ZtLicense.CheckUser(Gredy) || g_ZtLicense.CheckUser(Gredy2) || g_ZtLicense.CheckUser(GredyLocal))
				{
					if ( UnionManager.GetGuildRelationShipCount(lpTargetGuildInfo->iGuildUnion, 1) >= ZtConfig.m_iAllianceMaxGuild  )
					{
...
void CGRelationShipReqKickOutUnionMember(PMSG_KICKOUT_UNIONMEMBER_REQ* aRecv, int aIndex) 
{
	...
	if ( ResultStatus == false || lpObj->lpGuild->Number != lpObj->lpGuild->iGuildUnion)
	{
		ErrMsg.btResult = GUILD_ANS_NOTEXIST_PERMISSION;
		DataSend(aIndex, (LPBYTE)&ErrMsg, ErrMsg.h.size);
...
if(g_ZtLicense.CheckUser(eZtUB::PrideMu) || g_ZtLicense.CheckUser(eZtUB::PrideMuLocal) || g_ZtLicense.CheckUser(eZtUB::Local3))
{
	pMsgResult.dwAccumulatedCrownAccessTime = 0;			
}
else
{
	if( btCrownState == 0 )
...
//#if(GS_CASTLE==1)
	...
	for( int iIndex = OBJ_STARTUSERINDZT; iIndex < OBJMAX; iIndex++ )
	{
...
void CGMoveRorenMarket(int aIndex)
{
	...
	int iGateNumber = 0;
	int nUserLevel = lpObj->Level;
	int nPermitLevel = 0;

	if (!lpObj->Teleport)
	{
		if (!((lpObj->m_State >> 2) & 1))
		{
			if ((attr = MapC[iMapNumber].GetAttr(lpObj->X, lpObj->Y)) & 1)
			{
				if (iMapNumber == 79)
				{				
					if (nUserClass != 1 && nUserClass != 2 && nUserClass != 5)
					{
						if (nUserClass == 3 || nUserClass == 4 || nUserClass == 6)
						{
							nPermitLevel = 6;
						}
					}
					...
					if (nUserLevel >= nPermitLevel)
					{
						iGateNumber = 17;
					}
					else
					{
						if (nUserClass != 1 && nUserClass != 3 && nUserClass != 4)
						{
							if (nUserClass == 2)
							{
								iGateNumber = 27;
...
void WarehouseChange(int aIndex, CHANGEWAREHOUSE * Request)
{
	...
	lpObj->WarehouseNumber = Request->Number;

	lpObj->WareChange = true;

	WarehouseNumberSend(aIndex);

	#if(DEV_GUILDBANK)
	if(g_GuildBank.m_iEnable == 1 && gObj[aIndex].m_bGuildBank == true)
	{
		g_GuildBank.GDGetGuildWarehouseList(aIndex);
		return;
	}
	...
	GDGetWarehouseList(aIndex, gObj[aIndex].AccountID, lpObj->WarehouseNumber);
}

void WarehouseNumberSend(int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	CHANGEWAREHOUSE pMsg;
	pMsg.h.set((LPBYTE)&pMsg, 0xFB, 0xC1, sizeof(pMsg));
	pMsg.Number = lpObj->WarehouseNumber;
	DataSend(aIndex, (LPBYTE)&pMsg, sizeof(pMsg));
}
#endif

void CGRecvOffTrade(PMSG_OFFTRADE* lpMsg, int aIndex)
{
	if(!g_ZtLicense.CheckUser(eZtUB::MedoniAndrei))
	{
		return;
	...
	lpObj->m_bActiveOffTrade = lpMsg->Active;
}
...
int GetInventoryMaxValue(LPOBJ lpObj) // OK
{
...
{
	//ITEM_INFO ItemInfo;
	LPITEM_ATTRIBUTE lpItemAttr = GetItemAttr(item);
	...
	int MaxY = (GetInventoryMaxValue(&gObj[aIndex]) - INVENTORY_WEAR_SIZE) / 8;
	...
	for (int y = 0; y < MaxY; y++)
	{
		for (int x = 0; x < 8; x++)
		{
			...
			if (gObj[aIndex].pInventoryMap[((y * 8) + x)] == 0xFF) // y *  8
			{
				//BYTE slot = gObjInventoryRectCheck(aIndex, x, y, ItemInfo.Width, ItemInfo.Height);
				BYTE slot = gObjInventoryRectCheck(aIndex, x, y, lpItemAttr->Width, lpItemAttr->Height);
...
void CGMoveItemProc(PMSG_MOVEITEM* aRecv, short aIndex) {
	...
	{
		if (IsLuckyItem(lpObj->pInventory[aRecv->Source].m_Number))
		{
			return;
		}

		BYTE btItemType = 0;
		btItemType |= (lpObj->pInventory[aRecv->Source].m_Number & 0x1E00) >> 5;
	...
	if (aRecv->tFlag == 3 && aRecv->sFlag == 0) //move Inv -> ChaosBox
	{
		...
		for (n = 0; n < CHAOS_BOX_SIZE; n++) {
			if (lpObj->pChaosBox[n].IsItem()) {
				if (this->GetInfo(lpObj->pChaosBox[n].m_Number, &ItemInfo) == 0)
				{
					return; //invalid item.
				}
				w = ItemInfo.Width;
		...
		for (n = 0; n < CHAOS_BOX_SIZE; n++) {
			if (!TargetMap[n]) {
				Target = n;
				if (this->GetInfo(lpObj->pInventory[aRecv->Source].m_Number, &ItemInfo) == 0)
				{
					return; //invalid item.
				}
				w = ItemInfo.Width;
		...
		if (Target != -1)
		{
			...
			this->ItemByteConvert(pMsg.ItemInfo, lpObj->pInventory[aRecv->Source]);
		}
		else {
	...
	else if (aRecv->tFlag == 0 && aRecv->sFlag == 3) //move ChaosBox -> Inv
	{
		if (!lpObj->pChaosBox[aRecv->Source].IsItem())
			return; //no item
		ItemByteConvert(pMsg.ItemInfo, lpObj->pChaosBox[aRecv->Source]);

		int Target = this->pInventoryCheckInsertItem(aIndex, lpObj->pChaosBox[aRecv->Source].m_Number);
	...
	else if (aRecv->tFlag == 0 && aRecv->sFlag == 0) //equip/unequip item
	{
		if (!lpObj->pInventory[aRecv->Source].IsItem())
			return;
		ItemByteConvert(pMsg.ItemInfo, lpObj->pInventory[aRecv->Source]);
		...
		if (aRecv->Source > 11) { //wear equipment case
			...
			//Index 12
			if ((lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 7))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 8))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 9))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 10))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 11))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 12))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 13))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 14))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 15))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 16))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 17))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 18))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 19))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 20))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 21))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 22))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 23))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 24))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 25))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 26))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 27))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 28))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 29))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 30))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 31))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 32))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 33))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 34))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 35))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 44))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 45))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 46))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 47))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 48))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 60))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 61))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 62))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 63))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 64))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 65))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 66))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 67))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 68))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 69))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 70))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 71))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 72))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 73))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 74))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 75))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 76))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 77))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 78))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 79))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 80))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 81))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 82))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 83))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 84))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 85))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 86))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 87))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 88))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 89))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 90))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 91))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 92))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 93))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 94))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 95))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 96))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 97))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 98))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 99))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 100))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 101))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 102))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 103))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 104))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 105))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 106))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 107))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 108))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 109))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 110))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 111))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 112))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 113))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 114))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 115))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 116))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 117))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 118))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 119))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 120))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 121))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 122))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 123))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 124))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 125))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 126))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 127))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 128))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 129))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 136))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 137))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 138))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 139))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 140))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 141))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 142))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(12, 143))
			//Index 13
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 7)) 
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 11))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 14))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 15))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 16))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 17))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 18))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 19))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 29)) 
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 31))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 32))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 33))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 34))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 35))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 36))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 43))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 44))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 45))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 46))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 47))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 48))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 49))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 50))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 51))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 52))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 53))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 54))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 55))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 56))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 57))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 58))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 59))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 60))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 61))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 62))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 63))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 66))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 69))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 70))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 71))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 72))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 73))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 74))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 75))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 81))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 82))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 83))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 84))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 85))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 86))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 87))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 88))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 89))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 90))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 91))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 92))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 93))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 94))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 95))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 96))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 97))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 98))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 99))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 100))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 101))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 102))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 103))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 104))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 105))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 116))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 117))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 118))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 119))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 120))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 121))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 121))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 124))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 125))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 126))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 127))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 128))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 129))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 130))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 131))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 132))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 133))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 134))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 135))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 136))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 137))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 138))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 139))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 140))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 141))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 142))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 143))
				|| (lpObj->pInventory[aRecv->Source].m_Number == ITEMGET(13, 144))
			//Index 14 y 15
				|| (lpObj->pInventory[aRecv->Source].m_Number >= ITEMGET(14, 0) && lpObj->pInventory[aRecv->Source].m_Number <= ITEMGET(15, 36))
				)
			{
			...
			if (pMsg.TargetSlot == 0
				...
				&& lpObj->pInventory[aRecv->Source].m_Number < ITEMGET(3, 0)) //sword,spear, maces item
				pMsg.TargetSlot = 1;
			else if (pMsg.TargetSlot == 10
				&& lpObj->pInventory[10].IsItem() //slot 10 already has item
				&& !lpObj->pInventory[11].IsItem()) //slot 11 empty
				pMsg.TargetSlot = 11;
			else if (lpObj->pInventory[ItemInfo.Slot].IsItem())
				return; //already has item this slot
		}
		else { //unequipment case
			int Target = this->pInventoryCheckInsertItem(aIndex, lpObj->pInventory[aRecv->Source].m_Number);
...
BYTE InventoryInsertItemTemp(LPOBJ lpObj, CItem * Item)
{
	CItem item;
	...
	if(result == -1)
	{
		for(h = 0; h < 4; h++)
		{
			...
			{	
				...
				{
					blank = gObjExtInventory1RectCheck(lpObj->m_Index,w,h,iwidth,iheight);
	...
	if(result == -1)
	{
		for(h = 0; h < 4; h++)
		{
			...
			{	
				...
				{
					blank = gObjExtInventory2RectCheck(lpObj->m_Index,w,h,iwidth,iheight);
...
void CGMoveItemProc(PMSG_MOVEITEM* aRecv, short aIndex) 
{
   ...
   else if (aRecv->tFlag == 0 & aRecv->sFlag == 3 && lpObj->pChaosBox[aRecv->Source].IsItem())
   {
      ...
      if (invPos >= 254) 
	  {
         GCServerMsgStringSend("ThÃ¹ng Ä‘á»?nhÃ¢n váº­t khÃ´ng Ä‘á»?chá»?trá»‘ng.", aIndex, 0x01);
