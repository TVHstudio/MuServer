...
void SaveExUser(LPOBJ lpObj)
{
	...
	for(int i=0; i<EXUSERMAX; i++)
	{
		...
		if(!strcmp(ExUser[i].Name,lpObj->Name))
		{
			ExIndex = i;
			AddExIndex = false;
			break;
	...
	strcpy(ExUser[ExIndex].Name,lpObj->Name);
	ExUser[ExIndex].Party = lpObj->PartyNumber;
	ExUser[ExIndex].TickParty = GetTickCount();
...
void SearchPartyExUSer(LPOBJ lpObj, int &Party, int &PartyTick)
{
	for(int i=0; i<EXUSERMAX; i++)
	{
		if(!strcmp(ExUser[i].Name,lpObj->Name))
		{
			Party = ExUser[i].Party;
			PartyTick = ExUser[i].TickParty;
			return;
		}
	}
}
...
EX_ACC_DATA ExAccData[EXUSERMAX];
...
//#endif
	...
	for(int a = 0; a < EW_MAX_ACT; a++)
	{
		...
		lpObj->ExWQuestNum[a] = 0;
		for(int m = 0; m < EW_MAX_MISSION; m++)
		{
			lpObj->ExWQuestCount[a][m] = 0;
		}
	}
#endif
...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
	...
	if ( lpObj->Level < 6 || DS_MAP_RANGE(lpObj->MapNumber) != FALSE && lpObj->MapNumber != 79 )
	{
		...
		{
			lpObj->MapNumber = ExConfig.DW_MapStart;
			if(ExConfig.DW_X == -1 || ExConfig.DW_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ExConfig.DW_X;
				lpObj->Y = ExConfig.DW_Y;			
			}
		}
		else if ( lpObj->Class == CLASS_KNIGHT )
		{
			lpObj->MapNumber = ExConfig.DK_MapStart;
			if(ExConfig.DK_X == -1 || ExConfig.DK_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ExConfig.DK_X;
				lpObj->Y = ExConfig.DK_Y;				
			}
		}
		else if ( lpObj->Class == CLASS_ELF )
		{
			lpObj->MapNumber = ExConfig.EE_MapStart;
			if(ExConfig.EE_X == -1 || ExConfig.EE_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ExConfig.EE_X;
				lpObj->Y = ExConfig.EE_Y;				
			}
		}
		else if ( lpObj->Class == CLASS_SUMMONER )
		{
			lpObj->MapNumber = ExConfig.SU_MapStart;
			if(ExConfig.SU_X == -1 || ExConfig.SU_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{	
				lpObj->X = ExConfig.SU_X;
				lpObj->Y = ExConfig.SU_Y;
			}
		}
		else if ( lpObj->Class == CLASS_MAGUMSA )
		{
			lpObj->MapNumber = ExConfig.MG_MapStart;
			if(ExConfig.MG_X == -1 || ExConfig.MG_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ExConfig.MG_X;
				lpObj->Y = ExConfig.MG_Y;				
			}
		}
		else if ( lpObj->Class == CLASS_DARKLORD )
		{
			lpObj->MapNumber = ExConfig.DL_MapStart;
			if(ExConfig.DL_X == -1 || ExConfig.DL_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ExConfig.DL_X;
				lpObj->Y = ExConfig.DL_Y;				
			}
		}
		else if( lpObj->Class == CLASS_MONK )
		{
			lpObj->MapNumber = ExConfig.RF_MapStart;
			if(ExConfig.RF_X == -1 || ExConfig.RF_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ExConfig.RF_X;
				lpObj->Y = ExConfig.RF_Y;				
			}
		}
		else
		{
		...
		//if(g_ExLicense.CheckUser(MU2Play))
		//{
	...
	if (lpObj->MapNumber == MAP_INDEX_GM_SUMMONZONE) //Season 2.5 add-on
	{
		...
		{
			//gObjTeleport(lpObj->m_Index,ExConfig.DryneaConfig.GMKillMap,ExConfig.DryneaConfig.GMKillMapX,ExConfig.DryneaConfig.GMKillMapY);
		/*	lpObj->X = ExConfig.DryneaConfig.GMKillMapX;
			lpObj->Y = ExConfig.DryneaConfig.GMKillMapY;*/
			//MapC[lpObj->MapNumber].GetMapPos(ExConfig.DryneaConfig.GMKillMap, ExConfig.DryneaConfig.GMKillMapX, ExConfig.DryneaConfig.GMKillMapY);
		}
		else if(g_ExLicense.CheckUser(eExUB::MedoniAndrei) || g_ExLicense.CheckUser(eExUB::Local) || g_ExLicense.CheckUser(eExUB::Artem) || g_ExLicense.CheckUser(eExUB::Artem2))
		{
			...
			//lpObj->X = ExConfig.DryneaConfig.GMKillMapX + randomX;
			//lpObj->Y = ExConfig.DryneaConfig.GMKillMapY + randomY;
...
//#endif
	...
	lpObj->ExFreePoints = lpMsg->ExFreePoints;
	...
	{
		if (lpObj->Class == CLASS_WIZARD)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ExConfig.PrideMu.LevelLifeSM  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ExConfig.PrideMu.VitalityToLifeSM);
		if (lpObj->Class == CLASS_KNIGHT)//
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ExConfig.PrideMu.LevelLifeDK  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ExConfig.PrideMu.VitalityToLifeDK);
		if (lpObj->Class == CLASS_ELF)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ExConfig.PrideMu.LevelLifeELF  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ExConfig.PrideMu.VitalityToLifeELF);
		if (lpObj->Class == CLASS_MAGUMSA)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ExConfig.PrideMu.LevelLifeMG  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ExConfig.PrideMu.VitalityToLifeMG);
		if (lpObj->Class == CLASS_DARKLORD)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ExConfig.PrideMu.LevelLifeDL  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ExConfig.PrideMu.VitalityToLifeDL);
		if (lpObj->Class == CLASS_SUMMONER)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ExConfig.PrideMu.LevelLifeSUM  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ExConfig.PrideMu.VitalityToLifeSUM);
		if (lpObj->Class == CLASS_MONK)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ExConfig.PrideMu.LevelLifeRF + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ExConfig.PrideMu.VitalityToLifeRF);
...
{
	//int Ex_PersonalID = GetPrivateProfileInt("Common","PersonalID",0,"..\\Data\\ExData\\CommonServer.ini");
...
#ifdef PRIVATE_MODULE
	//if(g_ExLicense.CheckUser(Encode) || g_ExLicense.CheckUser(MU2Play) || g_ExLicense.CheckUser(Local))
	//{
	//	// ----
	//}
	//else
...
bool gObjLevelUp(LPOBJ lpObj, __int64 & addexp, int iMonsterType, int iEventType) //0051F140
{
	...
	if(lpObj->Level >= ExConfig.ExCustom.MaxLevel && lpObj->Experience >= gLevelExperience[-1+lpObj->Level])
	{
	...
	lpObj->Level++;
		...
		if ( lpObj->Class == CLASS_DARKLORD )
		{
			lpObj->LevelUpPoint += ExConfig.AddLvlPoint.DlLevelPoints;
		}
		else if(lpObj->Class == CLASS_KNIGHT)
		{
			lpObj->LevelUpPoint += ExConfig.AddLvlPoint.DkLevelPoints;
		}
		else if(lpObj->Class == CLASS_WIZARD)
		{
			lpObj->LevelUpPoint += ExConfig.AddLvlPoint.DwLevelPoints;
		}
		else if(lpObj->Class == CLASS_ELF)
		{
			lpObj->LevelUpPoint += ExConfig.AddLvlPoint.ElfLevelPoints;
		}
		else if(lpObj->Class == CLASS_SUMMONER)
		{
			lpObj->LevelUpPoint += ExConfig.AddLvlPoint.SumLevelPoints;
		}
		else if ( lpObj->Class == CLASS_MAGUMSA )
		{
			lpObj->LevelUpPoint += ExConfig.AddLvlPoint.MgLevelPoints;
		}
#ifdef MONK
		else if ( lpObj->Class == CLASS_MONK )
		{
			lpObj->LevelUpPoint += ExConfig.AddLvlPoint.RfLevelPoints;
		}
#endif
	...
	if ( lpObj->PlusStatQuestClear != false )
	{
		...
		if ( lpObj->Class == CLASS_DARKLORD )
		{
			lpObj->LevelUpPoint += ExConfig.AddMarlonPoint.DlLevelPoints;
		}
		else if(lpObj->Class == CLASS_KNIGHT)
		{
			lpObj->LevelUpPoint += ExConfig.AddMarlonPoint.DkLevelPoints;
		}
		else if(lpObj->Class == CLASS_WIZARD)
		{
			lpObj->LevelUpPoint += ExConfig.AddMarlonPoint.DwLevelPoints;
		}
		else if(lpObj->Class == CLASS_ELF)
		{
			lpObj->LevelUpPoint += ExConfig.AddMarlonPoint.ElfLevelPoints;
		}
		else if(lpObj->Class == CLASS_SUMMONER)
		{
			lpObj->LevelUpPoint += ExConfig.AddMarlonPoint.SumLevelPoints;
		}
		else if ( lpObj->Class == CLASS_MAGUMSA )
		{
			lpObj->LevelUpPoint += ExConfig.AddMarlonPoint.MgLevelPoints;
		}
#ifdef MONK
		else if ( lpObj->Class == CLASS_MONK )
		{
			lpObj->LevelUpPoint += ExConfig.AddMarlonPoint.RfLevelPoints;
		}
#endif
#else
...
BOOL gObjLevelUpPointAdd(BYTE type, LPOBJ lpObj)
{
	...
	switch ( type )
	{
		case 0:
			{
				  if ((lpObj->Strength) >= ExConfig.Character.MaxStats) return false;
				lpObj->Strength++;
			}
			break;
		...
		case 1:
			{
				  if ((lpObj->Dexterity) >= ExConfig.Character.MaxStats) return false;
				lpObj->Dexterity++;
			}
			break;
		...
		case 2:
			{
				  if ((lpObj->Vitality) >= ExConfig.Character.MaxStats) return false;
				lpObj->Vitality++;
		...
		case 3:
			{
				  if ((lpObj->Energy) >= ExConfig.Character.MaxStats) return false;
				lpObj->Energy++;
...
{
	g_ExUser.PlayerKillInfoMail(lpObj->m_Index, lpTargetObj->m_Index);
...
#if(_ARENA_ITEM_)
	if(ExConfig.CommonServer.ArenaItem !=-1 && lpObj->MapNumber == 6)
	{
		if(gObjGetItemCountInInventory(lpObj->m_Index,ExConfig.CommonServer.ArenaItem,0) > 0)  // 2 < 1
		{
			gObjDeleteItemsCount(lpObj->m_Index,ExConfig.CommonServer.ArenaItem,0,1);
		}
		else
		{
	...
	if(g_ExLicense.CheckUser(eExUB::Local) || g_ExLicense.CheckUser(eExUB::Gredy) || g_ExLicense.CheckUser(eExUB::Gredy2) || g_ExLicense.CheckUser(eExUB::GredyLocal))
	{
		if(ExConfig.CommonServer.ArenaItem == -1 && lpObj->MapNumber == 6)
		{
			lpObj->MapNumber = ExConfig.iTeleportMapForDieMap6;
			lpObj->X = 62;
			lpObj->Y = 134;
		}
		else if(lpObj->MapNumber == 40)
		{
			lpObj->MapNumber = ExConfig.iTeleportMapForDieMap40;
			lpObj->X = 148;
			lpObj->Y = 17;
		}
		else if(lpObj->MapNumber == 51)
		{
			lpObj->MapNumber = ExConfig.iTeleportMapForDieMap51;
			lpObj->X = 215;
			lpObj->Y = 31;
		}		
	}
...
void gObjSpriteDamage(LPOBJ lpObj, int damage)
{
	...
	if(g_PetEx.IsPet(sprite->m_Type))
	{
		return;
	}

	if(g_PetEx.IsPetImmortal(sprite->m_Type))
	{
	...
	else if(sprite->m_Type == ITEMGET(13,37))
	{
		...
		if(g_ExLicense.CheckUser(eExUB::Gredy) || g_ExLicense.CheckUser(eExUB::GredyLocal) || g_ExLicense.CheckUser(eExUB::Gredy2))
		{
			...
			{
				iMaxDurSmall = ( iMaxDurSmall * ExConfig.Pet.BlackFenrirProcDurability ) / 100; 
			}
			else if ( sprite->IsFenrirDecLastAttackDamage() != FALSE )	// Blue Fenrir
			{
				iMaxDurSmall = ( iMaxDurSmall * ExConfig.Pet.BlueFenrirProcDurability ) / 100; 	 
			}
			else if ( sprite->IsFenrirSpecial() != FALSE )				// Illusion Fenrir
			{
				iMaxDurSmall = ( iMaxDurSmall * ExConfig.Pet.GoldFenrirProcDurability ) / 100; 	 
			}
			else														//red 
			{
				iMaxDurSmall = ( iMaxDurSmall * ExConfig.Pet.RedFenrirProcDurability ) / 100; 	 
			}
		}
		...
		if(sprite->m_DurabilitySmall > iMaxDurSmall)
		{
			if(g_ExLicense.CheckUser(eExUB::PrideMuLocal) || g_ExLicense.CheckUser(eExUB::PrideMu) || g_ExLicense.CheckUser(eExUB::Local3) || g_ExLicense.CheckUser(eExUB::Local))
			{
				...
				{
					if(ExConfig.PrideMu.OffDecreaseDur)
					{
					}
					else
					{
			...
			else if(g_ExLicense.CheckUser(eExUB::SILVER1) || g_ExLicense.CheckUser(eExUB::SILVER2))	//Fenrir Durability Off
			{
...
void gObjExpParty(LPOBJ lpObj , LPOBJ lpTargetObj, int AttackDamage, int MSBFlag )
{
	...
	if(viewplayer > 1)
	{
		...
		if(bApplaySetParty != 0)
		{
			...
			{
				viewpercent = ExParty7ExpPercent;
			}
			else if(viewplayer == 8)
			{
				viewpercent = ExParty8ExpPercent;
			}
			else if(viewplayer == 9)
			{
				viewpercent = ExParty9ExpPercent;
			}
		...
		else
		{
			...
			{
				viewpercent = ExParty7ExpSetPercent;
			}
			else if(viewplayer == 8)
			{
				viewpercent = ExParty8ExpSetPercent;
			}
			else if(viewplayer == 9)
			{
				viewpercent = ExParty9ExpSetPercent;
			}
			else if(viewplayer >= 10)
			{
			...
			if(lpTargetObj->MapNumber == lpPartyObj->MapNumber && dis[n] < 10)
			{
				...
				if(exp > 0)
				{
					if(lpPartyObj->Type == OBJ_USER)
					{
						...
						{
							exp += (( exp * ExConfig.m_iExpPartyPlus ) / 100 );
						}
...
BYTE gObjWerehouseRectCheck(int aIndex, int sx, int sy, int width, int height)
{
	...
	if(gObj[aIndex].ExpandedWarehouse == 0)
	{
		if(result == -1)
		{
			return -1;
...
void gObjTradeOkButton(int aIndex)
{
    ...
    if(THINNAKORN_MAC || g_ExLicense.CheckUser(eExUB::Gredy) || g_ExLicense.CheckUser(eExUB::GredyLocal) || g_ExLicense.CheckUser(eExUB::Gredy2))
	{
...
#define SLOTNEGATIVE -1
	...
	switch (lpObj->pInventory[7].m_Type)
	{
		...
		default:
			{
				#if(ENABLE_FIX_MINIWINGS)
				if(lpObj->pInventory[7].m_Type >= 130 && lpObj->pInventory[7].m_Type <= 135)
				{
					lpObj->CharSet[5] |= 12;
					lpObj->CharSet[17] |= (lpObj->pInventory[7].m_Type-129) << 5;
				}
				#endif
#ifdef NEWWINGS_2_5
	...
	default:
		#if(ENABLE_PETEX)
		if(g_PetEx.IsPet(lpObj->pInventory[8].m_Type))
		{
...
#if(NEW_WINDOW==1)
	int iOldBkMode = SetBkMode(hdc, TRANSPARENT);
	SetTextColor(hdc, RGB(245, 237, 88));
...
{
#if(CUSTOM_CAMERA_MOVE)
	if(g_ExLicense.user.CustomCameraMove)
	{
...
#ifdef DRYNEA_13_11
						else if((lpObj->MapNumber == MAP_INDEX_BATTLESOCCER) &&  g_ExLicense.CheckUser(eExUB::drynea))
						{
							lpObj->MapNumber = MAP_INDEX_RORENCIA;
							MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber,(short &)lpObj->X,(short &)lpObj->Y);
						}
...
//#endif
						...
						else if(lpObj->MapNumber == MAP_INDEX_SANTATOWN)
						{
							...
							else if(g_ExLicense.CheckUser(eExUB::ulasevich) || g_ExLicense.CheckUser(eExUB::ulasevich))
							{
								int mgt = 17;
								short x, y, level;
								BYTE map = lpObj->MapNumber, dir;
						...
						else if (lpObj->MapNumber == MAP_INDEX_GM_SUMMONZONE)
						{
							if(g_ExLicense.CheckUser(eExUB::drynea))
							{
								lpObj->MapNumber = ExConfig.DryneaConfig.GMKillMap;
							...
							else if(g_ExLicense.CheckUser(eExUB::MedoniAndrei) || g_ExLicense.CheckUser(eExUB::Local))
							{
								//int iGMKillMapX = 215;
								//int iGMKillMapY = 46;
...
void gObjSecondProc()
{
	...
	for(n = 0; n < OBJMAX;n++)
	{
		...
		if(lpObj->Connected > PLAYER_LOGGED)
		{
			...
			if(ExConfig.CommonServer.MarlonTeleport)
			{
...
//#endif
			...
			{
				if(lpObj->Reset < ExConfig.CommonServer.iNoobBuffMaxReset && lpObj->GReset == 0)
				{
...
#endif
#if(CUSTOM_NEWBUFF)
			if(ExConfig.m_bNewBuffPremiumEnable)
			{
				if(lpObj->PremiumTime > 0)
				{
			...
			if(ExConfig.m_bNewBuffGuildEnable)
			{
				...
				if(lpObj->GuildNumber > 0 && lpGuildUser)
				{
					...
					{
						if(g_ExUser.GetGuildMemberOnline(lpObj->GuildNumber) > ExConfig.m_bNewBuffGuildMembOnline)
						{
					...
					{
						if(lpObj->lpGuild->Count > ExConfig.m_bNewBuffGuildMembOnline)
						{
							if(gObjSearchActiveEffect(lpObj,AT_NEWGUILD_BUFF) == FALSE)
							{
								gObjApplyBuffEffectDuration(lpObj,AT_NEWGUILD_BUFF,0,0,0,0,600);
			...
			if(ExConfig.m_bNewBuffPartyEnable)
			{
				if(lpObj->PartyNumber >= 0)
				{
					if(gParty.GetPartyCount(lpObj->PartyNumber) >= ExConfig.m_bNewBuffPartyMembCount)
					{
			...
			if(g_ExLicense.CheckUser(Local) || g_ExLicense.CheckUser(SILVER1) || g_ExLicense.CheckUser(SILVER2))	//Need Delete Local
			{
...
void gObjViewportListProtocolCreate(LPOBJ lpObj)
{
	...
	if(lpObj->Type == OBJ_USER)
	{
		if(lpObj->m_Change >= 0)
		{
			...
			pViewportCreateChange.DirAndPkLevel = lpObj->Dir << 4;
//#ifdef LORA_BATTLE_EVENT
//			if((g_ExLicense.CheckUser(eExUB::Gredy) || g_ExLicense.CheckUser(eExUB::GredyLocal)) && (LoraBattle.CheckStartEvent() == true))
//			{
...
BOOL gObjItemRandomLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	//int Ex_SoulRate = GetPrivateProfileInt("JewelRates","SoulRate",50,"..\\Data\\ExData\\CommonServer.ini");
	//int Ex_SoulRateLuck = GetPrivateProfileInt("JewelRates","SoulRateLuck",20,"..\\Data\\ExData\\CommonServer.ini");
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	}
	...
	else
	{
		...
		{
			if(_r < Ex_SoulRate)	//40
			{
				bSuccess = true;
			}
		}
		else if(_r < Ex_SoulRate)	//60
		{
			bSuccess = true;
		}
	}
...
BOOL gObjItemRandomOption3Up(LPOBJ lpObj, int source, int target)
{
	...
	{
		//int Ex_LifeRate = GetPrivateProfileInt("JewelRates","LifeRate",50,"..\\Data\\ExData\\CommonServer.ini");
...
void gObjSendUserStatistic(int aIndex, int startLevel, int endLevel)
{
	...
	{
		endLevel = ExConfig.ExCustom.MaxLevel;
	}
...
void gObjUserSetExp(LPOBJ lpObj, __int64 & Experience)	//Panda User
{
	...
	{
		Experience = Experience + ( Experience * ExConfig.Pet.SkeletonPetAndRingExp / 100 );
	}
	else if(gObjSkeletonSprite(lpObj) == TRUE)
	{
		Experience = Experience + ( Experience * ExConfig.Pet.SkeletonPetExp / 100 );
	}
	...
	if(g_ExLicense.user.WinQuest) gWinQuestSystem.ExpOption(lpObj->m_Index,Experience);
...
#if(CUSTOM_PREMIUM_SYSTEM==TRUE)
	int PlusPremiumExp = g_PremiumSystemEx.UserExpPlus(lpObj->m_Index);

	if(PlusPremiumExp > 0)
	{
		Experience += (( Experience * PlusPremiumExp ) / 100 );
	}
#endif
	...
	if(lpObj->PartyNumber >= 0)
	{
		//if(ExConfig.m_iExpPartyPlus > 0)
		//{
		//	Experience += (( Experience * ExConfig.m_iExpPartyPlus ) / 100 );
		//}
	}
}
