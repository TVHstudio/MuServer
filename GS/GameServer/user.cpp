...
ZtMessageStateMachine gSMAttackProcMsg[OBJMAX][MAX_MONSTER_SEND_ATTACK_MSG];

OBJECTSTRUCT*	ObjectStruct	=	NULL; // pointer changed to 0
OBJECTSTRUCT*	gObj;
/*********************************************/
Zt__User Zt_User[ZTUSERMAX];
int ZtIndex = 0;
void SaveZtUser(LPOBJ lpObj)
{
	bool AddExIndex = true;
	for(int i=0; i<ZTUSERMAX; i++)
	{

		if(!strcmp(Zt_User[i].Name,lpObj->Name))
		{
			ZtIndex = i;
			AddExIndex = false;
			break;
...
#ifdef DEBUG_CODE
		LogAddC(2,"Zt_User[ZtIndex].Name: %s | lpObj->PartyNumber: %d",Zt_User[ZtIndex].Name, lpObj->PartyNumber);
#endif

	strcpy(Zt_User[ZtIndex].Name,lpObj->Name);
	Zt_User[ZtIndex].Party = lpObj->PartyNumber;
	Zt_User[ZtIndex].TickParty = GetTickCount();
	...
	if(ZtIndex >= ZTUSERMAX-1)
	{
		ZtIndex = 0;
	}
}
void SearchPartyZtUSer(LPOBJ lpObj, int &Party, int &PartyTick)
{
	for(int i=0; i<ZTUSERMAX; i++)
	{
		if(!strcmp(Zt_User[i].Name,lpObj->Name))
		{
			Party = Zt_User[i].Party;
			PartyTick = Zt_User[i].TickParty;
			return;
		}
	}
}
...
EX_ACC_DATA ExAccData[ZTUSERMAX];

void AccountDataReset()
{
	SYSTEMTIME tm;

	if(tm.wHour == 00 && tm.wMinute == 00 && tm.wSecond == 00)
	{
		iAccIndex = 0;
		memset(ExAccData, 0, sizeof(ExAccData));
	}
}
void AccountDataAdd(LPOBJ lpObj)
{
	for(int i=0; i<ZTUSERMAX; i++)
	{
		if(!strcmp(ExAccData[i].Account,lpObj->AccountID))
		{
			return;
		}
	}

	strcpy(ExAccData[iAccIndex].Account,lpObj->AccountID);

	ExAccData[iAccIndex].BloodCastleCount = 0;

	ExAccData[iAccIndex].DevilSqueryCount = 0;

	iAccIndex++;

	if(iAccIndex >= ZTUSERMAX-1)
	{
		iAccIndex = 0;
	}
}

int AccountDataGet(LPOBJ lpObj)
{
	for(int i=0; i<ZTUSERMAX; i++)
	{
		if(!strcmp(ExAccData[i].Account,lpObj->AccountID))
...
void gObjSkillUseProcTime500(LPOBJ lpObj) //
{
	if ( lpObj->SkillHellFire2State != 0 )
	{
		...
		else
		{
			...
			mana -= mana * lpObj->m_MPSkillOpt.MpsDecreaseMana / 100;
...
void MonsterAndMsgProc()
{
	...
	{
		lpObj = &gObj[n+OBJ_STARTUSERINDZT];
...
void gObjCloseSet(int aIndex, int Flag)
{
	...
	{
		lpObj->CloseCount = gSwitchCharacterSecondCount;
	}
	else if(lpObj->CloseType == 2)
	{
		lpObj->CloseCount = gSelectServerSecondCount;
	}
#endif
...
void gObjCharZeroSet(int aIndex)
{
	...
	if ( (aIndex-OBJ_STARTUSERINDZT) >= 0 )
	{
		for ( i=0;i<MAX_MONSTER_SEND_ATTACK_MSG;i++)
		{
			gSMAttackProcMsg[aIndex-OBJ_STARTUSERINDZT][i].MsgCode = -1;
		}
	}
...
//#endif
	...
	lpObj->MixManagerSocket = 0;
#ifdef CFIX_INV
	lpObj->ExpandedInventory = 0;
#endif // CFIX_INV

#if(SYSTEMOF_BROTHER==TRUE)
	lpObj->ActiveBrother = false;
	memset(lpObj->Brotherhood, 0, sizeof(lpObj->Brotherhood));
	lpObj->m_BroTeleportTime = 0;
#endif

	if(g_ZtLicense.user.TvTEvent)
	{
		TvT.RestoreUser(aIndex);
		TvT.RestoreTEvent(aIndex);
	}

#if(CUSTOM_PARTY_SEARCH==TRUE)
	lpObj->pt_Active = false;
	lpObj->pt_MaxLevel = 400;
	lpObj->pt_bAllClass = false;
	lpObj->pt_bOnlyOneClass = false;
	lpObj->pt_bDarkWizard = false;
	lpObj->pt_bDarkKnight = false;
	lpObj->pt_bElf = false;
	lpObj->pt_bMagicGladiator = false;
	lpObj->pt_bDarkLord = false;
	lpObj->pt_bSummoner = false;
#endif
	...
	lpObj->StatusTime = 0;
	lpObj->StatusCode = 0;
	lpObj->StatusNeedHelper = 0;
#endif

#if(CUSTOM_NPC_BUFFER==TRUE)
	lpObj->ExpBuffTime = 0;
	lpObj->DamageBuffTime = 0;
	lpObj->DefenceBuffTime = 0;
	lpObj->BattleBuffTime = 0;
	lpObj->DropBuffTime = 0;
	lpObj->DoubleBuffTime = 0;
#endif

	lpObj->BMQuest_num = 0;
	lpObj->BMQuest_kill = 0;
	lpObj->BMQuest_start = 0;
	lpObj->BMDamageReflect = 0;
	lpObj->BMDamage = 0;
	lpObj->BMDefense = 0;
	lpObj->BMCriticalDamage = 0;
	lpObj->BMExcelentDamage = 0;
	lpObj->BMLife = 0;
	...
	lpObj->MuHelperTeleportFix = false;
#endif
#if(CUSTOM_SAVE_PT==TRUE)
	lpObj->PartyIndex = 0;
#endif
#if(OFFLINE_MODE == TRUE)
	lpObj->m_OfflineMode = false;
	lpObj->m_OfflineSkill = 0;
	lpObj->m_OfflineBuffTime = 0;
	lpObj->m_OfflineAutoBuff = 0;
	lpObj->m_OfflineAttackTime = 0;
#if(ADD_OFFMODE_PICKUP==TRUE)
	lpObj->m_OfflinePickZen = false;
	lpObj->m_OfflinePickJewel = false;
	lpObj->m_OfflinePickExel = false;
	lpObj->m_OfflinePickAnc = false;
	#if(ADD_OFFMODE_TIMER)
	lpObj->m_OfflineTimerUser = 0;
	lpObj->m_OfflineTimerMax = 24;
	#endif
#endif
#endif
#if(CUSTOM_MARRIAGE)
	lpObj->bMarriage = false;
	memset(lpObj->MarriageName, 0, MAX_ACCOUNT_LEN);
#endif
#if(FIX_REF2)
	//lpObj->FixRef2 = false;
#endif
	...
	gSystemOfRage.CharacterZero(aIndex);

	lpObj->bAutoPt = true;
	lpObj->WaitWcoinConnect = false;

#if(EVENT_DUNGEON_SIEGE)
	lpObj->DSiege.bNpcClick = false;
#endif
	...
	lpObj->m_bCheckBuyDonateItem = false;
	...
	lpObj->m_iLoadConfigTickCount = GetTickCount();	//0;
	...
	{
		lpObj->ZtWQuestStart[a] = 0;
		lpObj->ZtWQuestNum[a] = 0;
		for(int m = 0; m < EW_MAX_MISSION; m++)
		{
			lpObj->ZtWQuestCount[a][m] = 0;
		}
	}
#endif
	lpObj->m_bActiveOffTrade = false;
	lpObj->m_iDonateCredit = 0;
...
void DbItemSetInByte(LPOBJ lpObj, struct SDHP_DBCHAR_INFORESULT* lpMsg, int ItemDbByte, bool* bAllItemExist)
{
	...
	for(n = 0; n < INVENTORY_SIZE;n++)
	{
		...
		if(itype != -1)
		{
			...
			{
				if(item.m_Level > 15)	//Season 5 +15 Items
				{
					item.m_Level = 0;
				}
			}
			...
			{
				item.Convert(itype,item.m_Option1,item.m_Option2,item.m_Option3,DBI_GET_NOPTION(lpMsg->dbInventory[n*ItemDbByte+7]),lpMsg->dbInventory[n*ItemDbByte+8],item.m_ItemOptionEx,SocketOption, SocketOptionIndex, 0, 1); //season4 changed
			}
			else
			{
				item.Convert(itype,item.m_Option1,item.m_Option2,item.m_Option3,DBI_GET_NOPTION(lpMsg->dbInventory[n*ItemDbByte+7]),lpMsg->dbInventory[n*ItemDbByte+8],item.m_ItemOptionEx,SocketOption, SocketOptionIndex, 0, 3); //season4 changed
			}
#endif
...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
	...
	if ( lpObj->Level < 6 || DS_MAP_RANGE(lpObj->MapNumber) != FALSE && lpObj->MapNumber != 79 )
	{
#ifdef PRIVATE_MODULE

		if ( lpObj->Class == CLASS_WIZARD )
		{
			lpObj->MapNumber = ZtConfig.DW_MapStart;
			if(ZtConfig.DW_X == -1 || ZtConfig.DW_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ZtConfig.DW_X;
				lpObj->Y = ZtConfig.DW_Y;			
			}
		}
		else if ( lpObj->Class == CLASS_KNIGHT )
		{
			lpObj->MapNumber = ZtConfig.DK_MapStart;
			if(ZtConfig.DK_X == -1 || ZtConfig.DK_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ZtConfig.DK_X;
				lpObj->Y = ZtConfig.DK_Y;				
			}
		}
		else if ( lpObj->Class == CLASS_ELF )
		{
			lpObj->MapNumber = ZtConfig.EE_MapStart;
			if(ZtConfig.EE_X == -1 || ZtConfig.EE_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ZtConfig.EE_X;
				lpObj->Y = ZtConfig.EE_Y;				
			}
		}
		else if ( lpObj->Class == CLASS_SUMMONER )
		{
			lpObj->MapNumber = ZtConfig.SU_MapStart;
			if(ZtConfig.SU_X == -1 || ZtConfig.SU_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{	
				lpObj->X = ZtConfig.SU_X;
				lpObj->Y = ZtConfig.SU_Y;
			}
		}
		else if ( lpObj->Class == CLASS_MAGUMSA )
		{
			lpObj->MapNumber = ZtConfig.MG_MapStart;
			if(ZtConfig.MG_X == -1 || ZtConfig.MG_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ZtConfig.MG_X;
				lpObj->Y = ZtConfig.MG_Y;				
			}
		}
		else if ( lpObj->Class == CLASS_DARKLORD )
		{
			lpObj->MapNumber = ZtConfig.DL_MapStart;
			if(ZtConfig.DL_X == -1 || ZtConfig.DL_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ZtConfig.DL_X;
				lpObj->Y = ZtConfig.DL_Y;				
			}
		}
		else if( lpObj->Class == CLASS_MONK )
		{
			lpObj->MapNumber = ZtConfig.RF_MapStart;
			if(ZtConfig.RF_X == -1 || ZtConfig.RF_Y == -1)
			{
				MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
			}
			else
			{
				lpObj->X = ZtConfig.RF_X;
				lpObj->Y = ZtConfig.RF_Y;				
			}
		}
		else
		{
		...
		//if(g_ZtLicense.CheckUser(MU2Play))
		//{
		//	lpObj->MapNumber = MAP_INDEX_DEVIAS;
		//	lpObj->X = 236;
		//	lpObj->Y = 10;
		//}
		//else if(g_ZtLicense.CheckUser(Encode))
		//{
		//	if ( lpObj->Class == CLASS_ELF )
		//	{
	...
	if ( DG_MAP_RANGE(lpObj->MapNumber) != FALSE )	//Season 5 DoppelGanger
	{
		lpObj->MapNumber = 51;
		MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
	}	

	if ( IMPERIAL_MAP_RANGE(lpObj->MapNumber) != FALSE )	//Season 5 ImperialFort
	{
		lpObj->MapNumber = MAP_INDEX_DEVIAS;
		MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
	}
	...
	if (lpObj->MapNumber == MAP_INDEX_GM_SUMMONZONE) //Season 2.5 add-on
	{
		...
		{
			//gObjTeleport(lpObj->m_Index,ZtConfig.DryneaConfig.GMKillMap,ZtConfig.DryneaConfig.GMKillMapX,ZtConfig.DryneaConfig.GMKillMapY);
		/*	lpObj->X = ZtConfig.DryneaConfig.GMKillMapX;
			lpObj->Y = ZtConfig.DryneaConfig.GMKillMapY;*/
			//MapC[lpObj->MapNumber].GetMapPos(ZtConfig.DryneaConfig.GMKillMap, ZtConfig.DryneaConfig.GMKillMapX, ZtConfig.DryneaConfig.GMKillMapY);
		}
		else if(g_ZtLicense.CheckUser(eZtUB::MedoniAndrei) || g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::Artem) || g_ZtLicense.CheckUser(eZtUB::Artem2))
		{
			//kordinati dlya safe zone 215 46
			int GMKillMapX = 215;
			int GMKillMapY = 46;
			...
			//lpObj->X = ZtConfig.DryneaConfig.GMKillMapX + randomX;
			//lpObj->Y = ZtConfig.DryneaConfig.GMKillMapY + randomY;
...
//#endif
...
#endif
#if(EVENT_TEAM_VS_TEAM==TRUE)
	g_TeamVsTeam.PlayerConnect(aIndex);
#endif
	//lpObj->MapNumber = MAP_INDEX_DEVIAS;
	...
	lpObj->PCPoint = lpMsg->PCPoint;
	lpObj->ZtFreePoints = lpMsg->ZtFreePoints;
	lpObj->Reset = lpMsg->Reset;
	lpObj->GReset = lpMsg->GReset;
	lpObj->ZtQuestNum = lpMsg->ZtQuestNum;
	lpObj->ZtQuestKill = lpMsg->ZtQuestKill;

	//if(g_ZtLicense.CheckUser(SILVER1) || g_ZtLicense.CheckUser(SILVER2))
	//{
	//	int iTimer = time(NULL);
	//	lpObj->PremiumTime = lpMsg->PremiumTime - iTimer;
	...
	//else
	//{
		lpObj->PremiumTime = lpMsg->PremiumTime;
	//}

#ifdef PREM_TYPE
	lpObj->PremiumTimeType = lpMsg->PremiumTimeType;
#endif
	lpObj->ZtCred = lpMsg->ZtCred;
	lpObj->BanChat = lpMsg->BanChat;
	lpObj->BanPost = lpMsg->BanPost;
	lpObj->BanChar = lpMsg->BanChar;
	...
	if(lpObj->ChaosBank < 0)
	{
		lpObj->ChaosBank = 0;
	}
	if(lpObj->BlessBank < 0)
	{
		lpObj->BlessBank = 0;
	}
	if(lpObj->SoulBank < 0)
	...
	lpObj->AutoPt = lpMsg->AutoParty;
	lpObj->Zen = lpMsg->Zen;
	lpObj->QTDate = lpMsg->QTDate;

	char szBroth[MAX_ACCOUNT_LEN+1];
	szBroth[MAX_ACCOUNT_LEN] = 0;
	memcpy(szBroth, lpMsg->Brother , MAX_ACCOUNT_LEN);
	strcpy(lpObj->Brotherhood, szBroth);
#ifdef ZTPINV
	lpObj->ExpandedInventory = lpMsg->ExpandedInventory;
#endif
#if(CUSTOM_SAVE_PT==TRUE)
	lpObj->PartyIndex = lpMsg->PartyIndex;
#endif
	lpObj->m_iDonateCredit = lpMsg->DonateCredit;
	#if(ENABLE_VOTER_ANONS)
	lpObj->ATDate = lpMsg->ATDate;
	#endif
	if ( lpObj->Mana < 1.0f )
	{
		lpObj->Mana = 1.0f;
	...
	{
		if (lpObj->Class == CLASS_WIZARD)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ZtConfig.PrideMu.LevelLifeSM  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ZtConfig.PrideMu.VitalityToLifeSM);
		if (lpObj->Class == CLASS_KNIGHT)//
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ZtConfig.PrideMu.LevelLifeDK  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ZtConfig.PrideMu.VitalityToLifeDK);
		if (lpObj->Class == CLASS_ELF)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ZtConfig.PrideMu.LevelLifeELF  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ZtConfig.PrideMu.VitalityToLifeELF);
		if (lpObj->Class == CLASS_MAGUMSA)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ZtConfig.PrideMu.LevelLifeMG  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ZtConfig.PrideMu.VitalityToLifeMG);
		if (lpObj->Class == CLASS_DARKLORD)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ZtConfig.PrideMu.LevelLifeDL  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ZtConfig.PrideMu.VitalityToLifeDL);
		if (lpObj->Class == CLASS_SUMMONER)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ZtConfig.PrideMu.LevelLifeSUM  + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ZtConfig.PrideMu.VitalityToLifeSUM);
		if (lpObj->Class == CLASS_MONK)
			lpObj->MaxLife = DCInfo.DefClass[lpObj->Class].Life + (lpObj->Level + lpObj->MLevel - 1) * ZtConfig.PrideMu.LevelLifeRF + ((lpObj->Vitality - DCInfo.DefClass[lpObj->Class].Vitality ) * ZtConfig.PrideMu.VitalityToLifeRF);
	...
	{
		if(ZtConfig.CommonServer.MarlonQuestReset)
		{
...
#endif
#ifdef PERIOD
	g_PeriodItemZt.AddPeriodInfo(lpObj);
#endif
#ifdef OLDCASHSHOP
...
//#endif
	...
	if(MonsterClass == 529 || MonsterClass == 530 ||
		MonsterClass == 531 || MonsterClass == 532 ||
		MonsterClass == 533 || MonsterClass == 534 ||
		MonsterClass == 535 || MonsterClass == 536 ||
		MonsterClass == 537 || MonsterClass == 538 ||
		MonsterClass == 539)
	{
		lpObj->Type = OBJ_MONSTER;
	}
	...
	if ( lpObj->m_AttackType != 0 )
	{
		...
		if ( lpObj->Class == 539 )
		{
			gObjMonsterMagicAdd(lpObj, 230 , 1);	//AT_SKILL_RED_STORM
		}
...
{
	#if(ENABLE_OPTIMIZATION)
	return g_Optimization.AddObjectUser(aSocket, ip);
	#endif
...
short gObjAddMonster(int iMapNumber)
{
	...
	#if(ENABLE_OPTIMIZATION)
	return g_Optimization.AddObjectMonster();
	#endif
...
{
	#if(ENABLE_OPTIMIZATION)
	return g_Optimization.AddObjectReCall();
	#endif
	...
	while ( true )
	{
		if ( gObj[count].Connected == PLAYER_EMPTY )
		{
			...
			if ( gObjCallMonCount >= OBJ_STARTUSERINDZT )
			{
				gObjCallMonCount = OBJ_MAXMONSTER;
...
#ifdef PERIOD
	g_PeriodItemZt.RemovePeriodInfo(lpObj);
#endif
...
bool gObjIsConnectedZt(int aIndex)
{
	if ( aIndex < 0 || aIndex > OBJMAX-1)
	{
		return FALSE;
...
{
	//int Zt_PersonalID = GetPrivateProfileInt("Common","PersonalID",0,"..\\Data\\ZtData\\CommonServer.ini");
	if(!ZtConfig.CommonServer.Zt_PersonalID)
		return true;
...
BOOL gObjCheckXYMapTile(LPOBJ lpObj, int iDbgName)
{
	...
	if(lpObj->GuildLoadTick > 5)
	{
		...
		{
			if(ZtConfig.CommonServer.CheckOtherGuildLand)
			{
				GCServerMsgStringSend("Your clan is not the owner of the castle.", lpObj->m_Index, 0);
...
#ifdef PRIVATE_MODULE
	//if(g_ZtLicense.CheckUser(Encode) || g_ZtLicense.CheckUser(MU2Play) || g_ZtLicense.CheckUser(Local))
	//{
	//	// ----
	//}
	//else
...
BOOL gObjSetAccountLogin(int aIndex, char* szId, int aUserNumber, int aDBNumber,int ukn_30, char* JoominNumber,BYTE PcBangRoom)
{
	...
	if(g_AccountSecurity.m_bEnable)
	{
		gObj[aIndex].Join_aUserNumber = aUserNumber;
		gObj[aIndex].Join_aDBNumber = aDBNumber;
		strcpy(gObj[aIndex].Join_JoominNumber, JoominNumber);
		gObj[aIndex].Join_ukn_30 = ukn_30;
		gObj[aIndex].Join_PcBangRoom = PcBangRoom;
...
#if(CUSTOM_ACCOUNT_SECURITY)
	g_AccountSecurity.GDReqLoad(aIndex);
#endif
...
{
#ifdef _FIX_
	if(szId == NULL) return -1;
#endif

	int n;

	for (n=OBJ_STARTUSERINDZT;n<OBJMAX; n++)
	{
...
void gObjAddMsgSendDelay(LPOBJ lpObj, int aMsgCode, int aIndex, int delay, int SubCode)
{
	for ( int n=0;n<MAX_MONSTER_SEND_MSG;n++)
	{
		...
		{
#if(FIX_REF_ON_REF == TRUE)
			//if(lpObj->FixRefOnRef == true && aMsgCode == 10)
			//{
			//	return;
			//}
...
void gObjAddAttackProcMsgSendDelay(LPOBJ lpObj, int aMsgCode, int aIndex, int delay, int SubCode, int SubCode2)
{
	...
	{
		int iIndex = lpObj->m_Index - OBJ_STARTUSERINDZT;
...
void gObjStateProc(LPOBJ lpObj, int aMsgCode, int aIndex, int SubCode)
{
		...
		case 10:
			{
				...
				if ( lpObj->DamageReflect != 0 )
				{
					gObjAttack(lpObj, &gObj[aIndex], 0, 0, 0, SubCode, 0, 0, 1);
#if(FIX_REF2==TRUE)
					//gSMAttackProcMsg[lpObj->m_Index][i].MsgCode = -1;
					//lpObj->FixRef2 = true;
#endif
				}
#endif
	...
	//Season 2.5 add-on for third wings
		...
		case 13:
			{
				lpObj->Life = lpObj->AddLife + lpObj->MaxLife; //season4.5 changed
				GCReFillSend(lpObj->m_Index,lpObj->Life, -1, 0, lpObj->iShield);
			}
			break;
		case 14:
			{
				lpObj->Mana = lpObj->AddMana + lpObj->MaxMana; //season 4.5 changed
				GCManaSend(lpObj->m_Index, lpObj->Mana, -1, 0, lpObj->BP);
			}
			break;
...
#if(FIX_DL_HORSE_IN_CROWN==FALSE)	
...
bool gObjLevelUp(LPOBJ lpObj, __int64 & addexp, int iMonsterType, int iEventType) //0051F140
{
	...
	#if(ENABLE_CHARLVL1000)

	if(lpObj->Level >= ZtConfig.ZtCustom.MaxLevel && lpObj->Experience >= gLevelExperience[-1+lpObj->Level])
	{
	...
	lpObj->Level++;
		...
		if ( lpObj->Class == CLASS_DARKLORD )
		{
			lpObj->LevelUpPoint += ZtConfig.AddLvlPoint.DlLevelPoints;
		}
		else if(lpObj->Class == CLASS_KNIGHT)
		{
			lpObj->LevelUpPoint += ZtConfig.AddLvlPoint.DkLevelPoints;
		}
		else if(lpObj->Class == CLASS_WIZARD)
		{
			lpObj->LevelUpPoint += ZtConfig.AddLvlPoint.DwLevelPoints;
		}
		else if(lpObj->Class == CLASS_ELF)
		{
			lpObj->LevelUpPoint += ZtConfig.AddLvlPoint.ElfLevelPoints;
		}
		else if(lpObj->Class == CLASS_SUMMONER)
		{
			lpObj->LevelUpPoint += ZtConfig.AddLvlPoint.SumLevelPoints;
		}
		else if ( lpObj->Class == CLASS_MAGUMSA )
		{
			lpObj->LevelUpPoint += ZtConfig.AddLvlPoint.MgLevelPoints;
		}
#ifdef MONK
		else if ( lpObj->Class == CLASS_MONK )
		{
			lpObj->LevelUpPoint += ZtConfig.AddLvlPoint.RfLevelPoints;
		}
#endif
	...
	if ( lpObj->PlusStatQuestClear != false )
	{
		...
		if ( lpObj->Class == CLASS_DARKLORD )
		{
			lpObj->LevelUpPoint += ZtConfig.AddMarlonPoint.DlLevelPoints;
		}
		else if(lpObj->Class == CLASS_KNIGHT)
		{
			lpObj->LevelUpPoint += ZtConfig.AddMarlonPoint.DkLevelPoints;
		}
		else if(lpObj->Class == CLASS_WIZARD)
		{
			lpObj->LevelUpPoint += ZtConfig.AddMarlonPoint.DwLevelPoints;
		}
		else if(lpObj->Class == CLASS_ELF)
		{
			lpObj->LevelUpPoint += ZtConfig.AddMarlonPoint.ElfLevelPoints;
		}
		else if(lpObj->Class == CLASS_SUMMONER)
		{
			lpObj->LevelUpPoint += ZtConfig.AddMarlonPoint.SumLevelPoints;
		}
		else if ( lpObj->Class == CLASS_MAGUMSA )
		{
			lpObj->LevelUpPoint += ZtConfig.AddMarlonPoint.MgLevelPoints;
		}
#ifdef MONK
		else if ( lpObj->Class == CLASS_MONK )
		{
			lpObj->LevelUpPoint += ZtConfig.AddMarlonPoint.RfLevelPoints;
		}
#endif
#else
		if ( lpObj->Class == CLASS_WIZARD || lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_ELF || lpObj->Class == CLASS_SUMMONER )
		{
...
BOOL gObjLevelUpPointAdd(BYTE type, LPOBJ lpObj)
{
	...
	switch ( type )
	{
		case 0:
			{
				  if ((lpObj->Strength) >= ZtConfig.Character.MaxStats) return false;
				lpObj->Strength++;
			}
			break;
		...
		case 1:
			{
				  if ((lpObj->Dexterity) >= ZtConfig.Character.MaxStats) return false;
				lpObj->Dexterity++;
			}
			break;
		...
		case 2:
			{
				  if ((lpObj->Vitality) >= ZtConfig.Character.MaxStats) return false;
				lpObj->Vitality++;
		...
		case 3:
			{
				  if ((lpObj->Energy) >= ZtConfig.Character.MaxStats) return false;
				lpObj->Energy++;
		...
		case 4:
			{
				if((lpObj->Leadership) >= 32767) return false;
				lpObj->Leadership++;
			}
			break;
	}
...
void gObjPlayerKiller(LPOBJ lpObj, LPOBJ lpTargetObj, int MSBDamage)
{
...
#if(EVENT_DUNGEON_SIEGE)
	if(!g_DungeonSiege.ChangePkLevel(lpObj->m_Index, lpTargetObj->m_Index))
	{
		return;
	}
#endif
	...
	if(!g_TeamVsTeam.PKCheck(lpObj->m_Index, lpTargetObj->m_Index))
	{
		return;
	}
#endif
	...
	if(g_ZtLicense.user.TvTEvent)
	{
		if(!TvT.PkCheck(lpObj,lpTargetObj))return;
	}
#ifdef PANDORA_EVENT
	if (g_ZtLicense.user.PandoraEvent)
	{
		if(!gPandoraBoxEvent.PkCheck(lpObj,lpTargetObj))return;
	}
#endif // PANDORA_EVENT


#ifdef UPDATE_FIX
	if(lpObj->m_iDuelUser != -1 || lpTargetObj->m_iDuelUser != -1)
	{
		return;
	}
#endif
	...
	if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
	{
		if(g_DeviasBattle.Start)
			g_DeviasBattle.BattleKillSocer(lpObj->m_Index,lpTargetObj->m_Index);
	}
#endif
...
//#endif
	...
	if(ZtConfig.PvP.ZtPkPartySystem && lpTargetObj->PartyNumber >= 0) //Season 2.5 add-on
	{
...
#if(FIX_PK_SELFDEFENCE)
	for ( int n=0;n<MAX_SELF_DEFENSE;n++)
	{
		if ( lpObj->SelfDefense[n] >= 0 )
		{
			if ( lpObj->SelfDefense[n] == lpTargetObj->m_Index )
			{
...
{
	g_ZtUser.PlayerKillInfoMail(lpObj->m_Index, lpTargetObj->m_Index);

#if(FIX_REF2)
	//lpObj->FixRef2 = false;
	//lpTargetObj->FixRef2 = false;
#endif
#if(EVOMU_CASTLE_SEGE==TRUE)
	if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) && g_ZtLicense.CheckUser(eZtUB::Gredy))
	{
		
		if(lpObj->m_btCsJoinSide >= 2 && lpObj->m_iAccumulatedCrownAccessTime > 0)
		{
			//lpObj->m_iAccumulatedCrownAccessTime -= 4000;
			lpObj->m_iAccumulatedCrownAccessTime -= g_UserConfig.m_EvoMU.CSRegTimeMinus;
...
#if(SYSTEM_ACHIEVEMENTS)
	g_Achievements.PlayerKill(lpTargetObj->m_Index, lpObj->m_Index);
#endif

#if(EVENT_DUNGEON_SIEGE)
	if(g_DungeonSiege.PlayerScore(lpObj->m_Index, lpTargetObj->m_Index))
	{
		return;
	}
#endif
	...
	//if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
	{
		if(!LoraBattle.PkCheck(lpObj, lpTargetObj))
		{
			return;
		}
	}
	...
	if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
	{
		if(!g_DeviasBattle.PkCheck(lpObj, lpTargetObj))
		{
			return;
		}
	}
	...
	if (g_ZtLicense.user.PandoraEvent)
	{
		gPandoraBoxEvent.ProcessKill(lpTargetObj,lpObj);
		if(!gPandoraBoxEvent.PkCheck(lpObj,lpTargetObj))return;
	}
#endif // PANDORA_EVENT

#if(_ARENA_ITEM_)
	if(ZtConfig.CommonServer.ArenaItem !=-1 && lpObj->MapNumber == 6)
	{
		if(gObjGetItemCountInInventory(lpObj->m_Index,ZtConfig.CommonServer.ArenaItem,0) > 0)  // 2 < 1
		{
			gObjDeleteItemsCount(lpObj->m_Index,ZtConfig.CommonServer.ArenaItem,0,1);
		}
		else
		{
			...
			{
				lpObj->MapNumber = MAP_INDEX_BATTLESOCCER;
				lpObj->X = 62;
				lpObj->Y = 134;
			}
			else
...
#if(CUSTOM_RETURNSPOT==TRUE)
	g_SpotReturn.StatusSave(lpObj->m_Index);
#endif
...
#if(EVENT_TEAM_VS_TEAM)
	if(g_TeamVsTeam.PlayerKill(lpObj->m_Index, lpTargetObj->m_Index))
	{
		return;
	}
#endif
	...
	if(lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER) 
	{
		MessaageAllGlobal("%s Killed %s", lpTargetObj->Name, lpObj->Name);
	}
#endif
	...
	if(g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal))
	{
		if(ZtConfig.CommonServer.ArenaItem == -1 && lpObj->MapNumber == 6)
		{
			lpObj->MapNumber = ZtConfig.iTeleportMapForDieMap6;
			lpObj->X = 62;
			lpObj->Y = 134;
		}
		else if(lpObj->MapNumber == 40)
		{
			lpObj->MapNumber = ZtConfig.iTeleportMapForDieMap40;
			lpObj->X = 148;
			lpObj->Y = 17;
		}
		else if(lpObj->MapNumber == 51)
		{
			lpObj->MapNumber = ZtConfig.iTeleportMapForDieMap51;
			lpObj->X = 215;
			lpObj->Y = 31;
		}		
	}
...
//#endif
	...
	if(itemdrop == 0)
	{
		...
		if(dropresult == 0)
		{
			...
			while(count-- != 0)
			{
				if( lpObj->m_PK_Level >= 4 && gPkItemDrop )	//New line
				{
...
BOOL gObjSkeletonSprite(LPOBJ lpObj)	//Season 5 Skeleton Pet
{
	if ( lpObj->Type != OBJ_USER )
	{
		return FALSE;
...
void gObjSpriteDamage(LPOBJ lpObj, int damage)
{
	...
	if(g_DonateManager.CheckItem(lpObj->m_Index))
	{
		return;
	}
#endif
		...
		g_ZtLicense.CheckUser(eZtUB::ulasevich) || g_ZtLicense.CheckUser(eZtUB::ulasevich2))
	{
		...
		{
			fN = ZtConfig.Pet.AngelDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,67))
		{
			fN = ZtConfig.Pet.RudolfDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,1))
		{
			fN = ZtConfig.Pet.SatanDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,64))
		{
			fN = ZtConfig.Pet.DemonDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,65))
		{
			fN = ZtConfig.Pet.GuardianDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,80))
		{
			fN = ZtConfig.Pet.PandaDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,123))
		{
			fN = ZtConfig.Pet.SkeletonDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,106))
		{
			fN = ZtConfig.Pet.UnicornDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,2))
		{
			fN = ZtConfig.Pet.UniriaDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,3))
		{
			fN = ZtConfig.Pet.DinorantDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,76))
		{
			fN = ZtConfig.Pet.RingPandaDurability;
		}
		else if(sprite->m_Type == ITEMGET(13,122))
		{
			fN = ZtConfig.Pet.RingSkeletonDurability;
		}
	}
	...
	if(g_PetZt.IsPet(sprite->m_Type))
	{
		return;
	}

	if(g_PetZt.IsPetImmortal(sprite->m_Type))
	{
	...
	else if ( sprite->m_Type == ITEMGET(13,123) ) //Season 5 Skeleton
	{
		fdamage = (damage)/10.0f;
		fdamage /= 10.0f;
		sprite->m_Durability -= fdamage;
	...
	else if ( sprite->m_Type == ITEMGET(13,4) )
	{
		...
		inc_def = lpObj->m_MPSkillOpt.MpsPetDurDownSpeed + 1500;

		//if ( sprite->m_DurabilitySmall > lpObj->m_MLPassiveSkill.m_iML_DecreasePetDurReduction + 1500 )
		if ( sprite->m_DurabilitySmall > inc_def )
		{
	...
	else if(sprite->m_Type == ITEMGET(13,37))
	{
		...
		if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
		{
			if ( sprite->IsFenrirIncLastAttackDamage() != FALSE )		// Black Fenrir
			{
				iMaxDurSmall = ( iMaxDurSmall * ZtConfig.Pet.BlackFenrirProcDurability ) / 100; 
			}
			else if ( sprite->IsFenrirDecLastAttackDamage() != FALSE )	// Blue Fenrir
			{
				iMaxDurSmall = ( iMaxDurSmall * ZtConfig.Pet.BlueFenrirProcDurability ) / 100; 	 
			}
			else if ( sprite->IsFenrirSpecial() != FALSE )				// Illusion Fenrir
			{
				iMaxDurSmall = ( iMaxDurSmall * ZtConfig.Pet.GoldFenrirProcDurability ) / 100; 	 
			}
			else														//red 
			{
				iMaxDurSmall = ( iMaxDurSmall * ZtConfig.Pet.RedFenrirProcDurability ) / 100; 	 
			}
		}
		...
		if(sprite->m_DurabilitySmall > iMaxDurSmall)
		{
			if(g_ZtLicense.CheckUser(eZtUB::PrideMuLocal) || g_ZtLicense.CheckUser(eZtUB::PrideMu) || g_ZtLicense.CheckUser(eZtUB::Local3) || g_ZtLicense.CheckUser(eZtUB::Local))
			{
				if (sprite->m_Type == ITEMGET(13,37))
				{
					if(ZtConfig.PrideMu.OffDecreaseDur)
					{
					}
					else
					{
			...
			else if(g_ZtLicense.CheckUser(eZtUB::SILVER1) || g_ZtLicense.CheckUser(eZtUB::SILVER2))	//Fenrir Durability Off
			{
...
void gObjSecondDurDown(LPOBJ lpObj)
{
	...
	for( int i = 0; i < 5; i++ )
	{
		...
		if( g_LuckyItemManager.IsLuckyItemEquipment(DurItem->m_Type) )
		{
			...
			if( ret == 2 )
			{
				...
				//Lucky Item Delete Invenary
				//gObjInventoryItemSet(lpObj->m_Index, item_num[i], -1);
				//gObj[aIndex].pInventory[pos].Clear();
				DurItem->Clear();
				//GCInventoryItemOneSend(aIndex, lpMsg->invenrotyTarget);
				GCInventoryItemDeleteSend(lpObj->m_Index, item_num[i], 1);
			}
		}
	}
#endif
	...
	if((attr&1)==0)
	{
		if(lpObj->pInventory[10].IsItem() == 1)
		{
			...
			{
				if(!lpObj->pInventory[10].IsPeriodItem())
				{
				...
				{
					ret = (float)lpObj->pInventory[10].m_Durability;
				}
		...
		if(lpObj->pInventory[11].IsItem() == 1)
		{
			...
			{
				if(!lpObj->pInventory[11].IsPeriodItem())
				{
					if(gObjCheckIsInPCBang(lpObj) != FALSE) //season4.5 add-on
					{
				...
				{
					ret = (float)lpObj->pInventory[11].m_Durability;
				}
...
void gObjChangeDurProc2(LPOBJ lpObj) //season 4.5 add-on
{
		...
		)
	{
		...
		else if(lpObj->pInventory[10].m_Type == ITEMGET(13,122)) //Season 5 Skeleton Ring
		{
			int m_Durability = (int)lpObj->pInventory[10].m_Durability;
		...
		)
	{
		...
		else if(lpObj->pInventory[11].m_Type == ITEMGET(13,122)) //Season 5 Skeleton Ring
		{
			int m_Durability = (int)lpObj->pInventory[11].m_Durability;
...
#if(EVENT_DUNGEON_SIEGE)
	if(!g_DungeonSiege.CheckEventStart(aTargetIndex))
	{
		return;
	}
#endif
...
#if(EVENT_TEAM_VS_TEAM==TRUE)
	if(!g_TeamVsTeam.PKCheck(lpObj->m_Index, aTargetIndex))
	{
		return;
	}
#endif

	if(!PvPMapSystem.CheckPkMap(lpObj, &gObj[aTargetIndex])) return;
...
int gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg, bool& bSendExp) //005289E0
{
	...
	{
		int NewExp = exp;
		NewExp = gResetSystem.Main(lpObj);
		NewExp = gGrandReset.DinamicExp(lpObj->m_Index, NewExp);
		exp *= NewExp;
		...
		{
			exp = int( ( exp * gPartyBonus.nearbycount1 ) / 100 );
		}
		#endif
	...
	if(exp > 0)
	{
		if(lpObj->Type == OBJ_USER)
		{
			gObjSealUserSetExp(lpObj, exp, FALSE);
			gObjUserSetExp(lpObj,exp);
...
void gObjExpParty(LPOBJ lpObj , LPOBJ lpTargetObj, int AttackDamage, int MSBFlag )
{
	...
	if(bCheckSetParty[0] != false && bCheckSetParty[1] != false && bCheckSetParty[2] != false || bCheckSetParty[3] != false && bCheckSetParty[4] != false && bCheckSetParty[5] != false )
	{
		bApplaySetParty = 1;
	}
	...
	{
		#ifdef DRYNEA_LIST_CUSTOM
		if(g_ZtLicense.CheckUser(eZtUB::drynea) && ZtConfig.DryneaConfig.PartySystemEnable)
		{			
			for(int j(0); j <= ZtConfig.DryneaConfig.CountPartyDiapason; j++)
			{
				for(int CountPartyMember = 0; CountPartyMember < gParty.m_PartyS[lpPartyObj->PartyNumber].Count; CountPartyMember++)
				{
					if( (gObj[gParty.m_PartyS[lpPartyObj->PartyNumber].Number[0]].Reset >= ZtConfig.DryneaConfig.PartyDiapason[j].StartDiapason && gObj[gParty.m_PartyS[lpPartyObj->PartyNumber].Number[0]].Reset <= ZtConfig.DryneaConfig.PartyDiapason[j].EndDiapason) 
						&& ( (gObj[gParty.m_PartyS[lpPartyObj->PartyNumber].Number[CountPartyMember]].Reset < ZtConfig.DryneaConfig.PartyDiapason[j].StartDiapason) || (gObj[gParty.m_PartyS[lpPartyObj->PartyNumber].Number[CountPartyMember]].Reset > ZtConfig.DryneaConfig.PartyDiapason[j].EndDiapason) ) )
						viewpercent -= ZtConfig.DryneaConfig.ExpMinus;
					else if(viewplayer == 5)
					{
						viewpercent += ZtConfig.DryneaConfig.Gold;
					}
					else if(viewplayer == 4)
					{
						viewpercent += ZtConfig.DryneaConfig.Silver;
					}
				}	
			}
		}
		...
		if(bApplaySetParty != 0)
		{
			if(viewplayer == 3)
			{
				viewpercent = ZtParty3ExpPercent;
			}
			else if(viewplayer == 4)
			{
				viewpercent = ZtParty4ExpPercent;
			}
			else if(viewplayer == 5)
			{
				viewpercent = ZtParty5ExpPercent;
			}
			#if _NEW_PARTY_SYSTEM_ == TRUE
			else if(viewplayer == 6)
			{
				viewpercent = ZtParty6ExpPercent;
			}
			else if(viewplayer == 7)
			{
				viewpercent = ZtParty7ExpPercent;
			}
			else if(viewplayer == 8)
			{
				viewpercent = ZtParty8ExpPercent;
			}
			else if(viewplayer == 9)
			{
				viewpercent = ZtParty9ExpPercent;
			}
			else if(viewplayer >= 10)
			{
				viewpercent = ZtParty10ExpPercent;
			}
			#endif
			else
			{
				viewpercent = ZtPartyExpPercentOther;
			}
		}
		else
		{
			if(viewplayer == 2)
			{
				viewpercent = ZtParty2ExpSetPercent;
			}
			else if(viewplayer == 3)
			{
				viewpercent = ZtParty3ExpSetPercent;
			}
			else if(viewplayer == 4)
			{
				viewpercent = ZtParty4ExpSetPercent;
			}
			else if(viewplayer == 5)
			{
				viewpercent = ZtParty5ExpSetPercent;
			}
			#if _NEW_PARTY_SYSTEM_ == TRUE
			else if(viewplayer == 6)
			{
				viewpercent = ZtParty6ExpSetPercent;
			}
			else if(viewplayer == 7)
			{
				viewpercent = ZtParty7ExpSetPercent;
			}
			else if(viewplayer == 8)
			{
				viewpercent = ZtParty8ExpSetPercent;
			}
			else if(viewplayer == 9)
			{
				viewpercent = ZtParty9ExpSetPercent;
			}
			else if(viewplayer >= 10)
			{
				viewpercent = ZtParty10ExpSetPercent;
			}
			#endif
			else
			{
				viewpercent = ZtParty1ExpSetPercent;
			}
		}
	...
	if(!g_ZtLicense.CheckUser(eZtUB::bassreflexive))
	{
		if(lpTargetObj->Level >= 65)
		{
			if(viewplayer == 1)
	...
	for(n = 0; n < MAX_USER_IN_PARTY; n++)
	{
		...
		if(number >= 0)
		{
			...
			if(lpTargetObj->MapNumber == lpPartyObj->MapNumber && dis[n] < 10)
			{
				...
				{
					int NewExp = exp;
					NewExp = gResetSystem.Main(lpPartyObj);
					NewExp = gGrandReset.DinamicExp(lpPartyObj->m_Index, NewExp);
					exp *= NewExp;

					#ifdef PARTY_BONUS
					if(g_ZtLicense.CheckUser(eZtUB::ulasevich) || g_ZtLicense.CheckUser(eZtUB::ConflictNoCustom) || g_ZtLicense.CheckUser(eZtUB::ulasevich2))
					{
						if (gPartyBonus.isEnable(viewplayer))
						{
							exp = int( ( exp * gPartyBonus.ExpNearbyCount(lpObj, lpTargetObj, viewplayer) / 100 ) );
							exp = int( ( exp * gPartyBonus.ExpClassBonus(lpObj, lpTargetObj, viewplayer) / 100 ) );
						}
					}
					#endif
				}
				...
				if(exp > 0)
				{
					...
					{
						gObjSealUserSetExp(lpPartyObj, exp, FALSE);

						gObjUserSetExp(lpPartyObj,exp);

						if(ZtConfig.m_iExpPartyPlus > 0)
						{
							exp += (( exp * ZtConfig.m_iExpPartyPlus ) / 100 );
						}
	...
	#if(PARTY_ZEN_DROP==TRUE)
	//if(lpTargetObj->Type == OBJ_MONSTER && expMoney > 0)
	//{
	//	if (expMoney > MAX_ZEN)
	//	{
	//		lpTargetObj->Money = MAX_ZEN;
...
//#endif
...
#if(SYSTEM_ACHIEVEMENTS)
			g_Achievements.MonsterKill(lpObj->m_Index, lpTargetObj->m_Index);
#endif

			#if(EVENT_CARNAGE)
			g_CarnageEvent.DieMonster(lpObj->m_Index, lpTargetObj->m_Index);
			#endif

			#if(DEV_MONSTERTIME)
			g_MonsterTime.DieMonster(lpObj->m_Index, lpTargetObj->m_Index);
			#endif
		...
		else if(lpTargetObj->Type == OBJ_USER)
		{
			...
			if(DG_MAP_RANGE(lpTargetObj->MapNumber))	//Season 5 DoppelGanger
			{
			...
			if(iGuildWar == 0 && iDuel == 0)	//Season 5
			{
	...
	if(result == 1)
	{
		...
		for(int y = 0; y < yl; y ++)
		{
			...
			for(int x = 0; x < xl; x++)
			{
				...
				if(ExtentCheck(xx,yy,8,4)==1) //12
				{
					*(BYTE*)(gObj[aIndex].pInventoryMap+64 + (itemposy + y)*8+(itemposx + x)) = set_byte;
				}
				else
				{
					result = 2;
	...
	{	
		...
		for(int y = 0; y < yl; y ++)
		{
			...
			for(int x = 0; x < xl; x++)
			{
				...
				if(ExtentCheck(xx,yy,8,4)==1) //12
				{
					*(BYTE*)(gObj[aIndex].pInventoryMap+96 + (itemposy + y)*8+(itemposx + x)) = set_byte;
				}
				else
				{
					result = 3;
	...
	{	
		...
		for(int y = 0; y < yl; y ++)
		{
			...
			for(int x = 0; x < xl; x++)
			{
				...
				if(ExtentCheck(xx,yy,8,4)==1) //12
				{	
					*(BYTE*)(gObj[aIndex].pInventoryMap+192 + (itemposy + y)*8+(itemposx + x)) = set_byte;
				}
				else
				{
					LogAdd("error : xx:%d yy:%d %s %d",xx,yy,__FILE__,__LINE__);
					return;
				}
			}
		}
...
BYTE gObjInventoryInsertItem(int aIndex, int item_num)
{
	...
	if(gObj[aIndex].ExpandedInventory == 0)
	{
		return -1;
	}
	...
	if(result == -1)
	{
		for(h = 0; h < 4; h++)
		{
			for(w = 0; w < 8; w++)
			{
				if(*(BYTE*)((gObj[aIndex].pInventoryMap+64)+h*8+w) == 255)
				{
					blank = gObjExtInventory1RectCheck(aIndex,w,h,iwidth,iheight);
	...
	if(gObj[aIndex].ExpandedInventory == 1)
	{
		return -1;
	}
	...
	if(result == -1)
	{
		for(h = 0; h < 4; h++)
		{
			for(w = 0; w < 8; w++)
			{
				if(*(BYTE*)((gObj[aIndex].pInventoryMap+96)+h*8+w) == 255)
				{
					blank = gObjExtInventory2RectCheck(aIndex,w,h,iwidth,iheight);
		...
		for(w = 0; w < 8; w++)
		{
			if(*(BYTE*)(gObj[aIndex].pInventoryMap+h*8+w) == 255)
			{
				...
				if(blank != 255)
				{
					...
					copyitem.Convert(item->m_Type,item->m_Option1,item->m_Option2,item->m_Option3,item->m_NewOption,item->m_SetOption,item->m_ItemOptionEx,item->m_SocketOption, item->m_SocketOptionIndex, 0, CURRENT_DB_VERSION);
	...
	if(lpObj->pInventory[n].IsItem() == 1)
	{
		if((int)lpObj->pInventory[n].m_Durability - m_dur > 0)
		{
				...
				g_ZtLicense.CheckUser(eZtUB::Stone) )
			{
				...
				{
					if(lpObj->pInventory[n].m_Type >= ITEMGET(14, 35) && lpObj->pInventory[n].m_Type <= ITEMGET(14, 40))
					{
						lpObj->m_PotionTick = GetTickCount();
					}
				}
	...
	if(result == FALSE)
	{
		for(h=0;h<4;h++)
		{
			for(w=0;w<8;w++)
			{
				if(*(BYTE*)((lpObj->pInventoryMap+64)+(h)*8+(w)) == 255)
				{
					blank = gObjExtInventory1RectCheck(lpObj->m_Index, w, h, iItemWidth, iItemHeight);
	...
	if(result == FALSE)
	{
		for(h=0;h<4;h++)
		{
			for(w=0;w<8;w++)
			{
				if(*(BYTE*)((lpObj->pInventoryMap+96)+(h)*8+(w)) == 255)
				{
					blank = gObjExtInventory2RectCheck(lpObj->m_Index, w, h, iItemWidth, iItemHeight);
...
#ifdef NEWWINGS_2_5
	if( IS_NEWWINGS_2_5(lpItem->m_Type) )
	{
		if(lpObj->Level < lpItem->m_RequireLevel)
		{
			return false;
...
#endif
#ifdef NEWWINGS_4
	if( IS_NEWWINGS_4(lpItem->m_Type) )
	{
		if(lpObj->Level < lpItem->m_RequireLevel)
		{
			return false;
...
#endif
#ifdef NEWWINGS_5
	if( IS_NEWWINGS_5(lpItem->m_Type) )
	{
		if(lpObj->Level < lpItem->m_RequireLevel)
		{
			return false;
...
#endif
#ifdef NEWWINGS_6
	if( IS_NEWWINGS_6(lpItem->m_Type) )
	{
		//if(g_ZtLicense.CheckUser(eZtUB::PrideMuLocal || eZtUB::PrideMu || eZtUB::Local3 || eZtUB::Local2)){
...
BYTE gObjWerehouseRectCheck(int aIndex, int sx, int sy, int width, int height)
{
	...
	if(gObj[aIndex].ZtpandedWarehouse == 0)
	{
		if(result == -1)
		{
			return -1;
	...
	{	
		...
		for(y = 0; y < height; y ++)
		{
			...
			for(x = 0; x < width; x++)
			{
				...
				if(ExtentCheck(xx,yy,8,15)==1)
				{
					if(*(BYTE*)((gObj[aIndex].pWarehouseMap+120)+(sy+y)*8+(sx+x)) != 255)
					{
						blank += 1;
						result = -2;
						break;
	...
	if(result == true)
	{
		...
		for(int y = 0; y < yl; y ++)
		{
			...
			for(int x = 0; x < xl; x++)
			{
				...
				if(ExtentCheck(xx,yy,8,15)==1)
				{
					*(BYTE*)((gObj[aIndex].pWarehouseMap+120) + (itemposy + y)*8+(itemposx + x)) = set_byte;
				}
				else
				{
...
BYTE gObjInventoryInsertItemPos(int aIndex, CItem item, int pos, BOOL RequestCheck)
{
	...
	if(pos < 12)
	{
		...
		if(lpItem->m_Option1)
		{
			...
			for(int i=0;i<12;i++)
			{
				if(lpObj->pInventory[i].m_Special[0] == lpItem->m_Special[0])
					iSkillRet++;
			}
			...
			if(iSkillRet < 2)
				result = gObjWeaponMagicAdd(lpObj,lpItem->m_Special[0],lpItem->m_Level);
...
BYTE gObjInventoryMoveItem(int aIndex, BYTE source, BYTE target, int& durSsend, int& durTsend, BYTE sFlag, BYTE tFlag, LPBYTE siteminfo)
{
...
#ifdef DEBUG_CODE
	LogAddC(2,"sFlag: %d || tFlag: %d",sFlag,tFlag);
#endif
	...
	__try
	{
			...
			if(sFlag == 0)
			{
				...
				else
				{
					if(sitem->m_Level < 4 && sitem->m_Option3*4 < 4)
					{
						...
						else if(sitem->m_Type == ITEMGET(14,195))
						{

						}
						else if (sitem->m_Type == ITEMGET(14,209))
						{
		...
		case 4:
			...
			if(gBlockMoveIteml.Move(aIndex,source) == false)
			{
...
#if(CUSTOM_ACCOUNT_SECURITY)
			if(g_AccountSecurity.CheckTrade(lpObj->m_Index) == false)
			{
				return -1;
			}
#endif
			...
			#if(CUSTOM_ITEM_MOVE)
			if(g_ItemMove.CheckTrade(sitem->m_Type))
			{
		...
		{
			if ( lpObj->pInventory[source].m_Type == ITEMGET(13,147) ||
				lpObj->pInventory[source].m_Type == ITEMGET(13,148) ||
				lpObj->pInventory[source].m_Type == ITEMGET(13,149) ||
				lpObj->pInventory[source].m_Type == ITEMGET(13,150) ||
				lpObj->pInventory[source].m_Type == ITEMGET(13,151) ||
				lpObj->pInventory[source].m_Type == ITEMGET(13,152) ||
				lpObj->pInventory[source].m_Type == ITEMGET(13,153) ||
				lpObj->pInventory[source].m_Type == ITEMGET(13,154) ||
				lpObj->pInventory[source].m_Type == ITEMGET(13,155) ||
				lpObj->pInventory[source].m_Type == ITEMGET(13,156)
				)
			{
			...
			//}
#endif
#ifdef GRAND_SYSTEM_DEV
			//if( g_LuckyItemTrade == false ) //Custom
			//{
				if( sitem->m_Type == ITEMGET(14, 195))
				{
					return -1;
				}
			//}
		...
		case 4:
			...
			else 
			{
				...
				//	return -1;
				//}
				if(ExtentCheck(w,h,8,28) == 0)//Danger fix
				{
					return -1;
				}
//#ifdef ZTPINV
...
//#endif
				...
				if(*(BYTE*)(gObj[aIndex].pInventoryMap + h * 8 + w) != 255)
				{
					memcpy(lpObj->pInventoryMap,TempInventoryMap,64*3);
					return -1;
				}
				...
				if(sFlag == 0)
				{
					if(source < INVETORY_WEAR_SIZE)
					{
						if(source == 10 || source == 11)
						{
							...
							if(lpObj->pWarehouse[target].m_Type == ITEMGET(13,122)) //Season 5 Skeleton Ring
							{
								gObjUseSkill.SkillChangeUse(aIndex);
							}
						}
				...
				if(sFlag == 0)
				{
					if(source < INVETORY_WEAR_SIZE)
					{
						if(source == 10 || source == 11)
						{
							...
							if(lpObj->pChaosBox[target].m_Type == ITEMGET(13,122)) //Season 5 Skeleton Ring
							{
								gObjUseSkill.SkillChangeUse(aIndex);
							}
						}
		...
		if(sFlag == 0 && source < INVETORY_WEAR_SIZE)
		{
			if(lpObj->pInventory[source].IsItem() == 1)
			{
				...
				if(lpObj->pInventory[source].m_Type == ITEMGET(13,122)) //Season 5 Skeleton Ring
				{
					gObjUseSkill.SkillChangeUse(aIndex);
					LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[source].m_Level);
				}
			...
			else
			{
				if(source == 10 || source == 11)
				{
					...
					if(lpObj->pInventory[target].m_Type == ITEMGET(13,122)) //Season 5 Skeleton Ring
					{
						gObjUseSkill.SkillChangeUse(aIndex);
						LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[target].m_Level);
					}
...
BYTE gObjInventoryTradeMove(LPOBJ lpObj, BYTE source, BYTE target)
{
	...
	if(gBlockMoveIteml.Move(lpObj->m_Index,source) == false)
	{
		GCServerMsgStringSend("Move Item Block",lpObj->m_Index,1);
		return -1;
	}
	...
	#if(CUSTOM_ITEM_MOVE)
	if(g_ItemMove.CheckTrade(lpObj->pInventory[source].m_Type))
	{
		GCServerMsgStringSend("Block Trade Item",lpObj->m_Index,1);
		return -1;
	}
...
BYTE gObjTempInventoryInsertItem(LPOBJ lpObj, CItem item, BYTE * TempMap)
{
	...
	if(result == -1)
	{
		for(h = 0; h < 4; h++)
		{
			for(w = 0; w < 8; w++)
			{
				if(*(BYTE*)(TempMap+64+h*8+w) == 255)
				{
					blank = gObjTempExtInventory1RectCheck(TempMap,w,h,iwidth,iheight);
	...
	if(result == -1)
	{
		for(h = 0; h < 4; h++)
		{
			for(w = 0; w < 8; w++)
			{
				if(*(BYTE*)(TempMap+96+h*8+w) == 255)
				{
					blank = gObjTempExtInventory2RectCheck(TempMap,w,h,iwidth,iheight);
...
#ifdef PERIOD
	g_PeriodItemZt.OnRequestPeriodItemList(&gObj[aIndex]);
	g_PeriodItemZt.OnRequestPeriodItemList(&gObj[number]);
#endif
...
BYTE LevelSmallConvert(int aIndex, int inventoryindex)	//Season 5 +15 Items
{
...
BYTE LevelSmallConvert(int level)	//Season 5 +15 Items
{
	if(level >= 15)
	{
		return 7;
...
void gObjMakePreviewCharSet(int aIndex)
{
	...
	lpObj->CharSet[CS_CLASS] |= CS_SET_3RD_CHANGEUP(lpObj->ChangeUP3rd);
...
#define SLOTNEGATIVE -1
	...
	switch (lpObj->pInventory[7].m_Type)
	{
		...
		default:
			{
#ifdef NEWWINGS_2_5
				if( IS_NEWWINGS_2_5(lpObj->pInventory[7].m_Type) )
				{
					lpObj->CharSet[5]	|= CS_SET_WING1(3);
...
#endif
#ifdef NEWWINGS_4
				if( IS_NEWWINGS_4(lpObj->pInventory[7].m_Type) )
				{
					lpObj->CharSet[5]	|= CS_SET_WING1(3);
					lpObj->CharSet[17]	|= (lpObj->pInventory[7].m_Type - ITEMGET(12, 180) + 1) << 2;
					break;
...
#endif
#ifdef NEWWINGS_5
				if( IS_NEWWINGS_5(lpObj->pInventory[7].m_Type) )
				{
					lpObj->CharSet[5]	|= CS_SET_WING1(3);
					lpObj->CharSet[17]	|= (lpObj->pInventory[7].m_Type - ITEMGET(12, 180) + 1) << 2;
					break;
...
#endif
#ifdef NEWWINGS_6
				if( IS_NEWWINGS_6(lpObj->pInventory[7].m_Type) )
				{
					//if(g_ZtLicense.CheckUser(eZtUB::PrideMuLocal || eZtUB::PrideMu || eZtUB::Local3 || eZtUB::Local2)){
	...
	//gObjCalcMLSkillItemOption(lpObj);	//nowork
	...
	default:
		#if(ENABLE_PETZT)
		if(g_PetZt.IsPet(lpObj->pInventory[8].m_Type))
		{
			index |= CS_SET_HELPER(3);
			lpObj->CharSet[16] |=  lpObj->pInventory[8].m_Type - ITEMGET(13,0);
			break;
		}
		#endif
...
void gObjViewportPaint(HWND hWnd, short aIndex)
{
...
#else
	wsprintf(szTemp, "Monster: %d | Online: %d  Player(%d):%d VpCount:%d(%d/%d) : item count:%d max:%d",
	...
	TextOut(hdc, 10, GAMESERVER_HEIGHT-75, szTemp, strlen(szTemp));
	SetBkMode(hdc, iOldBkMode);
#else
...
BOOL gObjCheckViewport(int aIndex, int x, int y)
{
	...
	if(g_ZtLicense.user.CustomCameraMove)
	{
		...
		if(lpUser->m_CamMove)
		{
			return g_CameraMove.CheckViewport(aIndex, x, y);
		}
	}
#endif
#if(CUSTOM_CAMERA_MOVE)
	//if(g_ZtLicense.user.CustomCameraMove)
	{
		...
		if(lpUser->m_CameraPartyActive && OBJMAX_RANGE(lpUser->m_CameraPartyIndex) )
		{
			return g_CameraParty.CheckViewport(aIndex, x, y);
		}
	...
	/*int DisViewe = ZtConfig.CommonServer.DisatanceObjViewe;

		if(x < lpObj->X - DisViewe  || x > lpObj->X + DisViewe || y < lpObj->Y - DisViewe || y > lpObj->Y + DisViewe)
		{
			return false;
		}*/
#ifdef FIX_DC_DISTANCE_OBJ_VIEWE
...
#endif // FIX_DC_DISTANCE_OBJ_VIEWE
...
//#else
							...
							if(g_Crywolf.GetCrywolfState() == 4 || bGateLicense == true)
							{
								int mgt = 118;
								short x,y,level;
								BYTE map = lpObj->MapNumber,dir;
...
#ifdef DRYNEA_13_11
						else if((lpObj->MapNumber == MAP_INDEX_BATTLESOCCER) &&  g_ZtLicense.CheckUser(eZtUB::drynea))
						{
							lpObj->MapNumber = MAP_INDEX_RORENCIA;
							MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber,(short &)lpObj->X,(short &)lpObj->Y);
						}
...
//#endif
...
#endif
#if(EVENT_TEAM_VS_TEAM)
						else if(g_TeamVsTeam.PlayerRegen(lpObj->m_Index))
						{

						}
#endif

						else if(lpObj->MapNumber == MAP_INDEX_SANTATOWN)
						{
							if(g_ZtLicense.CheckUser(eZtUB::Artem) || g_ZtLicense.CheckUser(eZtUB::Artem2))
							{
								//lpObj->MapNumber = MAP_INDEX_RORENCIA;
							...
							else if(g_ZtLicense.CheckUser(eZtUB::ulasevich) || g_ZtLicense.CheckUser(eZtUB::ulasevich))
							{
								int mgt = 17;
								short x, y, level;
								BYTE map = lpObj->MapNumber, dir;
						...
						else if (lpObj->MapNumber == MAP_INDEX_GM_SUMMONZONE)
						{
							if(g_ZtLicense.CheckUser(eZtUB::drynea))
							{
								lpObj->MapNumber = ZtConfig.DryneaConfig.GMKillMap;
							...
							else if(g_ZtLicense.CheckUser(eZtUB::MedoniAndrei) || g_ZtLicense.CheckUser(eZtUB::Local))
							{
								//int iGMKillMapX = 215;
								//int iGMKillMapY = 46;
								...
								//lpObj->X = iGMKillMapX + randomX;
								//lpObj->Y = iGMKillMapY + randomY;
...
void gObjSecondProc()
{
	...
	for(n = 0; n < OBJMAX;n++)
	{
		...
		if(lpObj->Connected > PLAYER_LOGGED)
		{
			...
			if(ZtConfig.CommonServer.MarlonTeleport)
			{
...
#if(CUSTOM_NPC_BUFFER==TRUE)
				//g_BufferSystem.Timer(n);
#endif

#if(EVENT_DUNGEON_SIEGE)
				g_DungeonSiege.PlayerCheck(n);
#endif

#if(CUSTOM_MARRIAGE && MARRI_BUFF) 
				g_Marriage.BuffAdded(n);
#endif
#if(SYSTEM_ACHIEVEMENTS)
				g_Achievements.TimeInGame(n);
#endif
#if(CUSTOM_PREMIUM_SYSTEM==TRUE)
				g_PremiumSystemZt.PremiumTime(n);
#endif
...
//#endif
			...
			gObjTimeCheckSelfDefense(lpObj);
#ifdef ZTCLUSIVE_AMYLET
			gExclusiveOptions.TickTime(n);
#endif

#if(SYSTEMOF_BROTHER==TRUE)
			gBrotherhood.TickTime(n);
#endif
			...
			{
				if(lpObj->Reset < ZtConfig.CommonServer.iNoobBuffMaxReset && lpObj->GReset == 0)
				{
					if(gObjSearchActiveEffect(lpObj,AT_NOOB_BUFF) == FALSE )
					{
						gObjApplyBuffEffectDuration(lpObj,AT_NOOB_BUFF,0,0,0,0,600);
					}
				}
				else
				{
...
#endif
#if(CUSTOM_NEWBUFF)
			if(ZtConfig.m_bNewBuffPremiumEnable)
			{
				if(lpObj->PremiumTime > 0)
				{
					if(gObjSearchActiveEffect(lpObj,AT_NEWPREMIUM_BUFF) == FALSE)
					{
						gObjApplyBuffEffectDuration(lpObj,AT_NEWPREMIUM_BUFF,0,0,0,0,600);
					}
				}
				else
				{
					...
					{
						gObjRemoveBuffEffect(lpObj,AT_NEWPREMIUM_BUFF);
					}
				}
			}

			if(ZtConfig.m_bNewBuffGuildEnable)
			{
				_GUILD_INFO_STRUCT * lpGuildUser = lpObj->lpGuild;
			
				if(lpObj->GuildNumber > 0 && lpGuildUser)
				{
					if(g_ZtLicense.CheckUser(eZtUB::ulasevich) || g_ZtLicense.CheckUser(eZtUB::ulasevich2))
					{
						if(g_ZtUser.GetGuildMemberOnline(lpObj->GuildNumber) > ZtConfig.m_bNewBuffGuildMembOnline)
						{
							if(gObjSearchActiveEffect(lpObj,AT_NEWGUILD_BUFF) == FALSE)
							{
								gObjApplyBuffEffectDuration(lpObj,AT_NEWGUILD_BUFF,0,0,0,0,600);
							}
						}
						else
						{
							...
							{
								gObjRemoveBuffEffect(lpObj,AT_NEWGUILD_BUFF);
							}
						}
					}
					else
					{
						if(lpObj->lpGuild->Count > ZtConfig.m_bNewBuffGuildMembOnline)
						{
							if(gObjSearchActiveEffect(lpObj,AT_NEWGUILD_BUFF) == FALSE)
							{
								gObjApplyBuffEffectDuration(lpObj,AT_NEWGUILD_BUFF,0,0,0,0,600);
			...
			if(ZtConfig.m_bNewBuffPartyEnable)
			{
				if(lpObj->PartyNumber >= 0)
				{
					if(gParty.GetPartyCount(lpObj->PartyNumber) >= ZtConfig.m_bNewBuffPartyMembCount)
					{
						if(gObjSearchActiveEffect(lpObj,AT_NEWPARTY_BUFF) == FALSE)
						{
							gObjApplyBuffEffectDuration(lpObj,AT_NEWPARTY_BUFF,0,0,0,0,600);
						}
					}
					else
					{
						...
						{
							gObjRemoveBuffEffect(lpObj,AT_NEWPARTY_BUFF);
						}
					}
				}
				else
			...
			if(g_ZtLicense.CheckUser(Local) || g_ZtLicense.CheckUser(SILVER1) || g_ZtLicense.CheckUser(SILVER2))	//Need Delete Local
			{
				if(lpObj->MapNumber == MAP_INDEX_VULCAN || lpObj->MapNumber == MAP_INDEX_VULCANROOM )
				{
					if(lpObj->Level < 200)
					{
						MsgNormal(n,"[Vulcanus Map]: Need 200 Level");
						gObjTeleport(n,0,140,135);
					}
				}
			}
			...
			if(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal))
			{
				if(g_UserConfig.m_EvoMU.PvpMap400LvlTpActive)
				{
					if(lpObj->Level >= 400)
					{
						if(!PvPMapSystem.CheckPvPMap(lpObj))
						{
							gObjTeleport(n, g_UserConfig.m_EvoMU.PvpMap400LvlTpMap, g_UserConfig.m_EvoMU.PvpMap400LvlTpX, g_UserConfig.m_EvoMU.PvpMap400LvlTpY);
						}
					}
				}
			}
		...
		if(lpObj->Connected == PLAYER_CONNECTED ||	lpObj->Connected == PLAYER_LOGGED || lpObj->Connected == PLAYER_PLAYING)
		{
			if(lpObj->Type == OBJ_USER)
			{
				...
				if(g_ZtLicense.CheckUser(eZtUB::Local) || g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::Gredy2) || g_ZtLicense.CheckUser(eZtUB::GredyLocal))
				{
					if(lpObj->Connected == PLAYER_LOGGED)
					{
						lpObj->m_iSwitchDisconnectCount++;

						if(lpObj->m_iSwitchDisconnectCount > 60)
						{
							CloseClient(lpObj->m_Index);
						}
					}
					...
					{
						lpObj->m_iSwitchDisconnectCount = 0;
					}
				}
...
void gObjManaPotionFill(LPOBJ lpObj)
{
	...
	if(lpObj->m_ManaFillCount > 2)
	{
		...
		if(lpObj->MaxMana + lpObj->AddMana > lpObj->Mana)
		{
			...
			if(lpObj->RecoveryMp > 0)
			{
				loc3 += lpObj->RecoveryMp;
			}
...
void gObjRestPotionFill(LPOBJ lpObj)
{
	...
	if(lpObj->m_Rest != 0)
	{
		...
		if(lpObj->m_LifeFillCount > 4)
		{
			...
			if(lpObj->RecoveryHp > 0)
			{
				percent += lpObj->RecoveryHp;
			}
...
//
//			pViewportCreateChange.ViewSkillState = lpObj->m_ViewState;//m_ViewSkillState;
...
//			}
//
//			pViewportCreate.ViewSkillState = lpObj->m_ViewState;//lpObj->m_ViewSkillState;
//
...
//			pCallMonsterViewportCreate.ViewState = lpObj->m_ViewState;//lpObj->m_ViewSkillState;
//	
...
//	
//			pMonsterViewportCreate.ViewState = lpObj->m_ViewState;//llpObj->m_ViewSkillState;
...
//								pViewportCreateChange.SkinL = SET_NUMBERL((lpTargetObj->m_Change & 0xFFFF) & 0xFFFF);									pViewportCreateChange.ViewSkillState = lpTargetObj->m_ViewState;//lpTargetObj->m_ViewSkillState;
...
//								}
//		
//								pViewportCreate.ViewSkillState = lpTargetObj->m_ViewState;//llpTargetObj->m_ViewSkillState;
...
//		
//									pCallMonsterViewportCreate.ViewState = lpTargetObj->m_ViewState;//llpTargetObj->m_ViewSkillState;
...
//#endif
//		
//									pMonsterViewportCreate.ViewState = lpTargetObj->m_ViewState;//llpTargetObj->m_ViewSkillState;
...
void gObjViewportListProtocolCreate(LPOBJ lpObj)
{
	...
	if(lpObj->Type == OBJ_USER)
	{
		if(lpObj->m_Change >= 0)
		{
			...
			pViewportCreateChange.DirAndPkLevel = lpObj->Dir << 4;
//#ifdef LORA_BATTLE_EVENT
//			if((g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal)) && (LoraBattle.CheckStartEvent() == true))
//			{
...
//			}
//			else if(g_ZtLicense.user.TvTEvent)
//#else
//			if(g_ZtLicense.user.TvTEvent)
//#endif // LORA_BATTLE_EVENT
//			{
...
//					if((tObjNum > OBJ_MAXMONSTER) && (tObjNum < OBJMAX))
//#endif
//					pViewportCreateChange.DirAndPkLevel |= TvT.ReturnStatus(lpObj->m_Index,tObjNum) & 0x0F;//out
//				}
//				else
//				{
//					pViewportCreateChange.DirAndPkLevel |= lpObj->m_PK_Level & 0x0F;
...
//			}
			...
			//LogAddC(2, "[DEBUG] pViewportCreate: %d size", sizeof(pViewportCreate)); 
					...
					if(tObjNum >= 0)
					{
						...
						case 1:
							...
							if(lpTargetObj->m_Change >= 0)
							{
...
#if(EVENT_DEVIASBATTLE)
								else if((g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2))
								...
								lOfsChange += (sizeof(pViewportCreateChange)) - (MAX_STATE_COUNT - iViewSkillCount); //Season3 changed
							...
							else
							{
								...
								pViewportCreate.DirAndPkLevel = lpTargetObj->Dir << 4;
#ifdef LORA_BATTLE_EVENT
								if(/*(g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2)) &&*/ (LoraBattle.CheckStartEvent() == true))
								{
									if((tObjNum > OBJ_MAXMONSTER) && (tObjNum < OBJMAX))
										pViewportCreate.DirAndPkLevel |= LoraBattle.ReturnStatus(lpObj->m_Index,tObjNum) & 0x0F;//out
								}
#if(EVENT_DEVIASBATTLE)
								else if((g_ZtLicense.CheckUser(eZtUB::Gredy) || g_ZtLicense.CheckUser(eZtUB::GredyLocal) || g_ZtLicense.CheckUser(eZtUB::Gredy2)) && (g_DeviasBattle.CheckStartEvent() == true))
								{
									if((tObjNum > OBJ_MAXMONSTER) && (tObjNum < OBJMAX))
						...
						case 3:
							if(lpObj->Type == OBJ_USER)
							{
								...
								if(lpTargetObj->m_RecallMon >= 0)
								{
									...
									callMonlOfs += (sizeof(pCallMonsterViewportCreate)) - (MAX_STATE_COUNT - iViewSkillCount); //Season3 changed
...
//#endif
									...
									MonlOfs += (sizeof(pMonsterViewportCreate)) - (MAX_STATE_COUNT - iViewSkillCount); //Season3 changed
...
BOOL gObjMoveGate(int aIndex, int gt)
{
	...
	if(g_ZtLicense.user.PandoraEvent)
	{
		if(!gPandoraBoxEvent.BlockTeleport(aIndex))return false;	//TvTEvent
	}
#endif // PANDORA_BOX
...
#if(EVENT_DUNGEON_SIEGE)
	/*if(!g_DungeonSiege.CheckMoveGateSiege(aIndex, mapNumber))
	{
		return false;
	}*/
#endif
	if(g_ZtLicense.CheckUser(SILVER1) || g_ZtLicense.CheckUser(SILVER2))
	{
		if(mapNumber == MAP_INDEX_VULCAN || mapNumber == MAP_INDEX_VULCANROOM )
		{
			if(lpObj->Level < 200)
			{
				MsgNormal(aIndex,"[Vulcanus Map]: Need 200 Level");
				return false;
			}
		}
	}
...
#if(SYSTEM_ACHIEVEMENTS)
	if(CC_MAP_RANGE(mapNumber))
	{
		g_Achievements.TeleportInCC(aIndex);
	}
#endif
...
int gObjMagicAdd(LPOBJ lpObj, BYTE Type, BYTE Index, BYTE Level, int & SkillNumber)
{
...
int gObjWeaponMagicAdd(LPOBJ lpObj, int aSkill, BYTE Level)
{
	int skill = -1,n;
	skill = -1;
	...
	if(g_MasterSkillSystem.WeaponMagicAdd(lpObj,aSkill, Level) != FALSE) //season 4 add-on
	{
		return -1;
	}
...
#if(SYSTEM_ACHIEVEMENTS)
	g_Achievements.UsedJewel(lpObj->m_Index);
#endif
		return true;
	}
...
#ifdef NEWWINGS_2_5
		  || IS_NEWWINGS_2_5(lpObj->pInventory[target].m_Type)
#endif
#ifdef NEWWINGS_4
		  || IS_NEWWINGS_4(lpObj->pInventory[target].m_Type)
#endif
#ifdef NEWWINGS_5
		  || IS_NEWWINGS_5(lpObj->pInventory[target].m_Type)
#endif
#ifdef NEWWINGS_6
...
BOOL gObjItemRandomLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	//int Zt_SoulRate = GetPrivateProfileInt("JewelRates","SoulRate",50,"..\\Data\\ZtData\\CommonServer.ini");
	//int Zt_SoulRateLuck = GetPrivateProfileInt("JewelRates","SoulRateLuck",20,"..\\Data\\ZtData\\CommonServer.ini");
	if(source < 0 || source > MAIN_INVENTORY_SIZE -1)
	{
		return false;
	}
	...
	else
	{
		...
		{
			if(_r < Zt_SoulRate)	//40
			{
				bSuccess = true;
			}
		}
		else if(_r < Zt_SoulRate)	//60
		{
			bSuccess = true;
		}
	}
...
BOOL gObjItemRandomOption3Up(LPOBJ lpObj, int source, int target)
{
	...
	{
		//int Zt_LifeRate = GetPrivateProfileInt("JewelRates","LifeRate",50,"..\\Data\\ZtData\\CommonServer.ini");
...
#if(SYSTEM_ACHIEVEMENTS)
			g_Achievements.GuildWarWin(lpGuild1->Index[n]);
#endif
		}
	}
...
void gObjSendUserStatistic(int aIndex, int startLevel, int endLevel)
{
	...
	{
		endLevel = ZtConfig.ZtCustom.MaxLevel;
	}
	...
	for(n = OBJ_STARTUSERINDZT; n < OBJMAX; n++) //fixed macro
	{
		lpObj = &gObj[n];
...
void gObjUseCashShopStatFruit(LPOBJ lpObj, int pos)
{
	...
	LogAdd("[CashShop][ZtMinusStatFruit] [%s][%s] Use StatDownItem Level:%d,Pos:%d,Dur:%d,Serial:%u", lpObj->AccountID,lpObj->Name,iStatType,pos,lpObj->pInventory[pos].m_Durability,iItemSerial);
	...
	LogAdd("[CashShop][ZtMinusStatFruit] [%s][%s] MinusStat[%d] -> LevelUpPoint Old(%d)/New(%d)",
...
{
	gObjCalCharacter(lpObj->m_Index);
	//GCReFillSend(lpObj->m_Index,(WORD)(lpObj->MaxLife + lpObj->AddLife),0xFE,0,(WORD)(lpObj->iMaxShield + lpObj->iAddShield));	//DeBug TEST
}
...
void MsgOutput(int aIndex, char* msg, ...)
{
	...
	//if(g_ZtLicense.CheckUser(eZtUB::masonX))
	{
		msg = g_Text.Change(msg);
	}
#endif
...
void gObjUserSetExp(LPOBJ lpObj, __int64 & Experience)	//Panda User
{
	...
	{
		Experience = Experience + ( Experience * ZtConfig.Pet.Zt_PandaExp / 100 );
	}

	//Season 5 Skeleton Ring
	if(gObjSkeletonSprite(lpObj) == TRUE && lpObj->m_Change == 548)
	{
		Experience = Experience + ( Experience * ZtConfig.Pet.SkeletonPetAndRingExp / 100 );
	}
	else if(gObjSkeletonSprite(lpObj) == TRUE)
	{
		Experience = Experience + ( Experience * ZtConfig.Pet.SkeletonPetExp / 100 );
	}
	...
	if(g_ZtLicense.user.WinQuest) gWinQuestSystem.ExpOption(lpObj->m_Index,Experience);

	gExclusiveOptions.ExpFunction(lpObj->m_Index,Experience);


#if(SYSTEMOF_BROTHER==TRUE)
	gBrotherhood.UserExp(lpObj,Experience);
#endif
#if(CUSTOM_NOOBBUFF)
	if(gObjSearchActiveEffect(lpObj,AT_NOOB_BUFF) == TRUE)
	{
		Experience += ( Experience * 35 ) / 100;
	}
#endif

#if(CUSTOM_PREMIUM_SYSTEM==TRUE)
	int PlusPremiumZtp = g_PremiumSystemZt.UserExpPlus(lpObj->m_Index);

	if(PlusPremiumZtp > 0)
	{
		Experience += (( Experience * PlusPremiumZtp ) / 100 );
	}
#endif

	#if(ENABLE_PETZT)
	g_PetZt.Exp(lpObj->m_Index, Experience);
	#endif
	...
	if(lpObj->PartyNumber >= 0)
	{
		//if(ZtConfig.m_iExpPartyPlus > 0)
		//{
		//	Experience += (( Experience * ZtConfig.m_iExpPartyPlus ) / 100 );
		//}
	}
}
...
#ifdef DEBUG_CODE
	LogAdd("sy: %d | height:%d | nEndofItemY: %d",sy,height,nEndofItemY);
#endif
	// ----
	if( sy < 0 )
	{
...
{
	if( sy >= 15 * gObj[aIndex].ZtpandedWarehouse + 15 )
	{
		return false;
	}
	// ----
...
BOOL gObjInventoryEquipment(LPOBJ lpObj)
{
	...
	// ----
	BUFF_EFFECT_DATA * lpBuffData	= 0;
	ITEMEFFECT * lpItemEffect		= 0;	//???

#ifdef DEBUG_CODE
	LogAddC(1,"lpObj->m_btSculptPos: %d | m_Type: %d",lpObj->m_btSculptPos,lpObj->pInventory[lpObj->m_btSculptPos].m_Type);
#endif
		...
		&&	lpObj->m_btSculptPos < MAIN_INVENTORY_SIZE )
	{
		...
		lpBuffData = g_BuffManager.GetEffectDataFromItemCode(lpObj->pInventory[lpObj->m_btSculptPos].m_Type);
		//lpBuffData = (LPBUFF_EFFECT_DATE)g_BuffManager.GetBuffInfo(lpObj->pInventory[lpObj->m_btSculptPos].m_Type);
		// ----
		if( !lpBuffData )
		{
			return false;
		...
		{
			gObjSetItemEffect(lpObj, lpBuffData->btIndex);
		}
		else
		{
			lpObj->m_btSculptPos = 0;
	...
	{
		lpBuffData = g_BuffManager.GetEffectDataFromItemCode(lpObj->pInventory[lpObj->m_btCharmPos].m_Type);
		//lpBuffData = (LPBUFF_EFFECT_DATE)g_BuffManager.GetBuffInfo(lpObj->pInventory[lpObj->m_btCharmPos].m_Type);
		// ----
		if( !lpBuffData )
		{
			return false;
		...
		{
#ifdef DEBUG_CODE
			LogAddC(1,"lpBuffData->btIndex: %d",lpBuffData->btIndex);
#endif
			gObjSetItemEffect(lpObj, lpBuffData->btIndex);
		}
		else
	...
	{
		lpBuffData = g_BuffManager.GetEffectDataFromItemCode(lpObj->pInventory[lpObj->m_btArtifactPos].m_Type);
		//lpBuffData = (LPBUFF_EFFECT_DATE)g_BuffManager.GetBuffInfo(lpObj->pInventory[lpObj->m_btArtifactPos].m_Type);
		// ----
		if( !lpBuffData )
		{
			return false;
...
BYTE gObjTempExtInventory1RectCheck(BYTE * TempMap, int sx, int sy, int width, int height)
{
	int x,y,blank = 0;
	...
	for(y = 0; y < height; y++)
	{
		for(x = 0; x < width; x++)
		{
			if(*(BYTE*)((TempMap+64) + (sy+y)*8 + (sx+x))!= 255)
			{
				blank += 1;
				return -1;
			}
	...
	if(blank == 0)
	{
		return 64+sx+sy*8+INVETORY_WEAR_SIZE;
	}
	return -1;
}

BYTE gObjTempExtInventory2RectCheck(BYTE * TempMap, int sx, int sy, int width, int height)
{
	int x,y,blank = 0;
	...
	for(y = 0; y < height; y++)
	{
		for(x = 0; x < width; x++)
		{
			if(*(BYTE*)((TempMap+96) + (sy+y)*8 + (sx+x))!= 255)
			{
				blank += 1;
				return -1;
			}
	...
	if(blank == 0)
	{
		return 96+sx+sy*8+INVETORY_WEAR_SIZE;
	}
	return -1;
}
...
BYTE gObjExtInventory1RectCheck(int aIndex, int sx, int sy, int width, int height)
{
	...
	for(y = 0; y < height; y ++)
	{
		...
		for(x = 0; x < width; x++)
		{
			...
			if(ExtentCheck(xx,yy,8,4)==1) //8
			{
				if(*(BYTE*)((gObj[aIndex].pInventoryMap+64)+(sy+y)*8+(sx+x)) != 255)
				{
					blank += 1;
					return -1;
				}
...
BYTE gObjExtInventory2RectCheck(int aIndex, int sx, int sy, int width, int height)
{
	...
	for(y = 0; y < height; y ++)
	{
		...
		for(x = 0; x < width; x++)
		{
			...
			if(ExtentCheck(xx,yy,8,4)==1) //8
			{
				if(*(BYTE*)((gObj[aIndex].pInventoryMap+96)+(sy+y)*8+(sx+x)) != 255)
				{
					blank += 1;
					return -1;
				}
