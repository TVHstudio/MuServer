...
#define OBJ_STARTUSERINDZT (OBJMAX - OBJMAXUSER)
#define OBJ_CALLMONCOUNT (OBJ_STARTUSERINDZT - OBJ_MAXMONSTER)
#define OBJECT_RANGE(x) (((x)<0)?0:((x)>=OBJMAX)?0:1)
...
#define OBJECT_USER_RANGE(x) (((x)<OBJ_STARTUSERINDZT)?0:((x)>=OBJMAX)?0:1)

#if(FIX_MONSTERDROP_MSG==TRUE)
#define MAX_MONSTER_SEND_MSG		50
#else
#define MAX_MONSTER_SEND_MSG		10
#endif
#define MAX_MONSTER_SEND_ATTACK_MSG	50
//#define MAX_MONSTER_TYPE 580	//512
...
#define MAGIC_SIZE MAX_MAGIC//60
...
extern ZtMessageStateMachine gSMAttackProcMsg[OBJMAX][MAX_MONSTER_SEND_ATTACK_MSG];
...
//#endif
	...
	BYTE m_btDoubleGoerRoom;	//Season 5 DoppelGanger
	//BYTE m_ViewState;	// 1E2	
	BYTE m_ViewSkillState[MAX_VIEWSKILL];	// 1E4	//Season 5 DoppelGanger
	WORD m_ImmuneToMagicCount;	// 249	//Season 5 ImperialFort
	WORD m_ImmuneToHarmCount;	// 24A	//Season 5 ImperialFort
	...
	BYTE MixManagerSocket;

	int		m_AddMixMoney;
	int		m_AddMixSuccessRate;

#ifdef PRIVATE_MODULE
	MonsterQuestUser m_MonsterQuest;
#endif
#ifdef FLOOD_IN_POST
	int PostTickCount;
#endif
#ifdef ZTCLUSIVE_AMYLET
	int ExclusiveItemTime[10];
#endif
...
//#endif
	...
	bool ActiveBrother;
	char Brotherhood[11];
	int m_BroTeleportTime;
#endif
#if(CUSTOM_PARTY_SEARCH==TRUE)
	bool pt_Active;
	short pt_MaxLevel;
	bool pt_bAllClass;
	bool pt_bOnlyOneClass;
	bool pt_bDarkWizard;
	bool pt_bDarkKnight;
	bool pt_bElf;
	bool pt_bMagicGladiator;
	bool pt_bDarkLord;
	bool pt_bSummoner;
	//bool pt_bRageFighter;
#endif
	//int Bonus;
	...
	bool	m_OfflineMode;
	WORD	m_OfflineSkill;
	BYTE    m_OfflineBuffTime;
	bool    m_OfflineAutoBuff;
	int     m_OfflineAttackTime;
#if(ADD_OFFMODE_PICKUP==TRUE)
	bool	m_OfflinePickZen;
	bool	m_OfflinePickJewel;
	bool	m_OfflinePickExel;
	bool	m_OfflinePickAnc;
	#if(ADD_OFFMODE_TIMER)
	int     m_OfflineTimerUser;
	int     m_OfflineTimerMax;
	#endif
#endif
	bool	m_ReOfflineMode;
#endif
#if(CUSTOM_MARRIAGE)
	bool bMarriage;
	char MarriageName[11];
#endif
#if(FIX_REF2)
	//bool FixRef2;
#endif
	...
	bool bAutoPt;
	bool WaitWcoinConnect;
#if(EVENT_DUNGEON_SIEGE)
	DUNGEON_SIEGE_USER DSiege;
#endif
...
#endif
	bool m_bCheckBuyDonateItem;
	BYTE m_btCheckBuyDonatePos;
#if(CUSTOM_ACCOUNT_SECURITY)
	ACCOUNT_SECURITY_INFO AccountSecurity;

	int Join_aUserNumber; 
	int Join_aDBNumber;
	int Join_ukn_30;
	char Join_JoominNumber[14]; 
	BYTE Join_PcBangRoom;
#endif

#if(CUSTOM_REFERAL_SYSTEM)
	int RefferalUser;
	int RefferalMaster[10];
	int RefferalUserLevel[10];
	int RefferalUserReset[10];
	int RefferalUserGrand[10];
#endif

	int m_iLoadConfigNumber;
	int m_iLoadConfigTickCount;
...
#endif
	bool m_bActiveOffTrade;
	int m_iDonateCredit;
#if(ADD_CTRL_KEY)
	bool CtrlKey;
#endif
	int m_iSwitchDisconnectCount;
	BYTE m_MiniMap;
	#if(DEV_GUILDBANK)
	bool m_bGuildBank;
	#endif
	...
	bool m_CameraPartyActive;
	int m_CameraPartyIndex;
	#endif
...
extern Zt__User Zt_User[ZTUSERMAX];
...
extern EX_ACC_DATA ExAccData[ZTUSERMAX];
