...
void CItem::Clear()
{
	...
	this->m_iPShopBonus=-1;
	this->m_PShopBlessValue=-1;
	this->m_PShopSoulValue=-1;
	this->m_PShopChaosValue=-1;
	this->m_PShopLifeValue=-1;
	//#if(DEV_PERSONAL_CREDIT)
	this->m_iPShopCredit=-1;
	//#endif
#endif
...
void CItem::Convert(int type, BYTE Option1, BYTE Option2, BYTE Option3,  BYTE Attribute2, BYTE SetOption, BYTE ItemEffectEx, BYTE SocketOption[5], BYTE SocketIndex, BYTE PeriodItemOption, BYTE DbVersion)
{
	int _type; //2
	int ItemLevel; //3
	...
	if ( gSetItemOption.IsSetItem(_type) != 0 && SetOption != 255 && SOption != 0)
	{
		...
		{
			//int ZtExcellentAncient = GetPrivateProfileInt("Items","ExcellentAncient",1,"..\\Data\\ZtData\\CommonServer.ini");
			if(!ZtExcellentAncient)
				this->m_NewOption = 0;
			Attribute2 = 0;
	...
	if(g_SocketOption.IsActive(this) == 1)
	{
		...
		{	//Zero SET
			//this->m_ItemOptionEx = 0;
			//int ZtAncientSocket = GetPrivateProfileInt("Items","AncientSocket",1,"..\\Data\\ZtData\\CommonServer.ini");
			if(!ZtAncientSocket)
				this->m_SetOption = 0;
			
			//int ZtExcellentSocket = GetPrivateProfileInt("Items","ExcellentSocket",1,"..\\Data\\ZtData\\CommonServer.ini");
			if(!ZtExcellentSocket)
			{
				Attribute2 = 0;
				this->m_NewOption = 0;
			}
	...
	if ( p->RequireLevel != 0 )
	{
		...
		else if ( _type >= ITEMGET(12,180) && _type <= ITEMGET(12,188) )	//Custom Wings4
		{
			this->m_RequireLevel = p->RequireLevel;
		}
#ifdef NEWWINGS_2_5
		else if( IS_NEWWINGS_2_5(_type) )
		{
			this->m_RequireLevel = p->RequireLevel;
		}
#endif
#ifdef NEWWINGS_4
		else if( IS_NEWWINGS_4(_type) )
		{
			this->m_RequireLevel = p->RequireLevel;
		}
#endif
#ifdef NEWWINGS_5
		else if( IS_NEWWINGS_5(_type) )
		{
			this->m_RequireLevel = p->RequireLevel;
		}
#endif
#ifdef NEWWINGS_6
		else if( IS_NEWWINGS_6(_type) )
		{
...
#ifdef NEWWINGS_4
			else if ( IS_NEWWINGS_4(_type) )	//Custom Wings4
			{
				this->m_Defense += this->m_Level * 4;
...
#endif
#ifdef NEWWINGS_2_5
			else if ( IS_NEWWINGS_2_5(_type) )	//Custom Wings4
			{
				this->m_Defense += this->m_Level * 4;
	...
	if ( Option2 != 0 )
	{
...
#ifdef NEWWINGS_6
		if ( IS_NEWWINGS_6(_type) )	//Custom Wings4
		{
			this->m_Special[this->m_SpecialNum] = 84;
			this->m_Option2 = 1;
		}
#endif
#ifdef NEWWINGS_5
		if ( IS_NEWWINGS_5(_type) )	//Custom Wings4
		{
			this->m_Special[this->m_SpecialNum] = 84;
			this->m_Option2 = 1;
		}
#endif
#ifdef NEWWINGS_2_5
		if ( IS_NEWWINGS_2_5(_type) )	//Custom Wings4
		{
			this->m_Special[this->m_SpecialNum] = 84;
			this->m_Option2 = 1;
		}
#endif
#ifdef NEWWINGS_4
		if ( IS_NEWWINGS_4(_type) )	//Custom Wings4
		{
			this->m_Special[this->m_SpecialNum] = 84;
			this->m_Option2 = 1;
		}
		...
		if ( _type >= ITEMGET(12,36) && _type <= ITEMGET(12,43) )	//Season 3.5 Summoner Wings
		{
			this->m_Special[this->m_SpecialNum] = 84;
			this->m_Option2 = 1;
		}
	...
	if ( Option3 != 0 )
	{
		...
		else if ( _type == ITEMGET(12,49) ) // Wing of Dimension
		{
			this->m_Option3 = Option3;
			this->m_Special[this->m_SpecialNum] = 0x55;
		...
		else if ( _type == ITEMGET(12,50) ) // Wing of Dimension
		{
			this->m_Option3 = Option3;
			this->m_Special[this->m_SpecialNum] = 0x55;
...
void CItem::Value()
{
	...
	else if ( this->m_Type == ITEMGET(14,110) ) //Season 5 DoppelGanger
	{
		Gold = (__int64)((10000.0f) * this->m_Durability) * 3.0f;
	}
	else if ( this->m_Type == ITEMGET(14,101) ) //Season 5 ImperialFort
	{
		Gold = (__int64)((10000.0f) * this->m_Durability) * 3.0f;
	}
...
#ifdef NEWWINGS_6
	else if ( (Type == 12 && this->m_Type > ITEMGET(12,6) && (this->m_Type < ITEMGET(12,36) && this->m_Type > ITEMGET(12,43)) && (this->m_Type < ITEMGET(12,180) && this->m_Type > ITEMGET(12,220)) ) || Type == 13 || Type == 15 )	//196
#else
	else if ( (Type == 12 && this->m_Type > ITEMGET(12,6) && (this->m_Type < ITEMGET(12,36) && this->m_Type > ITEMGET(12,43)) && (this->m_Type < ITEMGET(12,180) && this->m_Type > ITEMGET(12,194)) ) || Type == 13 || Type == 15 )
#endif
	{
	...
	else
	{
		...
		else if( Type == 12 && (this->m_Type >= ITEMGET(12,180) && this->m_Type <= ITEMGET(12,188)) ) //Custom Wings4
		{
			Gold = (Level2+40)*Level2*Level2*11+40000000;
		}
#ifdef NEWWINGS_6
		else if( Type == 12 && (this->m_Type >= ITEMGET(12,189) && this->m_Type <= ITEMGET(12,220)) )	//196
#else
		else if( Type == 12 && (this->m_Type >= ITEMGET(12,189) && this->m_Type <= ITEMGET(12,194)) )
#endif
		{
			Gold = (Level2+40)*Level2*Level2*11+40000000;
		}
...
BOOL CItem::IsExtLifeAdd()
{
	if ( (this->m_NewOption & 0x20) == 32 )
	{
		...
		int iRankID = g_ItemRank.IsRankItem(this->m_Type, this->m_Level);
		if(iRankID >= 0)
		{
			int iRankValue = g_ItemRank.GetRankValueOption(iRankID, 0);
			if(iRankValue != -1)
			{
				return iRankValue;
			}
...
BOOL CItem::IsExtManaAdd()
{
	if ( (this->m_NewOption & 0x10) == 16 )
	{
		...
		if(iRankID >= 0)
		{
			int iRankValue = g_ItemRank.GetRankValueOption(iRankID, 1);
			if(iRankValue != -1)
			{
				return iRankValue;
			}
...
BOOL CItem::IsExtDamageMinus()
{
	if ( (this->m_NewOption & 8) == 8 )
	{
		...
		if(iRankID >= 0)
		{
			int iRankValue = g_ItemRank.GetRankValueOption(iRankID, 2);
			if(iRankValue != -1)
			{
				return iRankValue;
			}
...
BOOL CItem::IsExtDamageReflect()
{
	if ( (this->m_NewOption & 4) == 4 )
	{
		...
		if(iRankID >= 0)
		{
			int iRankValue = g_ItemRank.GetRankValueOption(iRankID, 3);
			if(iRankValue != -1)
			{
				return iRankValue;
			}
...
BOOL CItem::IsExtDefenseSuccessfull()
{
	if ( (this->m_NewOption & 2) == 2 )
	{
		...
		if(iRankID >= 0)
		{
			int iRankValue = g_ItemRank.GetRankValueOption(iRankID, 4);
			if(iRankValue != -1)
			{
				return iRankValue;
			}
...
BOOL CItem::IsExtMonsterMoney()
{
	if ( (this->m_NewOption & 1) == 1 )
	{
		...
		if(iRankID >= 0)
		{
			int iRankValue = g_ItemRank.GetRankValueOption(iRankID, 5);
			if(iRankValue != -1)
			{
				return iRankValue;
			}
...
#if(ENABLE_PETZT)
int CItem::IsWhiteTiger() //New pet
{
	if (this->m_Durability < 1.0f) {
		return 0;
	}
	...
	if ((this->m_NewOption & 8) == 8) {
		return 20;//LeeHenry.Fenrir_DecDamagePercent;
	}
	/*else if ((this->m_NewOption & 4) == 4) {
		if (LeeHenry.Fenrir_FixGold != 0) {
			return LeeHenry.Fenrir_DecDamagePercent;
		}
	}*/
...
int CItem::IsHullWolf() //New pet
{
	if (this->m_Durability < 1.0f) {
		return 0;
	}

	if ((this->m_NewOption & 16) == 16) {
		return 20;//LeeHenry.Fenrir_DecDamagePercent;
	}
	/*else if ((this->m_NewOption & 4) == 4) {
		if (LeeHenry.Fenrir_FixGold != 0) {
...
int CItem::IsBeast() //New pet
{
	if (this->m_Durability < 1.0f) {
		return 0;
	}

	if ((this->m_NewOption & 32) == 32) {
		return 20;//LeeHenry.Fenrir_DecDamagePercent;
	}
	/*else if ((this->m_NewOption & 4) == 4) {
		if (LeeHenry.Fenrir_FixGold != 0) {
...
#ifdef NEWWINGS_2_5
		IS_NEWWINGS_2_5(this->m_Type) ||
#endif
#ifdef NEWWINGS_4
		IS_NEWWINGS_4(this->m_Type) ||
#endif
#ifdef NEWWINGS_5
		IS_NEWWINGS_5(this->m_Type) ||
#endif
#ifdef NEWWINGS_6
		IS_NEWWINGS_6(this->m_Type) ||
#endif
		this->m_Type == ITEMGET(12,43))	//Custom Wings4
...
int CItem::DurabilityDown(int dur, int iDecValue)
{
...
#if(WAITENI_CUSTOM)
	if(!ZtConfig.CommonServer.DurabilityEnable)
	{
		return 0;
	}
#endif
	...
	if(gObj[iDecValue].m_MPSkillOpt.MpsDownDur2 > 0)
	{
		DurabilitySmall += DurabilitySmall * gObj[iDecValue].m_MPSkillOpt.MpsDownDur2 / 100;
	}
	...
	if(ZtConfig.CommonServer.DurabilityEnable)
	{
		DurabilitySmall *= ZtConfig.CommonServer.DurabilityMaxCount;
	}
#endif
...
int CItem::DurabilityDown2(int dur, int iDecValue)
{
	...
	if(gObj[iDecValue].m_MPSkillOpt.MpsDownDur1 > 0) //season3 add-on
	{
		DurabilitySmall += DurabilitySmall * gObj[iDecValue].m_MPSkillOpt.MpsDownDur1 / 100;
	}
...
int CItem::NormalWeaponDurabilityDown(int defense, int aIndex)
{
	...
	{
		DurabilitySmall += DurabilitySmall * gObj[aIndex].m_MPSkillOpt.MpsDownDur1 / 100;
	}
...
int CItem::ArmorDurabilityDown(int damagemin, int aIndex)
{
...
#ifdef MART_FIX
	else if ( this->m_RequireClass[6] != 0 )	// Rage Fight
	{
		this->PlusSpecial(&plusdef, 83);
		DecreaseDur = damagemin/(def * 6 + plusdef);
	}
	...
	//LogAddC(2, "%d = %d > %d", this->m_Type, this->m_DurabilitySmall, DurabilitySmall);

#endif

	if ( (int)this->m_DurabilitySmall > DurabilitySmall )
	{
		this->m_DurabilitySmall = 0;
		this->m_Durability -= 1.0f;
...
{
#ifdef DEBUG_CODE
	LogAddC(2,"m_PeriodItemOption: %d",m_PeriodItemOption);
#endif
	return ((this->m_PeriodItemOption & 1) == 1);
}

// 1.01.00 ok
bool CItem::IsPeriodItemZtpire()
{
...
// 1.01.00 ok
void CItem::SetPeriodItemZtpire()
{
...
#ifdef DEBUG_CODE
	LogAddC(5,"this->m_DurabilitySmall: %d | g_LuckyItemManager.GetLuckyItemDurabilityTime(): %d",this->m_DurabilitySmall,g_LuckyItemManager.GetLuckyItemDurabilityTime());
#endif
...
void ItemByteConvertEx(LPBYTE buf, CItem item)
{
	int n=0;
...
int ItemGetDurability(int index, int itemLevel, int ExcellentItem, int SetItem)
{
	...
	//LogAddC(2,"Durability: %d, ExcellentItem: %d",dur, ExcellentItem);
