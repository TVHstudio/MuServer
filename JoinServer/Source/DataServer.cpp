...
//	g_ServerInfoDisplayer.SetState(TRUE,gSObj[aIndex].SType);
	...
	case 0x14:
		...
		break;
	case 0x15:
		DGSaveQuestZtInfo((PMSG_REQ_QUESTZtInfo_SAVE*)aRecv,aIndex);
		break;
	case 0x16:
		DGReqQuestZtInfo((PMSG_REQ_QUESTZtInfo*)aRecv,aIndex);
		break;
	...
	case 0xA2:
		...
		break;
	case 0xA3:
		DGExpandRequest((LPCSHOP_SET_ZTPANDINVENTORY)aRecv);
		break;
	case 0xA4:
...
void JGCharacterCreateRequest(LPSDHP_CREATECHAR lpMsg,int aIndex)
{
	...
	if ( SQLSyntztCheck(szName) == FALSE )
	{
		pResult.Result = 0;
	}
	else
...
void DGSaveQuestZtInfo(PMSG_REQ_QUESTZtInfo_SAVE* lpMsg,int aIndex)
{
	...
	UpdateQuestInfoZt(lpMsg,szName);
}
