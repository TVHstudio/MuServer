...
#define DS2_INISECTION "DataServer2"
#endif
...
struct PMSG_REQ_QUESTZtInfo_SAVE
{
	PWMSG_HEAD h;
	char szName[11];

	QUESTZT_USER	QuestInfo[20];
};

struct PMSG_REQ_QUESTZtInfo
{
	PBMSG_HEAD h;
	int aIndex;
	char szName[11];
...
typedef struct
{
	...
	char szAccountID[11];
	char szName[11];
}CSHOP_SET_ZTPANDINVENTORY, *LPCSHOP_SET_ZTPANDINVENTORY;
...
struct _tagSDHP_REQ_DBMUUN_INVEN_SAVE 
{
    ...
    char AccountID[11];
    char Name[11];
    BYTE dbInventory[1632]; // 16 (item size) * MUUN_INVENTORY_SIZE
};
// ----------------------------------------------------------------
...
void DGSaveQuestZtInfo(PMSG_REQ_QUESTZtInfo_SAVE* lpMsg,int aIndex);
void DGReqQuestZtInfo(PMSG_REQ_QUESTZtInfo* lpMsg,int aIndex);
...
void DGExpandRequest(LPCSHOP_SET_ZTPANDINVENTORY lpMsg);
...
void GDReqSaveMuunInvenItem(_tagSDHP_REQ_DBMUUN_INVEN_SAVE* lpMsg,int aIndex);
