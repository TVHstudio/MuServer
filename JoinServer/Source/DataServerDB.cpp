...
int DeleteCharacter(char* szId,char* szName,char* szGuild,int G_Status)
{
	...
	sprintf(szQuery,"DELETE FROM T_QuestZt_Info WHERE memb_char = '%s'",szName);
	g_DataServerDB.Exec(szQuery);
	g_DataServerDB.Clear();
...
int GetCharacterInfo(char* szAccountID,char* szName,LPSDHP_DBCHAR_INFORESULT lpResult)
{
	...
	{
#if (S6EP2==1 || ZT700==1 )
		sprintf(szQuery,"SELECT cLevel, Class, LevelUpPoint, Experience, Strength, Dexterity, Vitality, Energy, Money, Life, MaxLife, Mana, MaxMana, MapNumber, MapPosX, MapPosY, MapDir, PkCount, PkLevel, PkTime, CtlCode, DbVersion, Leadership, ChatLimitTime, FruitPoint,ExInventory,%s,%s FROM Character WHERE Name='%s' AND AccountID = '%s'",g_DSInfo.dbResetCol, g_DSInfo.dbGrandCol, szName, szAccountID);
#else
	...
	if(g_DataServerDB.Exec(szQuery) == TRUE && g_DataServerDB.Fetch() != SQL_NO_DATA)
	{
		...
		if( g_DSInfo.bResetCol == TRUE )
		{
			...
			lpResult->GrandReset = g_DataServerDB.GetInt(g_DSInfo.dbGrandCol);
		}
...
int GetOptionData(char* szName,LPSDHP_SKILLKEYDATA_SEND lpResult)
{
	int Result = FALSE;
	char szQuery[256];
...
void SetCharacterInfo(char* szName,LPSDHP_DBCHAR_INFOSAVE lpMsg)
{
	...
	if( g_DSInfo.bResetCol == TRUE )
	{
		sprintf(szQuery,"UPDATE Character SET cLevel=%d,Class=%d,LevelUpPoint=%d, Experience=%u, Strength=%d, Dexterity=%d, Vitality=%d, Energy=%d, Money=%d, Life=%f, MaxLife=%f, Mana=%f, MaxMana=%f, MapNumber=%d, MapPosX=%d, MapPosY=%d, MapDir=%d, PkCount=%d, PkLevel=%d, PkTime=%d, DbVersion=3, Leadership=%d, ChatLimitTime=%d, FruitPoint=%d,%s=%d,%s=%d WHERE Name = '%s'",
			lpMsg->Level, lpMsg->Class, lpMsg->LevelUpPoint, lpMsg->Exp, lpMsg->Str, lpMsg->Dex, lpMsg->Vit, lpMsg->Energy, lpMsg->Money, float(lpMsg->Life), float(lpMsg->MaxLife), float(lpMsg->Mana), float(lpMsg->MaxMana), lpMsg->MapNumber, lpMsg->MapX, lpMsg->MapY, lpMsg->Dir, lpMsg->PkCount, lpMsg->PkLevel, lpMsg->PkTime, lpMsg->Leadership, lpMsg->ChatLitmitTime, lpMsg->iFruitPoint,g_DSInfo.dbResetCol,lpMsg->iReset,g_DSInfo.dbGrandCol,lpMsg->GrandReset,szName);
	}
	else{
...
char szQuery[256];
	...
	if( g_DataServerDB.Exec(szQuery) == TRUE )
	{
		...
		if( g_DataServerDB.Fetch() != SQL_NO_DATA )
		{
			...
			lpMsg->MasterExp = g_DataServerDB.GetInt64("ML_ZTP");
...
void UpdateQuestInfoZt(PMSG_REQ_QUESTZtInfo_SAVE* szInfo,char*szName)
{
char szQuery[256];
	sprintf(szQuery,"UPDATE T_QuestZt_Info SET memb_quest = ? WHERE memb_char = '%s'",szName);

	g_DataServerDB.WriteBlob(szQuery,szInfo->QuestInfo,sizeof(szInfo->QuestInfo));
}

void ReadQuestInfoZt(char* szName,PMSG_ANS_QUESTZtInfo* lpMsg)
{
char szQuery[256];

	lpMsg->btResult = TRUE;
	sprintf(szQuery,"SELECT memb_char FROM T_QuestZt_Info WHERE memb_char = '%s'",szName);
	...
	if( g_DataServerDB.Fetch() == SQL_NO_DATA )
	{
		...
		sprintf(szQuery,"INSERT INTO T_QuestZt_Info (memb_char,memb_quest) VALUES ('%s',CONVERT(BINARY(260),0))",szName);
		g_DataServerDB.Exec(szQuery);
		g_DataServerDB.Clear();
	}
	else
	{
		g_DataServerDB.Clear();
		sprintf(szQuery,"SELECT memb_quest FROM T_QuestZt_Info WHERE memb_char = '%s'",szName);
