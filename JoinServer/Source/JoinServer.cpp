...
void JoinServerInit()
{
	...
	g_JSInfo.ServerCode = GetPrivateProfileInt(JS_INISECTION,"ServerCode",-1,INI_FILE);
...
void GJJoinIdPassRequest(LPSDHP_IDPASS lpMsgIdPass,int aIndex )
{
	...
	memcpy(szPass,lpMsgIdPass->Pass, 10);

	BuxConvert(szId, MAX_IDSTRING);
	BuxConvert(szPass, 10);
	...
	if( SQLSyntztCheck(szId) == FALSE || SQLSyntztCheck(szPass) == FALSE || SpaceSyntztCheck(szId) == FALSE)
	{
		result = 0x02;
	}
	else
