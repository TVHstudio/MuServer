...
void Window::LogAdd(BYTE Type, LPSTR Text, ...)
{
	...
	//SendMessage(LogBox, EM_ZTSETSEL, 0, (LPARAM)&CharRange);
	...
	g_WindowZt.LogAdd(Color, TextBuff);
#else
...
void Window::ServerLogAdd(DWORD ServerType, LPSTR Text, ...)
{
	...
	case ST_DATASERVER2:
		sprintf(ServerName, "DataServer2");
		break;
#endif
#if(ENABLE_ZTDATASERVER)
	case ST_ZTDATASERVER:
		sprintf(ServerName, "ZtDataServer");
		break;
#endif
	case ST_EVENTSERVER:
