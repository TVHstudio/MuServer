...
typedef struct _MINIDUMP_ZTCEPTION_INFORMATION 
{
    DWORD ThreadId;
    PEXCEPTION_POINTERS ExceptionPointers;
    BOOL ClientPointers;
} MINIDUMP_ZTCEPTION_INFORMATION, *PMINIDUMP_ZTCEPTION_INFORMATION;
// -------------------------------------------------------------------------------

typedef BOOL (WINAPI *MINIDUMPWRITEDUMP)(HANDLE hProcess, DWORD dwPid, HANDLE hFile, DWORD DumpType, PMINIDUMP_ZTCEPTION_INFORMATION ExceptionParam, void* UserStreamParam, void* CallbackParam);
...
LONG CALLBACK make_minidump(EXCEPTION_POINTERS* e)
{
	...
	if( hDbgDll )
	{
		...
		if( _MiniDumpWrite )
		{
			...
			{
				_MINIDUMP_ZTCEPTION_INFORMATION ZtInfo;

				ZtInfo.ThreadId = ::GetCurrentThreadId();
				ZtInfo.ExceptionPointers = e;
				ZtInfo.ClientPointers = NULL;

				BOOL bDump = _MiniDumpWrite(GetCurrentProcess(),GetCurrentProcessId(),hFile,0x26,&ZtInfo,NULL,NULL);
...
#if(ENABLE_NEW_WINDOW)
	return g_WindowZt.ProcessMain(hWnd, message, wParam, lParam);
#endif
